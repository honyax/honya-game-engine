# Day 37: SSAO(スクリーンスペース環境遮蔽)

Phase 6 の7日目。**物が床に乗る**。

## 今日のゴール

`Ctrl+F9` で立方体の根元に寄った構図が出て、**接地の帯が暗くなる**。
`Ctrl+F1` で切ると、Day 36 までの「浮いている立方体」に戻る。

```
  Ctrl+F9   SSAO が効く構図。**今日の到達点はこの1枚**
  Ctrl+F1   SSAO ON / OFF(OFF で Day 36 に戻る)
  Ctrl+F2   遮蔽率 / ビュー法線 / 距離 を全画面に出す(**今日いちばん使う窓**)
  Ctrl+F3   半径      Ctrl+F4   標本数
  Ctrl+F5   ぼかし(OFF で 4x4 の格子が見える)
  Ctrl+F6   遮蔽バッファを半解像度に
  Ctrl+F7   強さ      Ctrl+F8   下駄(0 で平らな床が荒れる)
  Ctrl+F11  AO を直接光にも掛ける(**わざと間違える**)
  Ctrl+F10  SSAO の自己チェック
```

Day 36 で環境光が「まわりの景色ぜんぶ」になった。
ところが焼いた放射照度は<b>その点が空だけを見ている前提</b>の値で、
隣に壁があろうと箱の下だろうと、法線が同じなら同じ明るさを返す。

だから Day 36 までの絵には**物と物が接している場所の暗がりが無い**。
影(Day 33)は太陽が遮られたところを暗くするが、
**太陽と関係ない暗がり**——曇りの日でも隅が暗いこと——は誰も作っていなかった。

今日はそれを作る。**すでに描いてある深度バッファを「世界の模型」として使い回す**
のが手口で、三角形の数にもシーンの構成にも依存しない。

## 事前に読む資料

- [ゲームグラフィックス特論 B-13: 画面空間の環境遮蔽](https://tokoik.github.io/gg/)
  **今日の本命**。深度バッファから遮蔽を推定するという発想そのものと、
  「スクリーンスペースで何が失われるか」がここで手に入る
- 西川善司『「3Dゲームエンジン」を用いた3Dゲームグラフィックス講座』**第10章**
  SSAO が実際のゲームでどう使われてきたか。Crytek 版から HBAO まで、
  **手法が世代交代してきた理由**が読める
- [LearnOpenGL: SSAO](https://learnopengl.com/Advanced-Lighting/SSAO)
  **コードで追うならここが最短**。今日の実装とほぼ同じ構成で、
  幾何バッファ → 半球カーネル → ノイズ → ぼかし の順に進む。
  ただしあちらはディファードなので G バッファが既にある前提(今日は自分で焼く)
- [Vladimir Kajalin, "Screen-Space Ambient Occlusion" (ShaderX7, 2009)](https://en.wikipedia.org/wiki/Screen_space_ambient_occlusion)
  Crytek 版の一次資料。**球でサンプリングしていた**ので
  平らな面まで一律に 0.5 になり、全体が薄暗くなっていた——
  今日「半球にする理由」として説明するものの出どころ
- [Louis Bavoil et al., "Image-Space Horizon-Based Ambient Occlusion" (NVIDIA, 2008)](https://developer.nvidia.com/sites/default/files/akamai/gamedev/docs/HBAO_SIG08b.pdf)
  今日の手法の次の世代(HBAO)。改造課題3の下敷きになる。
  **点を数えるのではなく地平線角を求める**という発想の違いを見ておくとよい

## 理論の要点

### 1. AO は「レンダリング方程式の可視項」を先に抜き出したもの

Day 35 で書いたレンダリング方程式には、実は項が1つ足りていなかった。

```
  Lo(V) = ∫ f(V, L) * Li(L) * V(L) * (N・L) dω
                              ~~~~
                              可視項。その方向に何も無ければ 1、遮られていれば 0
```

平行光源1本のときは、これがシャドウマップ(Day 33)そのものだった。
IBL(Day 36)では**この項を丸ごと 1 と置いていた**——
「まわりの景色は全部見えている」という仮定で焼いたのが放射照度マップ。

環境遮蔽はその仮定を緩める。ただし方向ごとに解くのではなく、
**方向によらない1つの係数**に潰す。

```
  AO(P) = (1/π) ∫ V(L) (N・L) dω        … 半球のうち、どれだけ空が見えるか
```

これを環境光に掛ける。**厳密には嘘**で、遮られている方向と
明るい方向がずれていれば結果は合わない。それでも使うのは、
**方向を捨てたぶん、1画素あたり1つの数字で済む**から。

### 2. 「画面に写っているもの」だけで遮蔽を測る

正しく AO を出すなら、点 P から半球じゅうへレイを飛ばして当たり判定をする。
三角形が 100 万個あればその 100 万個が相手になる。

SSAO の発想は、**その世界の模型として深度バッファを使う**こと。

```
  1. P のまわりの半球に、点をばらまく
  2. その点が「画面のどこに写るか」を計算する
  3. その画素にすでに描かれている面が、点より手前にあるか?
       手前にある = その方向は何かで塞がっている
```

**当たり判定を1回もしていない**のが肝で、やっているのは
射影と、テクスチャの読み込みと、大小比較だけ。
だからコストは**画素数 × 標本数**で決まり、
三角形が 100 万個あろうと 100 個だろうと同じ時間で終わる。

代償は「深度バッファは世界の模型として不完全」なこと。

| 失われるもの | どう見えるか |
|---|---|
| 画面の外にあるもの | カメラを振ると画面の縁の暗がりが揺れる |
| 手前の物の裏側 | 柱の縁に沿って、背景側が不自然に明るい/暗い |
| 厚み | 薄い板と分厚い柱が同じに扱われる |

**この3つは手法の欠陥ではなく定義**で、どんなに丁寧に実装しても消えない。
消したければ画面の外の情報を持つしかなく、それが
ボクセル化(VXAO)やレイトレーシング(RTAO)へ進む道になる。

### 3. 半球でなければならない

標本を球全体にばらまくと、**平らな面でも半分が「遮られている」と出る**。
面の裏へ潜った点は、必ず面より奥にあるから。
...
Crytek の初代 SSAO(2007)はこれを球でやっていて、
**シーン全体が一律に薄暗くなる**のが特徴的な絵だった。
`1 - 0.5 = 0.5` が「何も無いところ」の値になるので、
そこから明るさを持ち上げる調整が必要になる。

半球にするには、**法線を軸にした接空間**が要る。Day 34 で法線マップのために
組んだ TBN とまったく同じ手順で、違うのは接線を UV の傾きではなく乱数から作ること。

### 4. 少ない標本は「ノイズにしてからぼかす」

半球の積分を 16 点や 32 点で済ませているので、当然誤差が出る。
**全画素で同じ 16 方向を使うと、誤差も全画素で同じ形になる**——
これが縞やまだら(バンディング)として出る。

そこで画素ごとに接空間をランダムに回す。すると誤差は
**規則的な模様から、目に付きにくい砂嵐へ変わる**。

砂嵐はぼかせば消える。縞はぼかしても縞のまま残る。
これが「ノイズを入れてからぼかす」が定番になる理由で、
Day 36 の事前フィルタ(重点サンプリング)とも、
今後のテンポラル AA(Day 38 以降)とも同じ発想の系列にある。

**ぼかしの幅はノイズの周期に合わせる**。4x4 で敷いたなら 4x4 で平均する。
そうすると 16 通りがちょうど1回ずつ混ざり、模様がきれいに消える。
5x5 や 3x3 だと割り切れず、薄い格子が残る。

### 5. 遠くの物に遮られたことにしない

素朴に「手前に面があれば遮蔽」とすると、
**柱の向こうにある遠くの壁**まで遮蔽物と数えてしまう。
柱の縁に沿って、背景が黒く縁取られる形で出る。

対処は、深度の差が半径を超えたら効き目を落とすこと。

```glsl
  float rangeCheck = smoothstep(0.0, 1.0, uRadius / abs(P.z - sampledZ));
```

**この1行がスクリーンスペースの性格を全部表している**。
本当の遮蔽を測っているなら、こんな補正は要らない。
深度バッファという不完全な模型を相手にしているから、
「模型として信用できる範囲」を自分で区切る必要がある。

### 6. 半径はシーンの大きさで決まる。万能の値は無い

SSAO のつまみの中で、**半径だけが物理的な長さ**を持っている。
1辺 1m の箱が並ぶシーンなら 1m 前後、
机の上の小物なら 5cm、街並みなら 3m——**その都度決めるもの**。

作例の数字をそのまま持ってくると効かない、というのがここの落とし穴で、
LearnOpenGL が 0.5 なのはあちらの箱が小さいから。
このリファレンスは 1辺 1m の立方体が並ぶ縮尺なので **1.0m** にしてある。

半径を上げると暗がりは広がるが、**遠くの物まで遮蔽物と数え始める**。
上の要点5の綱引きがそのまま出るので、
「広く効かせたい」と「嘘を増やしたくない」の間で決めることになる。

### 7. AO を直接光に掛けてはいけない

AO は<b>まわりから回り込んでくる光</b>がどれだけ遮られるかの値なので、
太陽から一直線に来る光とは無関係。
遮っているものがあるなら、それは影(Day 33)の仕事。

掛けてしまうと、日向の壁際まで暗くなり、**絵全体が薄汚れる**。
`Ctrl+F11` でわざとそうできるようにしてある——
「なんとなく汚い」がどこから来るのかを一度見ておくと、
他人の絵を見たときにも気づけるようになる。

同じ話が Day 33 でも出ている(影は直接光にだけ掛ける)。
**AO と影はちょうど裏返し**で、掛ける先が入れ替わっている。
...
| ファイル | 役割 |
|---|---|
| `Render/Ssao.cs` | **SSAO 一式**。幾何バッファを持ち、遮蔽を計算し、ぼかして配る |
| `shaders/geometry.vert` / `geometry.frag` | 1パス目。**ビュー空間の法線と距離**を1枚に書く |
| `shaders/ssao.frag` | 2パス目。半球にばらまいて数える。**今日の主役** |
| `shaders/ssao-blur.frag` | 3パス目。4x4 の単純平均でノイズのタイルをならす |
| `shaders/ssao-view.frag` | 遮蔽率 / ビュー法線 / 距離 を全画面に出す(Ctrl+F2) |

### 変更ファイル

| ファイル | 変更 |
|---|---|
| `Render/Texture.cs` | `RenderTargetFormat.R8`。**1成分の描き込み先**(AO は数字ひとつ) |
| `Render/Framebuffer.cs` | `BytesPerPixel`。形式が3つになったので表に分けた |
| `Render/Shader.cs` | `SetVector3Array`。カーネル 64 本を **1回の glUniform3fv** で送る |
| `shaders/textured.frag` | 遮蔽率を環境光に掛ける。uniform 4本、成分 21 |
| `Program.cs` | `RenderSsaoPass`、`_ssao.Apply`、`Ctrl+F1〜F11`、HUD の1行、`RunSsaoCheck` |

新規が 1045 行、変更ファイルへの追加が 966 行で、**合わせて約 2000 行**。
Day 35(約 1700 行)と同じくらい、Day 36(約 2300 行)より小さい。

### 写経する順番

依存の下から。**バッファの形式から先に手を入れる**のが今日の順番で、
`Ssao` はその上に乗る。

1. **`Render/Texture.cs`**(変更)
   `RenderTargetFormat` に `R8` を足し、`CreateTarget` を
   `(内部形式, 転送形式, 型)` の3つ組に変える。**転送形式も `Red` にそろえる**
2. **`Render/Framebuffer.cs`**(変更)
   `ByteSize` の三項演算子を `BytesPerPixel` に切り出す。
   ここを直さないと HUD の VRAM が 4 倍に見える
3. **`Render/Shader.cs`**(変更)
   `SetVector3Array` を `SetVector3` と `SetVector4` の間に。
   **配列の uniform は「先頭の位置 + 個数」で書ける**
4. **`shaders/geometry.vert`** / **`geometry.frag`**(新規)
   `uModelView` と `uProjection` に分けるのが本描画との違い。
   **ビュー空間の位置が途中で要る**ので、透視投影の手前で切る
5. **`shaders/ssao.frag`**(新規)
   uniform 群 → **`ViewPosition`**(深度から位置を戻す) → `main`。
   `uSampleStride` を忘れないこと(下の「検証の途中で分かったこと」)
6. **`shaders/ssao-blur.frag`**(新規)
   4x4 の単純平均。**ノイズの周期と同じ幅**であることが効いている
7. **`shaders/ssao-view.frag`**(新規)
   3つのモード。距離は線形なので `shadow-view.frag` のような戻しが要らない
8. **`Render/Ssao.cs`**(新規)
   `SsaoDebugView` → 定数 → フィールド → コンストラクタ → プロパティ群 →
   `BeginGeometry` / `Draw` / `EndGeometry` → **`Compute`** → `Apply` →
   `DrawDebug` → `Resize` / `SetHalfResolution` / `ReloadShaders` →
   **`BuildKernel`**(半球の作り方)→ `CreateNoiseTexture` → `NormalMatrix` →
   `CreateOcclusionBuffer` → `ResizeOcclusionBuffers` →
   **`ComputeOcclusion`** → `BlurOcclusion` → `DrawFullscreen` → `SetCap` → `Dispose`
9. **`shaders/textured.frag`**(変更)
   ユニットのコメントに 7〜10 → Day 37 の uniform 4本 →
   **`ScreenSpaceOcclusion()`** を `SrgbToLinear` の直前へ →
   `occlusion` の直後に `ssao` / `ambientOcclusion` / `directOcclusion` →
   成分 21 → 環境光の 5 か所を `ambientOcclusion` に差し替え →
   直接光の 3 か所に `directOcclusion` を掛ける
10. **`Program.cs`**(変更)
    Day 37 のフィールド2つ → `OnLoad`(生成とコンソール1行)→
    `OnFramebufferResize` の `_ssao.Resize` →
    `OnRender`(`RenderSsaoPass()` と `_ssao.DrawDebug`)→
    **`RenderSsaoPass`** → `Render3D` の `_ssao.Apply` →
    `GridMatrix` / `SurfaceWallMatrix` / `SurfaceFloorMatrix` の切り出しと
    `RenderMaterialGrid` / `RenderSurfaceDemo` の差し替え →
    `DebugChannelLabel` の 21 と巡回を 22 → `SsaoViewLabel` / `SsaoLabel` →
    HUD の行 → `OnKeyDown`(`Ctrl+F1〜F11`。**`case Key.F5:` より必ず上**)→
    **`RunSsaoCheck`** と補助4つ → F5 の `_ssao.ReloadShaders()` →
    `_ssao.Dispose()` → 操作説明 → クラス要約に Day 37 の項
11. **`Day37.csproj`**(リネームのみ)
    中身は Day 36 と同じ

## 設計書

**クラスが1つ増えた**(`Render/Ssao`)。層の構成は Day 36 のままで、
今日も全部 `Render/` の中に入る。

| 増えたもの | 何をするか |
|---|---|
| `Render/Ssao` | 幾何バッファを焼き、遮蔽率を計算し、ぼかして配る。**シーンを知らない** |
| `SsaoDebugView` | 中間結果のどれを全画面に出すか(Ctrl+F2) |

`Render/` の既存クラスも少し太った。

| 変わったもの | 何が増えたか |
|---|---|
| `RenderTargetFormat` | `R8`。**1成分の描き込み先**(1920x1080 で 8.3MB が 2.1MB) |
| `Framebuffer` | `BytesPerPixel`。形式が3つになったので三項演算子を表に切り出した |
| `Shader` | `SetVector3Array`。カーネル 64 本を1回の `glUniform3fv` で送る |

`Ssao` は `PostProcess`(Day 31)・`ShadowMap`(Day 33)・
`EnvironmentMap`(Day 36)に続く**4つ目の「自分でバッファを持ち、シェーダは借りる」クラス**。
...
**この形が4例あることの値打ち**は、Day 52(ディファードレンダリング)で
5つ目を足すときに設計を考え直さなくてよいこと。
Day 31 で「1回では終わらない描画が全部ここから始まる」と書いた予告が、
ここでちょうど回収された形になっている。

**設計の歪みが1つ表に出た**。今日、同じジオメトリを
**1フレームに3回描く**ようになった。

| パス | 何のために | 何を書くか |
|---|---|---|
| 影(Day 33) | 光から見た深度 | 深度だけ |
| 幾何(Day 37) | カメラから見た法線と距離 | 法線 + 距離 |
| 本描画 | 絵 | 色 |

`Program.cs` に**同じ描画リストが3つ**並ぶことになり、
どれか1つに足し忘れると「影が落ちない」「AO に穴が開く」という形で出る。
実際、今日 `GridMatrix` と `SurfaceWallMatrix` を切り出したのは、
グリッドと板の行列が本描画側にベタ書きされていて、
**幾何パスから同じ形を描けなかった**ため。

正しい直し方は**1回描いて複数のバッファへ同時に書く**(MRT / G バッファ)で、
それが Day 52 の主題になる。今日そこへ踏み込まないのは、
`Framebuffer` にカラーアタッチメントを複数挿す口が無く、
足すと差分が今日の主題から外れて広がるため。

Day 36 の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の3つ。

| 図 | 何が変わったか |
|---|---|
| `Render` のクラス図 | `Ssao` / `SsaoDebugView` を追加。`RenderTargetFormat` / `Shader` に追記 |
| 1フレームの流れ | **幾何パスと遮蔽の計算**が影パスの次に入った |
| 1画素の色が決まるまで | 環境光に掛ける AO が「マップ × 画面から作った遮蔽」になった |
...
`dotnet run --project reference/Day37 -c Release` で起動する。
HUD に `SSAO:32本  半径:1.00m  下駄:0.025  強さ:1.0  ぼかしON  960x640  幾何:N回  パス:2  0.29ms  7.6MB`
の行が増えていること。

### 1. `Ctrl+F9`: SSAO が効く構図

**今日の到達点はこの1枚**。立方体の根元に寄った構図が出る。
スプライトの群れは消える(`PageUp` で戻る)。

| 見るところ | 期待 |
|---|---|
| **立方体の根元** | 床側に暗がりの帯。**幅は半径 1m ぶん**で、外へ行くほど薄れる |
| 立方体の下側の面 | 床からの照り返しが減るぶん、上側より暗い |
| 左奥の立方体 | 同じ帯が出る。**距離によらず効く**(スクリーンスペースなので) |
| 開けた床 | 何も起きない。遮るものが無いので AO = 1.0 |

`Ctrl+F1` で切ると、**立方体が床から浮いて見える**のが分かる。
Day 36 までの絵がそれだった、というのが今日の主張。

### 2. `Ctrl+F2`: 遮蔽率を全画面に出す

**今日いちばん使う窓**。1回押すと遮蔽率、2回でビュー法線、3回で距離。

| 表示 | 見どころ |
|---|---|
| 遮蔽率 | 白が「遮られていない」。**立方体の根元と、面と面の内角が灰色になる** |
| ビュー法線 | 面ごとに一様な色。**カメラを回すと色が変わるのが正しい**(ビュー空間なので) |
| 距離 | 近いほど白い。**線形なので手前から奥へ素直に暗くなる**(深度バッファの絵と違う) |

ビュー法線の表示で色が変わらないなら `uNormalMatrix` が効いていない。
距離が真っ白か真っ黒なら、`vViewDepth` の符号を疑う。

### 3. `Ctrl+F5`: ぼかしを切る

**4x4 の格子模様がはっきり出る**のが正解。
標本が 32 本しか無いことによるノイズが、ノイズテクスチャのタイルの形で残っている。

`Ctrl+F4` で標本数を 8 まで落とすと、格子がさらに粗くなる。
**ノイズを消しているのは標本数ではなくぼかし**であることが、この2つの組み合わせで分かる。

自己チェックの実測(ノイズのタイル1枚の中のばらつき)。

```
  ぼかし前 0.0847 → ぼかし後 0.0367     16 本
  8本 0.0815 → 64本 0.0505              ぼかし無し
```

### 4. `Ctrl+F8`: 下駄を 0 にする

**平らな床一面にうっすら砂が乗る**のが正解。
自分自身を遮蔽物と数えてしまっている——Day 33 のシャドウアクネと同じ理屈。

**平均値はほとんど動かない**(1.000 のまま)ので、絵を見るときも
「暗くなった」ではなく「ざらついた」を探すこと。
自己チェックはタイルの中のばらつきで測っている(下駄なし 0.0221 / 下駄あり 0.0000)。

### 5. `Ctrl+F3`: 半径を動かす

| 半径 | 見え方 |
|---|---|
| 0.25m | 接地の線が細い。**ほとんど効いていないように見える** |
| 0.5m | 線がやや太る |
| 1.0m(既定) | 立方体の縮尺に合う。根元に帯が出る |
| 2.0m | 広く暗くなるが、**離れた床まで暗がりが伸びる**(遠くの物を遮蔽物と数え始める) |

**半径だけが物理的な長さを持つつまみ**なので、
シーンの縮尺を変えたら必ず見直すことになる(要点6)。

### 6. `Ctrl+F11`: AO を直接光にも掛ける

**日向の壁際まで暗くなり、絵全体が薄汚れる**のが正解。これは誤用。
HUD に `[直接光にも適用=誤用]` が出る。

AO は環境光にだけ掛かるもので、
太陽が遮られていることは影(Day 33)が別に扱っている(要点7)。

### 7. `Ctrl+F6`: 半解像度

遮蔽バッファが 480x320 になる。画素数は 1/4。

**VRAM はあまり減らない**(7.6MB → 6.7MB)のが見どころで、
幾何バッファ(RGBA16F + 深度)が大半を占めているため。
絵のほうは、輪郭のあたりで AO がにじむ。

### 8. `Ctrl+F10`: 自己チェック

13 項目すべて `[OK]` になること。
落ちたときにどこを疑うかは、項目名がそのまま指している。

| 落ちた項目 | 疑うところ |
|---|---|
| 標本が全部 z >= 0 | `BuildKernel` の z を `-1〜1` にしていないか |
| 焼いた距離が一致 | `geometry.vert` の `vViewDepth` の符号、`uModelView` の掛ける順 |
| 焼いた法線が一致 | `NormalMatrix` にビュー行列を入れ忘れていないか |
| 空は遮られない | `ssao.frag` の `depth <= 0.0` の枝 |
| 接地部は暗い | 比較の向き(`sampledZ >= samplePos.z + bias`) |
| ぼかしがばらつきを消す | `uTexelSize` が遮蔽バッファの大きさになっているか |

## 改造課題

### 課題1(易): 遮蔽率を画面に出さずに数字で確かめる

`Ctrl+F2` の全画面表示は便利だが、**「この点はいくつか」には答えてくれない**。

マウスカーソルの下の画素の遮蔽率を読み返して HUD に出せ。
`RunSsaoCheck` の `ReadOcclusion` がそのまま使える
(`_input.Mice[0].Position` が窓の左上原点なので、**Y を反転する**必要がある)。

**毎フレーム `glReadPixels` を呼ぶと GPU の同期待ちが入る**ので、
fps がどれだけ落ちるかも一緒に見ておくとよい——
これが「デバッグ表示は画面に出すのが安い」理由になっている。

### 課題2(中): 半径をワールドではなくスクリーンで頭打ちにする

いまの実装は半径をワールド単位で持っているので、
**カメラが遠ざかると、探る範囲が画面上で数画素まで縮む**。
すると標本が全部同じテクセルを読み、遮蔽が出なくなる。

`ssao.frag` で、投影後の半径が一定の画素数を下回らないように制限せよ。
ヒント: ビュー空間の半径 `r` は、画面上ではおよそ
`r * (画面の高さ/2) / (tan(fov/2) * depth)` 画素になる。

逆に**近づきすぎると画面の半分を探る**ことになって重くなるので、
上限も要る。実際のエンジンはどちらも入れている。

### 課題3(難): ぼかしを深度でエッジを守る形にする

いまの `ssao-blur.frag` は 4x4 の単純平均なので、
**手前の物と奥の壁の境目でも混ぜてしまう**。
柱の輪郭のまわりで AO がにじみ、輪郭が二重に見える。

幾何バッファの距離も一緒に読み、
中心との深度差が大きい画素の重みを落とせ(バイラテラルフィルタ)。

```glsl
  float w = exp(-abs(centerDepth - sampleDepth) / uDepthSigma);
```

**テクスチャの読み込みが 16 回増える**ので、
Ctrl+F10 の計測でコストがどれだけ上がるかを必ず測ること。
値打ちに対して割に合うかを自分で判断するのが、この課題の主題。

さらに進めるなら、**法線の差でも重みを落とす**と内角のにじみが止まる。
そこまで行くと HBAO(事前に読む資料の最後)の入口に立っている。
...
```
[SSAO の自己チェック]
  [OK] **標本が全部 z >= 0(半球)**  ※球にすると平らな面まで 0.5 になる  64 本
  [OK] 標本が単位球の中にある
  [OK] 標本が中心寄りに密(前半の平均長 < 後半の平均長)  前半 0.103 / 後半 0.314
  [OK] **焼いた距離が CPU の計算と一致**  焼いた 6.336 / 期待 6.344(誤差 0.1%)
  [OK] **焼いた法線がビュー空間の床の法線と一致**  内積 1.0000
  [OK] **空は遮られない**(距離 0 を素通りしている)  実際 1.000
  [OK] 開けた床はほぼ遮られない  実際 1.000
  [OK] **立方体の接地部は暗い**  接地 0.675 / 開けた床 1.000
  [OK] 半径 0 なら遮蔽ゼロ  実際 1.000
  [OK] **下駄 0 では平らな床に自己遮蔽が出る**  下駄なし 0.0221 / 下駄あり 0.0000
  [OK] 標本を増やすとノイズが減る  8本 0.0815 → 64本 0.0505
  [OK] **ぼかしがタイルの中のばらつきを消す**  ぼかし前 0.0847 → ぼかし後 0.0367
  [OK] 半解像度にすると遮蔽バッファが小さくなる  7.6MB ⇔ 6.7MB
  すべて合格
```

### 幾何パス + 遮蔽の計算にかかる時間

床1枚 + 立方体1個を描いて 200 回まわした平均(`glFinish` 込み)。

| 遮蔽バッファ | 8本 | 16本 | 32本 | 64本 |
|---|---|---|---|---|
| 960x640(原寸) | 0.173ms | 0.248ms | 0.375ms | 0.383ms |
| 480x320(半分) | 0.101ms | 0.091ms | 0.097ms | 0.166ms |

**原寸の 8本と 16本の差(0.075ms)より、半解像度にしたときの差のほうが大きい**。
標本数はループの回数なので素直に効きそうに見えるが、
実際に効いているのは**テクスチャの読み込み回数 × 画素数**で、
32本と 64本がほとんど変わらないのは、この規模では
帯域ではなく幾何パスと固定費のほうが支配的になっているため。

**大きい画面ほど標本数が効いてくる**ので、1920x1080 で測ると傾きが変わる。
「自分の環境で測る」が結論になるのは Day 31 から変わらない。

### 検証の途中で分かったこと

**1. カーネルを先頭から取ると、いちばん短い標本だけになる**

`BuildKernel` は「中心寄りに密」にするため、
標本を**短いものから長いものの順**に作っている。
そこへ「標本数を減らすときは先頭から N 本」と素直に書くと、
**8本にした瞬間に手元 5cm しか探らなくなる**。

最初はそれで、標本数を 8 に落とすと AO が全部 1.000 になった。
半径を上げても何も起きないので、しばらく `uRadius` の送り忘れを疑っていた。

直し方は間引いて読むこと(`uSampleStride = 64 / uSampleCount`)。
**「配列の順序に意味がある」ことを、使う側が知っている必要がある**——
順序に意味を持たせた時点で、部分集合の取り方も設計の一部になる。

**2. 平らな面の自己遮蔽は、平均値では捕まえられない**

下駄(`uBias`)を 0 にしたときの不具合を確かめようとして、
最初は「開けた床の AO が下がるはず」と考えた。**下がらなかった**(1.000 のまま)。

理屈のうえでも下がらない。完全に平らな面では、半球の標本は必ず面より手前にあるので、
下駄が無くても遮蔽と判定されない。実際に出るのは
**深度の量子化と長さがほぼ 0 の標本による、当たり外れのばらつき**——
つまり平均ではなく分散に出る。

そこで自己チェックの物差しを**ノイズのタイル1枚の中の標準偏差**に変えた
(`TileNoise`)。4x4 の中では幾何がほぼ同じで乱数の向きだけが違うので、
本物の濃淡が混ざらない。下駄なし 0.0221 / 下駄あり 0.0000 と、はっきり出る。

**「何を測れば違いが出るか」を先に決める**のが、この種の確認では半分を占める。

**3. 確認用の点をカメラの後ろに置いていた**

自己チェックで「開けた床」の点を `(4.5, -0.5, 3.5)` に取ったところ、
焼いた距離が **-0.239**(負)になった。カメラの後ろだった。

透視除算は `w` が負でも計算できてしまうので、
**符号が反転した座標が返り、画面の反対側の画素を読む**。
エラーにならず、それらしい数字が返るのがたちが悪い。

`WorldToPixel` に「カメラの前にあるか」の確認を入れる手もあるが、
自己チェックが「焼いた距離が CPU の計算と一致」を見ているので、
**そこで必ず落ちる**——チェックが1つあれば、点の取り違えもまとめて捕まる。

**4. スプライトの群れが 3D の上に乗ったままだった(Day 34〜36 の不備)**

`Ctrl+F9` の構図を確認しようとしたら、**1000 枚のスプライトが全面を覆っていた**。

調べると、`OnRender` は「モデル表示中」と「ゲーム中」しかスプライトを飛ばしていなかった。
材質グリッド(Ctrl+Shift+5)と材質テストの板(Alt+5)を選ぶ分岐は
**`Render3D()` の中**にあって、`OnRender` からは見えていない——
だから両方とも「ふつうのデモ」の枝に落ち、スプライトの群れとロードの帯が上に重なる。

**Day 34〜36 の計画書の「1フレームの流れ」は、正しいほうの絵を描いていた**
(`RG --> TX` と `RS2 --> TX` でスプライトを飛ばしている)。
図を実装から起こさずに意図から描くと、**コードより先に図が嘘になる**——
Day 36 の設計書で `Core` ⇔ `Render` について書いたのと同じことが、ここでも起きていた。

症状は3つあった。

| | 何が起きるか |
|---|---|
| 1 | グリッドと板の上にスプライトが重なり、**材質も凹凸も読めない** |
| 2 | HUD の `DC` がグリッドの 49 ではなく**スプライトのバッチ数**になる(後から上書きされる) |
| 3 | `G`(3D 背景)を切ると、**グリッドや板まで消える**(`if (_draw3D)` の内側なので) |

直し方は、モデルの枝とまったく同じ扱いにすること。
`OnRender` に枝を1本足し、Day 34(板)で導入して Day 35(グリッド)で条件を広げた。
...
これで計画書の図とコードが一致し、3つの症状が同時に消える。
**Day 34・35・36 の reference にも同じ修正を入れてある**
(Day 36 の `Ctrl+Alt+9` は、これでようやく計画書どおりの1枚が出る)。

今日の `Ctrl+F9` はそれでも `SetSpriteCount(0)` を呼んでいる。
こちらの見どころは**ふつうのデモの立方体と床**——専用の表示モードではないので、
上の枝には落ちないため。`PageUp` で戻せる。
