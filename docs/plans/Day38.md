...
Phase 6 の8日目。**輪郭の階段が消えて、絵に色が付く**。

## 今日のゴール

`Shift+F12` で市松の床が奥まで見える構図が出て、**画面の左半分だけが階段のまま**になる。
`Shift+F11` で比較を切れば、全面が均された絵になる。

```
  Shift+F12  FXAA が効く構図。**今日の到達点はこの1枚**(左=加工前 / 右=加工後)
  Shift+F1   FXAA ON / OFF(OFF で Day 37 に戻る)
  Shift+F2   輝度 / 縁の検出 / 混合量 を全画面に出す(**今日いちばん使う窓**)
  Shift+F4   効き(低 / 中 / 高 / 極)
  Shift+F5   UI を後処理に通すか(**Day 37 までの置き方に戻す**)
  Shift+F6   カラーグレーディング ON / OFF
  Shift+F7   下敷き(夕暮れ / 月夜 / 退色 / モノクロ)
  Shift+F8   コントラスト   Shift+F9   彩度   Shift+F10  色温度
  Shift+F11  左右比較(FXAA / グレーディング)
  Ctrl+F12   今日の自己チェック
```

Day 31〜37 で足してきたのは、どれも**光の計算**だった。
HDR、影、法線、PBR、IBL、環境遮蔽——全部「この点はどれだけ明るいか」の話で、
足すたびに絵は物理に近づいてきた。

今日はそこから離れて、**画面に出す最後の2手**を入れる。

1つ目は **1画素につき1回しか色を決めていないことの帳尻**。
三角形の縁が画素の途中を通っても、その画素は「入っている」か「いない」の
どちらかに倒れるので、斜めの縁が段々になる(ジャギー)。
正攻法は画素の中で何回も色を決めること(MSAA)だが、
後処理と噛み合わないので、**もう出来上がった絵を見て均す**ほうを採る。これが FXAA。

2つ目は **絵作り**。物理的に正しい明るさが出たあと、
「どういう絵にしたいか」を決める段が要る。
ホワイトバランス・コントラスト・彩度の3つだけでも、
同じシーンが夕暮れにも月夜にもなる。**ここから先は物理ではなく好み**で、
Day 31 のトーンマッピングと地続きの話になっている。

併せて、**UI が後処理の外に出る**。
Day 31 から HUD の文字はシーンと同じバッファに描かれていたので、
露出を上げると文字まで白飛びしていた。FXAA は文字を滲ませるので、
今日それを分けないと絵が成立しない——Day 37 の `OnRender` に
「分けるのは Day 38」と書いてあった宿題がこれ。

## 事前に読む資料

- [Timothy Lottes, "FXAA" (NVIDIA, 2009)](https://developer.download.nvidia.com/assets/gamedev/files/sdk/11/FXAA_WhitePaper.pdf)
  **今日の本命**。FXAA の一次資料。
  「輝度で段差を見つけ、縁に沿って混ぜる」という骨格と、
  **なぜガンマ後の値を入力に取るのか**がここに書いてある
- [FXAA 3.11 のソース](https://github.com/GameTechDev/CMAA2/blob/master/Projects/CMAA2/FXAA/Fxaa3_11.h)
  製品版の実装。1500 行あるが、読むのは `FxaaPixelShader` の
  前半 100 行だけでよい。今日書くのはここを削ぎ落としたもの
- [Alex Fry, "High Dynamic Range color grading and display in Frostbite" (SIGGRAPH 2017)](https://www.ea.com/frostbite/news/high-dynamic-range-color-grading-and-display-in-frostbite)
  **グレーディングをどの空間で、どの順番で掛けるか**の決定版。
  今日「トーンマップの前」に置く理由がここで裏取りできる
- [Krzysztof Narkowicz, "ACES Filmic Tone Mapping Curve"](https://knarkowicz.wordpress.com/2016/01/06/aces-filmic-tone-mapping-curve/)
  Day 31 で使った曲線の出どころ。今日はその**手前**に処理を挟むので、
  「何を入れる前提の曲線なのか」をもう一度見ておくとよい
- [Unity HDRP の White Balance](https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@17.0/manual/White-Balance.html)
  今日の色温度の実装はこれと同じ形(LMS 空間での von Kries 順応)。
  **つまみの目盛り(-100〜100)も合わせてある**ので、
  Unity で触ったことがあれば感覚がそのまま移る
- [Jorge Jimenez et al., "Filmic SMAA" (SIGGRAPH 2016)](https://www.iryoku.com/smaa/)
  FXAA の次の世代。**改造課題3の下敷き**で、
  「縁を検出してから、その形を分類して重みを引く」という発想の違いを見ておくとよい

## 理論の要点

### 1. ジャギーは「1画素につき1回しか色を決めていない」ことの帰結

Day 14 から使っているラスタライザは、三角形の中に**画素の中心が入っているか**で
塗るかどうかを決めている。縁が画素の途中を通っても、その画素は 0 か 1 に倒れる。
...
正しい答えは「その画素の面積のうち何割が三角形に入っているか」で、
これを**被覆率**と呼ぶ。被覆率が出せれば階段は消える——
斜めの縁に沿って、0 と 1 の間の値がなだらかに並ぶからだ。

アンチエイリアスの手法は、結局のところ**この被覆率をどう手に入れるか**の違いになる。

| 手法 | 被覆率の出し方 | 代償 |
|---|---|---|
| SSAA | 4倍の解像度で描いて縮める | **全部が4倍**(描画も帯域もメモリも) |
| MSAA | 深度と被覆だけ4点で取り、色は1回 | バッファが4倍。**後処理と噛み合わない**(要点2) |
| FXAA | **出来上がった絵から推測する** | フルスクリーン1パス。細い線が滲む |
| TAA | 前のフレームからもらう(Day 53) | 動くものに残像。実装が重い |

今日入れるのは3行目。**被覆率を測るのではなく、
「階段になっているところを見つけて、それらしい中間値を置く」**という割り切りで、
情報が増えるわけではないぶん、限界も最初から決まっている。

### 2. MSAA が後処理と噛み合わない

MSAA は「深度と被覆は4点、色の計算は1回」という賢い節約で、
前方レンダリング(Day 37 までの形)なら品質もコストも申し分ない。
ではなぜ現代のゲームで主流でなくなったのか。

理由は3つあり、**どれも Day 31 以降に自分で作ったものが原因**になっている。

1. **HDR バッファが4倍になる**。1920x1080 の RGBA16F は 15.8MB で、
   4x MSAA なら 63MB。ブルームの入力を作るには「解決(resolve)」も要る
2. **解決の順番が難しい**。トーンマップの前に平均すると、
   明るい画素1点が全体を白く引きずる(Day 31 で見た「畳む前と後」の話)
3. **ディファードだと G バッファまで4倍になる**(Day 51)。
   位置・法線・材質を全部4標本ずつ持つのは、帯域として現実的でない

FXAA はこれを全部避ける。**入力は最終画像1枚だけ**なので、
パイプラインのどこにも触らずに後ろへ足せる。
2009 年に出てから 10 年以上、事実上の標準として使われ続けた理由がここにある。

おまけとして、**ポリゴンの縁以外にも効く**。
MSAA は「三角形の縁」でしか標本を増やさないので、
テクスチャの模様や高輝度部の縁(今日のデモなら市松の床)はまったく直らない。
FXAA は絵しか見ていないので、そこも同じように均す。

### 3. FXAA が見ているのは輝度。それも**ガンマを掛けたあと**の値

`fxaa.frag` が最初にやるのは、中心と斜め4点の**輝度**を取ることだけ。

```glsl
float Luma(vec3 color) { return dot(color, vec3(0.299, 0.587, 0.114)); }
```

色ではなく輝度で判断してよいのは、**人の目が明るさの差に敏感で色の差に鈍い**から。
3成分を別々に見ても判定はほとんど変わらないのに、計算は3倍になる。

ここで大事なのは**どの空間の値に掛けるか**。

- 係数 (0.299, 0.587, 0.114) は NTSC 由来で、**ガンマ後の値**に掛けるもの
- 係数 (0.2126, 0.7152, 0.0722) は Rec.709 の輝度で、**リニアな明るさ**に掛けるもの

今日は両方が1つのファイルの中に出てくる——
`composite.frag` の彩度計算は後者、`fxaa.frag` は前者。
**同じ「輝度」でも係数が違う**のはそのためで、取り違えると
暗部の重みがずれる(絵は出るので、なかなか気づけない)。

そして FXAA が**ガンマ後の絵**を入力に取るのも同じ理由。
リニアな明るさで段差を測ると、暗部の差が小さく出すぎて縁を見逃す。
人の目は暗部の差にこそ敏感なので、その感覚に近いのはガンマ後の値のほうになる。

**だから FXAA はパイプラインのいちばん後ろにしか置けない**。
今日 `PostProcess` の出口が2段になった(合成 → LDR バッファ → FXAA → 画面)のは、
この制約がそのまま形になったもの。

### 4. 混ぜるのは「縁をまたぐ方向」ではなく「縁に沿う方向」

FXAA でいちばん誤解されやすいのがここ。

```glsl
dir.x = -((lumaNW + lumaNE) - (lumaSW + lumaSE));  // 上下の差 → x へ
dir.y =  ((lumaNW + lumaSW) - (lumaNE + lumaSE));  // 左右の差 → y へ
```

上下の差が大きい(=**横に走る縁**)とき、`dir` は横を向く。
つまり **`dir` は縁に沿った方向**で、そちらへ数テクセル進んだ色を混ぜる。

縁をまたいで混ぜたら、それはただのぼかしで、
輪郭が甘くなるだけで階段は消えない。
縁に沿って混ぜると何が起きるか——**階段の1段先の色が入ってくる**。
...
段差の角に中間色が置かれ、被覆率のプロファイルが直線に近づく。
これが今日の自己チェックで測っているものそのもので、
**「列ごとの白の量が、理想の直線からどれだけずれているか」**が
0.50 画素から 0.30 画素へ下がる(実測)。

`dir` の長さも意味がある。小さいほうの成分で正規化してあるので、
**縁の傾きがそのまま歩幅になる**——45 度なら両成分が 1 テクセル、
ほとんど水平な縁なら横成分だけが `uSpanMax` まで伸びる。
傾きが緩い縁ほど階段の段が長いので、遠くまで取りに行く必要がある、という理屈。

### 5. FXAA が苦手なもの。だから UI を外に出す

情報を増やしていないので、**本当は縁でないものまで縁に見えると滲む**。

- **1画素幅の線**。両側に段差があるので必ず縁と判定され、両方向から混ざる
- **文字**。1画素幅の線の塊なので、いちばん犠牲になる
- **時間方向のちらつき**。前のフレームを知らないので、
  カメラが少し動くたびに「縁と判定されるか」が画素ごとに入れ替わる

1つ目と2つ目への対処は簡単で、**FXAA を通さないところに描けばよい**。
今日 UI を後処理の外へ出したのはこれが決め手になっている
(露出やトーンマップから外れるのは、そのついでに直った副作用のほう)。

`Shift+F5` で Day 37 までの置き方に戻せる。露出(`Shift+5`)を上げると
HUD の文字が白飛びし、`Shift+F7` を4回押してモノクロにすると文字まで灰色になる。

3つ目は原理的に直せない。**時間方向の情報を持ち込む**のが唯一の解で、
それが TAA(Day 53)になる。

### 6. グレーディングはトーンマップの**前**。空間と順番が全部

`composite.frag` の中身は、今日で5段になった。
...
**3 が 4 の前**でなければならない。トーンマップは
「フィルムの特性曲線」に当たるものなので、味付けはフィルムに写す前——
まだ物理的な明るさである側——で行う。

後ろに置くとどうなるか。すでに 0〜1 に畳まれた絵のコントラストを上げれば、
上は 1.0 に張り付き、下は 0 に潰れる。**階調を作る前に階調を捨てる**ことになる。
前に置けば、コントラストで持ち上がったぶんもトーンマップが畳んでくれる。

グレーディング自身の中の順番も決まっている。

```
  ホワイトバランス → 色フィルタ → コントラスト → 彩度
  ←── 撮影条件の補正 ──→   ←── 表現としての味付け ──→
```

前半は「撮ったときの光の色を打ち消す」補正、後半は「どう見せたいか」。
実際の現像もこの順で、順番を入れ替えると
**補正のつもりの操作が味付けに引きずられる**。

### 7. ホワイトバランスは RGB ではなく LMS で掛ける

色温度のつまみを「暖色寄りにしたいから赤を 1.1 倍、青を 0.9 倍」と実装すると、
**肌色が緑や紫に転ぶ**。RGB の3軸は人の目の3種類の錐体と一致していないので、
軸ごとに定数を掛けると色相が引きずられる。

人の目が「白いものを白く見続ける」仕組み(色順応)は、
**3種類の錐体それぞれの感度が独立に変わる**ことで起きている——
これが**フォン・クリースの順応則**(1902)。
錐体の応答を並べた空間が **LMS**(Long / Medium / Short 波長)で、
そこで各成分に定数を掛けるのが、いちばん人の見え方に近い白の付け替えになる。

やることは3段。

1. つまみから**目標の白点の色度 (x, y)** を作る。
   色温度を変えるとは「何 K の黒体が出す光を白とみなすか」を変えることで、
   その白点は CIE xy 平面上の1本の曲線(**プランク軌跡**)に載っている
2. その色度を **LMS へ変換**する(xyY → XYZ → LMS)
3. **基準の白との比**を取る。これが各成分の増幅率

`ColorGrade.ComputeWhiteBalance` がここを担当する。
**1フレームに1回で済む定数**なので CPU に置き、
シェーダは受け取った vec3 を掛けるだけ(`composite.frag` の `WhiteBalance`)。

なお、コントラストのほうも同じく**空間の選択**が効いている。
リニアのまま `(x - 0.18) * c + 0.18` と書くと暗部が負になり、
露出(掛け算)との合成もおかしくなる。**対数の上で傾きを変える**と、
露出が平行移動、コントラストが傾きの変更になって素直に噛み合う——
人の明るさの感覚もほぼ対数なので、見た目にも自然になる。

中心を **18% グレー**に置くのは写真からの流用。
...
| ファイル | 役割 |
|---|---|
| `Render/ColorGrade.cs` | **グレーディングのつまみ一式**。GL の資源を1つも持たない、数字だけのクラス |
| `shaders/fxaa.frag` | **FXAA 本体**。輝度で段差を見つけ、縁に沿って混ぜる。デバッグ表示3種と左右比較つき |

### 変更ファイル

| ファイル | 変更 |
|---|---|
| `shaders/composite.frag` | グレーディングの4段(白点 / フィルタ / コントラスト / 彩度)と左右比較 |
| `Render/PostProcess.cs` | **LDR バッファと FXAA パス**。`End` を `EndCore` に割り、行き先を引数にした |
| `Program.cs` | `Shift+F1〜F12`、HUD の2行、`RunAaCheck`、**UI を後処理の外へ** |

新規が 475 行、変更ファイルへの追加が約 1300 行で、**合わせて約 1800 行**。
Day 37(約 2000 行)より少し小さい。

`Program.cs` の 900 行のうち 500 行あまりは自己チェック(`RunAaCheck` と補助6つ)。
**今日の2つは絵を見ても正しさが分からない**類のものなので、そのぶん厚くしてある。

### 写経する順番

グレーディング → FXAA → 配線、の順。
**シェーダを先に書く**のが今日の並びで、`PostProcess` はその2つを呼ぶだけ。

1. **`Render/ColorGrade.cs`**(新規)
   `GradePreset` → 定数 → プロパティ群 → `ApplyPreset` / `MarkCustom` / `Apply` →
   **`ComputeWhiteBalance`** → `StandardIlluminantY` → `CieXyToLms`。
   `Shader` の既存の setter しか使わないので、ここは単体で書き切れる
2. **`shaders/composite.frag`**(変更)
   冒頭のコメントを5段に → uniform 6本(`uGradeEnabled` 〜 `uGradeSplit`)→
   `LinearToSrgb` の**手前**に `LinearToLms` / `LmsToLinear` の2つの `mat3` と
   `WhiteBalance` / `Contrast` / `Saturation` / `Grade` →
   `main` の 2 と 3 の間に `Grade` の呼び出し → 段の番号を 4 / 5 へ
3. **`shaders/fxaa.frag`**(新規)
   uniform 群 → `Luma` / `Sample` → `main`(近傍5点 → しきい値 →
   **向きの決定** → 4点の混合 → 安全装置 → デバッグ表示 → 左右比較)。
   **`dir.x` に上下差、`dir.y` に左右差**を入れるところが要点4
4. **`Render/PostProcess.cs`**(変更)
   `FxaaQuality` / `FxaaDebugView` / `PostSplit` の3つの enum →
   クラス要約の図を2段に → `_ldr` と `_fxaaShader` のフィールド →
   コンストラクタ(`_ldr` の生成、`fxaa.frag` の読み込み、`SetFxaaQuality`)→
   プロパティ群(`Grade` / `FxaaEnabled` / …)と `ByteSize` への加算 →
   **`End` を `EndCore` に割り、`EndToTarget` を足す** →
   `EndCore` の中で FXAA の有無で行き先を分ける →
   `Composite` の引数を `Framebuffer?` に(`Grade.Apply` と `uGradeSplit`)→
   **`ApplyFxaa`** → `SetFxaaQuality` → `BindTarget` →
   `Resize` / `ReloadShaders` / `Dispose` に `_ldr` と `_fxaaShader`
5. **`Program.cs`**(変更)
   `_uiThroughPost` のフィールド → `OnLoad`(FXAA のコンソール1行と操作説明)→
   **`OnRender`**(4つの枝から `RenderText()` を抜き、`_post.End` の前後に置き直す)→
   `FxaaViewLabel` / `FxaaQualityLabel` / `SplitLabel` / `AaLabel` / `GradeLabel` →
   HUD の2行と1行目の `Day38` → `OnKeyDown`(`Shift+F1〜F12`。
   **`case Key.F5:` より必ず上**)→ `Ctrl+F12` の1件 →
   **`RunAaCheck`** と補助6つ(`DrawStaircase` / `CompositeProbe` / `BenchmarkPost` /
   `ReadRed` / `ReadPixel` / `MidTones` / `EdgeDeviation` / `Identical` / `MaxDifference`)→
   窓のタイトル → クラス要約に Day 38 の項
6. **`Day38.csproj`**(リネームのみ)
   中身は Day 37 と同じ

**細かい2箇所を直してある**(今日の主題とは無関係。差分を 0 にしたいなら合わせておく)。

- HUD の1行目が `Day36` のままだった(Day 37 で更新し忘れ)。`Day38` にした
- 窓のタイトルが `Day31 - FBOとHDRパイプライン…` のまま据え置かれていた。今日の題に更新した

## 設計書

**クラスが1つ増えた**(`Render/ColorGrade`)。層の構成は Day 37 のままで、
今日も全部 `Render/` の中に入る。

| 増えたもの | 何をするか |
|---|---|
| `Render/ColorGrade` | グレーディングのつまみと下敷き。**GL の資源を1つも持たない** |
| `GradePreset` | 夕暮れ / 月夜 / 退色 / モノクロ(Shift+F7) |
| `FxaaQuality` | しきい値と歩幅の組(Shift+F4) |
| `FxaaDebugView` | 輝度 / 縁 / 混合量(Shift+F2) |
| `PostSplit` | 左右比較をどちらの段で行うか(Shift+F11) |

`Render/PostProcess` が太った。

| 変わったもの | 何が増えたか |
|---|---|
| `PostProcess` | `_ldr`(RGBA8)と FXAA パス。`End` が `EndCore` に割れ、**行き先が引数に**なった |
| `PostProcess` | `EndToTarget`。自己チェックが本番と同じ経路を読み戻すための出口 |

### `ColorGrade` だけ形が違う

Day 31 以降に足したクラスは、`PostProcess` / `ShadowMap` /
`EnvironmentMap` / `Ssao` の4つとも
**「自分でバッファを持ち、シェーダは借りる」**という同じ形をしていた。

```csharp
xxx.Begin(...);         // 行き先を自分のバッファへ切り替える
// ここで好きなものを描いてもらう
xxx.End(width, height); // 画面へ戻し、借りた GL の状態を返す
xxx.Apply(shader);      // 本描画が読めるようにテクスチャと uniform を配る
```

`ColorGrade` は**この形に入らない**。持っているのは数字だけで、
FBO もテクスチャもシェーダのハンドルも握っていない。
だから `IDisposable` ですらなく、`Apply(shader)` だけがある。

これは怠慢ではなく、**そもそも自分のパスを持たない機能だから**。
グレーディングは合成パス(`composite.frag`)の中に数行足すだけで済む——
すでに全画素を1回触っているところに便乗している。

| | パス | VRAM | どこに住むか |
|---|---|---|---|
| FXAA | **+1**(フルスクリーン) | **+8.3MB**(1920x1080 の RGBA8) | `PostProcess` の中 |
| グレーディング | ±0 | ±0 | `composite.frag` に相乗り |

**「後処理を1つ足す」と言っても代償はまるで違う**、という対比が
今日いちばん設計として見てほしいところ。
似た見た目の機能でも、**自分のパスが要るかどうか**で置き場所が変わる。

### 出口が2段になった

Day 37 まで、画面(既定のフレームバッファ)へ書くのは `Composite` の1箇所だけだった。
今日 FXAA が後ろに付いたので、こうなる。

```
  FXAA ON   … Composite → [_ldr] → ApplyFxaa → 画面
  FXAA OFF  … Composite ────────────────────→ 画面
```

**素通しのコピーを1パス挟んで経路を1本にする**書き方もできる(コードは短くなる)。
そうしなかったのは、1920x1080 の読み書きが丸ごと1回増えるため。
後処理では「パスを1つ増やす/減らす」が、そのまま帯域の話になる。

代わりに `End` を `EndCore(Framebuffer? target, int w, int h)` に割り、
**行き先を引数にした**。`null` なら画面。これで

- `End(w, h)` … 本番。画面へ
- `EndToTarget(fb)` … 自己チェック。読み戻せるバッファへ

の2つが同じ経路を通る。**確かめる側が本番と違う道を通ったら意味が無い**ので、
分岐を増やさずに口だけ2つにしてある。

### UI が後処理の外へ出た

Day 31 から、HUD の文字は `_post.Begin` と `_post.End` の**間**で描かれていた。
シーンと同じ RGBA16F のバッファに入るので、露出もグレーディングも
トーンマップもガンマも、全部くぐっていたことになる。

今日それを `_post.End` の**後ろ**へ移した。決め手は FXAA で、
**字は1画素幅の線の塊**なので、輪郭を均す処理といちばん相性が悪い(要点5)。

副作用が2つある。どちらも「そのほうが正しい」側に転んだ。

- 露出を上げても HUD が白飛びしなくなった
- **文字がわずかに暗く見える**。前はガンマ(1/2.2 乗)を通っていたので
  0.95 が 0.977 に持ち上がっていた。UI の色は**表示空間で決めるもの**なので、
  こちらのほうが筋が通っている

`Shift+F5` で Day 37 までの置き方に戻せる。**わざと間違えるための窓**で、
Day 37 の `Ctrl+F11`(AO を直接光に掛ける)と同じ趣旨。

Day 37 の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の3つ。

| 図 | 何が変わったか |
|---|---|
| `Render` のクラス図 | `ColorGrade` と enum 3つを追加。`PostProcess` に追記 |
| 1フレームの流れ | **UI が後処理の外へ出た**。FXAA パスが後ろに付いた |
| HDR パイプラインの中身 | 10 パス → **11 パス**。合成の中にグレーディングが入った |
...
classDiagram
    ...
    class SkyImage {
        ...
        +int Width
        +int Height
        +Create(sunDirection, clampToLdr) float[]
...
flowchart TD
    ...
    SP --> SS["RenderSsaoPass()<br/>カメラの目から法線と距離を焼く<br/>→ 遮蔽を計算 → ぼかす<br/>ゲーム中は飛ばす"]
...
flowchart TD
    ...
    S --> T["Texture.FromFloatPixels<br/>RGB16F。横は Repeat"]
    T --> E["equirect.frag x 6面<br/>方向 → 横長の絵の UV<br/>**キューブ 256**"]
...
`dotnet run --project reference/Day38 -c Release` で起動して、順に確かめる。

### 1. `Shift+F12`: FXAA が効く構図

市松の床が奥まで見える構図になり、**画面の真ん中に黄色い縦線**が引かれる。

- 線の**左**が加工前、**右**が加工後
- 見るのは奥へ向かって細かくなる市松の斜めの縁と、立方体の輪郭
- 左は段々、右はなだらか。**同じ模様が左右に続いているので、線をまたいで目で追える**

分かりにくければ、ウィンドウを最大化してから左右の同じ高さの縁を見比べる。
それでも分からないときは `Shift+F2` を3回押して「混合量」にすると、
**FXAA が触った画素だけが光る**ので、どこが変わったのかがはっきりする。

この構図は空が白飛びしているので、**HUD の白い文字がほとんど読めない**。
数字を見たいときだけ `;` で表示を消し、絵を見てから戻すとよい
(HUD の色を背景に応じて変えるのは Day 40 のトグル UI 側の仕事になる)。
...
全面が加工前に戻る。市松の奥のほうで、
**斜めの縁がはっきり階段になる**のが見える。

もう一度押して戻す。HUD の `パス:11` が `パス:10` と行き来するのも確かめる——
FXAA がフルスクリーン1パスだけの手法である、という主張がここに出ている。

### 3. `Shift+F2`: FXAA の中身を見る

4回押すと1周する。

| 表示 | 何が見えるか |
|---|---|
| 輝度 | 灰色の絵。**FXAA が見ている世界そのもの**(色は見ていない) |
| 縁の検出 | 縁と判定された画素が**赤く**なる。市松の線と立方体の輪郭が全部赤い |
| 混合量 | 元の色からどれだけ動いたか。**輪郭の線だけが光るのが正解** |
| 通常 | もどる |

「縁の検出」で**空や壁の平らな面まで赤い**ならしきい値が低すぎる。
「混合量」で**面が光っている**なら混ぜすぎ。
`Shift+F4` で効きを「極」にすると、赤い画素も光る画素も増えるのが分かる。

### 4. `Shift+F4`: 効きを変える

低 → 中 → 高 → 極 と回る。HUD の `しきい値` と `歩幅` が一緒に変わる。

- **低**にすると、市松の奥のほうに階段が残る(拾えていない)
- **極**にすると、細い線が滲み始める。HUD を後処理に通した状態
  (`Shift+F5`)で極にすると、**文字がぼやける**のがいちばん分かりやすい

### 5. `Shift+F5`: UI を後処理に通す(Day 37 までの置き方)

HUD の文字が後処理をくぐるようになる。3つ起きる。

1. `Shift+5` で露出を上げると、**文字が白飛びして読めなくなる**
2. `Shift+F7` を4回押してモノクロにすると、**文字まで灰色になる**
3. `Shift+F4` で効きを「極」にすると、**文字の輪郭が滲む**

3 が Day 38 で UI を外へ出した決め手。もう一度 `Shift+F5` で戻すと、
どれだけ露出を上げても文字は白いまま、色も変わらない。

### 6. `Shift+F7`: 下敷きを回す

ニュートラル → 夕暮れ → 月夜 → 退色 → モノクロ と回る。
**同じシーンの印象がどれだけ変わるか**を見る。

`Shift+F11` を「グレーディング比較」まで回すと(なし → FXAA → グレーディング)、
画面の左半分だけが素通しになる。**色は隣に並べないと分からない**——
単体で見ていると、どんな色かぶりでも数秒で目が順応して「これが普通」に見えてくる。

### 7. `Shift+F8` / `Shift+F9` / `Shift+F10`: つまみを1つずつ

- **コントラスト**(0.8 / 1.0 / 1.2 / 1.4)。上げても**中間の灰色は動かない**。
  明るいところがより明るく、暗いところがより暗くなる
- **彩度**(0.0 / 0.5 / 1.0 / 1.3)。0 でモノクロ
- **色温度**(-40 / -20 / 0 / +20 / +40)。
  正が暖色、負が寒色。HUD の `白点:(…)` が動くのを一緒に見る——
  +40 で R 側が 1.06、B 側が 0.77 になる

つまみを動かすと HUD の下敷き名が「手動」に変わる。
`Shift+F7` を押せばいつでも既知の組に戻せる。

### 8. `Ctrl+F12`: 自己チェック

コンソールに 15 項目が出る。**全部 OK なら今日のコードは正しい**。

内容は下の「動作確認済み環境」に載せてある。
いちばん見てほしいのは6項目目で、
**縁の被覆率が理想の直線からどれだけずれているか**が
0.499 画素 → 0.302 画素へ下がる。これが「階段が均された」の数値化そのもの。

## 改造課題

### 課題1(易): グレーディングの下敷きを1つ足す

`GradePreset` に自分の1つを足して、`ApplyPreset` の `switch` に数字を書く。

作ってみると分かるが、**「なんとなく良い」は再現できない**。
つまみを回しているうちに元がどこだったか分からなくなるので、
`Shift+F6` で ON/OFF するか、`Shift+F11` で左右比較しながら決めること。

余裕があれば `Filter`(色フィルタ)と `Temperature`(ホワイトバランス)で
**同じ方向に振ったときの違い**を見比べてみるとよい。
色フィルタは白いものにも色が付き、ホワイトバランスは白を白のまま動かす。

### 課題2(中): 色温度を RGB の掛け算で実装して見比べる

`composite.frag` の `WhiteBalance` を、LMS を経由せずに

```glsl
vec3 WhiteBalance(vec3 color) { return color * uWhiteBalance; }
```

と書き換えてみる(CPU 側はそのまま使える)。

`Shift+F10` で色温度を ±40 まで振ると、違いが出る。
特に見てほしいのは**中間の明るさの色がどちらへ転ぶか**で、
市松の床の緑や、モデル(`Shift+0`)の肌色に近い部分が分かりやすい。

「見た目そんなに変わらない」と思ったら、それも正しい観察。
**±40 程度なら差は小さく、大きく振ったときに効いてくる**——
つまり LMS を使う値打ちは「壊れにくさ」であって「見栄え」ではない。
どこまでやるかを自分で決めるための課題で、
`ColorGrade.CieXyToLms` のコメントを読み直す口実でもある。

### 課題3(難): 縁の形を見てから重みを決める(SMAA の入口)

FXAA は「縁を見つけたら、その向きへ数テクセル混ぜる」だけで、
**縁がどんな形をしているかは見ていない**。
だから 45 度の縁も、ほとんど水平な縁も、同じ手続きで処理してしまう。

SMAA(事前に読む資料の最後)はここを変える。

1. 縁を検出する(ここまでは FXAA と同じ)
2. **縁に沿って両端まで歩き、どこで途切れるかを調べる**
3. その形(L 字 / Z 字 / U 字 …)から、**正しい被覆率を計算で出す**

3 が決定的な違いで、推測ではなく**幾何から答えを出している**。

最小の一歩として、`fxaa.frag` に「縁に沿って左右へ最大 N テクセル歩き、
輝度が変わるところまでの距離を測る」ループを足してみるとよい。
測った2つの距離から台形の面積として被覆率を出せば、
**同じ1パスのまま品質が上がる**——FXAA 3.11 の後半(今日削ぎ落とした部分)が
やっているのもだいたいこれになる。

`Ctrl+F12` の「縁が理想の直線に近づく」の数字が
0.302 からどこまで下がるかで、値打ちを測れる。

## 動作確認済み環境

- Windows 11 / .NET 10 / RTX 3070
- 960x640(既定)、Release ビルド
- Silk.NET 2.23.0

### 自己チェック(15 項目すべて合格)

```
[FXAA とカラーグレーディングの自己チェック]
  [OK] **色温度 0 の増幅率がちょうど 1**(素通しが本当に素通し)  (1.00000, 1.00000, 1.00000)
  [OK] 暖色へ回すと L(長波長=赤側)が上がり S(短波長=青側)が下がる  L 1.059 / M 1.000 / S 0.765
  [OK] 寒色はその逆(**軌跡の上を反対へ動いている**)  L 0.906 / M 1.029 / S 1.653
  [OK] FXAA なしでは**中間の明るさが1画素も無い**(白か黒しかない = 階段)  0 画素 / 全 10752 画素
  [OK] **FXAA を掛けると中間の明るさが現れる**(階段の角が埋まった)  295 画素(2.7%)
  [OK] **縁が理想の直線に近づく**(列ごとの被覆率のずれが減る)
       ずれ 0.499 → 0.302 画素(完全な直線なら 0。階段のままなら段の高さの 1/4 = 0.500)
  [OK] **明るさの総量はほとんど変わらない**(ぼかしではなく整形だから)  4608.0 → 4603.6 画素ぶん(-0.10%)
  [OK] **白一色の面は1ビットも動かない**(局所コントラストが 0 なので縁ではない)  最大差 0.00000
  [OK] **しきい値を 1.0 にすると FXAA なしと完全一致**(早期打ち切りの経路)  最大差 0.00000
  [OK] FXAA を掛けるとフルスクリーンパスが1つ増える  OFF 1 パス → ON 2 パス
  [OK] **素通しの設定なら ON/OFF で絵が一致**(恒等が恒等になっている)
       OFF (0.792,0.576,0.424) / ON (0.792,0.576,0.424)
  [OK] 彩度 0 で R = G = B(モノクロになる)  (0.624, 0.624, 0.624)
  [OK] **コントラストを上げても 18% グレーは動かない**(回転の中心)  0.4588 → 0.4588
  [OK] 中心より暗いところはコントラストでさらに沈む  0.2549 → 0.2039
  [OK] **暖色へ回すと絵の赤が上がり青が下がる**(白点の付け替えが届いている)
       R 0.729→0.780  G 0.729→0.725  B 0.729→0.631
  すべて合格
  合成のみ 0.009ms  合成+FXAA 0.035ms  → FXAA ぶん 0.026ms  (960x640)
  LDR バッファ 960x640(RGBA8) 2.3MB  後処理の合計 12.3MB
```

**確かめる側の絵を自分で描いている**のが今日の自己チェックの特徴。
シザーテスト付きの `glClear` で傾き 1/4 の階段を焼くだけなので、
三角形もシェーダも要らず、**入力が完全に決まった1枚**になる。
カメラの位置にもシーンの中身にも左右されないので、
「入力が同じなら出力も同じ」が成り立つ。

### FXAA にかかる時間

`Ctrl+F12` の最後の行。合成だけを 60 回まわしたものと、
合成 + FXAA を 60 回まわしたものの差を取っている
(`glFinish` を前後に挟んで、積んだだけの時間にならないようにしてある)。

| 解像度 | 合成のみ | 合成 + FXAA | FXAA ぶん |
|---|---|---|---|
| 960x640 | 0.009ms | 0.035ms | **0.026ms** |

RTX 3070 で 0.03ms 弱。1920x1080 なら画素数が 3.4 倍なので 0.1ms 弱の見当になる。
**フレーム予算(60fps で 16.7ms)の 1% 未満**。

HUD に出ている影パスの 0.08ms や SSAO の 0.07ms と**並べて比べてはいけない**。
あちらは `Stopwatch` で測った<b>CPU が命令を積むのにかかった時間</b>で、
GPU が実際に走った時間ではない(`glFinish` を挟んでいない)。
同じ土俵で測りたいなら、あちらにも `glFinish` を入れる必要がある——
**計測の作法が揃っていない数字を並べない**、というのは
Day 26 のブロードフェーズの計測でも一度ぶつかった話。

FXAA が 10 年以上使われ続けた理由がこの数字に出ている——
**ほとんど只で、どんな絵にも効く**。
代わりに払っているのは VRAM 2.3MB(1920x1080 なら 8.3MB)と、細い線の滲み。

### 検証の途中で分かったこと

**「ジャギーが減った」をどう測るかで、結論がひっくり返った**。

最初は**全変動**(隣り合う画素の差の絶対値の和)で測ろうとして、
これは駄目だとすぐ分かった。段差をなだらかな傾斜に均しても、
**全変動は変わらない**(単調な変化の全変動は端の差だけで決まる)。

次に**2階差分の絶対値の和**にした。直線なら 0 になるので筋はよさそうに見えたが、
実測すると **92.0 → 97.5 と増えた**。
FXAA が縁の周りに作る細かい起伏を拾ってしまい、
「まっすぐになった」ことより「局所的にでこぼこした」ことのほうが大きく出る。

最終的に**最小二乗で直線を当てはめ、そこからの平均のずれ**にしたら
**0.499 → 0.302** と素直に下がった。同じデータで、同じ現象で、
**測り方を変えただけで OK と NG がひっくり返る**——
性能や品質を数字で語るときにいちばん怖いのがこれ。

**しきい値のチェックが最初 NG になった**。
「相対しきい値を 1.0 にすれば何も縁にならないはず」と思って書いたら、
最大差 0.333 が出た。理由は単純で、テスト画像が**黒と白**なので
局所コントラストがちょうど 1.0 になり、`range >= 1.0` を満たしてしまう。
2.0 にしたら通った。**「最大値ちょうど」は境界条件**という、
浮動小数点以前の初歩的な取りこぼし。

**`glClear` はシザー矩形の中しか塗らない**という性質が、
確かめ用の絵を描くのに具合がよかった。
シェーダも頂点バッファも要らず、`glScissor` を 24 回動かすだけで
「傾き 1/4 の直線をラスタライズした階段」が手に入る。
**シザーテストを切り忘れると、そのあとのフルスクリーンパスが
最後の短冊の中だけに描かれて画面がほぼ真っ黒になる**ので、そこだけ注意。
