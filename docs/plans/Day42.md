# Day 42: アニメーション制御 — ブレンドと簡易ステートマシン

Day 41 で「クリップを1本、正しく再生する」ところまで来た。
今日はその上に**複数のクリップを混ぜる仕組み**を載せる。

## 今日のゴール

**`Shift+Alt+F1` を押すと、キツネの移動速度が 0 ↔ 3.2m/s を往復し、
それに応じて立ち止まり → 歩き → 走りが連続的に混ざる。**

`Shift+Alt+F3` で位相同期を切ると、同じ重みのまま**足が合わなくなる**。

| キー | 何が起きるか |
|---|---|
| `Shift+Alt+F1` | Fox を出して、速度を自動で上下させながらブレンド |
| `Shift+Alt+F2` | 決め方(OFF / ブレンドツリー / ステートマシン)を切り替える |
| `Shift+Alt+F3` | **位相同期 ON/OFF**。今日いちばん効く見比べ |
| `Shift+Alt+F8` | クロスフェード 0 / 0.1 / 0.25 / 0.5 秒。**0 秒だと足がワープする** |
| `Shift+Alt+F9` | 今の混合の内訳(重み・各クリップの再生位置) |

### 今日いちばん大事な2行

```csharp
// 「どのクリップを、どれだけ」——決め方はブレンドツリーでもステートマシンでもよい
_animation.SetBlend([new ClipWeight(walk, 0.5f), new ClipWeight(run, 0.5f)]);

// 混ぜるのは TRS の段。行列にしてしまうと、もうこれができない
_accumulator[i].Add(_scratch[i], entry.Weight);
```

Day 41 で `NodePose`(平行移動・回転・拡大の3点セット)という型をわざわざ作った。
**その配当を回収するのが今日**になる。

### 今日はシェーダに1文字も触らない

差分は `Model/` と `Program.cs` だけ。`.vert` も `.frag` も 1 行も変わらない。

混ぜる場所を**姿勢の段**に置いたので、下流(世界行列 → 関節行列 → 頂点ブレンディング)から見ると
「ポーズが1つ決まった」以上のことは起きていない。
Day 41 の設計がここで効いていて、**上流で混ぜる形にしておくと、
何本混ぜても GPU 側は何も知らなくてよい**。

## 事前に読む資料

- ゲームグラフィックス特論 A-5「変換(3) 座標値の生成とブレンディング」
  <https://tokoik.github.io/gg/>
  Day 41 と同じ回。**後半のブレンディングの節が今日の範囲**。
- 特論 A-4「変換(2) 四元数による回転と補間」
  <https://tokoik.github.io/gg/>
  slerp と nlerp の違い。今日は「3本以上を混ぜる」ので nlerp 側の話になる。
- glTF 2.0 仕様の Animations の節
  <https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html#animations>
  **仕様にブレンドの話は1行も無い**。「複数のアニメーションを同時に動かしてよいか、
  どう混ぜるか」は実装に任されている、という確認のために読む。
- Fox のクリップ構成(Survey / Walk / Run)
  <https://github.com/KhronosGroup/glTF-Sample-Assets/tree/main/Models/Fox>
  README に「これらは同じ関節を動かすので、一度に1本だけ再生すること」と書いてある。
  **今日はそれをあえて破る**——だから位相を合わせる必要が出る。

## 理論の要点

### 1. クリップの合成は、必ず TRS の段でやる

Day 41 の要点3で書いたことの回収。

```
  TRS(平行移動・回転・拡大)  →  混ぜられる
  行列                        →  混ぜられない
```

回転行列を成分ごとに平均すると、**回転行列でなくなる**。
90 度回転と 0 度回転を 50:50 で足すと、45 度回転ではなく縮んだ行列が出る。
クォータニオンなら slerp で 45 度が出る。

だから `AnimationPlayer.Evaluate` の段取りは、混ぜる段を**行列を作る前**に置く。

```
  1. クリップごとに姿勢を評価      NodePose[]
  2. 重み付きで足し込む            ← ここが Day 42 で増えた段
  3. 姿勢を確定
  4. 行列にして木を降りる          ← ここから先は Day 41 と1文字も同じ
  5. 関節行列
```

**4 と 5 に手を入れずに済んだ**のが、この順番にしておいた配当。
逆に「行列を混ぜる」設計にしていたら、今日は全部書き直しになっていた。

### 2. 3本以上を混ぜるときは、slerp ではなく weighted nlerp

2本なら slerp で足りる。問題は3本以上で、
**ブレンドツリーの2本 + クロスフェード中の前の状態の2本 = 4本**が同時に効くことがある。

slerp を入れ子にすると2つ困る。

- **混ぜる順番で答えが変わる**(slerp は結合的でない)
- 「重み 0.3」が何に対する 0.3 なのかが分からなくなる

そこで実際のエンジンと同じ手を使う。

```
  平行移動・拡大 … 重み付きの和 ÷ 重みの合計
  回転           … 重み付きで足してから正規化(weighted nlerp)
```

nlerp は slerp と厳密には違う——**回る速さが一定にならない**——が、
差が出るのは元の2つが大きく離れているときで、
**中点(t = 0.5)では slerp と完全に一致する**。
歩きと走りのように似た歩容を混ぜるぶんには、この近似で十分。
自己チェックが「50:50 の合成が Lerp と一致」で確かめている。

**符号を揃えるのが肝**。クォータニオンは `q` と `-q` が同じ向きを表すので、
揃えずに足すと**打ち消し合って長さが 0 に近づく**。
そのまま正規化すると、まったく関係のない向きが出る。

```csharp
if (Quaternion.Dot(_reference, rotation) < 0.0f) { rotation = -rotation; }
```

最初に足したものを基準にして、内積が負なら裏返してから足す。
自己チェックの「揃えずに足すと長さが 0 になる」がこの実演。

### 3. 位相同期 — 長さの違うクリップを、同じ物差しに乗せる

**今日いちばん大事な考え方**。Fox のクリップの長さはこうなっている。
...
歩きと走りを 50:50 で混ぜるとき、**秒で合わせてはいけない**。
0.3 秒の時点で、歩きは1周の 42%、走りは 26% を指す——
つまり「歩きの左足が前に出ている瞬間」と
「走りの右足が前に出ている瞬間」を混ぜることになる。
結果は足が地面をこすり、歩幅が縮み、体が沈む。

正しくは**正規化した位置(位相)で合わせる**。

```
  位相 0.37  →  歩き 0.37 × 0.708 = 0.261s
                走り 0.37 × 1.158 = 0.427s
```

こうすれば「1周のうち同じところ」どうしが混ざる。
リファレンスでは <b>再生位置そのものを秒から位相(0〜1)へ変えた</b>——
Day 41 の `_time` が `_phase` になったのがこの日の中心的な変更。

周期のほうは**重み付きの平均**にする。

```
  0.708 × 0.5 + 1.158 × 0.5 = 0.933 秒
```

走りに寄せるほど周期が短くなる、という当たり前のことが、この1行で手に入る。

**同期を切るとどうなるか**は `Shift+Alt+F3` で見られる。
2本の周期は 0.708 と 1.158 で約分できないので、位相の関係が毎周期ずれていき、
**同じ歩容の瞬間なのに毎回ちがう姿勢になる**。
自己チェックがこれを数字で出していて、
「1周ぶん進めたときの姿勢のずれ」が同期時 0、非同期時 70 になる。

### 4. ブレンドツリーとステートマシンは、別のものを表している

同じ入力(移動速度)を受けて、同じクリップを混ぜるのに、性格がまるで違う。

| | 速度 1.8m/s のとき | いつ混ざるか |
|---|---|---|
| ブレンドツリー | 歩き 0.50 / 走り 0.50 | **常に** |
| ステートマシン | 走り 1.00 | **移り変わる瞬間だけ** |

どちらが正しいというものではなく、**題材で決まる**。

- **移動の歩容は連続量**。0.5m/s と 0.6m/s の違いは連続的に見えるべきなので、
  ブレンドツリーが向く
- **攻撃・被弾・ジャンプは離散**。「半分だけ攻撃している状態」に意味は無いので、
  ステートマシンが向く

実際のゲームは**両方を重ねる**。「ロコモーション」という1つの状態の中身が
ブレンドツリーになっていて、そこから「ジャンプ」状態へクロスフェードする、という形。
今日は見比べるために2つを並列に置いてあるが、
**そのまま入れ子にできる形**(どちらも出口が `ClipWeight` の並び)にしてある。

### 5. クロスフェードとヒステリシス — 状態機械の2つの必需品

ステートマシンには、素朴に作ると必ず出る問題が2つある。

**1つ目: 切り替えが飛ぶ**。歩きから走りへ瞬間で切り替えると、
足の位置が別の場所へワープする。だから**クロスフェード**で
「前の状態の重みを 1 → 0、次の状態を 0 → 1」と時間をかけて移す。

`Shift+Alt+F8` で 0 秒にすると、この飛びをわざと見られる。
長すぎる(0.5 秒)と今度は**走り出しているのに歩きの脚が残る**ので締まりが無い。
実務では 0.1〜0.3 秒あたりに落ち着く。

**2つ目: しきい値の上でばたつく**。速度がちょうど境目にあると、
毎フレーム状態が行き来し、クロスフェードが始まっては打ち切られる。
絵がぶるぶる震えるので、すぐ分かるが原因は分かりにくい。

直し方は**上がるときと下がるときでしきい値をずらす**(ヒステリシス)。

```
  歩き → 走り  … 速度 ≧ 2.08 で上がる
  走り → 歩き  … 速度 < 2.08 - 0.25 = 1.83 で下がる
```

自己チェックがこれを数えていて、しきい値をまたいで微小に振動させたとき、
**ヒステリシス 0 なら 100 フレームで 99 回、0.25 なら 1 回**しか状態が変わらない。

### 6. 重みの合計は、ここでも 1 でなければならない

Day 41 の要点1(頂点の重み)とまったく同じ話が、クリップの階層でも出る。

合計が 0.9 のまま混ぜると、平行移動と拡大が 0.9 倍になる——
つまり**モデル全体がわずかに縮み、原点へ寄る**。
1フレームだけなら気づかないが、重みが動くたびに大きさが揺れるので、
「なんとなく息をしているように見える」という妙な症状になる。

リファレンスでは2か所で守っている。

- `AnimationPlayer.SetBlend` が受け取った重みを**合計 1 に正規化する**
- `PoseAccumulator.Resolve` が**重みの合計で割る**

二重に見えるが、後者は「呼ぶ側が 0.9999 を渡してきたとき」の保険。
**同じ不変条件を、作る側と使う側の両方で守る**のは、
値が層をまたいで運ばれるときの定石になる。

### 7. `ClipWeight` — 決め方を知らない型を1つ挟む

ブレンドツリーもステートマシンも、出口は同じ。

```csharp
internal readonly record struct ClipWeight(int Clip, float Weight);
```

`AnimationPlayer` は「なぜその重みなのか」を一切知らない。
速度で決めたのか、状態遷移の途中なのか、手で置いたのかは呼ぶ側の都合。

Day 40 の `FeatureToggles` が `Func<bool>` と `Action<bool>` しか知らなかったのと同じ形で、
**知識を持たない側に置くほうが、あとから別の決め方を足せる**。
Day 45 でキャラクターコントローラを繋ぐとき、
「接地しているか」「壁に当たっているか」で重みを決める経路が増えるが、
そのとき `AnimationPlayer` には手が入らない。
...
| ファイル | 役割 |
|---|---|
| `Model/BlendTree1D.cs` | 1次元ブレンドツリー。**速度 → 隣り合う2本の重み**。状態を持たない |
| `Model/AnimationStateMachine.cs` | 簡易ステートマシン。状態・しきい値・ヒステリシス・クロスフェード |

### 変更ファイル

| ファイル | 何が変わったか |
|---|---|
| `Model/ModelNode.cs` | `NodePose.Lerp` と `PoseAccumulator`(重み付き合成) |
| `Model/AnimationPlayer.cs` | `ClipWeight` / `SetBlend` / `SyncPhase` / `Phase` / `Rewind`。**時刻が秒から位相へ** |
| `Program.cs` | ロコモーションの組み立てと更新、`Shift+Alt+F1〜F12`、HUD、自己チェック |

**シェーダは1本も変わっていない**。`.vert` も `.frag` も Day 41 のまま。

### 写経する順番

**依存の順に並べてある**。上から順に写せば、途中でビルドが通らなくなることは無い。

1. **`Model/ModelNode.cs`** — `NodePose.Lerp` を足し、ファイル末尾に `PoseAccumulator` を新設。
   **符号を揃える `Add` が今日の要点2そのもの**
2. **`Model/AnimationPlayer.cs`** — いちばん変更が大きい。次の順に手を入れるとよい
   - ファイル先頭に `ClipWeight`(`AnimationPlayer` の外側、名前空間の直下)
   - フィールド: `MaxBlendClips` / `_scratch` / `_accumulator` / `_blend` / `_blendCount` /
     `_clipTimes` / `_warnedTooManyClips`。**`_time` を `_phase` に置き換える**
   - `ClipIndex` をフィールドから導出プロパティへ
   - `Blend` / `Phase` / `Time` / `SyncPhase` / `Duration`
   - `SetBlend` / `Rewind`。`SelectClip` は `SetBlend` を呼ぶ形へ
   - `Advance` に `_clipTimes` の更新を足し、`TimeOf` / `Wrap` を新設
   - `Evaluate` の 1〜3 段を差し替え(**4 と 5 は触らない**)
3. **`Model/BlendTree1D.cs`**(新規) — 短い。`Evaluate` の場合分けだけ
4. **`Model/AnimationStateMachine.cs`**(新規) — `Update` の while 2本がヒステリシス
5. **`Program.cs`** — 最後。次の順に足す
   - フィールド(`LocomotionMode` / `_locomotion` / `_locomotionTree` / `_locomotionStates` /
     `_moveSpeed` / `_speedSweep` / `_sweepPhase` / `SweepSeconds` / `MaxMoveSpeed` /
     `CrossFadeSeconds` / `_crossFadeIndex` / `_blendScratch`)
   - `BuildLocomotion` / `FindClip` / `UpdateLocomotion`
   - `LocomotionLabel` / `BlendLabel` / `DescribeBlend`
   - `SetModel` の2か所(捨てるところ / `BuildLocomotion` の呼び出しとクリップ一覧の表示)
   - `OnUpdate` の `UpdateLocomotion` 呼び出し
   - HUD に `LocomotionLabel()` の1行
   - `Alt+F10` を `SelectClip` から `Rewind` へ
   - キー(`Shift+Alt+F1〜F12`)。**`Alt+F` の塊より前**に置くこと
   - `RunLocomotionCheck` と `MeasureCycleDrift`
   - `OnLoad` のヘルプ

**キーの位置がいちばん間違えやすい**。`case Key.F1 when alt:`(Day 41)は
Shift+Alt+F1 でも成立するので、今日の塊を下に置くと**キーが1つも届かない**。

## 設計書

**層は増えていない**。`Model/` の中にクラスが2つ増えて、既存の2つに手が入っただけ。
`Render/` も `shaders/` も1文字も変わっていない。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Model/ClipWeight` | `Model/` | 「どのクリップを、どれだけ」。**ブレンドの共通語** |
| `Model/PoseAccumulator` | `Model/` | 姿勢を重み付きで足し合わせる。**符号を揃える** |
| `Model/BlendTree1D` | `Model/` | 速度 → 隣り合う2本の重み。**状態を持たない** |
| `Model/AnimationStateMachine` | `Model/` | 状態・ヒステリシス・クロスフェード。**状態を持つ** |

| 変わったもの | 何が増えたか |
|---|---|
| `Model/NodePose` | `Lerp`(2つの姿勢を混ぜる) |
| `Model/AnimationPlayer` | `SetBlend` / `Blend` / `SyncPhase` / `Phase` / `Rewind`。時刻が秒から位相へ |
...
```
  BlendTree1D          ─┐
                        ├→  ClipWeight[]  →  AnimationPlayer  →  NodePose[]  →  関節行列
  AnimationStateMachine ─┘
      (決める側)                              (混ぜる側)
```

決める側は**姿勢を1ミリも触らない**。ノードも関節も骨格も知らず、
知っているのは「クリップの番号」と「重み」だけ。
混ぜる側は**なぜその重みなのかを知らない**。

この線を引いた配当は3つ。

1つ目。**決め方を並べて見比べられる**。ブレンドツリーとステートマシンは
同じ口に刺さるので、`Shift+Alt+F2` で切り替えるだけで比較できる。
片方を `AnimationPlayer` の中に書いていたら、こうはならない。

2つ目。**自己チェックが軽い**。決める側は入力も出力もただの数値なので、
28 項目のうち 19 項目が**モデルを1体も読まずに走る**。
Day 40 の `CameraPath`、Day 39 の `SkyAnalysis` と同じ性格で、
「GL を呼ばないクラス」がまた2つ増えたことになる。

3つ目。**入れ子にできる**。ステートマシンの1つの状態の中身をブレンドツリーにする、
という実際のゲームでよくある形が、どちらの型にも手を入れずに書ける。
出口が同じ `ClipWeight[]` だから。

### `_time` を `_phase` に変えたこと

Day 41 の `AnimationPlayer` は再生位置を**秒**で持っていた。
今日それを**位相(0〜1)**に変えている。これが今日いちばん深い変更で、
理由は要点3のとおり——長さの違うクリップを同じ物差しに乗せるため。

副作用が2つある。

- `Duration` が**混ぜる重みで変わる**。歩き 0.708 と走り 1.158 を 50:50 なら 0.933 秒。
  つまり `Time`(= `Phase × Duration`)は、重みを動かすと**飛ぶ**
- 位相同期を切っている間は、`Phase` が「代表値」でしかなくなる。
  各クリップは `_clipTimes[]` の自分の秒で回っている

2つ目のために `_phase` と `_clipTimes` を**両方とも常に進めて**ある。
片方だけにすると、`Shift+Alt+F3` で切り替えた瞬間に姿勢が飛ぶ。
使わない値を進めておくのは無駄に見えるが、**切り替えを連続にするための代償**として払っている。

### `PoseAccumulator` を struct にした理由

ノードの数だけ(Fox なら 26 個)必要で、**毎フレーム作り直す**。
class にすると 26 個のオブジェクトが毎フレーム GC のゴミになる。

配列に入れておいて `Array.Clear` で初期化すれば、確保は起動時の1回だけになる。
Day 26 の均一グリッドで「毎フレーム List を作らない」と書いたのと同じ判断で、
**1フレームに何度も使い捨てるものは、struct の配列にして使い回す**。

`Add` が `in NodePose` を取るのも同じ理由(40 バイトの構造体をコピーしない)。

### 今日見つけた歪み — `Program.cs` が 13,000 行を超えた

正直に書いておく。`Program.cs` は Day 42 で **13,752 行**になった。
今日足した 530 行のうち、**300 行以上が自己チェックとキー操作**で、
エンジンの機能そのものは 200 行ほどしかない。

構造としては破綻していないが(`Program` は組み立てとデモの層で、
エンジン本体は `Model/` や `Render/` にある)、
**「どこに何があるか」を grep 以外で探せなくなっている**のは確か。

直すとしたら Phase 7 が終わったあと(Day 50)で、切り方は見えている。

| 切り出す先 | 何を移すか | 目安の行数 |
|---|---|---|
| `Demo/DemoKeys.cs` | `OnKeyDown` の巨大な switch | 1,200 |
| `Demo/SelfChecks.cs` | `RunXxxCheck` の 14 本 | 4,000 |
| `Demo/Hud.cs` | `DrawOverlayInfo` と `XxxLabel()` 群 | 800 |

今やらないのは、**写経の差分が丸ごとファイル移動になる**から。
Day ごとの差分を読む形式と、大きなリファクタリングは相性が悪い。
Phase の切れ目でまとめてやるのが筋になる。

Day 41 の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の3つ。

| 図 | 何が変わったか |
|---|---|
| `Model` のクラス図 | **クラスが4つ増えた**。`AnimationPlayer` と `NodePose` にもメンバーが増えた |
| 1フレームの姿勢が決まるまで | **混ぜる段が1つ増えた**。時刻の作り方が位相へ変わった |
| 1フレームの流れ | `OnUpdate` に `UpdateLocomotion` が入った |

新しく足した図が1つ。

| 図 | 何を描いたか |
|---|---|
| 速度から重みが決まるまで | ブレンドツリーとステートマシンの、2通りの決め方 |
...
`dotnet run --project reference/Day42 -c Release` で起動する。
**必ず `-c Release`**——`.hdr` の復号も太陽の抽出も CPU の総なめなので、
Debug だと読み込みが数倍かかる。

起動直後は Day 40 までの絵(DamagedHelmet)。今日の主役は `Shift+Alt+F1` から。

### 1. `Shift+Alt+F1`: キツネが歩き出し、走り出す

床の上にキツネが出て、**速度が 12 秒かけて 0 → 3.2m/s → 0 を往復する**。
それに合わせて立ち止まり → 歩き → 走り → 歩き → 立ち止まりと連続的に変わる。

コンソールに出る:

```
  ロコモーション: 立ち止まり(0.0m/s) → 歩き(1.0m/s) → 走り(3.2m/s)  (Shift+Alt+F1 で再生)
ロコモーション: ブレンドツリー。速度が 0 ↔ 3.2m/s を 12 秒で往復します
...
```
ロコモーション:ブレンドツリー  速度:2.10m/s(自動)  位相同期:ON  フェード:0.25s
  周期:0.93s  位相:0.37  混合:Walk0.50+Run0.50
```

**`混合:` の数字が連続に動いている**ことを確かめる。
`Walk1.00` から `Run1.00` へ飛んでいたら、ブレンドツリーではなく
ステートマシンになっている(`Shift+Alt+F2` を押しすぎ)。

**周期が動く**のも見どころ。走りに寄るほど `周期:` が短くなる
(0.708s ↔ 1.158s の重み付き平均)。

### 2. `Shift+Alt+F3`: 位相同期を切る

**足の運びが合わなくなる。** 重みも速度も何も変えていないのに、
脚がばらばらに動き、体が沈んだり跳ねたりする。

`Shift+Alt+F9` で内訳を出すと違いが分かる。

```
（同期 ON）
速度 2.10m/s  位相 0.369  周期 0.933s  位相同期 ON
  Walk     重み 0.500  長さ 0.708s  位置 0.261s
  Run      重み 0.500  長さ 1.158s  位置 0.427s

（同期 OFF）
速度 2.10m/s  位相 0.684  周期 0.933s  位相同期 OFF
  Walk     重み 0.500  長さ 0.708s  (各クリップが独立に進行中)
  Run      重み 0.500  長さ 1.158s  (各クリップが独立に進行中)
```

ON のときは `0.261 / 0.708 = 0.369` と `0.427 / 1.158 = 0.369` で**位相が一致している**。
これが「同じ物差しに乗せる」ということ。

もう一度 `Shift+Alt+F3` で戻す。**戻した瞬間に姿勢が飛ばない**ことも確かめておく
(`_phase` と `_clipTimes` を両方進めてあるため)。

### 3. `Shift+Alt+F2`: ブレンドツリーとステートマシンを見比べる

同じ速度スイープで、混ざり方がまったく違う。

| | HUD の `混合:` |
|---|---|
| ブレンドツリー | `Survey0.42+Walk0.58` のように**常に2本** |
| ステートマシン | ふだんは `Walk1.00` の**1本**。切り替わる瞬間だけ2本 |

ステートマシンのときは `ロコモーション:ステートマシン[歩き]` のように
今の状態が出る。速度が 2.08 を超えると `[走り]` へ移り、
**1.83 を下回るまで戻らない**(ヒステリシス)。

もう一度押すと `OFF`(Day 41 と同じ。クリップを1本そのまま再生)。

### 4. `Shift+Alt+F8`: クロスフェードを 0 秒にする

ステートマシンにしてから押していくと、`0.00s` のとき
**歩き → 走りの瞬間に脚がワープする**。0.25s に戻すと滑らかに繋がる。

0.50s まで伸ばすと、今度は**走り出しているのに歩きの脚が半分残る**ので、
締まりが無くなる。「長ければよい」ものではないことが目で分かる。

※このキーはステートマシンのときだけ効く(ブレンドツリーには遷移が無い)。

### 5. `Shift+Alt+F5` / `F6` / `F7`: 速度を手で置く

`F7` で自動スイープを止め、`F5` / `F6` で 0.2m/s ずつ動かす。
しきい値の前後(1.9 と 1.8)を往復させると、
**ステートマシンが 1.83 まで戻らない**ことを手で確かめられる。

### 6. `Shift+Alt+F9`: 内訳を出す

上の 2 で使ったもの。ツリーの点と状態のしきい値も一緒に出る。

```
  ツリーの点: 立ち止まり 0.00m/s / 歩き 1.00m/s / 走り 3.20m/s
  状態: 立ち止まり ≧-∞ / 歩き ≧0.65 / 走り ≧2.08  今 歩き ← 立ち止まり(48%)
```

最後の `← 立ち止まり(48%)` がクロスフェードの途中を表す。

### 7. Day 41 までの機能が全部生きていること

`Alt+F1`(CesiumMan が歩く)、`Alt+F6`(スキニング ON/OFF)、
`Alt+F9`(骨の重み)、`Ctrl+Alt+F1`(デモ v1)、`Shift+F12`(FXAA)——全部そのまま動く。

**`Alt+F10`(先頭へ戻す)だけ中身が変わっている**。Day 41 は
`SelectClip(ClipIndex)` だったが、それだと混ぜているものが1本に潰れるので `Rewind()` にした。
挙動は「混ぜていないときは同じ」なので、Day 41 の使い方では違いが出ない。

CesiumMan はクリップが1本なので、`Shift+Alt+F1` 以外では
ロコモーションが組まれない(HUD に `使えるモデルではない` と出る)。

### 8. `Shift+Alt+F12`: 自己チェック(28 項目すべて合格)

```
--- Day 42: アニメーション制御の自己チェック ---
  [OK] walk(1.0)と run(3.0)の中点 2.0 で 50:50  0.500 / 0.500
  [OK] 50:50 の合成が Lerp と一致(**中点では nlerp = slerp**)  平行移動 1.0000 / 内積 1.000000
  [OK] q と -q を混ぜても元の向きが出る(**符号を揃えている**)  内積 1.000000
  [OK] 揃えずに足すと長さが 0 になる(**揃えないとこうなる**)  長さ 0.00E+000
  [OK] ヒステリシス 0 だと境目でばたつく  100 フレームで 99 回
  [OK] ヒステリシスを入れると 1 回で収まる  100 フレームで 1 回
  [OK] 周期が重み付きの平均になる  0.9333s(walk 0.708 / run 1.158)
  [OK] 位相同期 ON: 1周ごとに同じ姿勢へ戻る  ずれ 0.00E+000
  [OK] 位相同期 OFF: **毎周期ちがう姿勢になる**  ずれ 7.01E+001(同期していれば 0.00E+000)
  すべて合格(ブレンドツリー・ステートマシン・位相同期が仕様どおり)
```

**28 項目のうち 19 項目はモデルを1体も読まずに走る**。
ブレンドツリーもステートマシンも `PoseAccumulator` も、
入口と出口がただの数値なので、絵にせずに確かめられる。

## 改造課題

### 課題1(易): ツリーの点の速度を、実測で決める

`Program.BuildLocomotion` の速度(歩き 1.0、走り 3.2)は**目分量**。
実際にクリップが表している速さとずれていると、**足が地面を滑る**。

測り方は素直で、**1周期のあいだに足が後ろへどれだけ流れるか**を見る。

```csharp
// 接地している足の骨(Fox なら b_LeftLeg / b_RightLeg の先)の
// 世界位置を1周ぶんサンプルして、後ろへ動いた総量 ÷ 周期
```

`Alt+F11` で骨の名前が出るので、そこから足の骨を選ぶ。

そのうえで確かめてほしいことが2つ。

- **測った値は目分量とどれだけ違うか**。Fox は 100 倍スケールのモデルなので、
  そのままでは「m/s」にならない。**単位を揃えるところがいちばん引っかかる**
- 直したあと、速度を手動(`Shift+Alt+F5` / `F6`)で動かしながら
  足の接地点を見る。滑りが減っているか

### 課題2(中): クロスフェードを「今のポーズから」始める

いまの `AnimationStateMachine` は**直前の状態を1つ覚えるだけ**。
走り出してすぐ止まると、混ざりかけの姿勢が捨てられて少し飛ぶ。

本物は**ポーズを1枚保存してから**フェードする。

```csharp
// AnimationPlayer 側に足す
public void SnapshotPose();                    // 今の _pose を控える
public void SetBlendFromSnapshot(float weight, ReadOnlySpan<ClipWeight> clips);
```

`Evaluate` の 3 段目で、控えたポーズと混ぜた結果をさらに `weight` で混ぜる。

引っかかるのが2か所ある。

- **控えたポーズは時間とともに古くなる**。フェード中に元のクリップは進まないので、
  0.5 秒もかけると「途中で止まった姿勢」から繋ぐことになる。
  実際のエンジンは短いフェード(0.1〜0.2 秒)を前提にしてこれを許している
- **ブレンドツリーとの併用**。ツリーの2本 + 控えたポーズ1枚で、
  `MaxBlendClips` の勘定が変わる

### 課題3(難): 部位マスク付きレイヤーを足す

「下半身は走り、上半身は手を振る」を作れるようにする。
実際のゲームでいちばん使われている仕組みで、必要なのは**ノードごとの重み**。

```csharp
internal sealed class AnimationLayer
{
    public float[] NodeWeights;             // ノードごとの効き具合(0〜1)
    public ClipWeight[] Blend;
}
```

`AnimationPlayer.Evaluate` を「レイヤーを下から順に適用する」形に変え、
各レイヤーの寄与を `NodeWeights[i]` で重み付けする。

設計で悩むところが3つある。

- **マスクをどう作るか**。骨の名前で選ぶ(`Spine` 以下を上半身とする)のが定番だが、
  `Model.Nodes[].Parent` をたどって**部分木ごと**に塗るほうが確実
- **重みが 1 にならない境目**。腰のあたりで上半身レイヤーの重みが 0.5 になると、
  そこだけ2つのクリップの中間になる。**滑らかに落とすほうが自然**
- **`PoseAccumulator` は使い回せるか**。レイヤーは「足す」ではなく「上書きに近い」ので、
  素直に足すと下のレイヤーが薄まる。**Resolve のあとに Lerp する**ほうが素直

まず1レイヤー(全身マスク = 今と同じ)で動くことを確かめてから、マスクを入れるとよい。

## 動作確認済み環境

- Windows 11 / .NET 10 / RTX 3070 / 1920x1080(DPI 100%)
- `dotnet run --project reference/Day42 -c Release`

Fox(576 三角形 / 24 関節)を影・SSAO・IBL・ブルーム・FXAA 込みで **658 fps**。
ブレンドしていないとき(Day 41 相当)と**差が測れない**。

理由は計算量を数えれば分かる。クリップを2本混ぜると、
姿勢の評価が 26 ノード × 2 回、足し込みが 26 回増えるだけ——
**1フレームあたり 100 回に満たない浮動小数点演算**で、
描画の 0.17ms(影パス)に対して桁が3つ違う。

**ブレンドは安い**。高いのは頂点スキニング(GPU)とドローコールのほうで、
そこは Day 41 から1文字も変わっていない。

### 自己チェック(28 項目すべて合格)

上の「完成条件 8」のとおり。数字の読み方をいくつか。

- **チャタリング 99 回 対 1 回**。「ヒステリシスが要る」という言葉が、
  **100 フレームで 99 回**という具体的な数字になる。
  ここが 0 回なら、しきい値を跨げていない(テストの速度が間違っている)
- **周期 0.9333s**。walk 0.708 と run 1.158 の平均そのもの。
  ここが 0.708 や 1.158 に張り付くなら、重みが片方に寄っている
- **位相同期 OFF のずれ 7.01E+001**。Fox は 100 倍スケールなので、
  世界単位で 70 = 実寸で 70cm 相当。**関節1本ぶんの長さより大きい**
- **同期 ON のずれが厳密に 0**。`Advance(Duration)` で位相がちょうど 1 周する。
  float の丸めが乗って 1e-7 くらいになるかと思ったが、
  `((1.0f % 1.0f) + 1.0f) % 1.0f` が正確に 0 を返すので誤差も出なかった

### 検証の途中で分かったこと

**キーの段が6つ目になって、順番の制約が効いてきた**。
`Shift+Alt+F1` は `case Key.F1 when alt:`(Day 41)にも
`case Key.F1 when shift:`(Day 38)にも一致してしまう。

C# の `switch` はガード付き `case` を**上から順に**照合するので、
今日の塊を Day 41 の下に置くと**キーが1つも届かない**——
しかもコンパイルは通るし、警告も出ない。押しても何も起きないだけ。

Day 39 から「具体的なものほど上」と書き続けてきたが、
段が6つになると**書く前に「自分より一般的な case がどこにあるか」を探す**のが
先になった。次に段を足すとき(Ctrl+Shift+Alt しか残っていない)は、
そろそろキー割り当ての表を別ファイルに出すべき時期だと思う。

**`Span<ClipWeight>` をローカル関数から触れなかった**。
チャタリングを数えるところで、外側の `stackalloc` した Span を
ローカル関数の中で使おうとして `CS8175` になった。
ref 構造体はクロージャに入らないので、当然といえば当然。
`static` なローカル関数にして中で `stackalloc` し直した。

**`Program.cs` が 13,752 行になった**。設計書に書いたとおり、
今日足した 530 行のうち 300 行以上が自己チェックとキー操作で、
エンジンの機能そのものは 200 行ほど。Phase 7 が終わったら切り出す。
