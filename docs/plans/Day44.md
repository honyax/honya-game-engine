...
Day 43 は形が球と平面の2つしか無く、**判定のコードは 30 行**だった。
残りの 500 行は全部「当たったあとどうするか」の話だった。

今日はその比が逆になる。**判定と接触点の作成だけで 700 行**書く。
物理エンジンでいちばん歯応えがあると言われるのがここで、
「当たったか」を答えるより「**どこで、何点で触れているか**」を答えるほうがずっと難しい。

## 今日のゴール

**`Ctrl+Shift+Alt+1` を押すと、傾きを振った5つの箱が落ちてきて、
角で跳ね、向きを変え、最後は全部が面で床に落ち着く。**

`Ctrl+Shift+Alt+2` で「箱を積む / 球と箱 / 箱が転がる」へ切り替えられる。

| キー | 何が起きるか |
|---|---|
| `Ctrl+Shift+Alt+1` | 箱デモ ON/OFF。**今日の到達点** |
| `Ctrl+Shift+Alt+2` | 筋書き(箱を落とす / 箱を積む / 球と箱 / 箱が転がる) |
| `Ctrl+Shift+Alt+3` | 接触点の表示。**光る玉がマニフォールドの点** |
| `Ctrl+Shift+Alt+4` | 1組あたりの接触点 1/2/4。**1 にすると箱が落ち着かない** |
| `Ctrl+Shift+Alt+5` | 箱を1つ降らせる |
| `Ctrl+Shift+Alt+6` | 接触の内訳(どの軸から出た接触か) |
| `Ctrl+Shift+Alt+7` | 接触点の解き方(同時 / **順番**)。**切ると積んだ箱が崩れる** |
| `Ctrl+Shift+Alt+0` | 今日の自己チェック(47 項目) |

### 今日いちばん大事な2つ

**1つ目は「15 本の軸」**。箱と箱が離れているかどうかは、
たった 15 通りの向きに投影して確かめれば決着する。

```csharp
for (int i = 0; i < 3; i++) TryAxis(a.Axis(i));            // A の面の法線 3 本
for (int i = 0; i < 3; i++) TryAxis(b.Axis(i));            // B の面の法線 3 本
for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++)
        TryAxis(Cross(a.Axis(i), b.Axis(j)));              // 辺 × 辺 9 本
```

**面の法線 6 本だけでは足りない**。角どうしが斜めに擦れ違う配置では、
どちらの面から見ても重なって見えるのに実際は離れている——
自己チェックはこの配置を**ランダム探索で実際に探し当てて**見せる(6 回目の試行で見つかる)。

**2つ目は「4点を同時に解く」**。

```csharp
// 4点ぶんを**同じ状態から**決めてから、まとめて掛ける
for (int k = 0; k < count; k++) magnitudes[k] = NormalImpulse(_contacts[start + k]);
for (int k = 0; k < count; k++) ApplyNormalImpulse(start + k, magnitudes[k] / count);
```

Day 43 と同じく「順番に1点ずつ」解くと、
**1点目だけが箱の重さを全部背負って**そこにトルクが立ち、
摩擦の無い世界では傾いた接触が横向きに押すので、
**積んだ箱が3段で崩れる**(`Ctrl+Shift+Alt+7` で切り替えて見られる)。

### 今日もシェーダには触らない

差分は `Physics/` に新しいファイル3本(949 行)と、既存4本の書き換え、そして `Program.cs`。
`.vert` も `.frag` も 1 行も変わらない。

**物理は今日も絵を1ミリも知らない**。おかげで自己チェック(47 項目)は
窓を1枚も出さずに走る——Day 43 からの線がそのまま保たれている。

## 事前に読む資料

- **Game Physics Engine Development**(Ian Millington)第 13〜14 章
  接触の生成(contact generation)の章。箱と箱、箱と点、箱と球が一通り出てくる。
  この本は SAT を「最小重なりの軸を探す」形で書いていて、今日のコードと同じ構成。
- Erin Catto, *Box2D Lite* のソースと GDC 資料
  <https://box2d.org/publications/>
  2D だが、**基準面を選んで入射面をクリップする**という接触点の作り方は 3D と同じ。
  `Collide` 関数(200 行ほど)を読んでおくと、今日の `Sat.BuildFaceContact` が短く感じる。
- Christer Ericson, *Real-Time Collision Detection* 4.4 節(OBB)
  15 本の軸の一覧と、平行な辺で外積が退化する話。
  今日 `ParallelEpsilon` で外している場合分けの出どころ。
- Dirk Gregorius, *Robust Contact Creation for Physics Simulations*(GDC 2015)
  <https://box2d.org/files/ErinCatto_ContactManifolds_GDC2007.pdf>(Erin Catto 版)
  接触マニフォールドをどう作り、どう4点に絞るかの決定版。
  **今日入れなかった「点の選び方(広がりを見る)」**はここに書いてある(改造課題2)。
- 慣性モーメントの表(箱)
  <https://en.wikipedia.org/wiki/List_of_moments_of_inertia>
  Day 43 で「Day 44 の箱 `1/12 m (h²+d²)` はここから引く」と書いた、その日が来た。

## 理論の要点

### 1. 箱の慣性テンソル — 軸ごとに違う、が全部の始まり

Day 43 の球は**等方**だった。どの軸まわりにも回りにくさが同じ(`2/5 m r²`)なので、
慣性テンソルは「同じ数が3つ並んだ対角行列」で、回しても変わらなかった。

箱は違う。

```
  I_x = 1/12 · m · (h² + d²)      h, d は y, z 方向の**全長**
  I_y = 1/12 · m · (d² + w²)
  I_z = 1/12 · m · (w² + h²)
```
...
長い棒を軸まわりに回すのが簡単で、横に振るのが大変なのはこれ。

半分の長さ e で書くと全長は 2e なので、コードは `1/3 m (e_y² + e_z²)` になる。
...
**3つの値が違うので、世界での慣性テンソルが向きによって変わる**。
Day 43 で「使わないうちから正しい形で書いておく」と言って
`R^T · D · R`(`UpdateInertiaWorld`)を書いておいたのが、今日ようやく効く。
球のときは入力と同じものが返るだけの、何もしない関数だった。

### 2. 分離軸定理 — 無限の向きが 15 本に減る

**2つの凸形状が離れているなら、その2つを分けてしまう平面が必ず1枚ある。**

裏を返すと、**どの向きに投影しても重なっているなら当たっている**。
これが分離軸定理(SAT)で、証明は「凸集合の分離超平面定理」そのもの。

実用にするには「どの向きを試せばよいか」が要る。向きは無限にあるが、
凸多面体どうしなら候補は有限個で足りる。

| 候補 | 箱と箱では | なぜ要るか |
|---|---|---|
| それぞれの**面の法線** | 3 + 3 = 6 本 | 面と面、面と頂点で分かれる場合を捕まえる |
| それぞれの**辺の向きの外積** | 3 × 3 = 9 本 | **辺と辺**が擦れ違う場合を捕まえる |

1本の軸を試すのは、**線分どうしの重なりを見るだけ**。

```
  重なり = r_A(n) + r_B(n) - |(中心間ベクトル)·n|
```

`r(n)` は箱を軸 n に落としたときの長さの半分で、
これは各軸の寄与の**絶対値の和**になる。
...
絶対値なのは、「どの角がいちばん n の側に突き出るか」を場合分けせずに済ませるため。

**1本でも重なりが負なら、その時点で離れている**——残りは調べなくてよい。
これが SAT が実用になる理由で、離れている組(大多数)は数本で抜ける。

#### 辺×辺の 9 本は本当に要るのか

要る。**角どうしが斜めに擦れ違う配置**では、
面の法線 6 本のどれで見ても重なって見えるのに、実際には離れている。
面の法線だけで済ませると、**斜めに置いた箱がすり抜ける**。

自己チェックはこれを理屈ではなく実物で見せる——
ランダムな配置を試して「面の法線だけなら当たったと答えてしまうが、
辺の軸を見れば離れている」配置を探し当てる。
**6 回目の試行で見つかる**ので、珍しい配置でもない。

#### いちばん浅い軸を採る

15 本すべてで重なっていたら当たっている。押し戻す向きは
**重なりがいちばん浅かった軸**を採る——そこがいちばん抜けやすい向きだから。
深い軸で押し戻すと、床にわずかに沈んだ箱が真横へ吹っ飛ぶ。

ただし**辺×辺の軸には 5% の下駄を履かせる**。
ほぼ平行な2つの辺の外積は長さがほとんど 0 で、正規化すると
向きが数値誤差でぐらぐら揺れる。その軸がわずかな差で最小になると、
面で触れているのに「辺どうし」と判定されて接触点が1つに落ち、箱が震え出す。

### 3. 形を体から切り離す — `Collider`

Day 43 の `RigidBody` は `Radius` を直接持っていた。形が球1つなら成立するが、
箱に半径は無い。今日は形を `Collider` という構造体に切り出す。

```csharp
internal readonly struct Collider
{
    public readonly ColliderKind Kind;      // Sphere / Box / Plane
    public readonly float Radius;           // 球
    public readonly Vector3 HalfExtents;    // 箱
    public readonly Vector3 Normal;         // 平面
}
```

**クラス階層にしない**のがここの判断。`SphereCollider : Collider` のように書くと、
判定のたびに仮想呼び出しと型チェックが入る——
N² 組を回す内側のループでいちばん高いものを毎回踏むことになる。
種類の札で分岐するほうが素直で速く、Box2D も Bullet も結局こうなっている。

代償は**使わないフィールドが眠ること**。球のときに `HalfExtents` は 0 のまま。
32 バイトなので気にしないことにした。

#### Day 43 の歪みが2つ同時に片付く

Day 43 の設計書に、片付ける予定として2つ書いた。**両方とも今日消えた**。

| Day 43 の歪み | どう消えたか |
|---|---|
| `RigidBody.Radius` | 形が `Shape` に移った。慣性の式も形の側(`Collider.InertiaLocal`)へ |
| `ContactPoint.B = -1`(相手が平面) | 平面が「平面の形を持った静的な体」になり、相手は必ず体の番号で指せる |

平面を体にすると `RigidBody.Position` が「重心」でなくなるが、
逆質量が 0 なので重心が使われる場所が無い——ここでは「平面が通る点」の意味になる。
Bullet の `btStaticPlaneShape` もまったく同じ割り切りをしている。

おかげで `PhysicsWorld` の解決から `b?.` という書き方が全部消えた。

### 4. 接触マニフォールド — なぜ1点では足りないのか

Day 43 の接触は点が1つだった。**球はどこで触れても1点**なので、それで足りていた。

箱では足りない。床に置いた箱を1点で支えると、その点まわりにトルクが立って箱が傾く。
次のステップでは傾いた先の角が最深点になり、そこを1点で押し戻すのでまた傾く——
**箱が永久にガタガタと揺れ続ける**。

`Ctrl+Shift+Alt+4` で上限を 1 にすると、この状態がそのまま見られる。
自己チェックでも数字で出る。

```
  4点: 最後の1秒の角速度 0.00000 rad/s、傾き 0.0000 度
  1点: 最後の1秒の角速度 0.73 rad/s、傾き 0.84 度
```

**面で触れているものは面で支えないと落ち着かない**、というのがマニフォールドが要る理由のすべて。

#### 点は4つまで

凸多面体どうしの接触面は凸多角形なので、理屈のうえでは頂点がいくつでも出うる
(箱どうしなら最大8つ)。ただし**凸多角形を支えるのに要るのは4点**——
外周から4つ取れば、残りはその内側に入る。Box2D も Bullet も同じ上限。

**法線は1本だけ**持つ。面どうしの接触では4点とも同じ向きに押し戻すのが正しく、
点ごとに法線を持たせると、床の箱が場所によって違う向きに押されて震える。

接触点の数がそのまま**支え方の数**になっているのが目で見える。
...
SAT が「面の軸」を選んだとき、接触点は多角形クリップで作る。手順は4段。

1. **基準面**を決める … 重なりが最小だった軸を持っていたほうの箱の、相手を向いた面
2. **入射面**を決める … 相手の面のうち、外向き法線が基準面と最も逆を向いているもの
   (= いちばん正面から当たっている面)
3. **クリップ** … 入射面の4頂点を、基準面の側面4枚で切り落とす
4. **ふるい** … 残った点のうち、基準面より内側(めり込んでいる)ものだけを採る

**なぜ「入射面を基準面の枠で切る」のか**。
触れているのは2つの面の重なっている部分で、凸どうしなのでそれは凸多角形になる。
入射面(4角形)を基準面の枠(側面4枚)で切ると、まさにその重なりが出てくる。

床の上に箱を置けば4点。**床から半分はみ出せば**、
はみ出した側の2点が切り落とされて、代わりに**床の縁の上に新しい2点が生まれる**。
自己チェックはこれを「いちばん外の点の x 座標が台の縁とぴったり一致する」で確かめている。

クリップの中身は Sutherland-Hodgman——頂点を順にたどって

- 今の点が内側なら、その点を残す
- 今の点と次の点で内外が入れ替わるなら、**境界との交点**を足す

の2つを繰り返すだけ。**Day 8 で視錐台に三角形をクリップしたのと同じ手続き**で、
2D でも 3D でも形は変わらない。

**点の数は減るとは限らない**。交点が新しく増えるので、4角形を4枚で切ると最大8つ。
そこから4つに絞るのは「深い順に4つ」という素朴なやり方にしてある(改造課題2)。

辺×辺が選ばれたときは話が簡単で、**接触点は1つだけ**。
角と角が擦れ違っている場面なので、触れているのは1点しかない。
2つの辺の最近接点を求めればそこが接触点になる。

### 6. 位置の補正の測り直し — 体に貼り付けた印

Day 43 の位置補正は、周ごとに**判定関数をもう一度呼んで**めり込みを測り直していた。
あれは「形が球しか無い」ことに寄りかかった書き方で、
実際 `Measure` の中で `a.Radius` を直接読んでいた。

箱では判定そのものが重い(15 本の軸 + クリップ)ので、
4周 × 接触の数だけ呼び直すと目に見えて遅くなる。

代わりに、接触点を**両方の体に貼り付けた印**として物体座標で覚えておく。

```csharp
Vector3 localA = Transform(point - a.Position, Conjugate(a.Orientation));
Vector3 localB = Transform(point - b.Position, Conjugate(b.Orientation));
```
...
で追いかけられる。**判定を1回も呼ばずに測り直せる**のがこの持ち方の値打ちで、
しかも**形を1つも知らない**——カプセル(Day 45)でも地形(Day 46)でもこのまま動く。

これは Box2D の位置ソルバとほぼ同じ手。

### 7. 4点をどう解くか — 今日いちばん深い分岐

マニフォールドを入れると、**その4点をどう解くか**という問題が新しく生まれる。

Day 43 と同じく「接触を順番に1つずつ」解くと、こうなる。

- 1点目を解いた時点で、その点の相対速度は 0 になる
- 残り3点は「もう離れている」と見て何もしない
- つまり**1点目だけが箱の重さを全部背負う**

角1つで支えているのと変わらないので、そこを中心にトルクが立つ。
摩擦の無い世界では傾いた接触が横向きに押すので、**積んだ箱がじりじり滑って崩れる**。
実測で3段の柱が 15 秒で完全に崩れ、箱が 10m 以上流れていった。

直し方は素直で、**4点ぶんを同じ状態から計算してから、まとめて掛ける**。
...
**点の数で割る**のは、「1点だけで速度差を消せる大きさ」を4点ぶん足すと
4倍になってしまうから。割ったぶん1周で消せる量は減るが、反復回数で取り返す——
**ヤコビ法(同時に更新)とガウス・ザイデル法(順に更新)の古典的なトレードオフ**で、
ここでは「速く収束すること」より「偏らないこと」を採った。

**組と組の間は今までどおり順番**(ガウス・ザイデル)。
そちらは偏りが問題にならず、むしろ「直前の組の結果を見る」ことで
柱の下から上へ荷重が伝わる(Day 43 の要点7)。

`Ctrl+Shift+Alt+7` でこの2つを切り替えられる。
Day 43 の積分法の切り替え(`Ctrl+Shift+Alt+F2`)と同じ性格のつまみで、
**正しい解き方と素朴な解き方を並べて見るため**だけにある。

### 8. 今日入れなかったもの

3つある。どれも「入れないと何が起きるか」が今日の絵に出ている。

| 無いもの | 絵に出る症状 | いつ入れるか |
|---|---|---|
| **摩擦** | 傾けた台に乗せた箱が必ず滑り落ちる。ドミノは倒れず滑る | Day 47 |
| **蓄積インパルス / 温存** | 柱が段数ぶん沈む(6段で 57mm) | Day 47 |
| **ブロードフェーズ** | 体が増えると N² 組。箱は 15 軸ぶん球より重い | Day 46 |

**沈むことと崩れることは別の話**なのが今日の学び。
4点を同時に解いたことで**崩れなく**なったが、**沈む**のは直っていない。
沈みは「1周ごとにインパルスをゼロから決め直している」ことが原因で、
そこは蓄積インパルスと温存(warm starting)の担当になる——
Day 43 の改造課題3で予告したとおり。
...
| ファイル | 役割 |
|---|---|
| `Physics/Collider.cs` | `ColliderKind` と `Collider`。**形を剛体から切り離した**(要点3) |
| `Physics/ContactManifold.cs` | `ManifoldSource` / `ManifoldPoint` / `ManifoldPointArray` / `ContactManifold`。**最大4点の束**(要点4) |
| `Physics/Sat.cs` | 分離軸定理と面のクリップ。**今日いちばん長い**(要点2・要点5) |

### 変更ファイル

| ファイル | 何が変わったか |
|---|---|
| `Physics/Shapes3D.cs` | `Box3D` を追加。`Sphere3D` / `Plane3D` のコメントを現在形に |
| `Physics/RigidBody.cs` | `Radius` → `Shape`。`CreateBox` / `Create` / `CreateStatic` / `CreatePlane` / `ToBox` / `ToPlane` |
| `Physics/Collision3D.cs` | `SphereBox` / `BoxPlane` / `Collide`(形の組み合わせで振り分ける窓口) |
| `Physics/PhysicsWorld.cs` | `ContactPoint` の作り直し、マニフォールド単位の解決、印による測り直し |
| `Program.cs` | 箱の筋書き4つ、接触点の描画、`Ctrl+Shift+Alt+数字キー`、HUD、自己チェック |

**`Render/` も `shaders/` も1文字も変わっていない**。
箱は Day 12 から使っている単位立方体をそのまま拡大して描いている。

### 写経する順番

**依存の順に並べてある**。ただし今日は**5 から 8 までが一続き**で、
`RigidBody.Radius` を消した時点で `PhysicsWorld` と `Program.cs` が壊れる。
**7・8 まで進めて初めてビルドが通る**ので、途中の赤は気にしないでよい。

1. **`Physics/Shapes3D.cs`**(変更) — 末尾に `Box3D` を足す。
   `ProjectedRadius` が今日の判定の土台になる(要点2)。
   `Sphere3D` と `Plane3D` の説明も少し直っている(**コメントだけ**)
2. **`Physics/Collider.cs`**(新規) — `ColliderKind` と `Collider`。
   `InertiaLocal` が慣性の式を引き受ける(要点1)
3. **`Physics/ContactManifold.cs`**(新規) — 次の順で
   - `ManifoldSource`(接触がどの軸から出たか)
   - `ManifoldPoint`(点と深さだけ)
   - `ManifoldPointArray`(`[InlineArray]`。**構造体の中に配列を持ちたいだけ**)
   - `ContactManifold`(`Add` / `Reduce` / `Flipped`)
4. **`Physics/Sat.cs`**(新規) — **今日の山場**。次の順で足すとよい
   - `EdgeBias` / `ParallelEpsilon` と `TryAxis`(**1本の軸を試すだけ**)
   - `BoxBox`(15 本の軸を回して、いちばん浅い1本を採る)
   - `MostAlignedAxis` / `EdgeMidpoint` / `SignedExtent`(小さい道具)
   - `Clip` と `Swap`(Sutherland-Hodgman。**`Span` はタプルで入れ替えられない**)
   - `BuildFaceContact`(要点5)/ `ClosestPointsOnLines` / `BuildEdgeContact`
5. **`Physics/RigidBody.cs`**(変更) — **ここからビルドが通らなくなる**
   - `Radius` フィールドを消して `Shape` を足す
   - `CreateSphere` を `Create` 経由に書き換え、`CreateBox` を2本足す
   - `CreateStaticSphere` を `CreateStatic` へ、`CreatePlane` を新設
   - `ToSphere` を `Shape.Radius` 参照に直し、`ToBox` / `ToPlane` を足す
   - コメントの「Day 44 で…」が現在形に変わっている箇所が3つ
6. **`Physics/Collision3D.cs`**(変更) — クラスの説明を書き換えてから
   - `SphereBox`(**中心が箱の中に入ったときだけ別扱い**)
   - `BoxPlane`(8つの頂点を落とすだけ。**いちばん分かりやすいマニフォールド**)
   - `Collide` と `Wrap`(形の組み合わせで振り分ける窓口)
7. **`Physics/PhysicsWorld.cs`**(変更) — 次の順で
   - `ContactPoint` を作り直す(`Plane` を消し、`LocalA` / `LocalB` / `Separation` を足す)
   - `_manifolds` / `_sourceCounts` と、つまみ2つ
     (`MaxContactsPerPair` / `SolveContactsTogether`)
   - `ContactPairs` / `DynamicCount` / `PlaneCount` / `SourceCount`
   - `AddPlane` を「静的な体を足す」に書き換え、`_planes` を消す
   - `Step` の3段目をマニフォールド単位の周回に
   - `GenerateContacts` と `Add`(**印をここで作る**)
   - `SolveManifold` / `ApplyNormalImpulse` / `NormalImpulse`(**要点7**)
   - `CorrectPositions`(印で測り直す。`Measure` は消える)
8. **`Program.cs`** — 最後。次の順に足す
   - `PhysicsScene` に `BoxDrop` / `BoxStack` / `BoxMix` / `BoxTumble` を追加
   - フィールド(`_showContacts` / `_contactMaterial` / `ContactLimits` /
     `_contactLimitIndex` / `_boxSpawnCount`)
   - `OnLoad` に `_contactMaterial` の生成(`_physicsMaterial` の直後。
     **`white` は同じ関数の中で作ってあるものを使う**)
   - `ShowPhysicsDemo` のカメラ(柱の筋書きだけ引いて上げる)
   - `BuildPhysicsScene` の平面を `AddPhysicsPlane` 経由にし、`switch` に4つ足す
   - `AddPhysicsPlane`(**色の配列と添字を揃えるため**)
   - `BuildBoxDropScene` / `BuildBoxStackScene` / `BuildBoxMixScene` /
     `BuildBoxTumbleScene` / `SpawnBox`(`SpawnBall` の直前)
   - `KickBodies` の `body.Radius` を `body.Shape.BoundingRadius` に
   - `RenderPhysics` / `RenderContacts` / `BodyMesh` / `BodyMatrix`
   - `RenderShadowPass` の物理デモの枝(**平面を飛ばす**)
   - `PhysicsSceneLabel` に4つ / `IsBoxScene` / `ShapeLabel`
   - `PhysicsLabel` の体の数を `DynamicCount` に / `ContactLabel` を新設
   - `DrawOverlayInfo` に `ContactLabel()` の1行
   - `DescribePhysics` の書き換えと `DescribeContacts`
   - **`RunRigidBodyCheck`(Day 43 の自己チェック)のしきい値を1つ緩める**
     — 「反復1周だと柱が沈む」の倍率が 3.0 から 2.0 へ。
     今日の位置補正で**数字が良くなったせいで通らなくなった**項目
   - `RunBoxCollisionCheck`(`RunRigidBodyCheck` の直後)
   - キー(`Ctrl+Shift+Alt+1〜7` と `0`)。**switch のいちばん上**に置くこと
   - `OnLoad` のヘルプ

**キーの位置は今日も間違えやすい**。`Ctrl+Shift+Alt+1` は
`case Key.Number1 when ctrl && shift:` にも `case Key.Number1 when ctrl:` にも一致するので、
今日の塊を下に置くと**キーが1つも届かない**。しかも警告は出ない。
Day 39 から毎日書いている「ガード付き case は具体的なものほど上」がそのまま続く。

## 設計書

**層は今日も増えていない**。`Physics/` の中で型が8つ増え、
既存の4つが書き換わっただけ——矢印は1本も足していない。
`Physics/` は今日も**どこにも依存しない層**のまま。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Physics/ColliderKind` | `Physics/` | 形の種類の札(球 / 箱 / 平面) |
| `Physics/Collider` | `Physics/` | 形。**寸法と慣性の式を体から引き取った**(要点3) |
| `Physics/Box3D` | `Physics/` | 向きを持った直方体(OBB) |
| `Physics/ManifoldSource` | `Physics/` | 接触がどの軸から出たか(面 / 辺 / 点) |
| `Physics/ManifoldPoint` | `Physics/` | 接触点1つ。位置と深さだけ |
| `Physics/ManifoldPointArray` | `Physics/` | `[InlineArray]`。**構造体の中に4つ並べたいだけ** |
| `Physics/ContactManifold` | `Physics/` | 法線1本 + 最大4点(要点4) |
| `Physics/Sat` | `Physics/` | 分離軸定理と面のクリップ。**今日いちばん長い 524 行** |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Physics/Shapes3D` | `Box3D` が入った | +166 / -4 |
| `Physics/RigidBody` | `Radius` が消えて `Shape` に。生成が `Create(mass, shape)` 経由へ | +105 / -23 |
| `Physics/Collision3D` | 判定が3本増え、`Collide` という窓口ができた | +193 / -12 |
| `Physics/PhysicsWorld` | 平面が体になり、解決がマニフォールド単位になった | +363 / -136 |
| `Program.cs` | 箱の筋書き4つ、接触点の描画、キー、HUD、自己チェック | +1,246 / -29 |
...
今日いちばん誘惑が強かったのは**接触点の描画**で、
`PhysicsWorld` に `DrawDebug` を生やせば3行で済む。
そうしなかったので、`Program.RenderContacts` が
`Physics.Contacts` を読んで光る玉を並べている——
物理の側は「接触点を持っている」だけで、それが絵になるかは知らない。

配当は今日も自己チェックに出る。**47 項目が窓を1枚も出さずに走る**。

### 形が体から出たこと — 今日の設計の中心

Day 43 の設計書に「Day 44 で箱を足すときに、この2つは同時に片付く」と書いた。
そのとおりになったので、変化を並べておく。

```
  Day 43:  RigidBody { float Radius; }          ← 形が球だと言い切っている
           PhysicsWorld { List<RigidBody>; List<Plane3D>; }   ← 平面だけ別枠
           ContactPoint { int A; int B; int Plane; }          ← B = -1 が「平面」

  Day 44:  RigidBody { Collider Shape; }        ← 形は札 + 寸法
           PhysicsWorld { List<RigidBody>; }    ← **リストが1本になった**
           ContactPoint { int A; int B; }       ← **必ず両方とも体の番号**
```

消えたコードのほうが分かりやすい。`SolveVelocity` にあった
...
が、全部ただの `_bodies[contact.B]` になった。`b?.` は6箇所あった。

**「形が1つしか無いうちに抽象化すると、たいてい間違った形の抽象になる」**
と Day 43 で書いて先送りしたのが正解だったと思う。
球しか無い時点で `Collider` を作っていたら、
おそらく「半径を持つ基底クラス」のような形になっていた——
実際に要ったのは<b>継承の無い、種類の札と数個の数だけの構造体</b>だった。

### 判定の窓口を1つにしたこと

Day 43 の `GenerateContacts` は、形の組み合わせを直に呼び分けていた。
...
形が5種類になると 15 通りになるので、今日から窓口を1つにした。

```csharp
ContactManifold manifold = Collision3D.Collide(a, b);
```

`Collide` の中は `switch (a.Shape.Kind, b.Shape.Kind)` の表引きで、
**逆順の組は `Flipped()` で法線を裏返す**。
おかげで「箱と球」を書かずに「球と箱」だけ書けばよく、
15 通りのうち実際に書くのは5通りで済んでいる。

**`Collision3D` が `RigidBody` を知ることになった**のは、今日の小さな譲歩。
Day 43 までは形しか知らない層だったが、
「どの形を持っているか」を尋ねる相手が体しか無いので、同じ層の中で参照を1本足した。
`Physics/` の外へは依然として出ていないので、層の線は破れていない。

### 5段の順番は変わっていない

`PhysicsWorld.Step` の5段は Day 43 のまま。
**2段目が返すものの形だけが変わった**(1点 → 最大4点の束)。

| 段 | やること | Day 44 で変わったか |
|---|---|---|
| 1 | 積分(速度 → 位置) | 変わらない |
| 2 | 判定(接触を集める) | **マニフォールドを返す。組の区切りも覚える** |
| 3 | 速度の解決 | **組の中は同時、組の間は順番**(要点7) |
| 4 | 位置の補正 | **印で測り直す**(要点6)。判定を呼ばなくなった |
| 5 | 力の溜め場を空にする | 変わらない |

### 今日残した歪み(3つ)

**1つ目: `BodyColors` という並行配列**。
色は `Program` 側の `List<Vector4>` で、添字を体の番号と揃えているだけ。
平面が体になった途端に「平面のぶんも席を作らないと添字がずれる」という手当てが要った
(`AddPhysicsPlane`)。**並行配列の弱いところ**が素直に出た箇所で、
体の数が増えたら `Dictionary` か体側の付帯情報に移すことになる。

**2つ目: 柱が沈む**。4点を同時に解いたことで**崩れなく**なったが、
6段の柱は 57mm 沈む(許容は 5mm)。
1周ごとにインパルスをゼロから決め直しているのが原因で、
蓄積インパルスと温存(warm starting)が Day 47 の担当になる。
Day 43 の改造課題3で予告したとおりの場所に、予告どおりの症状が出ている。

**3つ目: `Program.cs` が 16,349 行になった**。
Day 43 で 15,123 行、今日 1,226 行足した。内訳は Day 43 と同じ傾向。

| 何 | 行数 |
|---|---|
| 自己チェック(`RunBoxCollisionCheck`) | 約 610 |
| デモの組み立て・描画・HUD | 約 410 |
| キー操作(`Ctrl+Shift+Alt+数字キー`) | 約 105 |
| ヘルプ・フィールド・配線 | 約 120 |

**エンジンの機能そのものは1行も `Program.cs` に無い**——
今日の中身は全部 `Physics/` の新規 949 行 + 既存への 827 行の追記に入っている。
切り出しの計画は Day 42 のまま(Day 50 で `Demo/DemoKeys.cs` /
`Demo/SelfChecks.cs` / `Demo/Hud.cs` へ)。

**キーの段は数字キーでほぼ打ち止め**。Day 45 以降は
Day 40 の `FeatureToggles` のように、割り当ての表そのものをデータにするしかない。

Day 43 の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の2つ。

| 図 | 何が変わったか |
|---|---|
| `Ecs と Physics` のクラス図 | **3D の側にクラスが7つ増え、`RigidBody` と `PhysicsWorld` が書き換わった** |
| 剛体が1ステップ進むまで | 2段目がマニフォールドを返すようになり、3・4段目の中身が変わった |

新しく足した図が2つ。

| 図 | 何を描いたか |
|---|---|
| 箱と箱が当たるまで | 15 本の軸をどう回し、どこで接触点の作り方が分かれるか |
| 接触点が4つできるまで | 基準面 → 入射面 → クリップ → ふるい |
...
### Ecs と Physics — どこにも依存しない2つ(今日 `Physics` の3D 側が倍になった)
...
#### 2D の側(Day 26。Day 43 に続いて今日も1文字も変わっていない)
...
#### 3D の側(Day 44 でさらに7つ増え、2つが書き換わった)
...
| | Day 43 | Day 44 |
|---|---|---|
| 形の持ち方 | `RigidBody.Radius`(球だと言い切っている) | `RigidBody.Shape`(**札 + 寸法**) |
| 平面 | `PhysicsWorld` が別のリストで持つ | **形の1つ**。静的な体になった |
| 判定の戻り値 | `Contact3D`(1点) | `ContactManifold`(**最大4点 + 法線1本**) |

**2D 側との性格の違い**は Day 43 のまま変わっていない。

| | 2D(Day 26) | 3D(Day 43〜44) |
|---|---|---|
...
Day 43 のキー(`Ctrl+Shift+Alt+F1〜F12`)はそのまま使える。
今日のキーは**同じ修飾キー + 数字キー**。

### 1. `Ctrl+Shift+Alt+1`: 傾きを振った5つの箱が落ちて、面で落ち着く

**今日の到達点**。傾き 0 / 0.10 / 0.35 / 0.62 / 0.95 rad の5つの箱が
同じ高さから落ちてくる。色は緑(まっすぐ)から紫(大きく傾いている)へ。

- まっすぐな箱は**4点で着地して、そのまま止まる**
- 少し傾いた箱は**辺の2点で着地し、倒れて4点になる**
- 大きく傾いた箱は**角の1点で着地し、跳ねて向きを変えてから落ち着く**

3秒ほどで5つとも**面を下にして**床に落ち着く。
傾きの大きかった2つは、途中で 90 度転がって別の面で着地する。

**摩擦が無いので、落ち着いたあともゆっくり滑ったり回ったりは続く**
(縦軸まわりの回転は接触に何の抵抗も受けない)。
止めたければ Day 47 の摩擦を待つことになる。
一方で<b>ガタガタと上下に揺れているなら、それは別の話</b>——
接触点が1つしか出ていない。

HUD の2行目(今日足した行)がこうなっていれば正しい。

```
接触[5組 20点 上限4]  面:5  辺:0  点:0  解き方:同時  表示:ON
```

めり込みは 1cm 前後(許容 5mm の2倍)で落ち着く。
これも Day 47(蓄積インパルス)の宿題。

### 2. `Ctrl+Shift+Alt+3`: 接触点を消して、また出す

光る橙色の玉がマニフォールドの点。**深いほど大きい**。
床に平らに乗った箱では、4つの玉が箱の底面の四隅に等間隔で並ぶ。

角で着地している瞬間に止めて(`Ctrl+Shift+Alt+F10`)カメラを回すと、
玉が1つしか無いことが確かめられる。

### 3. `Ctrl+Shift+Alt+4`: 1組あたりの接触点を 1 にする

**今日いちばんの見どころ**。押すと `4 → 1` に落ちる(次に押すと 2、その次に 4)。

落ち着いていた箱が、**その場でカタカタと揺れ始める**。
1点で支えられた箱がその点まわりに傾き、傾いた先の角が次の最深点になり…を
延々と繰り返している。HUD の「接触」が `5組 5点` になり、めり込みも増える。

4 に戻すと、数フレームで静止に戻る。

### 4. `Ctrl+Shift+Alt+2`(1回目): 箱を積む

一辺 0.84m の箱を6段。カメラが少し引いて上がる。

**崩れない**。球の積み上げ(`Ctrl+Shift+Alt+F4` の2番目)と見比べると、
球は下が少しずれると転がり落ちるのに対し、箱は4点で支えるので横ずれに強い。

ただし**段数ぶん沈む**。HUD の「めり込み」が 4〜6cm まで増える。
これが Day 47(蓄積インパルスと温存)の宿題。

### 5. `Ctrl+Shift+Alt+7`: 解き方を「順番」にする

柱を出したまま押す。**数秒で柱が滑り出して崩れる**。
HUD の「解き方」が `**順番**` に変わっている。

もう一度押して「同時」に戻し、`Ctrl+Shift+Alt+2` で組み直すと、
また崩れなくなる。**この差が今日の要点7**。

### 6. `Ctrl+Shift+Alt+2`(2回目): 球と箱

傾けた静的な箱(台)の上へ、球と箱が交互に落ちてくる。
**今日足した判定が全部走る**——球×箱、箱×箱、箱×平面、そして Day 43 の2本。

HUD の「面 / 辺 / 点」の内訳が刻々と変わる。
`点` が球の接触(1点しか出ない)で、`面` が箱の接触。

摩擦が無いので、乗ったものは必ず台から滑り落ちる。
**台には端がある**ので、平面と違って縁から落ちていく——
Day 43 で「平面には端が無い」と書いた話への答えになっている。

### 7. `Ctrl+Shift+Alt+2`(3回目): 箱が転がる

細長い箱(1.5 × 0.32 × 0.48m)を回しながら落とす。
角と角がすれ違うように当たると HUD の `辺` が 0 でなくなる。

`Ctrl+Shift+Alt+6` で内訳をコンソールへ出すと、
どの組が辺×辺で解かれているかまで見える。

### 8. `Ctrl+Shift+Alt+5`: 箱を降らせる

押すたびに1つ増える。縦横比が3通りで巡回するので、
**細長いものほど倒れやすい**のが見える。

20 個ほど降らせても HUD の `ms` は 0.1ms 前後のまま。
箱は 15 本の軸を試すぶん球より重いが、まだブロードフェーズは要らない。

### 9. Day 43 までの機能が全部生きていること

- `Ctrl+Shift+Alt+F1`: 球の物理デモ(落下 / 積み上げ / 撞き玉)
- `Ctrl+Shift+Alt+F2`: 積分法の切り替え(数字キー `4` と併せて陽的オイラーの発散)
- `Ctrl+Shift+F1`: デモ v1 / `Alt+F1`: アニメーション / `2`: 材質グリッド
- `Ctrl+Shift+Alt+F12`: Day 43 の自己チェック(45 項目。**今日の書き換えのあとも全部通る**)

球の物理デモにも**接触点の光る玉が出る**ようになっている
(`Ctrl+Shift+Alt+3` で消せる)。球は1点でしか触れないので、
玉が1つずつしか出ないのが箱との違いとして見える。

### 10. `Ctrl+Shift+Alt+0`: 今日の自己チェック(47 項目すべて合格)

コンソールに 47 行の `[OK]` が並び、最後に
`すべて合格(箱は落ち、面で支えられ、積める)` と出る。

**窓は1枚も出ない**——`Physics/` が GL を知らないので、
このチェックはウィンドウを開かずに全部走る。

## 改造課題

### 課題1(易): 箱の重心をずらしてみる

`RigidBody.Create` は慣性テンソルを「重心が箱の中心にある」前提で作っている。
`Collider.InertiaLocal` の戻り値の1成分だけを 10 倍(あるいは 1/10)にして、
`Ctrl+Shift+Alt+1` で落としてみる。

- どの軸を重くすると、着地後の転がり方が変わるか
- 3つとも同じ値にすると(= 球と同じ等方にすると)何が起きるか

**慣性テンソルは「どの向きに回しにくいか」の表**なので、
値をいじると箱の性格が変わる。物理的に正しくない値でも動くのが確かめどころで、
「それらしく動く」ことは正しさの証拠にならない、を体で覚えるための課題。

### 課題2(中): マニフォールドの点の選び方を直す

`ContactManifold.Add` は満杯のとき「いちばん浅い点」を捨てている。
**深さだけで選ぶと、面のごく狭い一角に4点が固まることがある**——
そうなると面で支えているつもりが実質1点になり、また箱が揺れる。

Box2D と Bullet は**広がりを見て**選んでいる。

1. いちばん深い点を1つ採る
2. そこから**いちばん遠い**点を採る
3. その2点が作る線分から**いちばん遠い**点を採る
4. できた三角形の外にある点のうち、**面積がいちばん増える**点を採る

これを `Add` の代わりに「全部溜めてから4つ選ぶ」形で書き直す
(`Sat.BuildFaceContact` が最大8点をいったん配列に溜めるように変える)。

**効果が見えるのは、箱を台の角にわずかに載せたとき**。
今のやり方だと4点が角に固まるので、`Ctrl+Shift+Alt+3` で並びを見比べるとよい。

### 課題3(難): GJK で置き換えてみる

SAT は「凸多面体どうし」でしか使えない。面と辺を列挙できる形でないと
軸の候補が作れないからで、円柱や凸包には向かない。

GJK(Gilbert-Johnson-Keerthi)は**支持関数だけ**で距離を求めるアルゴリズムで、
`Box3D.Support` が既にある(今日の `BuildEdgeContact` で使っている考え方と同じ)。

1. `Support(direction)` だけを使って、2つの形のミンコフスキー差が原点を含むかを判定する
2. 含んでいたら EPA(Expanding Polytope Algorithm)で最小の押し戻し方向を求める
3. `Sat.BoxBox` と答えを突き合わせる自己チェックを書く

**接触点の生成は SAT のまま使える**——
EPA が返す法線から基準面を選べば、今日書いたクリップがそのまま動く。
ロードマップで「厳密に必要になったら convex hull + GJK を発展課題として」と
書いてあるのがこれ。

## 動作確認済み環境

- Windows 11 / .NET 10 / RTX 3070 / 1920x1080(DPI 100%)
- `dotnet run --project reference/Day44 -c Release`

箱5個 + 平面5枚を影・SSAO・IBL・ブルーム・FXAA 込みで回して、
HUD の表示は **0.05〜0.2ms**(移動平均なので、出した直後は高めに出る)。
描画のほうが2桁重い。

窓を出さずに `PhysicsWorld.Step` だけを測ると、
体の数に対してこうなる(平面5枚込み、速度の反復 8 周、Release)。

| 体の数 | 箱 | 球 | 総当たりの組 |
|---|---|---|---|
| 5 | 0.012ms | 0.006ms | 35 |
| 20 | 0.044ms | 0.029ms | 290 |
| 40 | 0.109ms | 0.068ms | 980 |
| 80 | 0.321ms | 0.149ms | 3,560 |

**箱は球のおよそ2倍**。15 本の軸を試し、当たったらクリップまで走るぶん重い。
ただし増え方を決めているのは<b>組の数 N(N-1)/2</b> のほうで、
80 体で 3,560 組を毎ステップ舐めている。
**ここがブロードフェーズ(Day 46)の出番**で、
形のコストより組の数のほうが先に効いてくるのが数字で見える。

### 自己チェック(47 項目すべて合格)

数字の読み方をいくつか。

- **面の法線6本だけでは足りない配置が「6 回目の試行」で見つかる**。
  ランダムに置いた立方体2つのうち、
  「面から見れば重なっているが、辺の軸で分離している」ものを探している。
  ここが「見つからなかった」になるなら、
  `Sat.BoxBox` の辺×辺のループが効いていない
- **中心がほぼ重なった箱 20,000 通りで取りこぼし 0**。
  逆向きの確認で、深く重なっているのに「離れている」と答えないこと。
  `ParallelEpsilon` を大きくしすぎるとここが落ちる
- **床に平らに置いた箱の4点の重心が (0.00000, -0.02500, 0.00000)**。
  x と z がちょうど 0 なのが大事で、ずれているとその方向へトルクが立つ
- **台から半分はみ出した箱の「いちばん外の点」が x = 1.0000**。
  台の縁とぴったり一致する。ここが 1.5(= 箱の角)になっているなら、
  クリップが効いていない
- **箱の慣性が (13.000, 10.000, 5.000)**。
  z 方向がいちばん長い箱なので、z 軸まわりがいちばん小さい(回しやすい)。
  順序が逆なら `InertiaLocal` の3成分の並びが1つずれている
- **4点なら最後の1秒の角速度が 0.00000、1点だと 0.73**。
  マニフォールドが要る理由がこの2行に出ている
- **6段の柱: 同時に解けば横ずれ 0.00mm、順番に解くと 10.48m**。
  同じ柱、同じ反復回数、違うのは「4点を同時に決めるかどうか」だけ

### 検証の途中で分かったこと

**最初に書いた解き方では、箱が3段で崩れた**。
接触を Day 43 と同じく1点ずつ順番に解いていたのが原因で、
反復を 50 周に増やしても、角速度の減衰を 0.9/秒 まで上げても直らなかった。

原因を追うと、**接触を作った最初のステップで既に角速度が 0.0013 rad/s 立っていた**。
完全に対称な配置なのに回り始めている——
1点目を解いた時点で相対速度が 0 になり、残り3点が仕事をしないので、
**1点目だけが重さを全部背負っていた**。

摩擦が無い世界では、少し傾いた接触は横向きに押す。
0.01 rad/s の傾きでも 5 秒あれば 3 度になり、
`mg·sin(3°)` の横向きの力が効いて柱が流れていく。**物理的には正しい挙動**で、
間違っていたのは解き方のほうだった。

`SolveManifold` を「同じ状態から4点ぶんを決めてから、まとめて掛ける」に変えたら、
横ずれが**測定できないほど 0** になった。
組の中だけヤコビ法にした形で、組と組の間はガウス・ザイデルのまま。

**蓄積インパルスを先に試したが、ほとんど効かなかった**。
Day 43 の改造課題3(そして Day 47)の内容を先に入れてみたところ、
3段の柱はやはり崩れた——**偏りの原因は反復のクランプではなく、
点を順番に解いていること**だったので、当然といえば当然だった。
蓄積インパルスは「沈み」には効くので、Day 47 の担当のままにしてある。

**平面を体にしたら、球が跳ねなくなった**。
Day 43 の自己チェックが2項目落ちて気づいた——
跳ね返り目標が `2.0` のはずが `1.6` になっていた。

原因は反発係数の採り方。`PhysicsWorld` は2つの体の**小さいほう**を採るが、
`RigidBody` の既定値は `0.4`。Day 43 の平面は体ではなく反発係数も持たず、
「ぶつけた側の値をそのまま使う」だった。
平面を体にした途端、**床の 0.4 が上限として効いてしまった**。

直し方は `CreatePlane` と `CreateStatic` で `Restitution = 1.0f` にすること。
最小を採る規則のもとでは、1 は「相手の値をそのまま通す」を意味する。
**静的な体の既定値は 1**、と覚えておくのが正しい。

**Day 43 の自己チェックのしきい値を1つ緩めた**。
「反復1周だと柱が沈む」が `deep1 > deep8 × 3.0` を求めていたが、
今日の位置補正(印で測り直す)で1周のときのめり込みが
**64.4mm から 37.4mm へ改善**したため、倍率が 2.9 に落ちて通らなくなった。
8周のほうも 14.7 → 12.8mm と良くなっている。
**数字が良くなったせいで落ちるチェック**という珍しい形なので、
主張は変えずに倍率だけ 2.0 へ緩めた。

**Span はタプルで入れ替えられない**。
クリップの入れ物を `(polygon, scratch) = (scratch, polygon)` で交換しようとしたら
`CS9244` で弾かれた。`Span<T>` は ref 構造体なのでタプルに入れられない
(タプルはヒープに載りうる)。一時変数で書くしかない。

**減衰は 1.0 を超えると NaN になる**。
`MathF.Pow(1.0f - damping, dt)` の底が負になるため。
Day 43 から入っている性質で、`LinearDamping` / `AngularDamping` は
「1秒あたりに失う割合」なので 1.0 を超える値に意味は無いが、
**知らずに 2.0 と書くと座標が全部 NaN になって物が画面から消える**。
今日は直していない(Day 47 で減衰まわりを触るときに `Clamp` を入れる)。

**接触点を光る玉で描いたら、デバッグが一気に楽になった**。
「4点出ているはず」が数字では追えても、
**どこに出ているか**は絵にしないと分からない。
クリップが効いていないときは玉が箱の角に飛び出るので一目で分かる。
物理のデバッグ表示を `Physics/` の中に置きたくなる誘惑は強いが、
`Program` 側に 20 行書けば済むと分かったのは収穫だった。
