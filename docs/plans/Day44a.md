# Day 44a: 箱を床に置く — 形を体から切り離し、面で支える

Day 44 の1日目で、reference は Day 43 の完全コピー + 差分。
Day 44 は差分が大きい(判定と接触点の作成だけで 700 行を超える)ので、2日に分けてある。
**44a で箱を床に置き、44b で箱どうしを当てる**(分離軸定理と面のクリップ)。

Day 43 の形は球と平面の2つだけで、`RigidBody` が `Radius` を直に持っていた。
今日は3つ目の形として**箱(OBB)**を入れる。ただし当てる相手は**床(平面)だけ**。
それでも、球では起きなかったことが一気に出てくる。

| | 球(Day 43) | 箱(Day 44a) |
|---|---|---|
| 形の持ち方 | `RigidBody.Radius`(体が「球だ」と言い切っている) | `RigidBody.Shape`(**形を体から切り離した**) |
| 慣性テンソル | 等方。回しても変わらない | **軸ごとに違う**。回すと世界での回しにくさが変わる |
| 床との接触 | どこで触れても1点 | **平らなら4点、辺なら2点、角なら1点** |
| 1点で支えると | 落ち着く | **いつまでも揺れる** |
| 4点を順番に解くと | (点が1つなので問題にならない) | **揺れが止まらない**。同時に解く |
...
| `Ctrl+Shift+Alt+7` | 接触点の解き方(同時 / 順番)。**順番にすると床の箱が揺れ止まない** |
| `Ctrl+Shift+Alt+0` | 今日の自己チェック(24 項目) |

**箱どうしはまだ当たらない**(Day 44b)。今日のデモが5つの箱を 1.7m ずつ離して並べているのはそのため。

### 今日いちばん大事な2つ

**1つ目は「面で触れているものは面で支える」**。床に平らに置いた箱は、4つの頂点が同時に沈む。
その4点を全部、接触点として持つ(**接触マニフォールド**)。

```csharp
// 箱と平面: 8つの頂点を平面に落として、沈んだものを全部拾う
for (int i = 0; i < 8; i++)
{
    Vector3 corner = box.Corner(i);
    float distance = plane.SignedDistance(corner);
    if (distance < 0.0f) manifold.Add(corner - (plane.Normal * (distance * 0.5f)), -distance);
}
```

1点だけで支えると、その点まわりにトルクが立って箱が傾き、傾いた先の角が次の最深点になり……を
延々と繰り返す。`Ctrl+Shift+Alt+4` で上限を 1 にすると、落ち着いていた箱がその場で揺れ始める。
...
Day 43 と同じく「順番に1点ずつ」解くと、**1点目だけが箱の重さを全部背負う**。
点が4つあっても1点で支えているのと変わらないので、床に置いた箱1つでも揺れが止まらない
(最後の1秒の角速度 0.118 rad/s。同時に解けば 0.00000)。
`Ctrl+Shift+Alt+7` で切り替えて見られる。

### 今日もシェーダには触らない

差分は `Physics/` に新しいファイル2本(422 行)と、既存4本の書き換え、そして `Program.cs`。
`.vert` も `.frag` も 1 行も変わらない。箱は Day 12 から使っている単位立方体をそのまま拡大して描いている。

**物理は今日も絵を1ミリも知らない**。おかげで自己チェック(24 項目)は
窓を1枚も出さずに走る——Day 43 からの線がそのまま保たれている。

## 事前に読む資料

- **Game Physics Engine Development**(Ian Millington)第 13〜14 章
  接触の生成(contact generation)の章。今日は「箱と平面」と「接触点を束で持つ」ところまで読めば足りる。
  箱と箱は Day 44b で読む
- Dirk Gregorius, *Robust Contact Creation for Physics Simulations*(GDC 2015)
  ...
  接触マニフォールドをどう作り、どう4点に絞るかの決定版。**なぜ点を束で持つのか**の前半だけ今日読む
- 慣性モーメントの表(箱)
...
```
  ...
  1点: 最後の1秒の角速度 0.73 rad/s、傾き 0.94 度
```
...
今日書く判定は「箱と平面」の1本だけで、中身は短い。
平面は片側しか無いので、**試すべき向きは平面の法線1本だけ**だと最初から分かっている。
8つの頂点それぞれの符号付き距離を測り、平面より下にあるものを全部接触点にする。

| 置き方 | 平面より下の頂点 | 接触点 |
|---|---|---|
| 箱を平らに置く | 底面の4つ | **4点** |
| 45度傾けて辺で立てる | 1本の辺の両端 | **2点** |
| 角で立てる | 角の1つ | **1点** |

**接触点の数がそのまま支え方の数**になっているのが、`Ctrl+Shift+Alt+3` の光る玉で目に見える。
接触点は頂点そのものではなく、**頂点とその真上の面上の点の中点**(めり込んだ領域の真ん中)に置く。

箱どうしになると話がまるで変わる。相手も向きを持っているので「試すべき向き」が決まっておらず、
15 本の軸を調べることになる(Day 44b の分離軸定理)。今日の短い判定は、平面という相手に甘えている。
...
しかも**形を1つも知らない**——箱どうし(Day 44b)でもカプセル(Day 45)でも地形(Day 46)でもこのまま動く。

これは Box2D の位置ソルバとほぼ同じ手。

### 5. 4点をどう解くか — 今日いちばん深い分岐
...
次のステップでは別の点が1点目になり、今度はそちらが全部を背負う——
**支える点が毎ステップ入れ替わって、床に置いた箱1つでも揺れが止まらない**。

```
  同時に解く: 最後の1秒の角速度 0.00000 rad/s
  順番に解く: 最後の1秒の角速度 0.118 rad/s
```
...
床の箱1つでは「わずかに揺れ続ける」程度だが、**積むと柱が崩れる**——それは箱どうしが当たる Day 44b で見る。

### 6. 今日入れなかったもの

| 無いもの | 絵に出る症状 | いつ入れるか |
|---|---|---|
| **箱と箱の判定**(分離軸定理) | 箱どうしが重なっても素通りする。積めない | Day 44b |
| **球と箱の判定** | 球が箱をすり抜ける | Day 44b |
| **摩擦** | 落ち着いた箱も、縦軸まわりにゆっくり回ったり滑ったりし続ける | Day 47 |
| **ブロードフェーズ** | 体が増えると N² 組 | Day 46 |

`Collide` の表に穴が2つ空いたまま、というのが今日の姿。
穴に落ちた組は `default`(当たっていない)を返すので、**すり抜けはするが壊れはしない**。
...
| ファイル | 行数 | 役割 |
|---|---|---|
| `Physics/Collider.cs` | 184 | `ColliderKind` と `Collider`。**形を剛体から切り離した**(要点2) |
| `Physics/ContactManifold.cs` | 238 | `ManifoldSource` / `ManifoldPoint` / `ManifoldPointArray` / `ContactManifold`。**最大4点の束**(要点3) |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Physics/Shapes3D.cs` | `Box3D` を追加(中心・半分の長さ・向きと、頂点を出す口だけ)。`Sphere3D` / `Plane3D` のコメント | +74 / -4 |
| `Physics/RigidBody.cs` | `Radius` → `Shape`。`CreateBox` / `Create` / `CreateStatic` / `CreatePlane` / `ToBox` / `ToPlane` | +137 / -24 |
| `Physics/Collision3D.cs` | `BoxPlane` / `Collide`(形の組み合わせで振り分ける窓口) | +118 / -12 |
| `Physics/PhysicsWorld.cs` | `ContactPoint` の作り直し、マニフォールド単位の解決、印による測り直し | +361 / -136 |
| `Program.cs` | 箱を落とす筋書き、接触点の描画、`Ctrl+Shift+Alt+数字キー`、HUD、自己チェック | +641 / -28 |
| `Day44a.csproj` | `Day43.csproj` からのリネームのみ | — |

**`Render/` も `shaders/` も1文字も変わっていない**。

### 写経する順番

**依存の順に並べてある**。ただし今日は**4 から 7 までが一続き**で、
`RigidBody.Radius` を消した時点で `PhysicsWorld` と `Program.cs` が壊れる。
**6・7 まで進めて初めてビルドが通る**ので、途中の赤は気にしないでよい。

1. **`Physics/Shapes3D.cs`**(変更) — 末尾に `Box3D` を足す。
   今日要るのは中心・半分の長さ・向きと、`ToWorld` / `Corner`(8つの頂点)だけ。
...
5. **`Physics/Collision3D.cs`**(変更) — クラスの説明を書き換えてから
   - `BoxPlane`(8つの頂点を落とすだけ。**いちばん分かりやすいマニフォールド**)
   - `Collide` と `Wrap`(形の組み合わせで振り分ける窓口。**球と箱・箱と箱の枝はまだ無い**)
6. **`Physics/PhysicsWorld.cs`**(変更) — 次の順で
   ...
   - `SolveManifold` / `ApplyNormalImpulse` / `NormalImpulse`(**要点5**)
   - `CorrectPositions`(印で測り直す。`Measure` は消える)
7. **`Program.cs`** — 最後。次の順に足す
   - `PhysicsScene` に `BoxDrop` を追加
   - フィールド(`_showContacts` / `_contactMaterial` / `ContactLimits` / `_contactLimitIndex`)
   - `OnLoad` に `_contactMaterial` の生成(`_physicsMaterial` の直後。
     **`white` は同じ関数の中で作ってあるものを使う**)
   - `BuildPhysicsScene` の平面を `AddPhysicsPlane` 経由にし、`switch` に `BoxDrop` を足す。
     `MaxContactsPerPair` を入れる1行も
   - `AddPhysicsPlane`(**色の配列と添字を揃えるため**)
   - `BuildBoxDropScene`(`SpawnBall` の直前)
   ...
   - キー(`Ctrl+Shift+Alt+1・3・4・7` と `0`)。**switch のいちばん上**に置くこと
   - `OnLoad` のヘルプ
8. **`Day44a.csproj`** — `Day43.csproj` からのリネームのみ
...
| `Physics/Collider` | `Physics/` | 形。**寸法と慣性の式を体から引き取った**(要点2) |
| `Physics/Box3D` | `Physics/` | 向きを持った直方体(OBB)。今日は頂点を出せれば足りる |
| `Physics/ManifoldSource` | `Physics/` | 接触がどこから出たか(面 / 点) |
| `Physics/ManifoldPoint` | `Physics/` | 接触点1つ。位置と深さだけ |
| `Physics/ManifoldPointArray` | `Physics/` | `[InlineArray]`。**構造体の中に4つ並べたいだけ** |
| `Physics/ContactManifold` | `Physics/` | 法線1本 + 最大4点(要点3) |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Physics/Shapes3D` | `Box3D` が入った | +74 / -4 |
| `Physics/RigidBody` | `Radius` が消えて `Shape` に。生成が `Create(mass, shape)` 経由へ | +137 / -24 |
| `Physics/Collision3D` | 箱と平面の判定と、`Collide` という窓口ができた | +118 / -12 |
| `Physics/PhysicsWorld` | 平面が体になり、解決がマニフォールド単位になった | +361 / -136 |
| `Program.cs` | 箱を落とす筋書き、接触点の描画、キー、HUD、自己チェック | +641 / -28 |
...
配当は今日も自己チェックに出る。**24 項目が窓を1枚も出さずに走る**。
...
```
  ...
  Day 44a: RigidBody { Collider Shape; }        ← 形は札 + 寸法
...
おかげで「平面と箱」を書かずに「箱と平面」だけ書けばよい。
今日埋まったのは3通り(球と球・球と平面・箱と平面)で、
**球と箱・箱と箱の2つは穴のまま**(Day 44b)。穴に落ちた組は `default` を返して素通りする。
...
| 段 | やること | Day 44a で変わったか |
|---|---|---|
| 1 | 積分(速度 → 位置) | 変わらない |
| 2 | 判定(接触を集める) | **マニフォールドを返す。組の区切りも覚える** |
| 3 | 速度の解決 | **組の中は同時、組の間は順番**(要点5) |
| 4 | 位置の補正 | **印で測り直す**(要点4)。判定を呼ばなくなった |
| 5 | 力の溜め場を空にする | 変わらない |
...
**2つ目: `Collide` の表に穴が2つある**。
球と箱、箱と箱は `default`(当たっていない)に落ちる。
壊れはしないが、**近づけた箱どうしは素通りする**。
今日のデモが箱を離して並べているのはこのためで、Day 44b で2つとも埋まる。

**3つ目: `Program.cs` が 15,747 行になった**。
Day 43 で 15,134 行、今日 613 行足した。内訳は Day 43 と同じ傾向。

| 何 | 行数 |
|---|---|
| 自己チェック(`RunBoxCollisionCheck`) | 約 290 |
| デモの組み立て・描画・HUD | 約 190 |
| キー操作(`Ctrl+Shift+Alt+数字キー`) | 約 90 |
| ヘルプ・フィールド・配線 | 約 40 |

**エンジンの機能そのものは1行も `Program.cs` に無い**——
今日の中身は全部 `Physics/` の新規 422 行 + 既存への 690 行の追記に入っている。
...
| 図 | 何が変わったか |
|---|---|
| `Ecs と Physics` のクラス図 | **3D の側にクラスが6つ増え、`RigidBody` と `PhysicsWorld` が書き換わった** |
| 剛体が1ステップ進むまで | 2段目がマニフォールドを返すようになり、3・4段目の中身が変わった |
| インパルスが決まるまで | Day 43 の「1つ決まるまで」を、**組の中は同時・組の間は順番**に描き直した |
...
```
接触[5組 20点 上限4]  面:5  点:0  解き方:同時  表示:ON
```
...
延々と繰り返している。HUD の「接触」が `5組 5点 上限1` になる。
`Ctrl+Shift+Alt+F9` の内訳で、5つの箱の角速度が 0.6〜0.9 rad/s を行き来しているのが見える。

4 に戻すと、数フレームで静止に戻る。

### 4. `Ctrl+Shift+Alt+7`: 解き方を「順番」にする

箱が落ち着いてから押す。HUD の「解き方」が `**順番**` に変わる。
**見た目ではほとんど分からない**のがこのつまみの面白いところで、
`Ctrl+Shift+Alt+F9` の内訳を出すと、5つの箱の角速度が `0.017rad/s` 前後から下がらない
(「同時」なら `0.000rad/s`)。

1点目が全部を背負う偏りは、床の箱1つではこの程度の揺れで済む。
**積むと偏りが段ごとに重なって柱が崩れる**——それは Day 44b で見る。
もう一度押して「同時」に戻しておく。

### 5. Day 43 までの機能が全部生きていること
...
### 6. `Ctrl+Shift+Alt+0`: 今日の自己チェック(24 項目すべて合格)

コンソールに 24 行の `[OK]` が並び、最後に
`すべて合格(箱は落ち、面で支えられる)` と出る。
...
`ContactLimits` に 3 を足して(`[1, 2, 3, 4]`)、`Ctrl+Shift+Alt+4` で 3 点にする。
**押す前に予想する**——四隅のうち3点で支えれば、三角形で支えることになる。三脚は倒れない。

実際には**揺れが止まらない**(最後の1秒の角速度 0.15 rad/s。4点なら 0.00000)。
理由は重心の真下がどこにあるかを描いてみると分かる。

- 正方形の四隅から3つ選ぶと、残った三角形の斜辺は**正方形の対角線**になる
- 箱の重心の真下は正方形の中心で、**ちょうどその対角線の上**に乗る
- 支える三角形の「縁の上」に重心があるので、少しでも傾くと支えの外へ出る

三脚が倒れないのは、重心の真下が三角形の**内側**にあるから。
「点が多いほど安定する」ではなく「**重心の真下を囲めているか**」が効いている——
凸多角形を支えるのに4点を採る理由(要点3)を、裏側から確かめる課題。
2点(辺で支える)と比べると、揺れ方の違いも分かる。

### 課題3(難): 位置の補正で向きも直す

`CorrectPositions` は**位置だけ**を動かして、向きは直していない(コメントに理由がある)。
Box2D の位置ソルバは、めり込みを並進と回転の両方に配って戻す。

1. 印(`LocalA` / `LocalB`)から今の接触点を出し、`rA` / `rB` を作る
2. 速度の解決と同じ分母(並進 + 回転)で「戻す量」を決める
3. 並進は `a.Position += n * (λ * InverseMass)`、回転は `I⁻¹ (r × n) λ` の角度ぶん `Orientation` を回す

**床の箱では差がほとんど出ない**(4点で支えていれば並進だけで十分まっすぐ立つ)。
差が出るのは、`Ctrl+Shift+Alt+4` で 1 点にしたときと、角から落ちた直後の深いめり込み。
向きを直すと反復が収束しにくくなる、というコメントの主張も、周回数を変えて確かめてみる。

## 動作確認済み環境

- Windows 11 / .NET 10 / RTX 3070 / 1920x1080(DPI 100%)
- `dotnet run --project reference/Day44a -c Release`
...
- **4点を順番に解くと 0.1179 rad/s、同時なら 0.00000**。
  同じ4点、同じ反復回数で、違うのは「4点を同じ状態から決めるかどうか」だけ

### 検証の途中で分かったこと

**4点を順番に解くと、床の箱1つでも揺れが止まらなかった**。
「同時に解く」理由は箱を積んで初めて見えるものだと思っていたが、
床に置いた箱1つでも、最後の1秒の角速度が 0.118 rad/s から下がらない。
1点目を解いた時点で相対速度が 0 になり、残り3点が仕事をしないので、
**1点目だけが重さを全部背負う**——その1点が毎ステップ入れ替わり、揺れが続く。
摩擦も減衰も無い今日の世界では、この小さな揺れを止めるものが何も無い。
デモの箱(減衰 0.25)でも 0.017 rad/s が残る。
