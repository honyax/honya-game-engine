# Day 44b: 箱と箱 — 分離軸定理と面のクリップ

Day 44 の2日目で、reference は Day 44a の完全コピー + 差分。
Day 44a で箱を床に置いた。形を体から切り離し、接触点を束(マニフォールド)で持ち、
4点を同時に解くところまでできている。**今日は箱どうしを当てる**。

Day 44a の判定は「箱と平面」の1本で、中身は8つの頂点を落とすだけだった。
平面は片側しか無いので、試すべき向きが法線1本と最初から分かっていたからだ。
相手も向きを持った箱になると、その甘えが効かなくなる。

| | 箱と平面(Day 44a) | 箱と箱(Day 44b) |
|---|---|---|
| 試す向き | 平面の法線 **1本** | **15 本**(面の法線 6 + 辺×辺 9) |
| 接触点の作り方 | 平面より下の頂点を拾うだけ | **基準面の枠で入射面を切る**(クリップ) |
| 判定の長さ | 20 行ほど | **524 行**(`Sat.cs`) |
| 積めるか | 積めない(箱どうしが素通りする) | **6段積める**。4点を同時に解く意味がここで効く |
...
**`Ctrl+Shift+Alt+1` で箱デモを出してから `Ctrl+Shift+Alt+2` を押すと、一辺 0.84m の箱が6段に積まれ、崩れずに立つ。
`Ctrl+Shift+Alt+7` で解き方を「順番」にすると、数秒で柱が滑り出して崩れる。**

`Ctrl+Shift+Alt+2` を押すたびに「箱を積む → 球と箱 → 箱が転がる → 箱を落とす」と巡る
(箱の筋書きを出していないときに押すと「箱を落とす」から始まる)。

| キー | 何が起きるか |
|---|---|
| `Ctrl+Shift+Alt+2` | 筋書き(箱を落とす / **箱を積む** / 球と箱 / 箱が転がる)。**今日の到達点** |
| `Ctrl+Shift+Alt+5` | 箱を1つ降らせる |
| `Ctrl+Shift+Alt+6` | 接触の内訳(どの組が、どの軸で、何点で触れているか) |
| `Ctrl+Shift+Alt+7` | 接触点の解き方(同時 / 順番)。**切ると積んだ箱が崩れる** |
| `Ctrl+Shift+Alt+0` | 今日の自己チェック(48 項目。Day 44a の 24 項目を含む) |
| `Ctrl+Shift+Alt+1` / `3` / `4` | Day 44a のまま(箱デモ / 接触点の表示 / 1組の接触点) |

HUD の「接触」の行に **`辺:`** が増える。面で触れているか、辺と辺がすれ違っているかが数字で見える。
...
**2つ目は「入射面を基準面の枠で切る」**。面どうしが触れているとき、触れているのは2つの面の重なりで、
凸どうしなのでそれは凸多角形になる。相手の面(入射面)の4頂点を、こちらの面(基準面)の側面4枚で切り落とすと、
まさにその重なりが出てくる。

```
  床の上に箱をまるごと置く  → 4頂点がそのまま残る                → 4点
  台から半分はみ出して置く  → はみ出した2点が切り落とされ、
                              代わりに台の縁の上に新しい2点が生まれる → 4点(縁の上に並び直す)
```

**Day 8 で視錐台に三角形をクリップしたのと同じ手続き**(Sutherland-Hodgman)が、ここでもう一度出てくる。

### 今日もシェーダには触らない

差分は `Physics/` に新しいファイル1本(`Sat.cs`、524 行)と、既存4本の書き足し、そして `Program.cs`。
`.vert` も `.frag` も 1 行も変わらない。

**物理は今日も絵を1ミリも知らない**。自己チェック(48 項目)は窓を1枚も出さずに走る。
...
- Dirk Gregorius, *Robust Contact Creation for Physics Simulations*(GDC 2015)
  ...
  Day 44a で前半を読んだ資料の後半。**今日入れなかった「点の選び方(広がりを見る)」**はここに書いてある(改造課題2)。

## 理論の要点

### 1. 分離軸定理 — 無限の向きが 15 本に減る
...
これは各軸の寄与の**絶対値の和**になる(`Box3D.ProjectedRadius`)。
...
そこから4つに絞るのは Day 44a の `ContactManifold.Add`(深い順に4つ)にそのまま任せる。
素朴なやり方で、これを直すのが改造課題2。

辺×辺が選ばれたときは話が簡単で、**接触点は1つだけ**。
角と角がすれ違っている場面なので、触れているのは1点しかない。
2つの辺の最近接点を求めればそこが接触点になる。

### 3. 球と箱 — 最近接点で済む

箱どうしが 500 行を超えるのに対し、**球と箱は 50 行ほど**で済む。
球は「中心から一定の距離」なので、**箱の上でいちばん近い点**までの距離を半径と比べればよい。

```csharp
Vector3 local = box.ToLocal(sphere.Center);                                   // 箱の物体座標へ
Vector3 clamped = Vector3.Clamp(local, -box.HalfExtents, box.HalfExtents);    // 各成分を切り詰める
```

傾いた箱でも物体座標へ持ち込めば軸に沿った箱になるので、
**Day 25 の 2D の円と箱(`Collision2D.CircleBox`)とほとんど同じ形**になる。

1つだけ別扱いが要るのは、**球の中心が箱の中に入ったとき**。
最近接点が中心そのものになり距離が 0 なので、法線が決められない。
3つの軸それぞれについて「面までどれだけ余裕があるか」を測り、**いちばん余裕の少ない面へ押し出す**。

これで `Collide` の表の穴(Day 44a で空けたままだった2つ)が両方埋まる。
15 通りのうち、今日で5通り。

### 4. 積むと、4点を同時に解く意味がもう一度出る

Day 44a の要点5で、4点を順番に解くと床の箱1つでも揺れが止まらない(0.118 rad/s)と書いた。
**積むと、この偏りが段ごとに重なる**。

- 1段目の箱は、1点目で床を強く押し、残り3点は仕事をしない
- わずかに傾いた1段目の上面が、2段目を斜めに押す
- 摩擦の無い世界では、傾いた接触は**横向きにも押す**ので、上の段ほど横へ流れていく

実測で6段の柱が 5 秒で崩れ落ち、**横ずれは 10m を超える**。
同時に解けば横ずれは**測定できないほど 0**(0.00mm)。

`Ctrl+Shift+Alt+7` の意味が、床の箱1つ(Day 44a)より**ずっと派手に**見えるのが今日。
同じつまみ、同じ偏りでも、積み重ねると物の運命が変わる。

**組と組の間は順番**(ガウス・ザイデル)のままなのも効いている。
下の組を先に解くと、その結果を見て上の組が解かれるので、柱の下から上へ荷重が伝わる。
...
| ファイル | 行数 | 役割 |
|---|---|---|
| `Physics/Sat.cs` | 524 | 分離軸定理と面のクリップ。**今日いちばん長い**(要点1・要点2) |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Physics/Shapes3D.cs` | `Box3D` に軸・投影半径・最近接点・支持点など、**判定に要る道具**を足した | +97 / -5 |
| `Physics/ContactManifold.cs` | `ManifoldSource.EdgeEdge` と、クリップの話を踏まえたコメント | +11 / -8 |
| `Physics/Collision3D.cs` | `SphereBox`(要点3)と、`Collide` に球と箱・箱と箱の枝 | +89 / -14 |
| `Physics/PhysicsWorld.cs` | 出どころ別の数え場所を5つに。コメント | +9 / -7 |
| `Program.cs` | 箱の筋書き3つ、箱を降らせる、接触の内訳、HUD の「辺」、自己チェックの追加 | +641 / -19 |
| `Day44b.csproj` | `Day44a.csproj` からのリネームのみ | — |

### 写経する順番

**依存の順に並べてある**。今日は**足すだけ**なので、途中でビルドが壊れることはない
(Day 44a の `RigidBody` の書き換えのような一続きの区間が無い)。

1. **`Physics/Shapes3D.cs`**(変更) — `Box3D` に判定の道具を足す。次の順で
   - `Box3D` の説明(分離軸定理の段落)と、`Sphere3D` / `Plane3D` の説明の書き換え(**コメントのみ**)
   - `AxisX` / `AxisY` / `AxisZ` / `Axis` / `Extent`(`ToWorld` の直前。**SAT のループが添字で回る**)
   - `ToLocal` / `ClosestPoint`(`ToWorld` の直後。球と箱が使う)
   - `ProjectedRadius`(`Corner` の直前。**要点1の式**)
   - `Support`(末尾。辺の接触と、改造課題3の GJK が使う)
2. **`Physics/ContactManifold.cs`**(変更) — `ManifoldSource.EdgeEdge` を**末尾に**足し、
   `ManifoldSource` と `Add` と `Flipped` のコメントを直す
3. **`Physics/Sat.cs`**(新規) — **今日の山場**。次の順で足すとよい
   ...
   - `BuildFaceContact`(要点2)/ `ClosestPointsOnLines` / `BuildEdgeContact`
4. **`Physics/Collision3D.cs`**(変更) — クラスの説明を書き換えてから
   - `SphereBox`(`BoxPlane` の直前。**中心が箱の中に入ったときだけ別扱い**)
   - `Collide` に球と箱の2枝と、箱と箱の1枝。コメントの「箱と球」「5 通り」
5. **`Physics/PhysicsWorld.cs`**(変更) — `_sourceCounts` を 5 つに(**`EdgeEdge` の席**)。
   クラスと `SourceCount` / `SolveContactsTogether` / `SolveManifold` / `CorrectPositions` のコメント
6. **`Program.cs`** — 最後。次の順に足す
   - `PhysicsScene` に `BoxStack` / `BoxMix` / `BoxTumble` を追加
   - フィールド `_boxSpawnCount`
   - `OnLoad` のヘルプに `2`・`5`・`6` の行、`7` の説明の書き換え
   - `ShowPhysicsDemo` のカメラ(柱の筋書きだけ引いて上げる)
   - `BuildPhysicsScene` の `switch` に3つ。「有限の床」のコメント
   - `BuildBoxStackScene` / `BuildBoxMixScene` / `BuildBoxTumbleScene` / `SpawnBox`(`BuildBoxDropScene` の直後)
   - `PhysicsSceneLabel` に3つ
   - `ContactLabel` に `辺:` の1行とコメント
   - `DescribeContacts`(`DescribePhysics` の直後)
   - キー: `Ctrl+Shift+Alt+1` の案内文、`2`・`5`・`6` の case、`7` の案内文
   - `RunBoxCollisionCheck` に Day 44b の項目を足す — 次の順で
     - 関数の説明(ランダム探索の段落)
     - 「2. 箱の形そのもの」「3. 分離軸定理」(`FaceOnlyOverlap` などのローカル関数を含む)
     - 「4. 接触マニフォールド」の見出しの書き換えと、台からはみ出した箱のクリップ
     - 「5. 球と箱」
     - 節の番号の振り直し(6・7・8)
     - 「7. 落として、支えられて、積む」に箱の柱(`BoxStack`)と、箱2つのめり込み(`BoxPenetrationAfter`)
     - 最後の合格メッセージ
7. **`Day44b.csproj`** — `Day44a.csproj` からのリネームのみ

**キーの位置は Day 44a で決めた塊の中**に足す。`Ctrl+Shift+Alt+2`・`5`・`6` も
**switch のいちばん上の塊**の中に置くこと。下の `case Key.Number2 when ctrl && shift:` などに吸われる。

## 設計書

**層は今日も増えていない**。`Physics/` の中で型が1つ増え(`Sat`)、
既存の5つに書き足しただけ——矢印は1本も足していない。
...
| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Physics/Shapes3D` | `Box3D` に判定の道具(軸・投影半径・最近接点・支持点)が入った | +97 / -5 |
| `Physics/ContactManifold` | `ManifoldSource.EdgeEdge` が増えた | +11 / -8 |
| `Physics/Collision3D` | `SphereBox` と、`Collide` の球と箱・箱と箱の枝 | +89 / -14 |
| `Physics/PhysicsWorld` | 出どころ別の数え場所が5つになった(ほかはコメント) | +9 / -7 |
| `Program.cs` | 箱の筋書き3つ、降らせる、接触の内訳、HUD、自己チェック | +641 / -19 |
...
`Sat` も同じ性格で、`BoxBox(a, b)` は箱2つを受け取って束を返すだけ。
体も世界も時間も知らない。接触点の描画は Day 44a のまま `Program.RenderContacts` が受け持つ。

配当は今日も自己チェックに出る。**48 項目が窓を1枚も出さずに走る**。

### `Collide` の表が埋まったこと

Day 44a で `Collide` という窓口を作り、`switch (a.Shape.Kind, b.Shape.Kind)` の表引きにした。
そのとき穴のまま残した2つが、今日埋まった。

| 組み合わせ | Day 44a | Day 44b |
|---|---|---|
| 球 × 球 / 球 × 平面 | Day 43 から | そのまま |
| 箱 × 平面 | **8つの頂点を落とす** | そのまま |
| 球 × 箱 | 穴(`default`。素通り) | **`SphereBox`**(箱の上の最近接点) |
| 箱 × 箱 | 穴(`default`。素通り) | **`Sat.BoxBox`**(15 本の軸 + クリップ) |

**逆順の組は `Flipped()` で法線を裏返す**ので、「箱と球」は書かずに「球と箱」だけ書けばよい。
15 通りのうち実際に書くのは5通りで済んでいる。**窓口の形は1文字も変わっていない**——
表の行が2つ増えただけで、`PhysicsWorld` は今日も判定の中身を知らない。

**判定が重くなったのに、`PhysicsWorld` は1行も困っていない**のも見どころ。
Day 44a で位置の補正を「印で測り直す」形にしておいたので、
箱どうしの判定(15 本の軸 + クリップ)を周ごとに呼び直さずに済んでいる。

### 5段の順番は変わっていない

`PhysicsWorld.Step` の5段は Day 43 のまま。Day 44a で「2段目がマニフォールドを返す」
「3段目は組の中を同時に解く」「4段目は印で測り直す」になったものが、今日もそのまま動く。
**今日の差分は2段目の中(`Collide` の先)にしか無い**。

### 今日残した歪み(3つ)

**1つ目: `BodyColors` という並行配列**(Day 44a から)。
...
**3つ目: `Program.cs` が 16,369 行になった**。
Day 44a で 15,747 行、今日 622 行足した。内訳は Day 44a と同じ傾向。

| 何 | 行数 |
|---|---|
| 自己チェック(`RunBoxCollisionCheck` の後半) | 約 330 |
| デモの組み立て(筋書き3つ・降らせる)・HUD | 約 190 |
| 接触の内訳(`DescribeContacts`) | 約 60 |
| キー操作・ヘルプ・配線 | 約 40 |

**エンジンの機能そのものは1行も `Program.cs` に無い**——
今日の中身は全部 `Physics/` の新規 524 行 + 既存への 206 行の追記に入っている。
切り出しの計画は Day 42 のまま(Day 51 で `Demo/DemoKeys.cs` /
`Demo/SelfChecks.cs` / `Demo/Hud.cs` へ)。

Day 44a の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の1つ。

| 図 | 何が変わったか |
|---|---|
| `Ecs と Physics` のクラス図 | **`Sat` が増え、`Box3D` に判定の道具が、`Collision3D` に `SphereBox` が入った** |
...
#### 3D の側(Day 44a で6つ、Day 44b で1つ増え、2つが書き換わった)
...
Day 43 のキー(`Ctrl+Shift+Alt+F1〜F12`)と、Day 44a のキー(`Ctrl+Shift+Alt+1・3・4・7・0`)はそのまま使える。
今日のキーは同じ段の `2`・`5`・`6`。

### 1. `Ctrl+Shift+Alt+1`: 箱を落とす(Day 44a のまま)

傾きを振った5つの箱が落ちて、面で落ち着く。Day 44a と同じ絵になる。
HUD の2行目に **`辺:` が増えている**。
...
### 2. `Ctrl+Shift+Alt+2`(1回目): 箱を積む

**今日の到達点**。一辺 0.84m の箱を6段。カメラが少し引いて上がる。
...
### 3. `Ctrl+Shift+Alt+7`: 解き方を「順番」にする

柱を出したまま押す。**数秒で柱が滑り出して崩れる**。
HUD の「解き方」が `**順番**` に変わっている。

Day 44a で床の箱1つに押したときは、揺れが少し残るだけだった。
**同じ偏りでも、積むと柱が崩れる**のがこの差(要点4)。

もう一度押して「同時」に戻し、`Ctrl+Shift+Alt+2` を4回押して(1周して)柱に戻ると、
また崩れなくなる。

### 4. `Ctrl+Shift+Alt+2`(2回目): 球と箱

傾けた静的な箱(台)の上へ、球と箱が交互に落ちてくる。
**今日までに書いた判定が全部走る**——球×箱、箱×箱(今日)、箱×平面(Day 44a)、そして Day 43 の2本。
...
### 7. Day 44a までの機能が全部生きていること

- `Ctrl+Shift+Alt+3` / `4`: 接触点の表示 / 1組の接触点 1/2/4
- `Ctrl+Shift+Alt+F1`: 球の物理デモ(落下 / 積み上げ / 撞き玉)
- `Ctrl+Shift+F1`: デモ v1 / `Alt+F1`: アニメーション / `2`: 材質グリッド
- `Ctrl+Shift+Alt+F12`: Day 43 の自己チェック(45 項目)

### 8. `Ctrl+Shift+Alt+0`: 今日の自己チェック(48 項目すべて合格)

Day 44a の 24 項目に、今日の 24 項目(箱の形・分離軸定理・クリップ・球と箱・柱)を足したもの。
コンソールに 48 行の `[OK]` が並び、最後に
...
`Sat.EdgeBias` は 1.05(辺×辺の軸に 5% の下駄)。これを **1.0** にして、
`Ctrl+Shift+Alt+2` で「箱を積む」と「箱が転がる」を出す。

- HUD の `辺:` が、面で触れているはずの場面で増えるか
- 積んだ箱の接触点(`Ctrl+Shift+Alt+3`)が、4点から1点へ飛ぶ瞬間があるか
- `Ctrl+Shift+Alt+6` の内訳で、辺×辺になった組の法線がどちらを向いているか

**下駄は「迷ったら面のほうが正しい」という経験則を数にしたもの**(要点1)。
理論上の根拠は無いので、外すと何が壊れるかを自分の目で確かめるための課題。
逆に 1.5 のように大きくしすぎると、本当に辺どうしが当たっている場面で面が選ばれ、何が起きるかも見てみる。
...
- `dotnet run --project reference/Day44b -c Release`
...
数字の読み方をいくつか(Day 44a の項目は Day 44a の計画書を参照)。
...
- **台から半分はみ出した箱の「いちばん外の点」が x = 1.0000**。
  ...
  クリップが効いていない
- **6段の柱: 同時に解けば横ずれ 0.00mm、順番に解くと 10.85m**。
...
間違っていたのは解き方のほうだった。
(このとき直した「同時に解く」は、Day 44 を2日に分けたときに Day 44a へ移した。
床の箱1つでも揺れが止まらないことが数字で出たので)
...
**接触点の光る玉は、クリップの確認にも効いた**。
クリップが効いていないときは玉が箱の角に飛び出るので一目で分かる。
Day 44a で「`Program` 側に 20 行書けば済む」と書いた道具が、今日いちばん働いた。
