# Day 45b: キャラクターコントローラ — 力で動かさないカプセル

Day 45 の2日目で、reference は Day 45a の完全コピー + 差分。
Day 45a でカプセルを剛体の形として入れた(判定4通りで 200 行に届かない)。
**今日はそのカプセルをキャラクターにする**。

重心はがらりと移る。今日の 830 行は「当たったか」ではなく、
...
| `Ctrl+Shift+Alt+X` | 今日の自己チェック(53 項目。Day 45a の 28 項目を含む) |
| `Ctrl+Alt+X` | Day 45a のまま(カプセルを1つ降らせる) |
...
**2つ目は「床は真上へ押し出す」**。押し戻しを法線の向きへ出すと、
30 度の坂に立っているだけで横へ滑り落ちる。**真上へ `深さ / 法線のY` だけ持ち上げれば**、
めり込みは同じだけ解けて横へは1ミリも動かない。「登れる坂」は物理ではなく決めごと(要点2)。

**3つ目は「段は先に覗く」**。Quake 以来の「持ち上げて・進んで・落とす」を書いて、**動かなかった**。
丸い底が段の角に当たったときの法線が寝ているので、落とした先で「壁」と判定されて押し戻される。
**半径ぶん前・段差ぶん上を先に覗いて**、段かどうかを決めてから持ち上げる(要点3)。

### 今日もシェーダには触らない

差分は `Physics/` に新しいファイル1本(830 行)と `PhysicsWorld` の問い合わせ、
`Core/InputMap.cs` にジャンプの割り当て、そして `Program.cs`。
`.vert` も `.frag` も 1 行も変わらない。

**物理は今日も絵を1ミリも知らない**。自己チェック(53 項目)は窓を1枚も出さずに走る——
そのうち 20 項目以上は「実際に数百ステップ歩かせてみる」形で、**窓を出さずに操作の手触りを測っている**。
...
### 1. キャラクターは剛体ではない — キネマティックという選択

**なぜキャラクターを `RigidBody` にしないのか**。
...
- **しゃがみ・匍匐** … 高さを変えるだけだが、「頭上が空いているか」の確認が要る(改造課題2)。
...
- **地形(Heightmap)** … Day 46。問い合わせ(`QueryCapsule`)は形を知らないので、地形の体が増えてもそのまま使える。
...
| ファイル | 行数 | 役割 |
|---|---|---|
| `Physics/CharacterController.cs` | 830 | キネマティックなキャラクターコントローラ。**今日の主役**(要点1〜4) |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Physics/PhysicsWorld.cs` | `QueryCapsule` / `OverlapCapsule`(**読むだけの問い合わせ**) | +104 / -1 |
| `Core/InputMap.cs` | `GameAction.Jump` と `Space` の割り当て | +20 / -1 |
| `Program.cs` | キャラクターデモ3つ、`X` の段の残り5つと `Space`、HUD、描画、自己チェックの後半 | +1,089 / -26 |
| `Physics/Collision3D.cs` | コメントのみ(キャラクターの話を `CharacterController` の参照に) | +13 / -5 |
| `Physics/RigidBody.cs` / `Physics/Shapes3D.cs` | コメントのみ(同上) | +1 / -1 ずつ |
| `Day45b.csproj` | `Day45a.csproj` からのリネームのみ | — |

**`shaders/` も `Render/` も1文字も変わっていない**。

### 写経する順番

**依存の順に並べてある**。今日は**1 から 3 までは足すだけ**なので、
途中でビルドが壊れることはない。

1. **`Core/InputMap.cs`**(変更) — `GameAction.Jump` と `map.Bind(Key.Space, ...)`
2. **`Physics/PhysicsWorld.cs`**(変更) — クラスの説明に1段落と
   - `QueryCapsule` / `OverlapCapsule` / `CapsuleQueries` / `CapsuleQueryTests` /
     `ResetQueryStats`(`Clear` の直前)
3. **`Physics/CharacterController.cs`**(新規) — **今日の山場**。次の順で足すとよい
   ...
   - `Move`(5段。要点1)
   - `MoveHorizontal` と `TryFindStep`(要点3)
   - `MoveAndSlide`(要点1・要点2。**床は真上へ押す**)
   - `UpdateGround` と `Probe`(要点4)
   - `UpdateFacing` / `Teleport`
4. **`Program.cs`** — 次の順に足す
   - `CharacterScene` を新設(`PhysicsScene` の直後)
   - フィールド(`_characterDemo` / `Character` / `_characterScene` /
     `_characterMaterial` / `_characterMilliseconds` / `_characterSpawn`)。
     `_capsuleSpawnCount` を挟むように置く
   - `OnLoad`: `_characterMaterial`(`_contactMaterial` の直後)、ヘルプの Day 45 の段の残り
   - `OnUpdate`: 注視点をキャラクターに合わせる行(`UpdateDemoCamera` の直後)と、
     アニメーションのコメントの直し
   - `FixedUpdate`: `_scene.Input` を `_characterDemo` で分ける行と、
   ...
   - `ShowPhysicsDemo` に `_characterDemo = false` の1行と、
     ...
     (**世界を消すならキャラクターも下ろす**)
   - Day 45 の塊に、`SpawnCapsule` の直後へ
     `ShowCharacterDemo` / `BuildCharacterScene` / `BuildSlopeCourse` / `BuildStepCourse` /
     `BuildObstacleCourse` / `UpdateCharacter` / `RenderCharacter`、
     `RotationTo` の直後へ `CharacterMatrix` / `CharacterLabel` / `CharacterSceneLabel` / `DescribeCharacter`
   - `RenderPhysics` に `RenderCharacter()`、`RenderShadowPass` にキャラクターの影
   - `PhysicsSceneLabel` をキャラクターの筋書きでも答える形に
   - `DrawOverlayInfo` に `CharacterLabel()` の枝
   - `RunCapsuleCheck` の後半(節 5〜9。キャラクターと問い合わせ)と、説明・見出し・合格メッセージ
   - キー(`X` の段の `Ctrl+X` / `Shift+X` / `Alt+X` / `Ctrl+Shift+X` / `Shift+Alt+X` と `Space` の横取り)。
     **Day 45a で作った X の塊の中**に置くこと
5. **`Physics/Collision3D.cs` / `Physics/RigidBody.cs` / `Physics/Shapes3D.cs`** — **コメントのみ**。
   Day 45a で「Day 45b のキャラクター」と書いておいた箇所が、`CharacterController` の参照に変わる。
   差分0にしたいなら合わせておく
6. **`Day45b.csproj`** — `Day45a.csproj` からのリネームのみ

**`Space` の横取りは X の塊の最後**に置く。`case Key.Space:`(一時停止)より上にないと、
ジャンプするたびに世界が止まる。

## 設計書

**層は今日も増えていない**。`Physics/` の中で型が1つ増え(`CharacterController`)、
`PhysicsWorld` に読むだけの入口が付いただけ——矢印は1本も足していない。
`Physics/` は今日も**どこにも依存しない層**のまま。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Physics/CharacterController` | `Physics/` | キネマティックなキャラクター。**今日の主役 830 行**(要点1〜4) |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Physics/PhysicsWorld` | **読むだけの問い合わせ**が入った | +104 / -1 |
| `Core/InputMap` | `GameAction.Jump` と `Space` の割り当て | +20 / -1 |
| `Program.cs` | キャラクターデモ3つ、`X` の段の残り、HUD、自己チェックの後半 | +1,089 / -26 |
| `Physics/Collision3D` / `RigidBody` / `Shapes3D` | コメントのみ | — |
...
### 判定の窓口が2つある理由が、今日はっきりする

Day 45a で、カプセルの判定の窓口を2つにしておいた。

```csharp
Collide(RigidBody a, RigidBody b)                       // 体どうし。Day 44a
CapsuleAgainst(in Capsule3D capsule, RigidBody body)    // 形と体。Day 45a
```

2つ目が**体ではなく形を受け取る**のは、**キャラクターのカプセルが体ではない**から。
今日 `QueryCapsule` がこの窓口を使う。
...
`PhysicsWorld.Step` の5段は Day 43 のまま、中身も Day 44・45a のまま。
**今日変わったのは `FixedUpdate` の側**で、物理のあとにキャラクターが1段入った。

| 段 | やること | Day 45b で変わったか |
|---|---|---|
| 1 | 積分(速度 → 位置) | 変わらない |
| 2 | 判定(接触を集める) | 変わらない(カプセルの4組は Day 45a から) |
| 3 | 速度の解決 | 変わらない |
| 4 | 位置の補正 | 変わらない |
| 5 | 力の溜め場を空にする | 変わらない |
...
要点3のとおり「段の高さまで持ち上げてから進む」ので、
足が段の縁に乗るまでの数フレーム(0.05 秒ほど)は空中を歩いている。
掃引(スイープ)を入れれば「触れたところで止まる」形にできるが、
そのぶん判定が重くなるので今日は入れなかった。

**2つ目: `Program.cs` が 18,000 行を超えた**。
Day 45a で 17,003 行、今日 1,063 行足して 18,066 行。

| 何 | 行数 |
|---|---|
| 自己チェック(`RunCapsuleCheck` の後半) | 約 380 |
| デモの組み立て(コース3つ)・描画・HUD | 約 470 |
| キー操作(`X` の段の残り + `Space` の横取り) | 約 100 |
| ヘルプ・フィールド・配線 | 約 110 |
...
**3つ目: キーの段がとうとう尽きた**。
Day 45a から使い始めた `X` の段(A〜Z のうち `case Key.X:` を持っていなかったのは X だけ)が、今日で埋まった。
**Day 46 以降は空きが無い**ので、Day 40 の `FeatureToggles` のように
割り当ての表そのものをデータにするしかない。

Day 45a の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の3つ。

| 図 | 何が変わったか |
|---|---|
| `Ecs と Physics` のクラス図 | **`CharacterController` が増え、`PhysicsWorld` に問い合わせが入った** |
| `FixedUpdate` の中身 | 物理の直後に**キャラクターの更新**が1段入った |
| `Core` のクラス図 | `GameAction` に `Jump` が増えた |

新しく足した図が4つ。**今日は図で説明したいことが多い**——
キャラクターの側が「なぜその順番でなければいけないか」の塊になっている。
...
#### 3D の側(Day 45a で2つ、Day 45b で1つ増えた)
...
flowchart TD
    ...
    V2 -->|接地| Z["**Velocity.Y = 0**<br/>床へ押し付けない(要点4)"]
...
flowchart TD
    ...
    C -->|はい・床| U["**真上へ** depth / normal.Y<br/>横へずれない(要点2)<br/>Velocity.Y を 0 に"]
...
**取り消すのは「動き」だけで、「接地している」という判断は残す**のが要点4。
...
`dotnet run --project reference/Day45b -c Release` で起動して、順に確かめる。
...
### 7. `Shift+Alt+X`: 内訳をコンソールへ

形・位置・速度・接地・坂の上限・段差・問い合わせの回数が並ぶ。
坂が登れない・段差が越えられないときは、まずここを見る——
**「上限の設定」か「引っかかり」のどちらか**しかないので、切り分けが1回で済む。

### 8. Day 45a までの機能が全部生きていること

`Ctrl+Shift+Alt+1` → `2` を4回(カプセルを落とす)と `Ctrl+Alt+X`(カプセルを降らせる)、
...
### 9. `Ctrl+Shift+Alt+X`: 今日の自己チェック(53 項目すべて合格)

窓を1枚も出さずに走る。節 1〜4 は Day 45a の 28 項目で、今日足したのは節 5〜9 の 25 項目。
...
- ビルド: `dotnet build reference/Day45b`(警告 0・エラー 0)
- 整形: `dotnet format whitespace reference/Day45b/Day45b.csproj --verify-no-changes` 差分なし
- 実行: `dotnet run --project reference/Day45b -c Release`
