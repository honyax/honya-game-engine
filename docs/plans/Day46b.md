# Day 46b: ブロードフェーズ(均一グリッド)— 答えを変えずに速くする

Day 46 の2日目で、reference は Day 46a の完全コピー + 差分。
Day 46a で地形を入れた。判定はまだ**全部の組を試す総当たり**のままで、
「地形に落とす」の 66 体で毎ステップ 2,130 組、自己チェックの 200 体なら 2 万組を試している。
**今日はこれを格子で絞る**——Day 26 の 2D の均一グリッドを 3D へ広げる。

地形が先にあったのには意味がある。**地形はブロードフェーズにとっていちばん厄介な客**で、
...
| キー | 何が起きるか |
|---|---|
| `Shift+G` | ブロードフェーズ(**均一グリッド / 総当たり**)。**今日の到達点** |
| `Alt+G` | 格子のマスを線で描く |
| `Ctrl+Shift+G` | マスの大きさ 0.5 / 1 / 2 / 4 / 8m |
| `Ctrl+Alt+G` | 体を 40 個降らせる(**組の数の増え方を見る**) |
| `Shift+Alt+G` | 地形とブロードフェーズの内訳をコンソールへ |
| `Ctrl+Shift+Alt+G` | 今日の自己チェック(59 項目。Day 46a の 44 項目を含む) |
| `Ctrl+G` | Day 46a のまま(地形デモ) |

HUD に**「広域」の1行**が増える。ブロードフェーズは効いていても絵が1ピクセルも変わらないので、
数字が出ていないと切り替えたことにすら気づけない。

### 今日いちばん大事な2つ

**1つ目は「ブロードフェーズは形を1つも知らない」**。
渡すのは外接箱(Day 46a の `Aabb3D`)の列だけで、返ってくるのは番号の組だけ。

```csharp
_grid.Build(_bounds);                    // 外接箱だけを受け取る
_grid.CollectPairs(_bounds);             // 番号の組だけを返す
foreach (BroadPair pair in _grid.Pairs) { TestPair(pair.A, pair.B); }
```

だから形が増えても格子は1行も変わらない。
**「答えを変えずに速くする」層**なので、格子と総当たりで接触の組がぴったり一致することを
自己チェックが 200 体で確かめている。

**2つ目は「格子に入らないものをどうするか」**。
...
差分は `Physics/` に新しいファイル1本(`SpatialGrid3D.cs`、652 行)と `PhysicsWorld` の書き換え、そして `Program.cs`。
`.vert` も `.frag` も 1 行も変わらない。**判定(`Collision3D`)も1行も変わらない**。

## 事前に読む資料

- **Real-Time Collision Detection**(Christer Ericson)7.1.2 Uniform Grids
...
### 1. ブロードフェーズ — 「答えを変えずに速くする」層

Day 43 から Day 46a まで、判定は全部の組を試す総当たりだった。
...
Day 46a で地形という5つ目の形が増えたが、`SpatialGrid3D` は
...
地形ははみ出し組なので全部の相手と組になるが、**組を作ったあと AABB で足切りする**。
ここで地形の外接箱が地面にぴったり張り付いていると、潜った物が足切りで落とされる。

そこで**地形の外接箱を下へ 2m 厚くしてある**(`Terrain3D.RecoveryDepth`)。
...
Day 46a の要点3の「潜った物体を押し上げる」枝が一度も走らない。
本当の直し方は掃引(スイープ)だが、今日は
**「抜けさせない」ではなく「抜けても戻す」**という安いほうで手当てしてある。

### 3. マスの大きさ — 3D では小さいほうへ外す罰が重い
...
- **BVH / Sweep and Prune** … 均一グリッドは「物の大きさが揃っている」ときに強い。
  大小が混ざると階層的な構造(BVH)か、軸ごとに区間を並べる SAP が要る(改造課題3)
- **差分更新** … 格子は毎ステップ丸ごと作り直している(要点3)。
  動いた物だけマスを移す作りは、全員が動く物理では割に合わないので入れていない
- **壁を格子に入れる** … 今日の壁は無限の平面なので、はみ出し組として全員と組になる(改造課題2)
- **掃引(スイープ)** … 地形には厚みが無いので、速い物は抜けうる。
  今日は地形の外接箱を下へ厚くして「抜けても戻す」で手当てしてある(要点2)
- **摩擦・スリープ** … Day 47。眠っている体を格子から外すと、候補はさらに減る
...
| ファイル | 行数 | 役割 |
|---|---|---|
| `Physics/SpatialGrid3D.cs` | 652 | 均一グリッドの 3D 版(要点1〜3) |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Physics/Shapes3D.cs` | `Aabb3D.Infinite` / `IsFinite`(**無限を値で持つ**)と、説明の書き換え | +36 / -6 |
| `Physics/RigidBody.cs` | `Bounds()`(**体を外接箱1つに落とす窓口**) | +23 / -0 |
| `Physics/PhysicsWorld.cs` | ブロードフェーズの組み込み、`TestPair` の切り出し、問い合わせの高速化 | +256 / -32 |
| `Physics/HeightField.cs` | コメントのみ(Day 46b を指していた言い回しを、今日の格子への参照に) | +3 / -3 |
| `Program.cs` | 格子の可視化、`G` の段の残り5つ、HUD の「広域」、自己チェックの後半 | +509 / -19 |
| `Day46b.csproj` | `Day46a.csproj` からのリネームのみ | — |

**`shaders/` も `Render/` も1文字も変わっていない**。**`Collision3D` も1行も変わっていない**。

### 写経する順番

**依存の順に並べてある**。4 の `GenerateContacts` の書き換えまでは足すだけ。

1. **`Physics/Shapes3D.cs`**(変更) — `Aabb3D` の説明の書き換えと、
   `Infinite`(`FromCenter` の後)/ `IsFinite`(`HalfSize` の後)。**要点2の主役**
2. **`Physics/RigidBody.cs`**(変更) — `Bounds()`(`ToTerrain` の直後)
3. **`Physics/SpatialGrid3D.cs`**(新規) — Day 26 の `SpatialGrid` を横に置いて写すとよい
...
4. **`Physics/PhysicsWorld.cs`**(変更) — 次の順で
   ...
   - プロパティ(`Broadphase` / `CellSize` / `GridOrigin` / `GridSize` / `Grid` / `BruteForcePairs`)と、
     `PairTests` の説明の書き換え
   - `QueryCapsule` / `OverlapCapsule` に格子の枝
   - `Clear` に `BruteForcePairs = 0` と `_gridReady = false`
   - **`GenerateContacts` を書き換え、`TestPair` と `UpdateBounds` を切り出す**
5. **`Program.cs`** — 次の順に足す
   - フィールド(`_showGridCells` / `GridCellSteps` / `_gridCellIndex`)。`_terrainSpawnCount` の説明の書き換え
   - `OnLoad` のヘルプに `G` の段の残り
   - `DrawOverlayInfo` に `BroadphaseLine()`
   - `BuildPhysicsScene` / `BuildCharacterScene` の頭に `ConfigureBroadphase()`
   - Day 46 の塊に `ConfigureBroadphase`(`DisposeTerrain` の直後)と、
     `RenderTerrain` の直後へ `RenderGridCells` / `CycleGridCellSize` / `BroadphaseLine` / `DescribeBroadphase`。
     `BuildTerrainScene` / `SpawnTerrainBodies` / `BuildTerrainCourse` の説明の書き換え
   - `RenderPhysics` の最後に `RenderGridCells()`
   - `RunTerrainCheck` の後半: 節1に無限の箱と平面の外接箱の3項目、
     節5「ブロードフェーズ」、節6(通しで動かす)の世界を格子で組む形に。説明と見出しの書き換え
   - 補助関数: `BuildTerrainOnlyWorld` に格子の設定と1ステップ、
     `BuildTerrainDropWorld` を `BuildBroadphaseWorld(mode, cellSize)` に書き換え、
     `StepAndCollectPairs` / `ContainsPair`(`WalkTowardHill` の直前)
   - キー(`G` の段の `Shift+G` / `Alt+G` / `Ctrl+Shift+G` / `Ctrl+Alt+G` / `Shift+Alt+G`)と、
     `Ctrl+G` の案内文、`Ctrl+Shift+Alt+2` の「地形に落とす」の案内文
6. **`Physics/HeightField.cs`** — **コメントのみ**。差分0にしたいなら合わせておく
7. **`Day46b.csproj`** — `Day46a.csproj` からのリネームのみ

**`G` の段は Day 46a で作った塊の中**に足す。素の `case Key.G:` より上にあること。

## 設計書

**層は今日も増えていない**。`Physics/` の中で型が2つ増え(`SpatialGrid3D` / `BroadphaseMode`)、
`PhysicsWorld` の判定の段が書き換わっただけ——矢印は1本も足していない。
`Physics/` は今日も**どこにも依存しない層**のまま。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Physics/SpatialGrid3D` | `Physics/` | 均一グリッドの 3D 版。**652 行**(要点1〜3) |
| `Physics/BroadphaseMode` | `Physics/` | 候補の作り方(総当たり / 均一グリッド)の札 |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Physics/Shapes3D` | `Aabb3D` が**無限を値で持てる**ようになった(`Infinite` / `IsFinite`) | +36 / -6 |
| `Physics/RigidBody` | `Bounds()`(体を外接箱1つに落とす) | +23 / -0 |
| `Physics/PhysicsWorld` | **候補の作り方が2通りになった**。判定そのものは1行も変わらない | +256 / -32 |
| `Program.cs` | 格子の可視化、`G` の段の残り、HUD、自己チェックの後半 | +509 / -19 |
...
おかげで、Day 46a で `ColliderKind` に5つ目の札が増えたのに `SpatialGrid3D` は無関係でいられる。
...
```csharp
// Day 46a: 二重ループの内側に判定・間引き・登録が直接書いてあった
// Day 46b: 「候補をどう作るか」と「候補をどう当てるか」を分けた
foreach (BroadPair pair in _grid.Pairs) { TestPair(pair.A, pair.B); }
```
...
**2つ目: `Program.cs` が 19,600 行を超えた**。
Day 46a で 19,139 行、今日 490 行足して 19,629 行。

| 何 | 行数 |
|---|---|
| 自己チェック(節5「ブロードフェーズ」と補助2本) | 約 200 |
| 格子の可視化・内訳・HUD | 約 190 |
| キー操作(`G` の段の残り5つ)・ヘルプ・配線 | 約 100 |

**エンジンの機能そのものは1行も `Program.cs` に無い**のは Day 45 と同じ。
切り出しの計画も変わらない(Day 51 で `Demo/DemoKeys.cs` /
`Demo/SelfChecks.cs` / `Demo/Hud.cs` へ)。

Day 46a の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の4つ。

| 図 | 何が変わったか |
|---|---|
| `Ecs と Physics` のクラス図 | **`SpatialGrid3D` / `BroadPair` / `BroadphaseMode` が増え、`PhysicsWorld` に格子の設定が入った** |
| 剛体が1ステップ進むまで | 判定の段の中で**候補の作り方が2通りに分かれ、`TestPair` へ合流する** |
| `衝突判定の3段` | **3D の側にもブロードフェーズが割り込んだ** |
| 均一グリッドの中身 | 3D の格子で変わったこと(3重の添字と、はみ出しの列) |
...
`dotnet run --project reference/Day46b -c Release` で起動して、`Ctrl+G` で地形デモを出してから、次を順に確かめる。

### 1. `Shift+G`: ブロードフェーズを総当たりに戻す
...
**「延べ 96 体」が Day 46a までは「延べ数千体」だった**数字。
`Shift+G` で総当たりに戻すと、その場で数十倍に跳ね上がるのが確かめられる。

### 5. Day 46a までの機能が全部生きていること

- `Ctrl+G` → 地形の上を歩ける。緩い丘は登れて、急な丘は登れない(Day 46a)
...
### 6. `Ctrl+Shift+Alt+G`: 今日の自己チェック(59 項目すべて合格)

窓を1枚も出さずに走る。Day 46a の 44 項目に、今日の 15 項目を足したもの。いちばん大事なのは次の3つ。
...
### 課題3(難): Sweep and Prune を書いて、格子と突き合わせる

均一グリッドは「物の大きさが揃っている」ときに強い(要点3)。
大小が混ざる世界でよく使われるもう1つの定番が **Sweep and Prune(SAP)**——
外接箱を x 軸に落とした区間を並べて、**区間が重なっている組だけ**を y・z で確かめる。

```
  1. 全員の外接箱の x 区間 [Min.X, Max.X] を、Min.X の小さい順に並べる
  2. 左から順に見て、「まだ閉じていない区間」の列と今の区間を比べる
  3. x で重なった組だけ、y と z でも重なっているか確かめる
```

`BroadphaseMode` に `SweepAndPrune` を足し、`GenerateContacts` の分岐を1つ増やす。
**`TestPair` へ合流させるだけで判定は1行も変わらない**のが、要点1の線引きの配当になる。

確かめ方は自己チェックの「格子と総当たりで接触の組がぴったり一致する」と同じ——
**同じ状態から1ステップだけ切り替えて、接触の組が一致する**ことを見る。
そのうえで、次を測って格子と比べる。

- 200 体の世界での候補の数と、1ステップの時間
- **はみ出し組(平面と地形)をどう扱うか**——SAP は無限の区間を並べられない。
  格子と同じく別の列に置くことになる
- 並べ替えを毎回やり直すか、前のステップの並びを使い回すか
  (使い回すと挿入ソートでほぼ O(n) になるのが SAP の値打ち)
...
| ブロードフェーズ(**今日**) | はみ出しの扱い、重複の除去、**総当たりとの一致**、マスの大きさへの不変性、問い合わせ | 12 |
...
**1. 地形には厚みが無いので、速い物は素直に抜ける**。

Day 46a の2つ(逆慣性テンソルと格子点の真下)を直しても、格子で組んだ世界では 200 体のうち1体(カプセル、落下速度 13m/s)が抜けた。
