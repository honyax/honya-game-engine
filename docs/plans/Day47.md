...
差分の形にそのまま出ている。次の Day 48(パーティクル)でようやく描画に戻る。
...
| `Program.cs` | 摩擦の坂、`F` の段、HUD、眠りの色、自己チェック 39 項目、**古くなった4項目の書き直し** | +1,059 / -34 |
...
5. **`Program.cs`** — 最後。次の順に足す
   ...
   - **`RunRigidBodyCheck` / `RunBoxCollisionCheck` の4項目を書き直す**(下記)
...
| Day 44 の1点接触 | 傾き 0.94 度 | 0.25 度。**しきい値を 0.005 → 0.002 rad へ緩める** |
| Day 44 の順番解き | 柱が崩れる(横ずれ 12.9m) | **崩れない**(35mm)。摩擦が支えている |
| Day 44 の沈み | 段数ぶん沈む(96.2mm)= **「Day 47 の宿題」** | **沈まない**(5.0mm)。宿題が片付いた |
...
切り出しの計画も変わらない(Day 50 で `Demo/DemoKeys.cs` /
`Demo/SelfChecks.cs` / `Demo/Hud.cs` へ)。
**Phase 7 の残りは Day 48〜50 の3日**なので、ここまで来たら切り出しは Day 50 でよい。
...
**4. Day 43・44 の自己チェックが4項目、嘘になった**。

「反復1周だと沈む」「順番に解くと崩れる」「段数ぶん沈む」——
