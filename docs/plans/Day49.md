# Day 49: パーティクルシステム(ビルボード・加算合成・コマ送り)

昨日は入力を片付ける日にあてた。**今日から描画に戻る**。

Phase 7 の残りは Day 49・50 のエフェクトと、Day 51 の組み上げ。
今日はその土台になる<b>パーティクルシステム</b>を作る——
点を撒いて、進めて、消して、それを<b>カメラを向いた四角形</b>として描く。

技術としては新しいものがほとんど無い。ビルボードは Day 18 のスプライトバッチを
3D に持っていっただけだし、粒の動きは Day 43 のセミインプリシット積分そのもの。
それでも1日を使うのは、**混ぜ方(ブレンド)と描く順番**という、
今まで一度も踏み込まなかった領域が丸ごと出てくるからになる。

> **今日いちばん時間を取られたのは「見えない」だった**
> パーティクルは<b>描けているのに画面に出ない</b>ということが起きる。
> しかも原因が3つある——**背景が明るい**(加算は暗いところでしか効かない)、
> **1粒が小さい**(数を増やしても濃さが積み上がらない)、
> **二重に薄れている**(テクスチャと色のカーブが両方フェードしている)。
> 実際にこの3つを順に踏んだので、要点2・3と設計書に残してある。

## 今日のゴール

**メニューの「パーティクル」ページを開いて `F2` を押すと、
中央の箱の上で焚き火が燃える。オレンジの炎が上へ細くなりながら消え、
火の粉が弧を描いて落ちる。`F4` で混ぜ方をアルファに変えると、
同じ粒が濁った灰色の塊になって「火」に見えなくなる。**

| キー(「パーティクル」ページ) | 何が起きるか |
|---|---|
| `F2` | パーティクルデモ ON/OFF。**今日の到達点** |
| `F3` | 筋書き(焚き火 / 煙 / 爆発 / 土埃 / 負荷) |
| `F4` | 混ぜ方(加算 / アルファ / 事前乗算)。**火が煙になる** |
| `F5` | 奥行きで並べ替え ON/OFF |
| `F6` | 深度を書く ON/OFF(**わざと間違える**) |
| `F7` | 明るさ x1 / x2 / x4 / x8 |
| `F8` | 放出量 x0.5 / x1 / x2 / x4 |
| `F9` | 重力 ON/OFF |
| `F10` | コマ送り(フリップブック)ON/OFF |
| `F11` | 一発撒く(バースト 400) |
| `F12` | 内訳(放出・寿命・釣り合いの数・転送量) |
| `1` | 今日の自己チェック(27 項目) |

筋書きは5つ。**どれも同じ `ParticleEmitter` の設定違いでしかない**。

| 筋書き | 混ぜ方 | 形 | 見どころ |
|---|---|---|---|
| 焚き火 | 加算 | 円錐 | 放出口が2つ(炎と火の粉)でも**ドローコールは1回** |
| 煙 | アルファ | 円錐 | **4x4 のコマ送り**と、奥行きの並べ替え |
| 爆発 | 加算 | 球 | **バースト**。抵抗で止めると「弾けた」形になる |
| 土埃 | アルファ | 円盤 | 外へ広がる。Day 50 でキャラクターの足元に付ける |
| 負荷 | 加算 | 球 | **8192 粒でプールを飽和**させて ms を見る |

### 今日いちばん大事な3つ

**1つ目はビルボード**。粒はただの点だが、画面には四角い絵として出したい。
そこで<b>カメラの右と上</b>で四角形を張る。
...
カメラの右と上は<b>ビュー行列から取れる</b>。回転部分は正規直交なので転置が逆行列で、
ビュー空間の `(1,0,0)` に対応するワールドのベクトルは<b>1列目</b>——
`(M11, M21, M31)` がそれになる。`cos` / `sin` は粒ごとの回転で、
**回した結果を頂点の位置に焼き込んでしまう**のでシェーダは何も知らなくてよい。

**2つ目は混ぜ方**。同じ粒が、混ぜ方1つで火にも煙にもなる。

```csharp
// 加算    重なるほど明るい。**暗いところでしか効かない**
glBlendFunc(GL_SRC_ALPHA, GL_ONE);

// アルファ 重ねても明るくならない。**順番が要る**
glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
```

加算は `dst + src·a` なので、**背景が既に明るいと何も起きない**。
花火が夜にしか上がらないのと同じ話で、演出ではなく前提条件になる。
今日のデモが空を消しているのはこのため(要点2)。

**3つ目は深度を書かないこと**。

```csharp
_gl.Enable(EnableCap.DepthTest);   // 読む: 箱の裏に回った粒は隠れてほしい
_gl.DepthMask(DepthWrite);         // 書かない: 既定は false
```

半透明が深度を書くと、**後から描く粒が深度テストで落ちる**——
手前の粒が壁になって、その裏の粒が四角く消える。
`F6` でわざと ON にすると一目で分かる。

### 今日は物理に触らない

差分は `Render/` に新しいファイル3本と `shaders/` に2本、そして `Program.cs`。
`Physics/` も `Model/` も `Scene/` も1行も変わっていない。

粒は<b>当たり判定を持たない</b>ので、`PhysicsWorld` とは無関係に動く。
だから可変 dt でよく(カメラワークやアニメと同じ扱い)、
決定性も要らない。**粒に物を押させたくなった日に初めて固定ステップへ移る**。

## 事前に読む資料

- [Blend Function(OpenGL Wiki)](https://www.khronos.org/opengl/wiki/Blending)
  — **今日の一次資料**。`glBlendFunc` の係数と、事前乗算アルファがなぜ1式で済むか
- [Particle Systems — A Technique for Modeling a Class of Fuzzy Objects(Reeves, 1982)](https://dl.acm.org/doi/10.1145/357318.357320)
  — パーティクルの原典。「境界のはっきりしないものを、多数の点で表す」という発想そのもの
- **Day 18 の `Render/SpriteBatch.cs`(このリポジトリ)**
  — ビルボードのバッチはこれの 3D 版になる。「差分を全部頂点に焼き込む」という判断が
  そのまま効いてくる
- **Day 43 の計画書 要点1(セミインプリシット積分)**
  — 粒の運動はこれと同じ。速度を先に進めてから位置に足す
- [Premultiplied alpha(Tom Forsyth)](https://web.archive.org/web/20200121143148/http://home.comcast.net/~tom_forsyth/blog.wiki.html#%5B%5BPremultiplied%20alpha%5D%5D)
  — 事前乗算がなぜ「正しい」のか。今日は選択肢として置くだけだが、理屈は読む価値がある

## 理論の要点

### 1. ビルボード — カメラの基底で四角形を張る

粒は点だが、画面には絵として出したい。
一番素直なのは<b>常にカメラを向いた四角形</b>を置くことで、これをビルボードと呼ぶ。

四隅は中心から<b>カメラの右と上</b>へ半分ずつずらせば作れる。
...
カメラの右と上をどこから取るか、が唯一の技術的な点になる。
ビュー行列 `V` はワールドからビューへの変換で、その回転部分 `R` は正規直交。
正規直交行列は<b>転置が逆行列</b>なので、ビュー空間の `(1,0,0)` に戻るワールドのベクトルは
`R` の1列目になる。System.Numerics は行ベクトル規約なので、これは `(M11, M21, M31)`。
...
**自己チェックで確かめているのはここ**(要点7)。長さが1で、直交していて、
外積が奥行き軸に一致すること——どれか1つでも崩れると四角形が歪むか裏を向く。

<b>CPU で四隅を作る</b>のは Day 18 と同じ判断になる。
頂点シェーダで展開する手(インスタンシングやジオメトリシェーダ)もあるが、
そちらは「粒ごとの uniform」か「追加の頂点属性」が要る。
CPU で焼き込めば **uniform はフレームに1本**で済み、1万粒でもドローコールは1回のまま。

代償は転送量で、1粒あたり4頂点 × 24 バイト = 96 バイト。
8000 粒なら毎フレーム 750KB を送ることになる。
**そこが効いてくるのが GPU パーティクル**で、教養編 Day 57 のコンピュートシェーダに繋がる。

### 2. 加算合成は「暗いところ」でしか効かない

ブレンドは `結果 = src × S + dst × D` の形で、`S` と `D` を選ぶのが `glBlendFunc`。

| 混ぜ方 | S | D | 式 | 性格 |
|---|---|---|---|---|
| 加算 | `SrcAlpha` | `One` | `dst + src·a` | **重なるほど明るい**。順番不問 |
| アルファ | `SrcAlpha` | `OneMinusSrcAlpha` | `src·a + dst·(1-a)` | 重ねても明るくならない。**順番が要る** |
| 事前乗算 | `One` | `OneMinusSrcAlpha` | `src + dst·(1-a)` | **1式で両方**。RGB に A を掛けておく |

加算の式をよく見ると、`dst` に足すだけで<b>引く手段が無い</b>。
つまり **`dst` が既に明るいと、何を足しても変わらない**。

これは理屈としては当たり前なのだが、実際にやると必ず一度は引っかかる。
今日のデモも、最初は<b>炎が1粒も見えなかった</b>——
空(スカイボックス)が白く、その前で足し算をしていたため。

```csharp
// ShowParticleDemo。**空を消す**のは演出ではなく前提条件
_env.SkyboxVisible = false;
```

花火が夜にしか上がらないのと同じ話になる。
**加算を使うなら背景を暗くする**、あるいは
明るい背景ではアルファ合成に切り替える、のどちらかしかない。

<b>事前乗算</b>はこの2つを1つの式に畳む手になる。
RGB にあらかじめ A を掛けておくと、`a = 0` の粒は `src + dst` で加算そのもの、
`a = 1` の粒は `src + 0` でアルファ合成そのものになる。
**1回のドローコールの中で火と煙を混ぜられる**のが値打ちで、
今日は選択肢として置くだけだが、Day 50 のエフェクト応用で効いてくる。

### 3. 粒が見えないときに疑う3つ

要点2の背景の話も含めて、**「描けているのに出ない」**には決まった原因がある。
今日はこの3つを順に踏んだので、順番に書いておく。

**1つ目: 背景が明るい**(要点2)。加算合成なら空を消す。

**2つ目: 1粒が小さい**。これがいちばん引っかかった。

テクスチャの減衰が `(1-r)²` なので、四角形の中で実効的な不透明度はかなり低い。
さらに寿命に沿ってアルファが 0 へ落ちるので、
**小さい粒はいくら数を増やしても濃さが積み上がらない**。

```
  0.5m の粒を 200 個   →  画面には何も出ない
  1.0m の粒を 130 個   →  はっきり煙に見える
```

画面に出るかどうかを決めているのは<b>数ではなく1粒の面積</b>になる。
煙や埃のように「もや」を出したいときは、**粒を大きくして数を減らす**のが正しい。

**3つ目: 二重に薄れている**。

コマ送りのテクスチャが最後のコマで透明になり、
かつ色のカーブも `EndColor` のアルファを 0 にしていると、**両方が掛かる**。
寿命の後半がまるごと消えることになり、「煙が続かない」という形で出る。

```csharp
// ParticleTextures.CreateSmokeFlipbook。0 まで落とさない
float fade = 1.0f - (0.55f * t);
```

<b>コマ送りが持つのは形、消え方は色のカーブが持つ</b>、と分けておく。
...
```csharp
_gl.Enable(EnableCap.DepthTest);   // 読む
_gl.DepthMask(false);              // 書かない
```

**読む**のは、不透明な床や箱の裏に回った粒に隠れてほしいから。
深度テストを切ると、粒が壁を突き抜けて手前に出てしまう。

**書かない**のは、半透明が深度を書くと<b>後から描く粒が落ちる</b>から。
手前の粒が「壁」になって、その裏の粒が四角く消える——
四角いのは、消えているのがビルボードの矩形そのものだからになる。

`F6` で ON にすると実演できる。焚き火の筋書きでいちばん分かりやすく、
炎が<b>四角い穴だらけ</b>になる。

**ここは「借りたら返す」も要点**。`End()` で `DepthMask(true)` に戻し忘れると、
次のフレームの<b>不透明な描画が深度を書かなくなり</b>、前後関係が崩れる。
しかも症状はパーティクルではなく床や箱のほうに出るので、原因を探すのが難しい。
Day 18 のスプライトバッチから続いている作法がここでも効く。

### 5. 並べ替えは「アルファのときだけ」要る

加算合成は足し算なので、**順番を変えても結果が同じ**になる(交換法則)。
アルファ合成はそうではない。`src·a + dst·(1-a)` は
`dst` を上書きしていく形なので、<b>奥から手前へ</b>描かないと正しくならない。

しかも `ParticleEmitter` は死んだ粒を末尾と入れ替えて詰めるので(要点6)、
**配列の順番は奥行きと何の関係も無い**。
だから描く直前に並べ替える。

```csharp
// 負の距離にして昇順に並べると、そのまま「遠い順」になる
_depths[i] = -Vector3.DistanceSquared(alive[i].Position, _cameraPosition);
Array.Sort(_depths, _order, 0, alive.Length);
```

平方根を取らないのは、大小の比較にしか使わないため
(平方根は単調増加なので、取っても順序は変わらない)。

**代償は HUD に出る**。`並替:ON 0.42ms` のように、
掛かった時間をそのまま出してある——
アルファに切り替えた瞬間に増えるので、出していないと気づけない。
8000 粒だと数 ms 掛かるので、**負荷の筋書きでは並べ替えを切ってある**。

### 6. プールと swap-remove — 順番を捨てて O(1) を買う

粒は毎フレーム生まれて死ぬ。**毎回 `new` しない**のが大前提で、
配列を1本持って「生きている数」だけを動かす。

死んだ粒は<b>末尾の生きた粒と入れ替えて</b>数を1つ減らす。
...
if (particle.Age >= particle.Lifetime)
{
    ...
    continue;   // **添字を進めない**。入れ替えた先はまだ進めていない
}
```

詰め直しが O(1) で済み、**生きている粒は常に配列の先頭から連続して並ぶ**。
描く側は `Alive` をそのまま舐めればよい。

代償は<b>順番が保たれないこと</b>で、これが要点5の並べ替えに繋がっている。
`continue` で添字を進めないのを忘れると、
**入れ替わってきた粒が1フレーム飛ばされる**——
症状は「たまに粒が1フレーム止まる」で、まず気づけない。

### 7. 撒く向きは「一様」が難しい

球面上や円錐の中に<b>一様に</b>向きを撒くのは、素朴にやると必ず偏る。

**球面**: 緯度と経度を両方一様に振ると、<b>極に密集する</b>。
地球儀の目盛りが極で詰まっているのと同じで、
緯度が同じ幅でも、そこが表す面積は極に近いほど狭い。
正しくは<b>高さ(cos θ)を一様に振る</b>(アルキメデスの定理)。

```csharp
float z = ((float)_random.NextDouble() * 2.0f) - 1.0f;   // 高さを一様に
float r = MathF.Sqrt(MathF.Max(0.0f, 1.0f - (z * z)));
```

**円錐**: 角度 θ を一様に振ると<b>軸の近くに密集する</b>。同じ理由になる。
`cos θ` を `[cos θmax, 1]` で一様に振るのが正しい。

**円盤**: 半径を一様に振ると<b>中心に寄る</b>。
面積は半径の2乗に比例するので、`√u` で振る。

```csharp
float radius = Radius * MathF.Sqrt((float)_random.NextDouble());
```

どれも<b>「一様」がどの測度で一様か</b>という話で、
自己チェックでは統計量で確かめている——
円錐なら平均 `cos` が `(1 + cos θmax) / 2` に一致すること、
円盤なら半径 `1/√2` の内側にちょうど半分入ること。
**偏っていても絵はそれらしく見える**ので、数で見るしかない。

### 8. 端数を持ち越す — fps で絵が変わらないために

毎秒 200 個を 60fps で撒くと、1フレームあたり 3.33 個。
切り捨てると毎秒 180 個にしかならず、144fps では 1.39 → 1 で毎秒 144 個。
**fps が変わると絵が変わる**ことになる。
...
Day 19 の固定ステップが「溜まった時間を持ち越す」のとまったく同じ形になる。
自己チェックでは 60fps と 144fps で 1 秒ぶん回し、
**どちらも 200 個**になることを確かめている。

**釣り合いの数**も押さえておく価値がある。
生きている粒の数は、放出が続いていれば `放出レート × 平均寿命` に落ち着く。

```
  焚き火   260 個/秒 × 0.75 秒 = 195 個
  煙        80 個/秒 × 2.8 秒  = 224 個
  負荷    6300 個/秒 × 1.3 秒  = 8190 個 ← 容量 8192 をわざと埋める
```

`F12` の内訳がこの数字を出す。**容量を超えると、超えたぶんは単に撒かれない**——
古いものを消したりはしないので、「粒が途中から出なくなる」という形で出る。

### 9. 今日入れなかったもの

- **ソフトパーティクル**(深度フェード)。粒が床や壁に刺さったところで
  <b>切り口が線になって見える</b>のを、深度の差で薄めて消す手。**Day 50 の主題**
- **トレイル**。粒の軌跡を帯として繋ぐ。これも Day 50
- **GPU パーティクル**。位置と速度をテクスチャかバッファに置いて、
  更新をコンピュートシェーダでやる。CPU からは「何個描け」しか言わなくなる。
  教養編 Day 57 で扱う
- **粒に当たり判定**。地面で跳ねる火花、壁で止まる煙。
  当たり判定を持たせた瞬間に<b>決定性が要る</b>ので、固定ステップへ移すことになる
- **カーブ**。今は始点と終点の線形補間だけで、途中に山を作れない
  (「出た瞬間だけ大きい」ができない)。改造課題1にしてある
- **ライト**。炎が周囲を照らさない。点光源を1つ置くだけでも効果は大きいが、
  そこは Day 52 のディファードで多光源を入れてからのほうが素直
...
| ファイル | 役割 |
|---|---|
| `Render/ParticleEmitter.cs` | 粒1つ(`Particle`)と放出口。撒く・進める・捨てる。**403 行**(要点6〜8) |
| `Render/ParticleRenderer.cs` | ビルボード化して1回で描くバッチ(`ParticleVertex` / `ParticleBlend` を含む)。**471 行**(要点1・4・5) |
| `Render/ParticleTextures.cs` | 粒の絵を数式で作る。丸1枚と煙の 4x4 コマ送り。**141 行** |
| `shaders/particle.vert` | ワールド座標に `uViewProjection` を掛けるだけ。**28 行** |
| `shaders/particle.frag` | テクスチャ × 頂点色 × 明るさ。事前乗算の分岐だけ持つ。**43 行** |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Program.cs` | 筋書き5つ、放出口2つ、描画パス1つ、HUD 1行、メニュー 12 項目、自己チェック 27 項目 | +979 / -4 |

`Physics/` も `Model/` も `Scene/` も `Ecs/` も **1行も変わっていない**。
`Render/` の既存ファイルも触っていない——
`SpriteVertex.PackColor` と `VertexAttribute` を<b>借りているだけ</b>で、
向こうに手を入れる必要は無かった。

**`Program.cs` の +979 行の内訳**はこうなっている。

| 何 | 行数 |
|---|---|
| 自己チェック(`RunParticleCheck`) | 約 300 |
| 筋書きの設定(`ApplyParticleScene`) | 約 200 |
| メニューのページ(12 項目) | 約 160 |
| デモの出し入れ・HUD・内訳 | 約 190 |
| フィールド・列挙・配線 | 約 130 |

**今日は重い日**になる。新規 1,086 行 + `Program.cs` 979 行で、
写経は<b>4〜5時間</b>を見込んでほしい。
コメントが4割ほどあるので、実際に打つ量はもう少し軽いはずだが、
1日で終わらなければ<b>「1〜4 まで」と「5 以降」で切る**のが素直な分け方になる
(4 まで終われば焚き火は出る)。

### 写経する順番

**依存の順に並べてある**。1〜3 はエンジン側で、`Program.cs` を知らない。

1. **`shaders/particle.vert`**(新規) — ワールド座標に行列を掛けるだけ。
   モデル行列が無いことと、その理由(差分を頂点に焼き込んである)を読む
2. **`shaders/particle.frag`**(新規) — テクスチャ × 頂点色。
   `uIntensity` と `uPremultiply` の2つだけ分岐がある
3. **`Render/ParticleEmitter.cs`**(新規) — 上から順に
   - `ParticleShape`(3つ)/ `Particle`(構造体。`Normalized` が全部の入力になる)
   - フィールドと設定のプロパティ(数が多いが、どれも1行)
   - `Burst` / `Clear`
   - **`Update`** — 撒く → 進める → 捨てる。**`continue` で添字を進めない**のが要点6
   - `ColorAt` / `SizeAt` / `FrameAt`
   - `Spawn` と、その下の `RandomDirection` / `RandomInCone`(要点7)
4. **`Render/ParticleTextures.cs`**(新規) — `CreateSoftDot` → `CreateSmokeFlipbook`。
   **`fade` を 0 まで落とさない**理由(要点3)を読んでおく
5. **`Render/ParticleRenderer.cs`**(新規) — 上から順に
   - `ParticleBlend`(3つ)/ `ParticleVertex`(24 バイト)
   - コンストラクタ(**Day 18 の `SpriteBatch` とほぼ同じ**。見比べると早い)
   - プロパティ(`Blend` / `SortByDepth` / `DepthWrite` / `Intensity` と計測用)
   - **`Begin`** — カメラの基底を取る(要点1)。GL の状態もここで作る
   - `Draw` / `End` / `AppendSorted`(要点5)
   - **`Append`** — 四隅を作る。フリップブックの UV もここ
   - `Flush` / `ApplyBlend`(要点2)/ `Dispose`
6. **`Program.cs` のフィールドと列挙**(変更) — `ParticleScene`(5つ)を
   `PhysicsScene` の直前に。`_menu` の直後に `_particles` 以下 12 個
7. **`Program.cs` の初期化**(変更) — `_spriteShader` の読み込みの直後。
   シェーダ・テクスチャ2枚・`ParticleRenderer`・放出口2つ・`ApplyParticleScene()`
8. **`Program.cs` の `ShowParticleDemo` / `HideParticleDemo` / `ApplyParticleScene`**(新規)
   — 筋書き5つぶんの設定。**`_env.SkyboxVisible = false`** の理由(要点2)は読む価値がある
9. **`Program.cs` の `ApplyParticleGravity` / `CurrentParticleTexture` / `UpdateParticles`**(新規)
10. **`Program.cs` の `RenderParticles` / `ParticleLabel` / `DescribeParticles`**(新規)
11. **`Program.cs` の更新と描画への差し込み**(変更) — 2箇所だけ
    - `OnUpdate` の可変 dt 側、`_animation?.Update(...)` の直後に `UpdateParticles`
    - `OnRender` の `if (_uiThroughPost)` の直前に `RenderParticles`。
      **`_post.Begin` と `_post.End` の間**であることが要る(ブルームに拾わせるため)
12. **`Program.cs` の HUD**(変更) — `_menu.HudLine()` の直後に `ParticleLabel()` を1行
13. **`Program.cs` の `RunParticleCheck`**(新規) — 27 項目。`CheckList` の直前に置く
14. **`Program.cs` のメニューのページ**(変更) — `BuildDebugMenu` の `return menu;` の直前。
    **12 行足すだけ**で、キーも一覧も HUD の案内も全部そこから起きる(Day 48 の値打ち)
15. **`Program.cs` の `RunDebugMenuCheck`**(変更) — 項目数の期待値を
    `159` から `171` へ。**ここを直さないと Day 48 の自己チェックが落ちる**

## 設計書

**層は今日も増えていない**。`Render/` に型が5つ増えただけで、矢印は1本も足していない。
`Physics/` も `Model/` も `Scene/` も `Ecs/` も1行も変わっていない。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Render/Particle` | `Render/` | 粒1つ。位置・速度・寿命・回転・種(**構造体**) |
| `Render/ParticleShape` | `Render/` | 撒く形(円錐 / 球 / 円盤) |
| `Render/ParticleEmitter` | `Render/` | 放出口。撒く・進める・捨てる。**403 行** |
| `Render/ParticleVertex` | `Render/` | 24 バイト。位置(3)+ UV(2)+ 色(4byte) |
| `Render/ParticleBlend` | `Render/` | 混ぜ方(加算 / アルファ / 事前乗算) |
| `Render/ParticleRenderer` | `Render/` | ビルボードのバッチ。**471 行** |
| `Render/ParticleTextures` | `Render/` | 粒の絵を数式で作る。**141 行**(static クラス) |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Program.cs` | 筋書き5つ、描画パス1つ、HUD 1行、メニュー 12 項目、自己チェック 27 項目 | +979 / -4 |

### 「進める」と「描く」を分けた

`ParticleEmitter` は **GL を1行も触らない**。逆に `ParticleRenderer` は
**粒を1つも持たない**。この分け方は `PhysicsWorld` と `RenderPhysics` の関係と同じで、
理由も同じになる——<b>片方だけをテストしたい</b>。

実際、今日の自己チェック 27 項目のうち **20 項目は窓を開かずに走る**。
端数の持ち越しも、円錐の分布も、抵抗の指数減衰も、
`ParticleEmitter` が GL を知らないからこそ数で確かめられる。

```mermaid
flowchart LR
    E["ParticleEmitter<br/>撒く・進める・捨てる"] -- "Alive: 生きている粒" --> R["ParticleRenderer<br/>四隅を作って1回で描く"]
    R -- "uViewProjection" --> S["particle.vert / .frag"]
    C["Camera"] -- "ViewMatrix の列 = 右と上" --> R
    T["ParticleTextures<br/>数式で作った絵"] -- "Texture" --> R
```

矢印が<b>片方向だけ</b>なのが要点になる。
`ParticleEmitter` は `Camera` も `Texture` も `GL` も知らない。
だから **Day 57 で GPU へ移すときに置き換わるのは右半分だけ**で、
設定の意味(寿命・重力・色のカーブ)はそのまま持っていける。

### 粒が1つ描かれるまで

```mermaid
flowchart TD
    U["Update&lpar;dt&rpar;"] --> C1["_carry += EmitRate × dt<br/>端数を持ち越す"]
    C1 --> SP["Spawn を int&lpar;_carry&rpar; 回"]
    SP --> IN["速度を進めて位置に足す<br/>セミインプリシット"]
    IN --> DE{"Age >= Lifetime ?"}
    DE -- "はい" --> SW["末尾と入れ替えて Count--<br/>**添字は進めない**"]
    DE -- "いいえ" --> NX["次の粒へ"]
    NX --> B["Begin&lpar;camera&rpar;<br/>右と上をビュー行列から"]
    SW --> B
    B --> SO{"SortByDepth ?"}
    SO -- "はい" --> SR["距離の2乗で並べ替え"]
    SO -- "いいえ" --> AP["Append: 四隅 + UV + 色"]
    SR --> AP
    AP --> FL["Flush: BufferSubData → DrawElements<br/>**1回**"]
```

**並べ替えが `Append` の手前にしかない**のが構造として大事なところ。
`ParticleEmitter` 側は順番を保証しない(swap-remove で壊れる)ので、
順番が要るのは<b>描く直前だけ</b>と決めてある。
ここを逆にして「常に順番を保つプール」にすると、
死んだ粒を消すたびに O(n) の詰め直しが要る——
**加算合成では必要ない仕事を、全部の筋書きに払わせる**ことになる。

### 状態を借りて返す

`ParticleRenderer.Begin` は GL の状態を4つ変える。

| 状態 | 何にするか | 戻すか |
|---|---|---|
| `Blend` | 有効 + `ApplyBlend()` | **`End` で無効に戻す** |
| `DepthTest` | 有効 | 戻さない(不透明の描画も有効のまま使う) |
| `DepthMask` | `DepthWrite`(既定 false) | **`End` で true に戻す** |
| `CullFace` | 無効 | 戻さない(次の `Render3D` が自分で設定する) |

**`DepthMask` を戻し忘れると症状が向こうに出る**。
次のフレームの不透明な描画が深度を書かなくなり、床と箱の前後関係が崩れる——
パーティクルを疑うところまで辿り着けない種類の壊れ方になる。

「戻すもの」と「戻さないもの」を分けているのは、
<b>次に描く人が必ず自分で設定するかどうか</b>で決めている。
`Render3D` は毎フレーム深度テストとカリングを設定し直すので、そこは戻さなくてよい。
`DepthMask` は誰も設定し直さない(既定が true のまま使われている)ので、返す。

### 絵を数式で作った

`ParticleTextures` は素材を1枚も使わない。

これは素材を増やさないための判断でもあるが、それ以上に
<b>パーティクルの絵は「形」ではなく「減衰の仕方」だ</b>ということが、
作ってみるといちばんよく分かる。
中心が明るくて縁へ滑らかに落ちる——それだけで火にも煙にも埃にも見える。

**縁を切り立たせない**のが唯一にして最大のコツになる。
`(1 - r)` と線形に落とすと粒が<b>円板</b>に見えて重なりが目立ち、
`(1 - r)²` にするだけで「もや」になる。

コマ送りのほうは、**形だけを持たせて消え方は持たせない**と決めてある(要点3)。
最初は最後のコマを透明にしていて、
色のカーブの `EndColor` アルファ 0 と<b>二重に掛かり</b>、
寿命の後半がまるごと消えていた。
「1つの見た目を2箇所で作らない」は Day 40 の `FeatureToggles` で
「状態の置き場所は1つ」と決めたのと同じ形の判断になる。

### 今日残した歪み(3つ)

**1つ目: 混ぜ方が1パスに1つしか無い**。
`ParticleRenderer.Blend` はレンダラ全体の設定なので、
**同じフレームに加算の炎とアルファの煙を同時に出せない**。
本当にやるなら「加算のものを全部描く → アルファのものを全部描く」と
2パスに分けるか、事前乗算に寄せて1式に畳むことになる。
今日は筋書きごとにどちらかを選ぶ形で逃げた。**Day 50 で必要になる**。

**2つ目: 並べ替えが放出口ごと**。
`AppendSorted` は `Draw` の中で呼ばれるので、
放出口が2つあると<b>それぞれの中でしか並ばない</b>。
放出口をまたいだ正しい順序にはならない。
今日は同時に出しているのが焚き火(加算・並べ替え不要)だけなので表に出ないが、
アルファの放出口を2つ出した日に必ず気づくことになる。

**3つ目: カーブが線形しかない**。
`ColorAt` も `SizeAt` も始点と終点の `Lerp` だけなので、
**「出た瞬間だけ大きい」「中盤で一番明るい」が作れない**。
本物のエンジンはここにカーブエディタを持っている。
足すなら「制御点の配列 + 線形補間」で十分で、改造課題1にしてある。

Day 48 の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の2つ。

| 図 | 何が変わったか |
|---|---|
| `Render` のクラス図 | **パーティクルの5つの型が増えた** |
| `1フレームの流れ` | 不透明の描画のあとに**粒のパスが1つ増えた** |

新しく足した図が2つ。

| 図 | 何を描いたか |
|---|---|
| 進めると描くの分かれ方 | `Emitter` → `Renderer` → シェーダの片方向の流れ |
| 粒が1つ描かれるまで | 端数 → 生成 → 積分 → 詰め直し → 並べ替え → 1回のドロー |

`Physics/` と `Model/` の図は**昨日のまま**で、今日は1つも触っていない。
...
### Render — OpenGL の薄い皮(Day 49 でパーティクルの5つが増えた)
...
**片方だけをテストしたい**。今日の自己チェック 27 項目のうち
...
flowchart TD
    ...
    RG --> PT
    RS2 --> PT
    PT["RenderParticles&lpar;&rpar;<br/><b>不透明を全部描いたあと</b><br/>深度は読むが書かない<br/>_post の中なのでブルームに乗る"]
    PT --> UQ
...
`dotnet run --project reference/Day49 -c Release` で起動して、次が確かめられること。
**メニューは 17 ページになっている**(`Ctrl+F1` で目次)。

### 1. 「パーティクル」ページの `F2`: 箱の上で焚き火が燃える

空が消えて背景が黒くなり、中央の箱(y = -0.5〜1.0)の天面に炎が立つ。

- オレンジ(`1.0, 0.72, 0.28`)から赤へ移りながら、上へ細くなって消える
- **火の粉**が円錐 35 度で飛び、重力 -3 で弧を描いて落ちる
- HUD に `粒:200/8192  DC:1  加算  並替:OFF  深度書込:OFF  x2  更新:0.0xms`

**`DC:1` が要点**。炎と火の粉で放出口は2つあるが、
テクスチャが同じなので<b>ドローコールは1回</b>に畳まれている。

**空が消えるのは正しい**(要点2)。白い空の前だと、加算合成では1粒も見えない。

### 2. `F4`: 混ぜ方を変えると火が煙になる

| 混ぜ方 | 何が起きるか |
|---|---|
| 加算 | 重なったところが明るい。**炎に見える** |
| アルファ | 重なっても明るくならない。**濁った灰色の塊**になり、火に見えない |
| 事前乗算 | アルファ寄りだが、アルファの低い粒は加算のように光る |

**同じ粒がまったく違う物質に見える**のが今日いちばん面白いところになる。

### 3. `F3` で「煙」へ: コマ送りと並べ替え

- 灰色の煙が箱の上からゆっくり広がる。**1粒が 1.0m → 3.0m** と大きい
- HUD が `アルファ  並替:ON 0.0xms` になる
- `F10` でコマ送りを切ると、**煙がずっと同じ形のまま薄れる**
- `F5` で並べ替えを切ると、粒の重なりが**四角く抜ける**ところが出る

**粒が小さいと見えない**(要点3)。`F8` で放出量を増やしても濃くならないことを
確かめておくと、要点3の「数ではなく面積」が腑に落ちる。

### 4. `F6`: 深度を書くと粒が四角く消える

焚き火の筋書きに戻して `F6`。**炎が四角い穴だらけ**になる。

手前の粒が深度を書き、その裏に来た粒が深度テストで落ちるため。
四角いのは、消えているのがビルボードの矩形そのものだから。

### 5. `F3` で「爆発」へ: バーストと抵抗

500 個が球状に一気に飛び出し、抵抗 2.5 で急に止まって落ちる。
`F11` でもう一度撒ける。

**箱の手前を通る粒は箱に隠れる**——深度を読んでいることの確認になる。

### 6. `F3` で「土埃」へ: 円盤から外へ

床の上(z = 2.4)に低い雲が広がる。円盤の上から**外向き**に撒いているので、
真上ではなく横へ這う。Day 50 でキャラクターの足元に付けるのがこれになる。

### 7. `F3` で「負荷」へ: 8192 粒で飽和する

- HUD が `粒:8150/8192` あたりで止まる。**放出 × 寿命 = 6300 × 1.3 ≒ 8190**
- `更新` が 0.5〜1.5ms 程度。**これが CPU パーティクルの上限の目安**になる
- `F5` で並べ替えを ON にすると `並替:` の ms が跳ねる(8000 個のソート)

`F12` の内訳で「釣り合いの目安」が出る。
`F8` で放出量を x2 にすると<b>容量を超えて撒かれなくなる</b>ことも確かめる。

### 8. `1`(数字): 自己チェックが 27 項目すべて合格

```
--- Day 49: パーティクルの自己チェック ---
  [OK] **60fps でも 144fps でも 1 秒で同じ数**(端数を持ち越している)  60fps:200 / 144fps:200(理論 200)
  ...
  [OK] **軸のまわりに偏っていない**(cos を一様に振っている)  平均 cos 0.9699(理論 0.9698)
  [OK] **面積で一様**(半径 1/√2 の内側が半分)  50.2%
  [OK] **セミインプリシット**(速度を先に進める)  Y = 0.9000(陽的オイラーなら 1.0)
  [OK] **抵抗で逆走しない**(指数減衰にしてある)  v = 0.00045(1 - Drag·dt なら -90)
  すべて合格
```

### 9. Day 48 の自己チェックも合格

「メニュー」は `Alt+F1`。**項目数が 159 から 171 に増えている**ので、
`RunDebugMenuCheck` の期待値を直していないとここで落ちる。
17 ページ 171 項目で全項目合格になること。

## 改造課題

### 課題1(易): 色と大きさにカーブを入れる

いまの `ColorAt` / `SizeAt` は始点と終点の `Lerp` だけなので、
**「出た瞬間だけ大きい」「中盤で一番明るい」が作れない**。

制御点の配列を持たせて、区間ごとに線形補間する。

```csharp
/// <summary>寿命に沿ったカーブ。制御点は 0〜1 の順に並んでいること。</summary>
internal sealed class ParticleCurve
{
    private readonly (float T, float Value)[] _keys;

    public float Evaluate(float t)
    {
        // 区間を探して線形補間。両端は端の値で止める
    }
}
```

`StartSize` / `EndSize` を `SizeCurve` に置き換えて、
焚き火を `(0, 0.1) (0.15, 0.5) (1, 0.05)` にすると、
**火が「ぼっ」と立ち上がる**ようになる。

**確かめること**: 制御点が1個・0個でも落ちないこと、
`t` が 0 未満や 1 超でも端で止まること。
自己チェックに「両端で止まる」を足しておくと、区間探索の境界がすぐ分かる。

### 課題2(中): 加算とアルファを1パスで混ぜる

いまは `ParticleRenderer.Blend` がレンダラ全体の設定なので、
**炎(加算)と煙(アルファ)を同時に出せない**。

事前乗算(`ParticleBlend.Premultiplied`)に寄せると、1つの式で両方を扱える。

```
  src + dst·(1-a)    a = 0 なら 加算そのもの
                     a = 1 なら アルファそのもの
```

`ParticleEmitter` に「加算寄りか、アルファ寄りか」を 0〜1 で持たせて、
`ColorAt` が返すアルファを掛け分ける。
炎の粒はアルファを 0 に寄せ、煙の粒は 1 に寄せる。

**確かめること**: 焚き火の上に煙を足して、
**炎が光ったまま、煙が背景を隠す**こと。
1パスで済んでいるか(HUD の `DC:` が 1 のまま)も見る。

**ここが今日と Day 50 の橋渡し**になる。土埃・爆発・トレイルを同じシーンに置くと、
混ぜ方の異なる粒が必ず同居することになる。

### 課題3(難): 更新をコンピュートシェーダへ移す

負荷の筋書きで `更新` が 1ms を超える。8192 粒で 1ms なら、
10 万粒なら 12ms で<b>それだけで 1 フレームを使い切る</b>。

粒の状態(位置・速度・寿命)を **SSBO** に置いて、更新をコンピュートシェーダでやる。
CPU からは「何個描け」しか言わなくなる。

段取りはこうなる。

1. `Particle` を GPU 側の構造体に合わせる(`std430` のアラインメントに注意)
2. 更新のシェーダを書く。**生死の判定と詰め直しが難所**——
   `atomicCounter` で生きている数を数えるか、死んだ粒を「大きさ 0」で描く
3. 描画は `glDrawArraysIndirect` にして、描く数も GPU が決める
4. **ビルボードは頂点シェーダへ移す**(CPU で四隅を作れなくなるため)

3 と 4 まで行くと「CPU がドローコールの中身を知らない」形になり、
これは教養編 Day 65 の GPU 駆動レンダリングと同じ発想になる。

**確かめること**: 10 万粒で `更新` が 0 に近づくこと。
そして<b>絵が CPU 版と同じに見えること</b>——
乱数の作り方が違うので粒の並びは一致しないが、
「釣り合いの数」と見た目の濃さは合うはず。

**手を出す前に読む**: 教養編 Day 57(コンピュートシェーダ入門)。
そこで GPGPU の実行モデル(ワークグループ・共有メモリ)を通してからのほうが早い。
...
`1`(「パーティクル」ページ)。**27 項目のうち 20 項目は窓を開かずに走る**——
`ParticleEmitter` が GL を知らないので、数だけで確かめられる。

| 範囲 | 何を確かめているか | 項目数 |
|---|---|---|
| 端数の持ち越し | **60fps と 144fps で 1 秒あたりの数が同じ** | 1 |
| プールと寿命 | Burst の数、先頭から連続、寿命で消える、**容量を超えない** | 5 |
| 寿命に沿った値 | 色の両端と中点、大きさ、コマ番号の両端と範囲 | 7 |
| 撒く向きの分布 | **円錐に収まる・偏らない**、円盤が水平・**面積で一様** | 5 |
| 積分と抵抗 | **セミインプリシット**、**指数減衰で逆走しない** | 2 |
| ビルボードの基底 | 長さ1、**直交**、外積が奥行き軸 | 3 |
| 手続きテクスチャ | 中心が濃い、コマ送りの大きさ、**透明になりきらない** | 4 |

とくに強いのは次の4つ。

- **60fps と 144fps で同じ数** — 端数を切り捨てていると 200 個が 180 個と 144 個に割れる。
  絵では気づけないが、数なら一発で出る
- **円錐の平均 cos が理論値と一致** — 実測 0.9699 / 理論 0.9698。
  角度を一様に振ると軸寄りに偏り、ここが 0.985 あたりまで上がる。
  **偏っていてもそれらしく見える**ので、数で見るしかない
- **円盤の半径 1/√2 の内側がちょうど半分** — 実測 50.2%。
  半径を一様に振ると 70% ほどになり、土埃が中心に固まる
- **抵抗で逆走しない** — `(1 - Drag·dt)` にしていると
  `Drag=20, dt=0.5` で速度が -90 になり、**粒が逆向きに吹き飛ぶ**。
  dt が跳ねたフレームで一度でも起きると目に見える

### 検証の途中で分かったこと

**1. 「描けているのに見えない」を3回踏んだ**(要点2・3)。

最初は炎が1粒も出なかった。`ParticleCount` も `DrawCallCount` も正しく、
粒の座標をクリップ空間まで出しても画面の中央に居る。
原因は順に3つあった。

| 症状 | 原因 | 直し方 |
|---|---|---|
| 炎が1粒も見えない | **空が白い**。加算は明るい背景では何も足せない | デモで `SkyboxVisible = false` |
| 炎が箱に埋まっている | 中央の箱が y = -0.5〜1.0 を占めていた | 放出口を天面(y = 1.0)へ |
| 煙が出ない | **1粒が小さい**。0.5m を 200 個出しても濃さが積み上がらない | 1.0m → 3.0m に |

**切り分けは「真っ赤・不透明・3m」に振ってみるのが早かった**。
それで出るなら経路は生きていて、あとは見た目の話でしかない。

**2. コマ送りと色のカーブが二重にフェードしていた**(要点3)。

コマ送りの最後のコマを透明にし、かつ `EndColor` のアルファを 0 にしていたので、
寿命の後半がまるごと消えていた。
**コマ送りが持つのは形、消え方は色のカーブが持つ**と分けて直した。

「1つの見た目を2箇所で作らない」は Day 40 で
「状態の置き場所は1つ」と決めたのと同じ形の判断になる。

**3. 500fps 出ているので、フレーム数で待つと足りない**。

検証中、決まったフレーム数でスクリーンショットを撮っていたら、
寿命 3 秒の煙が育たなかった。この構成は 400〜600fps 出るので、
**400 フレームでも 1 秒未満**になる。
時間で待つように直したら、粒の数が理論値(放出 × 寿命)にぴたりと合った。

`GameLoop` の固定ステップ(Day 19)は物理を守っているが、
**パーティクルは可変 dt 側に置いてある**ので、この速さで回っている。
そのぶん端数の持ち越し(要点8)が効いているのが、実測 200 個で確かめられる。
