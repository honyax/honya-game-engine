...
**1つ目は「本描画のシェーダを真ん中で切る」**。
`textured.frag` は「表面を決める」(視差 → ベースカラー → 法線マップ → 金属度・粗さ → AO → 発光)と
「光を当てる」(影 → BRDF → IBL → SSAO)の2つを続けてやっていた。
その切れ目に出口を1つ作り、<b>前半だけ走らせて G-Buffer に書いて帰る</b>。

```glsl
if (uGBufferPass == 1)
{
    FragColor    = vec4(sqrt(base.rgb), 1.0);               // 0: ベースカラー(平方根で詰める)
    GMaterial    = vec4(occlusion, roughness, metallic, 1.0); // 1: glTF の ORM と同じ並び
    GNormalDepth = vec4(viewNormal, viewDepth);             // 2: Day 37 の幾何バッファと同じ形
    GEmissive    = vec4(emissive, 1.0);                     // 3: 発光
    return;
}
```
...
- **Day 50 の計画書 要点3**(このリポジトリ)
  — `Framebuffer.BlitDepthTo`(深度を写す)。今日は G-Buffer の深度をシーンのバッファへ写すのに使う
...
| 深度 | 24bit | 深度テストのためと、あとでシーンのバッファへ写すため | 3 |

1画素 27 バイト、960x640 で 15.8MB。**全部を 16F にすると 35 バイト**になるので、
中身ごとに必要な精度を選んでいる。

1回の描画で4枚へ同時に書くのが **MRT**(Multiple Render Targets)。
画素シェーダが `layout(location = N) out vec4` で出口を分け、
フレームバッファ側は `glDrawBuffers` で「出口 N → アタッチメント N」の対応表を持つ。

```csharp
Span<DrawBufferMode> buffers = [ColorAttachment0, ColorAttachment1, ColorAttachment2, ColorAttachment3];
_gl.DrawBuffers(4, buffers);
```

**これを忘れると完全性チェックは通るのに、0 番のアルベドだけが書かれる**。
法線と材質が真っ黒のまま、エラーは1つも出ない(Day 33 の `glDrawBuffer(GL_NONE)` と同じ種類の呪文)。

**番号は `Material` の表を引き継ぐ**。読む側で G-Buffer を 0・1・2・4 番に刺す——
0 がベースカラー、1 がメタリック・ラフネス、2 が法線、4 が発光で、
...
vec3 viewPos  = vec3(ndc * uProjScale * depth, -depth);   // ssao.frag の ViewPosition と同じ
...
0.02 のあたりでは1段が値の 10% を超える。

```
  0.02〜1 での最大の相対誤差   リニアの 8bit  8.8%   平方根で詰める  2.7%
```

**平方根にしてから入れ、読むときに2乗する**。暗い側に段が寄るので誤差が 1/3 になる。
...
```csharp
_gbuffer.Target.BlitDepthTo(_post.Scene);   // Day 50 で足した口
...
```csharp
_ssao.Compute(_camera, width, height, _gbuffer.NormalDepth);   // 幾何パスを描かない
```

**`Ssao` は `GBuffer` を知らない**。受け取るのは `Texture` 1枚で、
自前か借り物かはシェーダからも区別が付かない。
自己チェックは、法線マップの無い物で両者の遮蔽が<b>完全に一致</b>(平均の差 0.0000)することを見ている。

1つだけ変わることがあり、G-Buffer の法線は<b>法線マップを当てたあと</b>のもの
...
<b>G-Buffer パスは `Render3D` をそのまま呼ぶ</b>ので(設計書)、何を描くかを書き写していない。
...
**矢印の出どころが `Render3D` と `ApplyLighting` の2つに集まっている**のが今日の姿になる。
...
| 場所 | 役割 |
|---|---|
| `Pbr.cs` | 定義と検算(自己チェックだけが呼ぶ) |
| `textured.frag` | フォワード。`CookTorrance` は D・G・F を外へ出す版 |
...
ところが**影のパス(`RenderShadowPass`)は今日も手書きの分岐**で、
Day 51 でキャラクターを足したときに3か所を直すことになった、その形が1か所ぶん残っている。
SSAO の自前の幾何パスと幾何バッファ(6.4MB)も、フォワードの絵のために残してある。
...
加えて、Day 36 の設計書が「Day 52 で複数のカラーアタッチメントが要るときにまとめて整理する」と書いた
`EnvironmentMap` の生の FBO は、**今日も生のまま**——MRT は足したが、
「外のテクスチャ(キューブマップの1面)を挿す」口は足していない。
...
### 1. `F2`: 描き方が変わっても、絵は変わらない

コンソールに次の1行が出る。

```
ディファード: **ON**。G-Buffer 960x640 x4枚 15.8MB。**絵が変わらないのが正しい**——HUD の「幾何パス」が 3 回から 2 回に減る
```
...
```
--- ディファードの内訳 ---
  0: ベースカラー(平方根)    Rgba8    4B/画素  2.3MB  読むときの番号 0
  1: AO / 粗さ / 金属度  Rgba8    4B/画素  2.3MB  読むときの番号 1
  2: ビュー法線 + 距離     Rgba16F  8B/画素  4.7MB  読むときの番号 2
...
1. `GBuffer` の深度をレンダーバッファから**深度テクスチャ**に替える
   (`Framebuffer` に「カラー N 枚 + 深度テクスチャ」の形を足す。Day 33 の `CreateDepthOnly` の応用)
2. `deferred.frag` で深度を読み、**逆射影行列**で戻す

```glsl
float depth = texelFetch(uGDepth, pixel, 0).r;                  // 0〜1
vec4 clip = vec4(ndc, depth * 2.0 - 1.0, 1.0);                  // OpenGL の NDC は z も -1〜1
vec4 view = uInverseProjection * clip;
vec3 viewPosition = view.xyz / view.w;                          // 透視除算を自分でやる
```

自己チェックの「位置の復元」(いまは 6.3m で 3.1mm)と、
フォワードとの一致(5% を超える画素)がどう変わるかを見る。
