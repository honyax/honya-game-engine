# Day 52b: 多光源 — 光の形 = 描く形(点光源・光の球・1024 個)

Day 52 の2日目で、reference は Day 52a の完全コピー + 差分。
Day 52a で描き方を2回に分けた。G-Buffer に表面が置いてあり、光は全画面1枚で当てている。
**絵は1ピクセルも変わらず、幾何パスが 3 回から 2 回に減っただけ**で終わっていた。

今日はその配当を取りに行く。光を<b>最大 1024 個</b>に増やす。
フォワードのままでは2つの壁にぶつかる。
...
ディファードにはどちらの壁も無い。表面はもう G-Buffer に置いてあるので、
**光を当てる仕事は「その光が画面をどれだけ覆うか」だけで決まる**。
1024 個を 16 個とほぼ同じ代償で描く。
...
> Day 35〜52a で落ちなかったのは、読み戻す前に別の VAO(文字のバッチ)が結び付いていたから。
> <b>どの VAO が結び付いているかで結果が変わる</b>、GL の状態機械らしい壊れ方だった。
> Day 35〜52a の `Mesh.cs` まで遡って直してあるので、今日の差分には出てこない(検証の途中で分かったこと 1)。

## 今日のゴール

**「ディファード」ページの `F3` で点光源を 16 → 64 → 256 → 1024 個に増やし、
`F4` で夜にすると、裏通りの床に色とりどりの光だまりが並ぶ。
`F2` でフォワードに戻すと、64 個を超えたぶんの光だまりが<b>目に見えて消える</b>。**

| キー(「ディファード」ページ) | 何が起きるか |
|---|---|
| `F2` | ディファード ON/OFF(Day 52a)。**今日は絵が変わる**——フォワードは 64 個で打ち切る |
| `F3` | 点光源の数(0 / 16 / 64 / 256 / 1024)。**球が塗る量は揃えたまま**。**今日の到達点** |
| `F4` | 夜にする(太陽と空を 3% に落とす)。点光源が主役になる |
| `F5` | G-Buffer を見る(Day 52a。席が1つ後ろへずれた) |
...
| `F10` | 今日の自己チェック(31 項目。Day 52a の 14 項目を含む) |

**メニューの席が5つ増えるので、Day 52a の `F3`〜`F5` が `F5`・`F8`・`F10` へずれる**。
点光源まわりを「ディファード」と「G-Buffer を見る」の間に差し込んだため。

### 今日いちばん大事な2つ

**1つ目は「光の形 = 描く形」**。Day 52a の太陽はどこにでも当たるので全画面の三角形1枚だった。
...
球が有限でいられるのは、<b>半径ちょうどで 0 に落ちる窓</b>を減衰に掛けてあるから(要点2)。
物理の 1/d² は 0 にならないので、そのままだと「どの画素に効くか」が全画面になり、球を描く意味が無い。

**2つ目は「球は裏面を描く」**(要点3)。
表面を描くと、カメラが球の中に入った瞬間に光が消える。
深度の比べ方も `GEQUAL`(奥なら通す)に変え、深度は書かない——
**ふだんの本描画と GL の状態が3か所違う**のが、この日いちばん手が滑るところ。

### 今日も Render とシェーダが主役

差分は `Render/` に新規1本(`PointLight`)と変更2本(`DeferredLighting` / `Shader`)、
`Demo/` に新規1本(`LightSwarm`)、シェーダが新規1本(`light-volume.vert`)と
`deferred.frag` / `textured.frag` の変更、そして `Program.cs`。
`Framebuffer` も `Ssao` も `GBuffer` も(コメント以外は)1行も変わっていない。

## 事前に読む資料

- [LearnOpenGL: Deferred Shading](https://learnopengl.com/Advanced-Lighting/Deferred-Shading)(**ライトボリュームの節**)
  — Day 52a で前半を読んだ回の続き。最後の "Light volumes" が今日そのもので、
  あちらが「実際にはやらない」と書いて終わっているところを、今日は最後までやる
- [Brian Karis, "Real Shading in Unreal Engine 4"(SIGGRAPH 2013 の講義ノート)](https://blog.selfshadow.com/publications/s2013-shading-course/karis/s2013_pbs_epic_notes_v2.pdf)
  — **要点2の減衰の式**の出どころ。`saturate(1 - (d/r)⁴)² / (d² + 1)`。
  Day 35 の PBR を読んだときと同じ資料の、ライトの章
- [ゲームグラフィックス特論 B-14: 遅延レンダリング](https://tokoik.github.io/gg/)(**後半**)
  — Day 52a の一次資料の続き。なぜ光源の数に強いのかが講義資料にまとまっている
- **Day 52a の計画書 要点1〜5**(このリポジトリ)
  — G-Buffer の中身・位置の復元・深度の写し。今日の球は全部その上に立つ
- **Day 26 の計画書**(このリポジトリ)
  — 「答えを変えずに速くする層」の考え方。視錐台での間引き(`F7`)がまさにそれ
...
完全に弾くにはステンシルを使った2パスにするが、今日は入れていない(要点6・改造課題2)。

### 4. フォワードの壁 — uniform の枠が光の数の上限になる
...
| ステンシルで弾く光の球 | 壁の裏の光も壁の手前の画素を塗る(要点3) | 改造課題2。Forward+ のほうが筋がよい |
| タイル / クラスターで光を選る | 光1個ずつの粗いカリングしかない | **Day 53(Forward+ / クラスタード)** |
| 深度バッファからの位置の復元 | 影の縁で 0.2% の画素が 5% 以上ずれる | Day 52a の改造課題2 |
...
深度を写した(Day 52a の要点4)ので壁の向こうに透けずに済んでいる。
...
| ファイル | 行数 | 役割 |
|---|---|---|
| `Render/PointLight.cs` | 90 | **点光源1つ**。位置・届く距離・色と、減衰の式の CPU 版(要点2) |
| `Demo/LightSwarm.cs` | 215 | **通りを漂う光の群れ**。数を変えても球が塗る量を揃える(要点5) |
...
| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Render/DeferredLighting.cs` | **光の球**(メッシュ1本・外接倍率・視錐台・`DrawPointLights`) | +250 / -10 |
| `Render/Shader.cs` | `SetVector4Array` を追加 | +24 / -0 |
| `shaders/deferred.frag` | 点光源の枝、`PointAttenuation` / `PointLightContribution`、光の重なりの表示 | +79 / -3 |
| `shaders/textured.frag` | **フォワードの点光源ループ**(64 個まで) | +82 / -1 |
| `Render/GBuffer.cs` | コメントのみ(メニューの席が `F3` から `F5` へずれた) | +4 / -5 |
| `shaders/gbuffer-view.frag` | コメントのみ(同上) | +1 / -1 |
| `Program.cs` | 光の群れ、夜、メニュー 5 項目、HUD、自己チェック 17 項目、計測 | +502 / -42 |
| `Day52b.csproj` | `Day52a.csproj` からのリネームのみ | — |

**`Framebuffer` も `Ssao` も1行も変わっていない**。
Day 52a で作った G-Buffer の形は、光が 1024 個になっても何も変える必要が無い——
<b>「表面を置く場所」と「光を当てる回数」が切れている</b>ことの、いちばん分かりやすい証拠になる。

### 写経する順番

```
1. Render/Shader.cs              SetVector4Array(7 の ApplyForwardLights が使う)
2. Render/PointLight.cs          点光源(新規。3・6・7 が使う)
3. Demo/LightSwarm.cs            光の群れ(新規。2 を使う)
4. shaders/light-volume.vert     光の球(新規。6 が読み込む)
5. shaders/deferred.frag         点光源の枝(変更。6 が読み込む)
6. Render/DeferredLighting.cs    光の球を描く(変更。2・4・5 を使う)
7. shaders/textured.frag         フォワードの点光源ループ(変更)
8. Program.cs                    全部を繋ぐ
9. Day52b.csproj                 Day52a.csproj からのリネームのみ
```

**5 と 7 は同じ関数を2つ持つ**(`PointAttenuation` と `PointLightContribution`)ので、
並べて写すと見比べやすい。文字まで揃っているかは自己チェックが見張っている。

`Program.cs` の中は次の順で読むとよい。

```
  フィールド(_swarm / _night / ForwardLightPositions / ForwardLightColors)
    → OnUpdate の _swarm.Update                            光を漂わせる
    → Render3D の ApplyForwardLights                       フォワードの点光源
    → ApplyDemoLighting / UnloadDemoScene                  夜
    → CurrentLights / RenderDeferredLighting の引数         ディファードの点光源
    → DeferredLabel / DescribeDeferred / BenchmarkDeferred
    → RunDeferredCheck の節1・3と、節4の 65 個の塊
    → BuildDebugMenu の「ディファード」ページ(5 項目を差し込む)と RunDebugMenuCheck の 199
```

**メニューの5項目は「ディファード」と「G-Buffer を見る」の間に差し込む**。
順番を変えると席(`F` キー)がずれて、コード中の「メニューの F8」といったコメントが全部嘘になる。

## 設計書

**層は今日も増えていない**。`Render/` に型が1つ(`PointLight`)、`Demo/` に型が1つ(`LightSwarm`)
増えて、`Render/DeferredLighting` の口が広がっただけ。
...
| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Render/DeferredLighting` | 光の球(メッシュ1本・外接倍率・視錐台の判定) | +250 / -10 |
| `Render/Shader` | `SetVector4Array` | +24 / -0 |
| `shaders/deferred.frag` | 点光源の枝と減衰 | +79 / -3 |
| `shaders/textured.frag` | フォワードの点光源ループ | +82 / -1 |
| `Program.cs` | 光の群れ、夜、メニュー、自己チェック、計測 | +502 / -42 |
...
**今日増えたのは `LightSwarm` から出る2本**(ディファードの球へ、フォワードのループへ)。
残りの線は Day 52a で引いたものがそのまま生きている。

**矢印の出どころが `Render3D` と `ApplyLighting` の2つに集まっている**のが Day 52a からの姿になる。
何を描くか(`Render3D`)と、どう照らすか(`ApplyLighting`)を<b>フォワードとディファードで共有</b>していて、
違うのは「表面を決めたあと、その場で光を当てるか、G-Buffer に置いて後で当てるか」だけ。

点線の1本(`ApplyLighting` → `DeferredLighting`)は Day 51 の `FollowCamera` と同じ形。
`DeferredLighting` は太陽の向きも IBL も知らず、<b>`Action<Shader>` 1本</b>で受け取る。
2か所に書き写すと、片方だけ直したときに2つの絵が食い違う——
しかも両方ともそれらしく見えるので、見比べるまで気づけない。

**`Ssao` から `GBuffer` への矢印は無い**。借りるのは `Texture` 1枚で、
`Ssao` は自分が G-Buffer を読んでいることを知らない。
Day 37 の幾何バッファと同じ形にしておいたことが、そのまま「知らずに済む」になっている。

### 1フレームが決まるまで — フォワードとディファードの分かれ道
...
**Day 52a から変わったのは、右の道の最後に光の球が1つ増えたこと**だけ(`PT`)。
道の形そのものは昨日のまま。

**ジオメトリを描く箱が、左の道には3つ、右の道には2つ**ある
(影・SSAO の幾何・本描画 と、影・G-Buffer)。これが HUD の「幾何パス」の数字になる。
光の球はジオメトリだが、G-Buffer に書かないので「幾何パス」には数えない。

**右の道は `Render3D` を呼ぶ場所が前に移っただけ**で、中身は1行も変わっていない。
その代わり、左の道の `Render3D` があった位置には<b>三角形を1つも描かない</b>パスが2本並ぶ。

**`RenderParticles` が両方の道の合流点に居る**のが大事なところ。
半透明は G-Buffer に入れられない(1画素に表面を1つしか持てない)ので、
どちらの道でも最後にフォワードで重ねる。深度を写してある(Day 52a の要点4)ので、
...
```mermaid
flowchart TD
    UV["ParallaxUv<br/>視差で UV をずらす(Day 34)"] --> BASE["ベースカラー × 頂点色 × Tint"]
    BASE --> NM["PerturbNormal<br/>法線マップを世界へ(Day 34)"]
    NM --> MR["金属度・粗さ + 上書き(Day 35)"]
    MR --> AOE["焼いた AO / 発光<br/>**発光を SSAO より前へ上げた**"]
    AOE --> CUT{"uGBufferPass == 1 ?"}
...
**この図の切れ目は Day 52a で入れたもの**。今日足したのは、後半のいちばん下に並ぶ
<b>点光源のループ</b>(フォワード)と、`deferred.frag` 側の球1個ぶんの計算になる。

**切る場所は SSAO の手前でなければならない**。SSAO はディファードのとき G-Buffer から作るので、
G-Buffer パスの時点ではまだ出来ていない。発光を SSAO の上へ移したのは、
出口で書くものが全部、切れ目より前に揃っている必要があるから。
...
**1つ目: 同じ式が2本のシェーダにもう2組増えた**。
Day 52a で `CookTorrance` と `ShadowFactor` を `textured.frag` と `deferred.frag` に
書き写したのに加えて、今日は `PointAttenuation` と `PointLightContribution` が
<b>そっくり同じ形で2本に載った</b>。

| 場所 | 役割 |
|---|---|
| `Pbr.cs` | BRDF の定義と検算(自己チェックだけが呼ぶ) |
| `PointLight.Attenuation` | 減衰の CPU 版。**検算用**(窓の形・単調性・半径で 0) |
| `textured.frag` | フォワード。全部の光を1画素の中でループする |
| `deferred.frag` | ディファード。球1個につき1回 |

GLSL には `#include` が無いので、書き写すしかなかった。
自己チェックは<b>文字列が揃っているか</b>(`MIN_ROUGHNESS`、窓の式、`MAX_FORWARD_LIGHTS`、出口の番号)と、
...
**2つ目: 光1個 = ドローコール1回**。1024 個で 940 回(画面外の 84 個は落とす)で、
1回ごとに uniform を3つ送り直している。
球はどれも同じメッシュなので、位置と色を頂点バッファに並べて1回で描ける(インスタンシング)。
それが改造課題3。

**3つ目: 壁の裏の光が、壁の手前の画素を塗る**(要点3)。
深度の比べ方を「奥なら通す」にしたので、球の裏面が G-Buffer の面より奥にあれば必ずシェーダが走る。
壁1枚を挟んだ向こうの光でも、球が壁の手前の画素を覆っていれば代償を払う——
シェーダの中で距離を測って 0 を足すだけになるが、その画素ぶんは確かに払っている。
「光の重なり」(`F6`)で壁の上のほうが焼けているのは全部これ。
完全に弾くにはステンシルを使った2パスにする(改造課題2)。

**Day 52a の歪みは3つともそのまま残っている**。
影のパス(`RenderShadowPass`)は今日も手書きの分岐、位置の精度は 16F のまま(Day 52a の要点2)、
材質グリッドと `EnvironmentMap` の生 FBO も手付かず。
どれも「光の設定をパスごとに持つ形」へ整理する Day 56 の仕事に回してある。

Day 52a の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。
変わった図は次の6つ。

| 図 | 何が変わったか |
|---|---|
| `全体構成` | `Demo/` → `Render/` の矢印に `PointLight` が乗った(層も矢印も増えていない) |
| `Render` のクラス図 | **`PointLight` が増えた**。`DeferredLighting` が球と視錐台を持った |
| `Demo` のクラス図 | `LightSwarm` が増えた |
| `今日足したものが繋がった場所` | `LightSwarm` から2本(ディファードの球と、フォワードのループ) |
| `1フレームの流れ` | 更新の段に `_swarm.Update`、ライティングパスの箱に**光の球**が並んだ |
| `1画素の色が決まるまで` | 直接光に**点光源のループ**が足された |
...
写してある(Day 52a の要点4)ので、粒は壁の向こうに透けない。

**デモ以外の絵(材質グリッド・モデル単体)とゲームモードは今日もフォワード**。
`UseDeferred` がデモ v1 のときだけ true を返す(Day 52a の設計書「今日残した歪み」の4つ目)。
...
`dotnet run --project reference/Day52b -c Release` で起動する。
先に「デモ v1」ページ(8 枚目)の `1`(決めの構図)でデモ v1 を出し、
`F1` を 20 回押すか `Ctrl+F1` の目次から「ディファード」ページ(20 枚目)を開く。
`F2` でディファードに切り替えてから下を試す。

### 1. `F3` と `F4`: 光を増やして、夜にする
...
**ここが今日の到達点**。Day 52a では数字にしか出なかった組み替えの配当が、
はじめて絵に出る。

### 2. フォワードの壁: 64 個を超えると光が消える

256 個か 1024 個のまま `F2` でフォワードに戻すと、HUD が
`**64個で打ち切り**` になり、<b>床の光だまりが目に見えて減る</b>。
64 個以下なら、ON/OFF で絵はほぼ同じ(夜・64 個で全画素の 0.07% が 5% 以上ずれる程度)。

**Day 52a では `F2` で絵が1ピクセルも変わらなかった**。
今日はじめて「描き方を変えると絵が変わる」状態になるが、
変わるのは<b>フォワードが描けていないぶん</b>だけで、ディファードの側は何も足していない。

### 3. `F6`: 光の重なり

画面が暗くなり、光の球が塗った画素だけが橙に焼ける。
**光の数を 16 ↔ 1024 で切り替えても、焼け方の総量がほぼ変わらない**——
大きな球 16 個と小さな球 1024 個で、塗る量を揃えてある(要点5)。
壁の上のほうにも焼けが出るのは、<b>壁の手前の画素を、壁の裏の球が塗っている</b>から(要点3)。
...
Day 52a の4枚の表の下に、点光源の段が増える。

```
  点光源: 1024 個  届く距離 0.50m  強さ 8.0(16 個のとき。数に合わせて半径に比例して弱める)
  光の球: 三角形 384 枚 x 倍率 1.028(外接させるため)  描いた 940 / 間引いた 84
  フォワードの上限: 64 個(uniform の枠)  **いまフォワードで描くと 960 個が消える**
```

**倍率 1.028** が要点2の最後に書いた「分割した球は面が内側へ入り込む」の手当て。
自己チェックが、いちばん内側に入り込む面でも届く距離の 1.0003 倍の外にあることを見ている。
...
Day 52a の 14 項目に 17 項目足して 31 項目。
**窓を開かずに走る項目が 11 ある**(減衰・群れ・視錐台)。
`PointLight` と `LightSwarm` が GL を知らないおかげで、数字だけで確かめられる。

### 8. Day 52a の絵と機能が1つも壊れていない

- **`F2`(光 0 個)で絵が1ピクセルも変わらない**。Day 52a の到達点はそのまま生きている
- **`F5`: G-Buffer を見る**。席が `F3` から `F5` へずれただけで、中身は同じ
- **プレイアブルデモもディファードで動く**。「プレイアブルデモ」ページ(19 枚目)の `F2` でキツネを出すと、
  ディファードのまま歩き回れる。夜にすると<b>キツネが光だまりを通るたびに照らされる</b>
- **ウィンドウを広げても G-Buffer が付いてくる**(`OnFramebufferResize` の `_gbuffer.Resize`)
- Day 48 の自己チェック(`Alt+F1`)は **199 項目**で通る(Day 52a の 194 に今日の5項目)

**既定はフォワード・光 0 個**なので、起動した直後の絵は Day 52a とまったく同じになる。

**IBL の自己チェック(Day 36)はデモ v1 を出す前に走らせる**こと。
あのチェックは手焼きの空(`SkyImage`)と焼いたキューブを比べるので、
HDRI の空を焼いたあとでは4項目が落ちる。これは Day 51 までと同じ振る舞いで、今日の変更とは関係ない。
...
### 課題2(中): ステンシルで壁の裏の光を弾く

いまの深度テストは「奥なら通す」だけなので、<b>壁の裏の光も壁の手前の画素を塗る</b>
(今日残した歪みの3つ目)。正しく弾くには、球1個につき2パスに分ける。

```
  1パス目  深度だけ・カラー書かない・両面
           表面が深度より奥  → ステンシルを +1(Z-fail)
           裏面が深度より手前 → ステンシルを -1
  2パス目  ステンシルが 0 でない画素だけ塗る
```

**「球の内側に面がある画素」だけが残る**という、影のボリューム(Stencil Shadow Volume)と
まったく同じ手口になる。`glStencilOpSeparate` で表裏それぞれの動きを決める。

代償は<b>ドローコールが倍になる</b>こと。「光の重なり」(`F6`)の焼けがどれだけ減るか、
計測(`F9`)の ms がどう動くかを並べて、<b>弾いた得と2パスの損のどちらが大きいか</b>を見る。
今日の規模(半径 0.5〜4m の球)ではたいてい割に合わず、
本番のエンジンがタイル / クラスター(Day 53)へ行った理由がここで腑に落ちる。
...
=== Day 52 自己チェック(G-Buffer・位置の復元・光の球・フォワードとの一致)===
  ...
  [OK] G-Buffer: 法線がビュー空間の床の法線と一致(2枚目の RGB)  内積 1.0000
  [OK] G-Buffer: AO・粗さ・金属度が材質どおり(**ORM の並び**)  R 1.000 / G 0.698 / B 0.000
  [OK] G-Buffer: ベースカラーは平方根で入り、2乗すると材質の色に戻る  入っている値 0.549(√0.30 = 0.548)→ 2乗 0.301
  [OK] G-Buffer: 空は距離 0(何も描かれていない印)  0.000
...
```
  Day 35〜52a  ReadIndices の直前に結び付いていた VAO = 文字のバッチ(フレームの最後に描く)
  Day 52b      ReadIndices の直前に結び付いていた VAO = 光の球(自己チェックが直前に描いた)
```

**Day 35〜52a で落ちなかったのは、たまたま**だった。
...
**Day 35〜52a の `Mesh.cs` まで遡って同じ直しを入れてある**。だから今日の差分
(`git diff --no-index Day52a Day52b`)には `Mesh.cs` が出てこない。
...
高さを半径に合わせて下げ、強さを半径の1乗で弱めると、どの数でも床に同じくらいの光が降るようになった(要点5)。
「光の重なり」(`F6`)を出して 16 個と 1024 個を並べたのが決め手になった。

**3. 点光源を足してもフォワードとの差は増えなかった**。Day 52a の時点で、
差は<b>影の縁の数画素だけ</b>(位置を 16F の距離から戻しているため。Day 52a の要点2)だった。

```
  夕暮れ・光 0    平均 0.34%   5% を超える画素 0.24%
  夜・光 64       平均 0.31%   5% を超える画素 0.07%
```

**光を 64 個足しても増えない**のが大事なところ。減衰も BRDF も2本のシェーダで
同じ式を書いたので、点光源のぶんはぴったり一致している——
差はもとから影の縁にしか無く、そこはフォワードもディファードも同じ影マップを引いている。
文字列が揃っているかを見る自己チェック(節5)が、この一致を支えている。

**4. フォワードの代償は「届く光」にも比例する**。計測(`F9`)で、
