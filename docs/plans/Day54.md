...
```
  FXAA 52%  /  TAA 22%(疑わなければ 23%)  /  ずらしの 8 点をただ平均 18%
```
...
graph TD
    ...
    DM -->|"Mesh / Material / Primitives / PointLight(Day 52)"| R
...
| 収束(64 点の答えとの縁の誤差。AA なし 100%) | FXAA 52% / **TAA 22%** / 8 点をただ平均 18% |
...
=== Day 54 自己チェック(ずらし・解決の式・速度・スーパーサンプリングへの収束)===
  ...
  [OK] **TAA はずらしの 8 点の平均まで近づく。FXAA より近い**(64 点の答えとの、縁の画素の平均誤差。AA なしを 100%)  縁 2678 画素  FXAA 52% / TAA 22%(疑わなければ 23%)/ 8 点をただ平均 18%
...
**3. 16 点にしても収束が良くならなかった**。自己チェックの収束の数字(TAA 22%)の内訳を見るため、
