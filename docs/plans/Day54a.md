# Day 54a: TAA(1画素未満ずらして描き、前のフレームと混ぜる)
...
| | 何をするか |
|---|---|
| **Day 54a(今日)** | **TAA の骨格**。ずらして描き、カメラの動きで履歴を引き戻し、近所の色で疑って混ぜる |
| Day 54b | **動く物のための3つ**。物の速度・Catmull-Rom での履歴の読み・明るさの重み |

ロードマップの題は「FXAA からの置き換え」。だから今日は、同じ AA という仕事を
**1枚の絵の中で解くか(FXAA)、時間の方向に解くか(TAA)** で並べ、何が変わって何を払ったかを見る。

| | Day 38 FXAA | Day 54a TAA |
|---|---|---|
...
| 払うもの | 全画面 1 パス、8bit 1枚(2.3MB) | 全画面 1 パス + 速度 1 パス、**RGBA16F 2枚(9.4MB)+ 速度(2.3MB)** |
| 壊れ方 | 細い線が滲む | **カメラが追いかけている物の輪郭が揺れる**(Day 54b で直す)、止めても 8 フレームの周期でわずかに揺れる |
...
> 箱に今の画素を必ず入れる1行で、差は 0.015% に消えた(要点5、検証の途中で分かったこと 2)。

## 今日のゴール

**メニューの「TAA」ページで `F2` を押すと、アンチエイリアスが FXAA → TAA → なし と回る。
止めた絵では TAA の輪郭が FXAA より滑らかになり、カメラワークを再生しても前の絵が尾を引かない。
`F5` で履歴を疑うのをやめると、カメラが回り込むたびに物の縁から尾が伸びる。**
...
| `F6` | ジッター(ON / OFF)。**OFF にすると AA が効かない** |
| `F7` | 内訳(揺らし方・履歴・速度バッファ・GPU の時間) |
| `F8` | 今日の自己チェック(20 項目) |
...
```csharp
// Camera.ProjectionMatrix(Day 54a)。ずらしは投影の「後ろ」に掛ける
...
**2つ目は「前にどこに居たかを知る」**。カメラが動くので、履歴を同じ画素の場所から読むと別の物の色を拾う。
読むべきは「この画素に今写っている点が、前のフレームで写っていた場所」で、その差を<b>速度バッファ</b>(RG16F)に書く。

```
  深度 → 世界の点 → 前のフレームの行列で写し直す      … 全画面1枚。**止まっている物は全部これで正しい**
```

深度に入っているのは「今どこにあるか」だけなので、<b>自分で動いた物の「前の位置」はこの1段では出てこない</b>。
それを直す2段目は Day 54b。

**3つ目は「履歴を疑う」**。カメラが回り込んで、それまで隠れていた場所が見えるようになると、そこの履歴は隠していた物の色になっている。
そのまま混ぜると、物の後ろに影(ゴースト)が尾を引く。そこで**今の絵の近所 3x3 の色の範囲**を「この画素がとりうる色」とみなし、
履歴がそこから外れていたら範囲まで引き戻す(要点5)。
...
差分は `Render/` に新規3本(`TemporalAA` / `MotionVectors` / `GpuTimer`)、シェーダ2本
(`taa.frag` / `camera-velocity.frag`)、`Camera` のずらし、`PostProcess` の入口、`Texture` の RG16F、そして `Program.cs`。
...
- [Brian Karis, "High Quality Temporal Supersampling"(SIGGRAPH 2014, Advances in Real-Time Rendering)](https://advances.realtimerendering.com/s2014/index.html)
  — **今日の一次資料**。UE4 の TAA。ずらし(Halton)、近所の色の箱を YCoCg で作ること(要点5)。
  スライドはこのページの一覧から取れる
- [Lasse Jon Fuglsang Pedersen, "Temporal Reprojection Anti-Aliasing in INSIDE"(GDC 2016)](https://github.com/playdeadgames/temporal)
  ...
  箱の中心へ向かって切る `clip_aabb` と、近所でいちばん手前の画素の速度を使う工夫(要点4・5)がそのまま読める
...
```
  FXAA 60%  /  TAA 21%(疑わなければ 22%)  /  ずらしの 8 点をただ平均 20%
```

(確認用のシーン(床と立方体)は、自己チェックが決めた光——起動直後の手書きの光、IBL・影・SSAO なし——で照らして測る。
Day 56 で直すまでは画面に出ている光を借りていて、押したときの光で数字と合否が変わっていた。検証の途中で分かったこと 6)

**TAA が近づけるのは「ずらしの 8 点の平均」まで**。残りは点が 8 つしか無いことと、
指数移動平均の重みの偏り(直近ほど重い)。
...
**描くのは全画面1枚**(`camera-velocity.frag`)。止まっている物の画素は、その点が前のフレームでどこに写っていたかが
カメラの動きだけで決まる。深度から世界の点を戻し(Day 52a の位置の復元と同じ発想)、前のフレームの行列で写し直す。
...
**自分で動いた物は、この1段では出てこない**。深度には「今どこにあるか」しか入っていないので、
キャラクターのように自分で動いた物の「前の位置」はどこにも書かれていない。
その画素は「止まっている物」として扱われ、背景と同じ速度で履歴を引く——
疑い(要点5)が色を引き戻すので残像は出ないが、履歴が溜まらないので輪郭が階段に戻って揺れる。
**前のモデル行列と前の関節行列で描き直す2段目は Day 54b**。
...
### 5. 疑う — ゴーストの正体と、近所の色の箱

**ゴーストは「隠れていた場所が見えるようになった」ときに出る**。カメラワークがゴミ箱の横を回り込むと、
それまでゴミ箱に隠れていた壁が毎フレーム現れる。その壁の画素が前のフレームの自分の場所を引くと、
そこはゴミ箱だった——履歴がゴミ箱の色になっている。
疑わずに混ぜると、そのゴミ箱の色が壁と一緒に流れ、<b>物の縁から尾が伸びる</b>(`F5` で「疑わない」にすると見える)。
...
**その手当て(混ぜる前に明るさで畳む)は Day 54b**。今日は HDR のまま混ぜる。

### 7. 繋がっていないフレーム — カットと、速度を挿したときだけ掛かる約束

**前のフレームと絵が繋がっていなければ、履歴を捨てる**。疑いがあるので捨てなくても数フレームで消えるが、その数フレームは前の絵が薄く重なる。
`MotionVectors` が次の2つを見て「カット」と判定し、`Program` が `TemporalAA.Reset` を呼ぶ。
...
**「絵が丸ごと切り替わった」を見る3つ目の条件は Day 54b**(描いたものの記録が要る)。
今日はデモ v1 から材質グリッドへ切り替えると、数フレームだけ前の絵が薄く重なる。
...
|---|---|---|
| **自分で動いた物の速度** | 深度には「今どこにあるか」しか無いので、動く物の履歴は背景の速度で引かれる。疑いが色を引き戻すので残像は出ないが、**履歴が溜まらず輪郭が揺れる** | **Day 54b** |
| **履歴を Catmull-Rom で読む** | 双線形で読むので、カメラが動いているあいだ<b>ぼけが積もって絵が眠くなる</b> | **Day 54b** |
| **明るさで重み** | HDR のまま混ぜるので、50 を超える明るい点がジッターで見え隠れすると、まわりごと白くちらつく | **Day 54b** |
| 鮮鋭化(シャープネス) | TAA は FXAA よりわずかに柔らかい(1画素の中を平均するので、テクスチャの細部も平均される) | Day 54b の改造課題 |
...
**今日の TAA は「カメラだけが動く絵」には完全に効く**。決めの構図もカメラワークも、
止めた絵も、これで FXAA よりずっと滑らかになる。
崩れるのは<b>カメラが物を追いかけたとき</b>——画面の上で止まって見える物の画素が「背景と同じだけ動いた」とみなされる。
それが Day 54b の出発点になる。
...
| `Render/TemporalAA.cs` | 417 | **今日の主役**。揺らし方(Halton)・履歴の2枚・解決パス・つまみ。後処理が持つ |
| `Render/MotionVectors.cs` | 234 | **速度バッファ**。カメラの動きから、全画面1枚で速度を描く |
| `shaders/taa.frag` | 227 | 解決のシェーダ。近所の箱・引き戻し・疑い・表示 |
| `shaders/camera-velocity.frag` | 52 | 速度(深度から、カメラの動きを全画面に) |
...
| `Program.cs` | ずらしの下ごしらえ、速度のパス、メニュー 7 項目、HUD 1行、内訳・自己チェック 20 項目 | +1141 / -2 |
| `Day54a.csproj` | `Day53.csproj` からのリネームのみ | — |

**`Program.cs` の差分の7割は自己チェック**(`RunTaaCheck` と道具)。
TAA の本体は `BeginTemporalFrame` と `RenderMotionVectors` の2つで、合わせて 30 行ほど——
**TAA も何を描くかを1行も書き換えない**。ずらしはカメラに、解決は後処理に入ったので、
...
| メニューの自己チェックの件数 207 → 214 | `RunDebugMenuCheck` | 「TAA」ページの 7 項目 |

### 写経する順番

```
1. Render/Texture.cs              RenderTargetFormat.Rg16F(7 が使う)
2. Render/Framebuffer.cs          Rg16F のバイト数(1 の続き)
3. Render/Camera.cs               ずらし(Jitter)と、ずらす前の行列
4. Render/GpuTimer.cs             GPU に測らせるストップウォッチ(新規。5・6・7 が使う)
5. shaders/taa.frag               解決のシェーダ(新規。6 が読む)
6. Render/TemporalAA.cs           揺らし方・履歴・解決(新規。4 を使う)
7. Render/PostProcess.cs          TAA を入口に置く(4・6 を使う)
8. shaders/camera-velocity.frag   速度(新規。9 が読む)
9. Render/MotionVectors.cs        速度バッファ(新規。1・3・4 を使う)
10. Program.cs                    全部を繋ぐ
11. Day54a.csproj                 Day53.csproj からのリネームのみ
```

**依存順に並べてある**。C# のビルドが通るのは 1〜4・6・7・9・10 の順で、シェーダ(5・8)は実行時に読むのでビルドには効かない——
ただし<b>10 より前に写しておく</b>こと。シェーダは起動時に読む(`TemporalAA` と `MotionVectors` のコンストラクタ)ので、
写し忘れると TAA を入れる前、起動の時点で落ちる。

`TemporalAA.cs` は半分がコメント。`NextJitter`(揺らす)→ `Resolve`(混ぜる)→ `Reset`(捨てる)の順に読む。
`MotionVectors` は `BeginFrame` → `Render` → `RenderCameraMotion`。
...
```
  フィールド(_motion / CurrentWeightSteps)
    ...
    → RunTaaCheck と道具(RenderTaaProbe / EdgeError / ReadTexture / Upload… / TaaSettings)
    → BuildDebugMenu の末尾(メニュー 1 ページ)と RunDebugMenuCheck の 214
```

## 設計書

**層は今日も増えていない**。`Render/` に型が5つ(`TemporalAA` / `MotionVectors` / `GpuTimer` と列挙2つ)増えて、
`Camera` と `PostProcess` の口が広がっただけ。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Render/TemporalAA` | `Render/` | 揺らし方・履歴の2枚・解決。**`Camera` も速度の出どころも知らない**(テクスチャを受け取って返すだけ) |
| `Render/MotionVectors` | `Render/` | 速度バッファ。カメラの動きを全画面1枚で描く。**`TemporalAA` も `PostProcess` も知らない** |
| `Render/GpuTimer` | `Render/` | タイマークエリの輪。**GL しか知らない** |
| `Render/TaaRectify` ほか1つ | `Render/` | 疑い方・表示の列挙 |
...
| `Program.cs` | 頭と尻の2か所、メニュー、自己チェック | +1141 / -2 |

### FXAA と TAA の設計比較 — 出口と入口

```mermaid
flowchart TD
    SC["シーンバッファ RGBA16F<br/>ずらして描いてある(Day 54a)"] --> TQ{"TAA を掛けるか<br/>このフレームの速度が挿さっているか"}
    TQ -->|"Yes"| TR["TAA の解決 taa.frag<br/>引き戻す → 疑う → 混ぜる"]
    HI["履歴 x2 RGBA16F<br/>前のフレームの結果"] --> TR
    VB["速度 RG16F<br/>カメラの動き"] --> TR
    TR -.->|"書いたものが次の履歴"| HI
    TR --> IN["後ろの段が読む絵"]
    TQ -->|"No"| IN
    IN --> BL["明部 → ぼかし(Day 31)"]
...
| | FXAA(Day 38) | TAA(Day 54a) |
|---|---|---|
| 置き場所 | 出口(合成のあと) | 入口(ブルームより前) |
| 読むもの | 8bit の絵1枚 | 今の絵・履歴・速度・深度 |
| 持つバッファ | LDR 1枚 2.3MB | 履歴 2枚 9.4MB(表示を使うと +4.7MB)+ 速度 2.3MB |
| 全画面パス | 1 | 1(解決)+ 1(速度) |
| シーンを知っているか | 知らない | **知らない**(速度はテクスチャで受け取る) |
| 前のフレームを覚えているか | いない | 履歴と、前の行列(速度の側) |
| GPU の時間(VSync で休みながら) | 0.04ms | 速度 0.02ms + 解決 0.05ms |
...
```mermaid
flowchart TD
    BT["BeginTemporalFrame<br/>ずらしを決めて Camera.Jitter へ"]
    BT --> PS["影 → G-Buffer → SSAO → 本描画 → 粒<br/><b>全部が同じずらしで描かれる</b>(影は光の目なのでずれない)"]
    PS --> MV["RenderMotionVectors<br/>深度を写す → カメラの速度を全画面に"]
...
flowchart LR
    ...
    CAM -- "ずらした VP と ずらす前の VP" --> MV["MotionVectors<br/>深度から、カメラの動きを描く"]
...
この形にしてあるので、Day 54b で速度に2段目が増えても `TemporalAA` は1行も変わらないし、
**Day 55 のモーションブラーは、この速度バッファをそのまま読める**。
...
自己チェックが 64x8 の作り物の絵で解決の式を確かめられる(要点3・5の数字)のはこのおかげ。
...
flowchart TD
    ...
    HV -->|"ある"| NB["近所 3x3 を見る<br/>YCoCg へ<br/>和・二乗和・最小・最大 + いちばん手前の画素"]
    NB --> V["速度 = いちばん手前の画素の速度"]
    V --> HU["履歴の場所 = 今の UV − 速度"]
    HU --> OFF{"画面の外か"}
    OFF -->|"外"| OUTC
    OFF -->|"中"| RH["履歴を双線形で読む<br/>(読むたびに少しぼける)"]
    RH --> RQ{"疑い方"}
    ...
    VC --> MIX
    NR --> MIX
    MIX["今の絵 1 割 : 履歴 9 割"] --> OUT["出力 = 次の履歴<br/>ブルームと合成が読む"]
```
...
**2つ目: 深度を2回写している**。ソフトパーティクル(Day 50)のために粒の前で1回、速度のために `_post.End` の前でもう1回。
粒は深度を書かないので中身は同じ。「フレームに1回、不透明を描き終えたところで写す」持ち主を1つ決めれば減るが、
Day 50 のコードの置き場所を動かすことになるので今日は見送った。

**3つ目: 速度を持たないものがある**。粒・トレイル・2D のスプライト・後処理を通す文字(「FXAA と色調整」の F5)、
そして<b>自分で動いた物すべて</b>。どれも後ろにある物の速度で履歴を引く。
疑いが色を引き戻すので残像は出ないが、履歴が溜まらずに揺れる。
自分で動いた物は Day 54b で直す。粒・文字は本物でも「反応マスク」で粒の画素だけ今の絵の割合を上げるか、TAA のあとに描く。

**4つ目: 履歴を双線形で読んでいる**(要点3)。前に居た場所は画素の中心から外れているので、読むたびに少しぼける。
止めた絵では中心をちょうど読むので差が無く、<b>カメラが動いているあいだだけ絵が眠くなる</b>。
Day 54b の Catmull-Rom で直す。
...
| 図 | 何が変わったか |
|---|---|
| `Render` のクラス図 | **`TemporalAA` / `MotionVectors` / `GpuTimer` と列挙2つが増えた**。`Camera` に `Jitter`、`PostProcess` に `Taa` と `Velocity`、`RenderTargetFormat` に `Rg16F` |
...
新しく足した図が4つ(この節の上に並べたもの)。Day 53 の設計書の冒頭にあった4つの図
...
**Day 54a でも層は増えていない**。増えたのは `Render/` の中の型5つで、矢印は1本も足していない——
`MotionVectors` が `Camera` を読むのも、`PostProcess` が `TemporalAA` を持つのも、全部 `Render/` の中の話。
...
### Render — OpenGL の薄い皮(Day 54a で `TemporalAA` と `MotionVectors` と `GpuTimer` が増えた)
...
**`MotionVectors` は6つ目の「自分でバッファを持ち、シェーダは借りる」クラス**
(`PostProcess` / `ShadowMap` / `EnvironmentMap` / `Ssao` / `GBuffer` に続く6つ目)。
ただし今日は<b>外が何も渡さない</b>——深度のコピーとカメラを受け取って、全画面に1枚描くだけ。
Day 54b で「本描画が描いたものを覚えて、動いたものだけ描き直す」2段目が入ると、
**ここまでの5つと違って「何を描くかを外が決めない」クラス**になる。
...
classDiagram
    ...
    MotionVectors *-- Framebuffer : RG16F 1枚
    MotionVectors *-- GpuTimer : 速度のパスを測る
...
**Day 54a でも `Demo/` は1行も変わっていない**。メニューのページが1枚増えた(22 枚)が、ページを組むのは `Program.BuildDebugMenu` の側。
...
flowchart TD
    ...
    GA --> BTF["BeginTemporalFrame()(Day 54a)<br/>TAA ならずらしを決めて Camera.Jitter へ<br/>描いたものの記録を始める"]
    ...
    TX1 --> RMV
    UQ -->|"通さない(既定)"| RMV["RenderMotionVectors()(Day 54a)<br/>TAA なら深度を写して速度を2段で描く<br/>カットなら履歴を捨てる → _post.Velocity へ"]
    ...
    TX2 --> JZ["_camera.Jitter = 0(Day 54a)<br/>ずらすのは描いている間だけ"]
```
...
### HDR パイプラインの中身 — 11 回のフルスクリーンパス(Day 54a で入口に TAA が入った)
...
| history A / B(Day 54a) | 画面と同じ | RGBA16F | なし | 1割ずつ混ぜる差が 8bit の刻みに負けないように。**TAA を入れたときに初めて作る** |
| 速度(Day 54a。`MotionVectors`) | 画面と同じ | **RG16F** | なし | 符号付きの2成分。深度テストを使わないので深度も要らない |
...
`dotnet run --project reference/Day54a -c Release` で起動する。
...
```
TAA:ON  揺らし:1/8(+0.00,-0.17)px  今の絵:10%  疑い方:分散で切る  速度:カメラだけ  GPU 速度:0.01ms 解決:0.05ms  捨てた:0回
```
...
階段の縁だけでなく、テクスチャの細部もわずかに平均される。レンガの模様が FXAA より少し柔らかく見えるのはそのため。

### 3. `F3`: TAA の表示

| 表示 | 何が見えるか |
|---|---|
| 揺れの素 | 履歴を混ぜない今の絵。**止めていても輪郭が震える** |
...
### 5. `F5`: 履歴を疑うのをやめるとゴーストが出る

「カメラワークと機能」ページ(9 枚目)の `F3` でカメラワークを再生し、「TAA」ページに戻って `F5` を2回押す
(`疑わない`)。カメラが回り込むと、**ゴミ箱や街灯の縁から、隠れていた所の色が尾を引く**(要点5)。
もう一度 `F5` で `分散で切る` に戻すと、尾がぴたりと消える。

`F5` の3つは自己チェックが数字で見ている(赤 4.0 の履歴に灰 0.1 の今の絵 → 疑わない 3.609 / 最小最大 0.100 / 分散 0.100)。

### 6. `F6`: ジッターを切る

`F6` で `ジッター: **OFF**` にすると、**TAA を掛けたまま、絵が「なし」とほとんど同じになる**(差 0.015%)。
同じ画素が毎フレーム同じ点を取るので、何枚混ぜても1点ぶんの情報しか無い。**TAA の AA の正体はジッター**で、
混ぜるのはそれを溜めるだけ。もう一度 `F6` で戻る。

### 7. `F7`: 内訳

```
--- TAA の内訳 ---
  揺らし方: Halton(2, 3) の 8 点 [画素]  いま 1 番
  ...
  疑い方: 分散で切る(YCoCg、分散の箱は ±1.0σ)  履歴を捨てた回数: 0

  速度バッファ: 960 x 640 RG16F = 2.3MB(**カメラの動きだけ**。物の速度は Day 54b)

  GPU の時間(タイマークエリの移動平均): 速度 0.015ms + 解決 0.052ms = 0.067ms
    ※FXAA は 0.038ms(最後に FXAA を掛けていたときの値)。TAA はこれに深度のコピーが1回乗る
  後処理の VRAM 合計: 28.1MB  いまのアンチエイリアス: TAA
```

**x の平均が -0.055 画素**なのが、要点3で書いた「Halton の平均が中心からずれている」の正体(改造課題1)。
**GPU の時間は走行ごとに倍以上ぶれる**(VSync で休みながら測るので、GPU のクロックが上下する)。
並びの傾向(TAA は FXAA の数倍)で読むこと。

### 8. `F8`: 自己チェックが 20 項目すべて合格

```
=== Day 54 自己チェック(ずらし・解決の式・カメラの速度・スーパーサンプリングへの収束)===
  [OK] Halton(2, 3) の最初の4点が (1/2, 1/3) (1/4, 2/3) (3/4, 1/9) (1/8, 4/9)
  ...
  すべて合格(ずらし・解決の式・カメラの速度・スーパーサンプリングへの収束が仕様どおり)
```

**GL を使わない項目が 4 つ**(ずらし)、**64x8 の作り物の絵で走る項目が 6 つ**(解決の式)。
`TemporalAA` がテクスチャを受け取って返すだけの部品なので、解決の式は本物のシーンを描かずに入力を作って確かめられる。

柱は**収束の1項目**——64 点のスーパーサンプリングを答えとして、縁の画素の誤差が
`FXAA 60% / TAA 21% / 8 点をただ平均 20%`(AA なしを 100%)。
**TAA が近づけるのは「ずらしの 8 点の平均」まで**で、そこにはほぼ届いている。

### 9. どの描き方でも TAA は同じ絵

TAA を入れたまま「Forward+」ページの `F2` で描き方を回しても、絵は変わらない。
**同じずらしの番号のフレームどうし**で比べること——ずらしの番号が違うフレームどうしで比べると、同じ描き方でも 0.109% 違う。
**TAA の絵は止めても 8 フレームの周期で揺れている**(要点3)。

### 10. Day 38〜53 の絵と自己チェックが1つも壊れていない

**既定は FXAA のまま**なので、起動した直後の絵は Day 53 とまったく同じ(ずらしが 0 のとき、射影行列は1ビットも変わらない)。
Day 48 の自己チェック(`Alt+F1`)は **214 項目**で通る。
...
いまカットになるのは2つ(前の行列が無い・カメラが 1 フレームで 2m 以上動いた)。
`MotionVectors.CutDistance` を 0.01m まで下げて、**カメラワークの再生中に毎フレーム履歴が捨てられる**様にしてみる。
HUD の `捨てた:N回` が毎フレーム増え、絵は「AA なし」に戻る——**TAA は履歴が続いていることが前提の手法**だと分かる。

逆に、プレイアブルデモに入る瞬間(追従カメラが位置を置き換える)にカットを掛けないとどうなるかも見る
(`CutDistance` を 100m にする)。前の構図の絵が薄く数フレーム残る。
**「疑いがあるから捨てなくてもいい」の限界**がここに出る——疑いは近所の色でしか判断できないので、
たまたま似た色のところは信じてしまう。
...
| GPU の時間 | 速度 0.02ms / 解決 0.05ms / FXAA 0.04ms(走行ごとにぶれる) |
| 収束(64 点の答えとの縁の誤差。AA なし 100%。確認用の光で測るので構図に依らない) | FXAA 60% / **TAA 21%** / 8 点をただ平均 20% |
...
=== Day 54 自己チェック(ずらし・解決の式・カメラの速度・スーパーサンプリングへの収束)===
  ...
  [OK] **止まった絵の速度は 0**(ずらしは毎フレーム変えている。ずらしが動きに化けない)  全画素の最大 0.0003 画素
  [OK] 別の絵から切り替えた最初のフレームは「繋がっていない」(カット)
  [OK] **カメラの動き**: 床の速度が、前と今の行列で写した位置の差と一致(深度から戻して写し直す)  (-3.655, +1.390) / 期待 (-3.667, +1.397) 画素
  [OK] **カメラが止まっていれば、動いた箱の速度も 0**(自分で動いた物は今日は拾えない。Day 54b)  箱の上面 0.0003 画素  全画素の最大 0.0008 画素
  [OK] カメラが 1 フレームで 2m 以上動いたら「繋がっていない」(カット)
  [OK] **TAA はずらしの 8 点の平均まで近づく。FXAA より近い**(64 点の答えとの、縁の画素の平均誤差。AA なしを 100%)  縁 1482 画素  FXAA 60% / TAA 21%(疑わなければ 22%)/ 8 点をただ平均 20%
  [OK] **速度を挿したフレームだけ TAA が掛かる**(自己チェックが後処理を単独で回しても割り込まない)  挿さない 1 パス / 挿す 2 パス
  [OK] 代償: 履歴は画面と同じ大きさの RGBA16F が2枚、速度は RG16F が1枚  履歴 9.4MB / 速度 2.3MB(FXAA の LDR バッファは 2.3MB)
  [OK] 速度の向きは「前 → 今」、単位は UV(camera-velocity.frag)
...
速度は、床の画素を 100m 先の点として写し直していた(遠い点ほど、カメラの平行移動で動かない)。
**速度は深度に全部を預けている**。深度が壊れると、エラーは出ずに、それらしい小さな速度が静かに出る。
...
**3. 16 点にしても収束が良くならなかった**。自己チェックの収束の数字の内訳を見るため、
...
**4. `EndToTarget` を続けて呼ぶと、パスの数が積み上がった**。自己チェックの「速度を挿したフレームだけ TAA が掛かる」が最初に落ちたとき、
...
**5. 今日は計測で PC が落ちなかった**。Day 53 は `glFinish` を挟んで数百フレームを連続で描く計測で GPU を全開にし、開発機の電源が落ちた。
...
**6.(Day 56 で追記)収束の項目が、押したときの光でしか通らなかった**。Day 56 の検証で、自己チェックを状態を変えて押し比べて見つかった。
確認用のシーン(床と立方体)を、いま画面に出ている空・太陽・影・SSAO を借りて照らしていたのが原因で、
起動直後に押すと数字が変わって落ちていた。

直し方は2つで、Day 54a・54b・55・56 に同じ文面で入れた。

- **節4の頭で、確認用の光を決める**: 起動直後の手書きの光(`_manualLightDirection` など)に揃え、IBL・影・SSAO を切る。終わったら戻す
- **節3の頭で、TAA のつまみを既定に揃える**: 今の絵の割合 10%・分散で切る・表示なし。
  節2の最後で本番のつまみに戻していたので、今の絵の割合を 50% にしたまま押すと数字が変わっていた。戻すのは節4の `saved.Restore`
