# Day 54b: 動く物のための TAA(物の速度・Catmull-Rom・明るさの重み)

Day 54 の2日目で、reference は Day 54a の完全コピー + 差分。
Day 54a で TAA の骨格ができた。ずらして描き、カメラの動きで履歴を引き戻し、近所の色で疑って混ぜる。
**カメラだけが動く絵には完全に効く**——決めの構図もカメラワークも、FXAA よりずっと滑らかになった。

崩れるのは<b>カメラが物を追いかけたとき</b>。プレイアブルデモでキツネを追うと、
画面の上でキツネは止まって見え、地面が下へ流れる。ところがキツネの画素に入っている速度は
「地面と同じだけ動いた」——深度には「今どこにあるか」しか書かれていないので、
自分で動いた物の前の位置はどこにも無い。別の場所から引いた履歴は疑いに捨てられ、
**履歴が溜まらないので、キツネの輪郭だけ TAA が効いていないのと同じ**になる。

今日はこれを直す。ついでに、Day 54a で残した2つの粗も片付ける。

| 今日入れるもの | 直るもの |
|---|---|
| **速度の2段目**(動いた物を前の行列で描き直す) | 追いかけている物の輪郭が止まる |
| **Catmull-Rom で履歴を読む** | カメラが動いているあいだのぼけが積もらない |
| **明るさで重みを付けて混ぜる** | 明るい点がジッターで見え隠れしても、まわりごと白くちらつかない |

> **今日の実装でいちばん面白かったのは、「何が動いたか」をどう知るかだった**
> 影パス(Day 33)と SSAO の幾何パス(Day 37)は、描くものの並びを `Program` の中で手で書き写している。
> Day 51 でキャラクターを足したときに3か所を直すことになった(Day 52a の `RenderGBufferPass` のコメント)。
> 速度で4か所目を作るのはやめて、**本描画が必ず通る `Program.Draw` で記録する**ことにした——
> `mesh.Draw()` の次の行に `_motion.Record(mesh, model, joints)` を1行足すだけで、
> どの絵(デモ・プレイアブル・物理・材質グリッド・モデル)でも「何を描いたか」が分かる(要点2)。

## 今日のゴール

**「プレイアブルデモ」でキツネを歩かせたまま「TAA」ページに戻り、`F7` で物の速度を切ると、
キツネの輪郭が階段に戻って揺れる。戻すと輪郭が止まる。
`F5` で履歴を疑うのをやめると、キツネの足元から画面の下へ尾が伸びる。**

| キー(「TAA」ページ) | 何が起きるか |
|---|---|
| `F2` | アンチエイリアス FXAA → TAA → なし(Day 54a) |
| `F3` | TAA の表示(通常 → 揺れの素 → 速度 → 履歴を疑った量。Day 54a) |
| `F4` | 今の絵の割合(Day 54a) |
| `F5` | 履歴の疑い方(Day 54a) |
| `F6` | 履歴の読み方(Catmull-Rom → 双線形)。**動かすと双線形は眠くなる** |
| `F7` | 物の速度(ON / OFF)。**OFF にすると追いかけている物の輪郭が揺れる**。**今日の到達点** |
| `F8` | 明るさで重み(ON / OFF)。OFF にすると HDR のまま混ぜる |
| `F9` | ジッター(ON / OFF。Day 54a) |
| `F10` | 内訳(揺らし方・履歴・速度バッファ・GPU の時間) |
| `F11` | 今日の自己チェック(26 項目。Day 54a の 20 項目を含む) |

**メニューの席が3つ増えるので、Day 54a の `F6`〜`F8` が `F9`・`F10`・`F11` へずれる**。
履歴まわりの3つを「疑い方」のすぐ後ろに差し込んだため。

### 今日いちばん大事な2つ

**1つ目は「前のフレームの姿勢をもう一度描く」**。自分で動いた物は、前のモデル行列と前の関節行列で頂点を運び、
速度バッファへ<b>上から描き直す</b>(`velocity.vert`)。深度テストはシーンの深度に対して <b>LEQUAL</b> ——
手前に柱があれば柱の画素は1段目(カメラの動き)のまま残る。

```
  1段目(全画面): 深度 → 世界の点 → 前のフレームの行列で写し直す   … Day 54a
  2段目(動いた物だけ): 前のモデル行列・前の関節行列で「前の位置」を出して上から描き直す   … 今日
```

**2つ目は「何が動いたかを、描画の分岐を書き写さずに知る」**(要点2)。
`Program.Draw` の最後で `MotionVectors.Record` に投げ、鍵 <b>(メッシュ, このフレームで何回目か)</b> で
行列と関節を覚えて前のフレームと比べる。1ビットでも違えば「動いた」、同じなら「止まっている」。
...
差分は `Render/MotionVectors` の2段目、シェーダ2本(`velocity.vert` / `velocity.frag`)、
`TemporalAA` のつまみ2つ、`taa.frag` の Catmull-Rom と明るさの重み、そして `Program.cs`。
`Camera` も `Framebuffer` も `Texture` も `GpuTimer` も1行も変わっていない。

## 事前に読む資料

- [Brian Karis, "High Quality Temporal Supersampling"(SIGGRAPH 2014)](https://advances.realtimerendering.com/s2014/index.html)
  — **明るさで重みを付ける**(要点4)の出どころ。Day 54a で読んだ資料の、`Weight` の節
- [Lasse Jon Fuglsang Pedersen, "Temporal Reprojection Anti-Aliasing in INSIDE"(GDC 2016)](https://github.com/playdeadgames/temporal)
  — 速度の2段目と、近所でいちばん手前の画素の速度を使う工夫がそのまま読める
- [Matt Pettineo, "Bindless Texturing / Filmic SMAA"(の Catmull-Rom の記事)](https://gist.github.com/TheRealMJP/c83b8c0f46b63f3a88a5986f4fa982b1)
  — **双線形 9 回で 4x4 を読む**(要点3)のコード。今日の `SampleCatmullRom` はこれとほぼ同じ形
- **Day 54a の計画書 要点1〜7**(このリポジトリ)
  — ずらし・混ぜ方・カメラの速度・疑い。今日の3つは全部その上に乗る
- **Day 41・42 の計画書**(このリポジトリ)
  — 関節行列(スキニング)。速度の2段目は「前のフレームの関節行列」をもう1組送る

## 理論の要点

### 1. 自分で動いた物 — 速度の2段目

Day 54a の速度は<b>深度から作った</b>。深度に入っているのは「この画素に今写っている点は、カメラから何 m か」だけで、
その点が<b>前のフレームにどこに居たか</b>は書かれていない。カメラの動きだけなら、世界の点は動いていないので
前の行列で写し直せば出る——けれど<b>物自身が動いた</b>ぶんは、どこからも出てこない。

```
  1段目(Day 54a)  深度 → 世界の点 → 前のフレームの VP で写し直す     … 世界の点が動いていない前提
  2段目(今日)     前のモデル行列・前の関節行列で頂点を運ぶ           … 物が動いたぶんを上から描く
```

2段目は<b>動いた物だけをもう一度描く</b>。頂点シェーダ(`velocity.vert`)が同じ頂点を2回運ぶ。

```glsl
vec4 currentWorld  = uModel         * skin(aPosition, uJoints);
vec4 previousWorld = uPreviousModel * skin(aPosition, uPreviousJoints);
// 画素シェーダで、それぞれを NDC にしてから引く
FragVelocity = (current - previous) * 0.5;      // 1段目とまったく同じ式
```
...
| 状態 | なぜ |
|---|---|
| 深度テスト **LEQUAL**、深度は書かない | シーンの深度と「同じ深さ」の画素だけが通る = その物が実際に見えている画素だけに書く。手前に柱があれば柱の画素は1段目のまま |
| **ポリゴンオフセット** −1.0 で少し手前へ | 本描画(`textured.vert`)とこちらは別のシェーダなので、同じ式を書いても深度が最後の1ビットまで一致する保証が無い。1ビット奥に出れば LEQUAL で落ちて、動く物に<b>点々と穴が開く</b> |
| **裏面を捨てない** | 材質ごとの両面の設定(`Material.DoubleSided`)を知らずに済む。閉じた物の裏面は表面より奥なので、深度テストで勝手に落ちる |

**比べる相手は自分で用意する**。速度バッファは今日から「RG16F + 深度」になり、
`Render` の頭でシーンの深度をそこへ写す(`Framebuffer.BlitDepthTo`)。
1段目が読む深度(`PostProcess.SceneDepth`)とは別の1枚——
<b>描き込み先に挿さっている深度を同時にテクスチャとして読むとフィードバックループになる</b>(Day 50 と同じ話)。

**TAA は近所 3x3 でいちばん手前の画素の速度を使う**(この仕掛けは Day 54a から入っている)。
動く物の輪郭の画素は、半分が物で半分が背景。ジッターでたまたま背景を取ったフレームに背景の速度で履歴を引くと、
輪郭がちぎれて残る。手前(= 物)の速度に寄せると、輪郭が物と一緒に動く——
**2段目を足して初めて、この寄せ方が効く**(Day 54a では手前も奥も同じカメラの速度だった)。

### 2. 何が動いたかを、描画の分岐を書き写さずに知る
...
```csharp
mesh.Draw();
_motion.Record(mesh, model, joints);      // Program.Draw の最後の1行(Day 54b)
```
...
別の物の行列と比べてしまう(設計書「今日残した歪み」の1つ目)。

**カットの3つ目の条件もここから来る**(Day 54a の要点7)。記録したもののうち<b>半分以上が前のフレームに居なかった</b>なら、
絵が丸ごと切り替わったとみなして履歴を捨てる——デモ v1 から材質グリッドへ切り替えたとき。
...
### 4. 明るさで畳む — 明るすぎる点が勝ちすぎないように

TAA は後処理の<b>入口</b>、リニアな HDR のまま走る(Day 54a の要点6)。混ぜる(平均する)なら、
光の量として足し算が効く値で混ぜるのが筋だから。
...
| ずらしを知らない位置の復元 | SSAO・ディファードの位置の復元と Forward+ の升目が、半画素ずれた点を扱う | Day 54a の改造課題3 |
| 16 点・中心合わせのずらし | 止めた絵の答えに 8 点の平均までしか近づかない | Day 54a の改造課題1 |
| アップスケーリング(TAAU / DLSS / FSR) | 画面と同じ解像度で描いている | Day 66b(講読) |
| Z プリパスでの速度 | 速度の2段目は動いた物をもう一度描く | 物が多くなったら G-Buffer / プリパスで一緒に書く |
| 物ごとの消えない番号 | 記録の鍵が「メッシュと、そのフレームで何回目か」なので、描く順番が変わると別の物と比べる | 設計書「今日残した歪み」の1つ目。Day 56 で `DemoScene` を整理するとき |
...
| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Render/MotionVectors.cs` | **記録(`Record`)と比較、2段目(`RenderObjectMotion`)**、深度付きの入れ物、カットの3つ目の条件 | +216 / -24 |
| `Render/TemporalAA.cs` | `HistoryFilter` と `ToneWeighted`(と列挙 `TaaHistoryFilter`) | +20 / -3 |
| `shaders/taa.frag` | `SampleCatmullRom`(双線形 9 回で 4x4)と `ToneWeigh` / `ToneUnweigh` | +88 / -9 |
| `Render/PostProcess.cs` | コメントのみ | +1 / -1 |
| `Program.cs` | `Draw` の1行、メニュー 3 項目、HUD、内訳、自己チェック 6 項目 | +357 / -44 |
| `Day54b.csproj` | `Day54a.csproj` からのリネームのみ | — |

**`Camera` も `Framebuffer` も `Texture` も `GpuTimer` も1行も変わっていない**。
Day 54a で `TemporalAA` を「4枚のテクスチャを混ぜるだけの部品」にしておいたので、
速度の中身が richer になっても `TemporalAA` 側は<b>つまみが2つ増えただけ</b>で済んでいる。

`Program.cs` の -44 行は、記録と描き直しの数を出すために HUD と内訳と自己チェックの
「カメラだけ」と書いてあった行を書き換えたぶん。**描画の分岐は今日も1行も変わらない**——
足したのは `Draw` の末尾の `_motion.Record(mesh, model, joints);` の1行だけ。

### 写経する順番

```
1. shaders/velocity.vert         速度の2段目の頂点(新規。4 が読む)
2. shaders/velocity.frag         速度の2段目の画素(新規。4 が読む)
3. shaders/taa.frag              SampleCatmullRom と ToneWeigh(変更。5 が読む)
4. Render/MotionVectors.cs       記録・比較・2段目(変更)
5. Render/TemporalAA.cs          HistoryFilter と ToneWeighted(変更)
6. Program.cs                    Draw の1行、メニュー、HUD、自己チェック
7. Day54b.csproj                 Day54a.csproj からのリネームのみ
```

**4 がいちばん重い**(+216 行)。`Entry` / `Moved` / 辞書2つ / 関節の並び2本を足し、
`BeginFrame` に引数を戻し、`Record` を書き、`Render` の中に2段目の枝を入れる。
`MotionVectors` は `BeginFrame` → `Record` → `Render`(`RenderCameraMotion` → `RenderObjectMotion`)の順に読む。

**メニューの3項目は「履歴の疑い方」と「ジッター」の間に差し込む**。
順番を変えると席(`F` キー)がずれて、コード中の「メニューの F10」といったコメントが全部嘘になる。

## 設計書

**層は今日も増えていない**。`Render/` に列挙が1つ(`TaaHistoryFilter`)増えて、
`MotionVectors` と `TemporalAA` の口が広がっただけ。

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Render/MotionVectors` | 記録と比較、2段目、カットの3つ目 | +216 / -24 |
| `Render/TemporalAA` | `HistoryFilter` / `ToneWeighted` | +20 / -3 |
| `shaders/taa.frag` | Catmull-Rom と明るさの重み | +88 / -9 |
| `Program.cs` | `Draw` の1行、メニュー、自己チェック | +357 / -44 |
...
**今日この図に足したのは2つ**。頭の `BeginTemporalFrame` が記録を始めるようになり、
その間の `Program.Draw` が毎回 `MotionVectors.Record` を呼ぶ。
**描画の分岐(デモ・プレイアブル・物理・材質グリッド・モデル)は今日も1行も変わっていない**——
`Draw` を必ず通ることだけを頼りにしているので、何を描くかの条件を1つも書き写さずに済んでいる。
...
今日 `MotionVectors` に2段目が入っても `TemporalAA` が1行も変わらなかったのは、この線引きの配当になる。
...
自己チェックが 64x8 の作り物の絵で解決の式を確かめられる(今日の要点3・4 の数字)のはこのおかげ。
...
<b>描き直すものを自分で選ぶ</b>。代わりに「鍵が何回目か」という弱い約束に乗っている(今日残した歪みの1つ目)。
...
疑いが TAA の品質を握っているのは Day 54a で見たとおり。

**今日この図に足したのは2か所**。近所の色を集める前に明るさで畳み(要点4)、最後に戻す。
履歴を読むところが双線形から Catmull-Rom になった(要点3)。
どちらも「混ぜ方」ではなく「混ぜる前の下ごしらえ」で、**指数移動平均そのものは Day 54a のまま**。

### 今日残した歪み(3つ)

**1つ目: 記録の鍵が「メッシュと何回目か」**。描く順番が毎フレーム同じなら同じ物を指すが、途中に1つ増えると後ろが全部ずれて、
...
**2つ目: スキニングの式が4か所目**(`textured.vert` / `depth.vert` / `geometry.vert` / `velocity.vert`)。
しかも `velocity.vert` は前のフレームのぶんも持つので、式が2本入っている。Day 52b の BRDF の写し(3か所)と同じ種類の歪みで、
見張り方も同じ(自己チェックの文字列の突き合わせ)。GLSL 3.3 に `#include` が無いのが根で、
シェーダを読み込むときに自前で `#include` を展開する仕組みを入れれば1か所にできる。

**3つ目: 速度を持たないものが残っている**。粒・トレイル・2D のスプライト・後処理を通す文字(「FXAA と色調整」の F5)。
どれも `Program.Draw` を通らないので記録されず、後ろにある物の速度で履歴を引く。疑いが色を引き戻すので残像は出ないが、
粒の縁は履歴が溜まらずに揺れる。本物は「反応マスク」で粒の画素だけ今の絵の割合を上げるか、TAA のあとに描く。

**Day 54a の歪みのうち、2つは今日消えた**。「速度を持たないもの」から自分で動いた物が抜け、
「履歴を双線形で読んでいる」は Catmull-Rom になった。残っているのは
「ずらしを知らない位置の復元が3か所」(Day 54a の改造課題3)と「深度を2回写している」の2つ。

Day 54a の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。変わった図は次の4つ。

| 図 | 何が変わったか |
|---|---|
| `Render` のクラス図 | `MotionVectors` に記録と2段目、`TemporalAA` に `HistoryFilter` / `ToneWeighted`、列挙 `TaaHistoryFilter` |
| `TAA の1フレーム` | 頭に「記録を始める」、`Program.Draw` のたびに `Record` |
| `今日足したものが繋がった場所` | `Program.Draw` → `MotionVectors` の線が増えた |
| `1画素の色が決まるまで(TAA)` | 明るさで畳む段と、Catmull-Rom での読み |
...
`Demo/`・`Model/`・`Scene/`・`Ecs/`・`Physics/`・`Text/`・`Audio/`・`Game/`・`Core/` の図は**昨日のまま**で、今日は1つも触っていない。
...
### Render — OpenGL の薄い皮(Day 54b で `MotionVectors` に記録と2段目が入った)
...
代わりに「鍵が何回目か」という弱い約束に乗っている(設計書の冒頭「今日残した歪み」の1つ目)。
...
`dotnet run --project reference/Day54b -c Release` で起動する。
先に「デモ v1」ページ(8 枚目)の `1`(決めの構図)でデモ v1 を出す。
メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「TAA」(22 枚目)が開き、`F2` で TAA に入る。
...
| 設定 | 見え方 |
|---|---|
| 既定(物の速度 ON) | キツネの輪郭は滑らかで、脚の縁も止まっている。HUD は `描き直し1/26`(キツネのパーツ) |
| `F7`(物の速度 OFF) | **キツネの輪郭(とくに脚)が階段に戻って揺れる**。HUD は `速度:**カメラだけ**` = Day 54a の状態 |
| `F3` を2回(速度の表示) | **キツネだけが黒く、地面が黄緑**——カメラがキツネを追っているので、画面の上でキツネは止まり、地面が流れている |

**ここが今日の到達点**。`F7` の見え方は、自己チェックの「追いかけている箱」がそのまま数字で見ている
(輪郭の画素の1フレームでの変化 0.0048 → 0.0142)。
物の速度が無いと、画面の上で止まっているキツネの画素が「地面と同じだけ動いた」とみなされ、
前のフレームの別の場所から履歴を引いて疑いに捨てられる——**履歴が溜まらないので、TAA が効いていないのと同じ**になる。

### 2. `F5`: 疑い方(キツネの尾)

同じくキツネを歩かせたまま `F5` を2回(`疑わない`)。
**キツネの足元から画面の下へ、キツネの色の尾が伸びる**(Day 54a の要点5)。
隠れていた地面が現れるたびに、キツネの色の履歴が地面と一緒に流れている。
**速度は正しい**(速度の表示ではキツネが黒く、地面が黄緑)のに尾が出る——
前のフレームに<b>そもそも写っていなかった</b>ものの色は、どこからも読めない。

### 3. `F6`: 履歴の読み方

カメラワークを再生しながら `F6` で双線形にすると、**動いている間だけ絵が眠くなる**。
止めると差は消える(止まっていれば、ちょうど画素の中心を読むので、どちらで読んでも同じ)。
差の大きさは自己チェックが見ている(縁の幅 双線形 3.61 画素 / Catmull-Rom 1.95 画素。今の絵そのものは 0.80 画素)。

### 4. `F8`: 明るさで重み

差は明るい点がジッターで見え隠れするところにしか出ないので、夕暮れの決めの構図では目で拾いにくい。
自己チェックの数字(ちらつきの振れ幅 2.67 → 0.21、代わりに平均 25 → 1.0)で確かめる。
**代償は偏り**——明るい画素ほど軽く数えるので、ジッターで見え隠れする明るい点は暗く寄る。

### 5. `F10`: 内訳

Day 54a の内訳に、記録の行が増える。
...
(キツネを歩かせているときのもの。)**26 回描いて描き直しは 1**——
動いていない 25 個は1段目(カメラの動き)で足りている。

### 6. `F11`: 自己チェックが 26 項目すべて合格
...
Day 54a の 20 項目に 6 項目足して 26 項目。今日足したのは
**明るさの重み・履歴の読み方・箱の動き・物の速度を切る・隠れた箱・追いかけている箱**。

### 7. Day 54a の絵と機能が1つも壊れていない

- **カメラだけが動く絵は Day 54a のまま**。決めの構図もカメラワークも、絵は変わらない
- **`F2` / `F3` / `F4` / `F5` / `F9`** は席が変わっただけで中身は同じ
- TAA を入れたまま「Forward+」ページの `F2` で描き方を回しても、絵は変わらない
  (**同じずらしの番号のフレームどうし**で比べること。フォワードと Forward+ の差は 0.008%、ディファードとは 0.076%)
- Day 48 の自己チェック(`Alt+F1`)は **217 項目**で通る(Day 54a の 214 に今日の3項目)
- **TAA を入れたままでも**、Day 53 の Forward+(23 項目)、Day 52b のディファード(31 項目)、
  Day 38 の FXAA の自己チェックは全部通る
...
設計書「今日残した歪み」の1つ目を、目で見る。物理デモ(「剛体力学」ページ)を出して箱を落とし、
接触点の光る玉が増えたり減ったりする瞬間の速度の表示(`F3` を2回)を見る。
**玉の数が変わったフレームだけ、後ろの箱の速度が一瞬おかしくなる**はず(鍵が1つずつずれて、別の物の行列と比べる)。

確かめたら、`MotionVectors.Record` の鍵をわざと「メッシュだけ」にしてみる(`occurrence` を常に 0)。
同じメッシュを何度も描くシーン(物理デモの箱、材質グリッド)で、**ほとんどの物が「動いた」と判定される**——
最後に記録した1つとしか比べられないため。HUD の `描き直しN/M` の N が跳ね上がる。
**鍵の設計がそのまま「何を同じ物とみなすか」の定義になっている**ことが分かる。

### 課題2(中): TAA のあとに鮮鋭化を足す

TAA の絵は FXAA より少し柔らかい。実際のエンジンは TAA のあとに鮮鋭化(アンシャープマスク)を1段足す
...
### 課題3(難): 速度を G-Buffer で一緒に書く

いまの2段目は、動いた物を<b>もう一度描いている</b>(ドローコールが増える)。
ディファード(Day 52)のときは G-Buffer パスで同じ物を描いているので、
**G-Buffer に5枚目(RG16F)を足して、本描画のついでに速度も書ける**。

1. `GBuffer.Layout` に `Rg16F` を足し、`textured.frag` の G-Buffer の枝に `layout (location = 4) out vec2 GVelocity;` を足す
2. `textured.vert` が前のモデル行列・前の関節行列でもう1つの位置を運ぶ(`velocity.vert` の中身をそのまま持ってくる)
3. `MotionVectors` は、ディファードのときだけ2段目を飛ばす

**難しいのは「前の行列をどこから取るか」**。いまは `MotionVectors` が記録して持っているが、
本描画の時点ではまだ記録の比較が終わっていない(`Draw` の中で記録している最中)。
前のフレームの記録を先に引ける形(`MotionVectors.PreviousOf(mesh, occurrence)`)を足すか、
物自身が前の行列を持つ形(今日残した歪みの1つ目の直し)へ進むことになる。
**歪みが2つ繋がっている**のが見えるのがこの課題の読みどころ。
...
| 解像度 | 960 x 640 |
| 確認の条件 | `-c Release`、**VSync を入れて**確認した。計測(`glFinish` を挟んで連続で描く)は無く、GPU の時間はタイマークエリで普段のフレームの中で測った |
...
| 追いかけている箱の輪郭の揺れ | 物の速度あり 0.0048 / カメラだけ 0.0142 |
...
**1. 疑わないと、キツネの足元から画面の下へ尾が伸びた**。完成条件2の絵。速度は正しく出ている(速度の表示ではキツネが黒く、地面が黄緑)のに尾が出るのは、
隠れていた地面が現れるときの履歴にキツネの色が入っているから。**速度が正しくても、前のフレームに写っていなかったものの色は、どこからも読めない**。
疑いは「速度の間違い」のためではなく、「現れたもの」のためにある。

**2. 物の速度を切ると、追いかけているキツネの輪郭が階段に戻った**。完成条件1の絵。物の速度を切ると、キツネの画素にはカメラの動きだけの速度——後ろの地面とほぼ同じ速度——が入る。
...
**3. 隠れているはずの箱が 322 画素の速度を書いていた**。自己チェックの「床に隠れた箱」が最初に落ちたとき。
原因は Day 54a と同じところで、確認用のシーンを描く前に深度テストが切れていた——
<b>深度を1画素も書かないまま描いた</b>ので、2段目の深度テストの相手が遠クリップ面(1.0)になり、床の下の箱を素通しにしていた。
Day 54a の時点で `RenderTaaProbe` が深度テストを自分で入れるように直してあるので、今日は踏まずに済んでいる。
**2段目は「シーンの深度と同じ深さか」だけで見える画素を決めている**ので、深度が壊れると静かに間違える。

**4. `velocity.vert` を写し忘れると、起動の時点で落ちる**。シェーダは `MotionVectors` のコンストラクタで読むので、
TAA を入れる前、`OnLoad` で落ちる。Day 54a で `camera-velocity.frag` だけを読んでいたのが2本になったぶん、
写経の順番(1・2 をシェーダ、4 をクラス)を守らないと引っかかりやすい。

**5.(Day 56 で追記)自己チェックのつまみを既定に揃えた**。Day 56 の検証で、自己チェックを状態を変えて押し比べて見つかった。
節2の最後で本番のつまみに戻していたので、「TAA」ページで今の絵の割合を 50% にしたまま押すと、
**「追いかける箱の輪郭が止まる」が落ちていた**。節3の頭で
「今の絵の割合 10%・分散で切る・Catmull-Rom・明るさで重み・表示なし・物の速度あり」に揃え、
戻すのは節4の `saved.Restore` にした。Day 54a・55・56 にも同じ文面で入れてある。
