# Day 55a: 被写界深度(レンズの口径を、カメラに戻す)

**Phase 8(デモ推奨編)の4日目**。Day 54 で絵の縁を時間の方向に均した。今日はその後ろに、
**被写界深度**(ピントの外れた物がぼける)の段を足す。

**Day 55 は2日に分けてある。**

| | 何をするか |
|---|---|
| **Day 55a(今日)** | **被写界深度**。錯乱円の式、散らすものを集めるで書く2つの層、半分の大きさで集めて原寸で重ねる。**ぼけの地図と升目**(明日も使う共通の下ごしらえ) |
| Day 55b | **モーションブラー**。Day 54 の速度バッファを読み、シャッターが開いている割合だけ尾を伸ばす。地図と升目は今日のものをそのまま読む |

Day 54 までのカメラは**穴が点(ピンホール)で、シャッターが一瞬**という、現実には作れないカメラだった。今日は穴のほうをやめる
(シャッターは Day 55b)。

| | Day 54 までのカメラ | Day 55a のカメラ |
|---|---|---|
| 光が通る穴 | 点(口径 0)。**どの奥行きにもピントが合っている** | 口径 A のレンズ。ピントの奥行きの点だけが1点に集まり、**ほかは円(錯乱円)に広がる** |
| 描き方 | 1回描いて終わり | 1回描いた絵を、**深度を手がかりに**ぼかす(後処理の近似) |
| 手がかり | — | 深度(Day 50 の写し)。奥行きに戻して錯乱円にする |
| 払うもの | — | 全画面 7 パス(地図 4 + 被写界深度 3)、VRAM 10.9MB(960x640)、GPU 0.5〜0.8ms |
...
| `F7` | 内訳(レンズの数字・錯乱円・VRAM・GPU の時間) |
| `F8` | 今日の自己チェック(14 項目) |

**既定は OFF** なので、起動した直後の絵は Day 54b と1ビットも変わらない(完成条件9 で画素ごとに確かめた)。
メニューを閉じた状態で `Shift+F1` を押すと、最後のページ(被写界深度、23 枚目)が直接開く。
...
**2つ目は「散らすものを、集めるで書く」**。ボケは、本来は「1つの点がまわりへ光を配る」処理(scatter)。
...
**画面を 16 画素の升目に切って、近所でいちばん大きく配っている物を先に記録しておく**(要点5)。
この地図と升目(`BlurField`)は、Day 55b のモーションブラーもそのまま読む共通の下ごしらえとして作る。

### 今日は Render とシェーダが主役

差分は `Render/` に新規2本(`BlurField` / `DepthOfField`)、シェーダ6本、
`PostProcess` の1段、`Shader` の `SetInt2`、そして `Program.cs`。
`Demo/` も `Model/` も `Scene/` も `Ecs/` も `Physics/` も `Text/` も `Audio/` も `Game/` も1行も変わっていない。
**`TemporalAA` と `MotionVectors`(Day 54)も1行も変わっていない**——後処理の入口の TAA の後ろに、1段を差し込んだだけ。
...
- [Jorge Jimenez, "Next Generation Post Processing in Call of Duty: Advanced Warfare"(SIGGRAPH 2014, Advances in Real-Time Rendering)](https://www.iryoku.com/next-generation-post-processing-in-call-of-duty-advanced-warfare)
  — **今日の被写界深度の一次資料**。散らしとして集める(scatter-as-gather)、手前と奥の層、半分の大きさで集めて原寸で重ねる(要点4・6)。
  被写界深度とモーションブラーを同じ枠組みで扱っているので、Day 55b でもう一度読む
...
- [Morgan McGuire, Padraic Hennessy, Michael Bukowski, Brian Osman, "A Reconstruction Filter for Plausible Motion Blur"(I3D 2012)](https://casual-effects.com/research/McGuire2012Blur/McGuire12Blur.pdf)
  — **升目の出どころ**。升目の最大(TileMax)と近所の最大(NeighborMax)(要点5)。本題のモーションブラーは Day 55b で読む
- [Earl Hammon Jr., "Practical Post-Process Depth of Field"(GPU Gems 3 第 28 章)](https://developer.nvidia.com/gpugems/gpugems3/part-iv-image-effects/chapter-28-practical-post-process-depth-field)
  — **一世代前の定番**。Call of Duty 4 の被写界深度。今日のものと比べると、升目と「相手の半径で決める」が無いぶん何を取りこぼすかが見える
- **Day 7 の計画書**(このリポジトリ) — 深度バッファの値が 1/z に比例して並ぶこと。錯乱円が深度の値の1次式になる理由(要点2)
- **Day 31 の計画書 設計書「HDR パイプラインの中身」**・**Day 53 の計画書 要点**・**Day 54a の計画書 要点6** — 今日の段を差し込む場所、
  升目という発想、TAA が入口で HDR のまま混ぜている理由
...
代わりに、1回の絵に写っていないもの(手前の物の縁の向こう側)は出せない——後処理の近似の限界(要点8)。

**今日は穴のほう(被写界深度)を入れる**。露光のほう(モーションブラー)は Day 55b で、今日作る地図と升目をそのまま読む。
...
集めるには「どこまで探すか」が要る。自分がぼけていなくても、隣の手前の物の大きな円が自分まで届く。
...
**速度を記録するのは Day 55b のモーションブラーのため**。動く物の尾も、隣の升目から自分まで伸びてくることがある。今日は速度を渡さないので、その欄は 0 のまま。
...
2. **集める**(半分、2枚同時。Day 52a の MRT): Vogel の円盤(i 番目の点を半径 √((i + 0.5) / 48)、角度 i × 黄金角に置く)に 48 点を撒き、奥の層と手前の層
...
Day 55b のモーションブラーは原寸のまま走らせる。尾は向きのあるぼけで、横に流れる絵でも縦の目地はくっきり残る——縮めると、残るべき縦の細部まで失う。

### 7. 並び — TAA の後ろ、ブルームの前、HDR のまま

| 置き場所 | なぜ |
|---|---|
| **TAA の後ろ** | 被写界深度は近所の色を混ぜる処理。ずらして描いた揺れる絵ではなく、TAA で落ち着いた絵を混ぜる。前に置くと、TAA が「近所の色の範囲」で履歴を疑うとき、ぼかした色の範囲で疑うことになる |
| **ブルームの前** | 明るい点をまず丸いボケに広げ、それからにじませる。後ろに置くと、ぼける前の鋭い点から出たにじみが、ぼけた絵の上に浮く |
...
**被写界深度は「このフレームのカメラの数字」が挿さっているときだけ掛かる**(`PostProcess.Frame`)。Day 54a の `Velocity` と TAA の関係と同じで、
挿すのは本番のフレーム(`Program.PrepareCameraBlur`)だけ、`_post.End` が使い切って外す。
自己チェックが後処理を単独で回しても割り込まない(挿さない 1 パス / 挿す 8 パス)ので、Day 38〜54 の自己チェックは入れたままでも全部通る。

### 8. 今日入れなかったもの

| 入れなかったもの | 何が起きるか | どこでやるか |
|---|---|---|
| モーションブラー | 動いている物も一瞬で写る。地図の升目は速度の口を空けたまま作る(今日は速度を渡さない) | **Day 55b**(Day 54 の速度バッファをそのまま読む) |
| オートフォーカス(画面の真ん中の深度にピント) | ピントは注視点まで。注視点が何も無い空間にあると、見ている物がぼける | 改造課題2(深度を GPU から読むと待たされるので、数フレーム遅れで拾う) |
| ボケの形(絞り羽根の多角形、画面の端の猫目) | どのボケも丸 | 改造課題1(六角形)・改造課題3(猫目) |
| 手前のボケの向こう側 | 本物のレンズは口径の端から手前の物の向こうが少し見える。1回の絵には写っていないので出せない | 何枚も描いて平均する(オフラインの手法) |
| 半透明のボケ | 粒・トレイルは深度を書かないので、後ろの物のボケ方を借りる | 今日残した歪みの2つ目 |
| 露出との連動 | 本物のカメラは F 値で明るさも変わる。今日はボケだけで、明るさは露出(Day 31)が別に決める | Day 55b の改造課題2(F 値とシャッターから露出を決める) |
...
**地図と升目の3本(`BlurField.cs` / `blur-tiles.frag` / `blur-max.frag`)は、速度を読む口も持っている**。
升目ごとに「いちばん手前の錯乱円」と一緒に「いちばん速い速度」も記録する形で、今日は速度を渡さない(`uHasVelocity = 0`)。
Day 55b のモーションブラーはこの3本を1文字も変えずに読むので、写すのは今日1回で済む。コメントに出てくる「モーションブラー」「尾」はその予告。
...
| `Render/PostProcess.cs` | 地図と被写界深度を持ち、TAA のすぐ後ろで掛ける。`Frame`(このフレームだけ挿す)、VRAM の合計 | +87 / -2 |
| `Program.cs` | `PrepareCameraBlur`、HUD 1行、メニュー 7 項目、内訳、自己チェック 14 項目 | +663 / -1 |

**`Program.cs` の差分の6割は自己チェック**(`RunCameraBlurCheck` と道具で 400 行ほど)。
本番の流れに入ったのは `PrepareCameraBlur`(15 行ほど)と、それを呼ぶ1行だけ——**何を描くかは1行も書き換えていない**。
`Render3D` 以下の描画の分岐はどれも Day 54b のまま。

Day 54b のコードに入った手直しは2か所(差分の -3 行はこれ)。

| 手直し | どこ | なぜ |
|---|---|---|
| メニューの自己チェックの件数 217 → 224 | `RunDebugMenuCheck` | 「被写界深度」ページの 7 項目 |
| VRAM の合計に地図と被写界深度(コメントと式の2行) | `PostProcess.ByteSize` | どちらも入れたときに初めて作るので、入れる前は 0 |

もう1つ、**プレイアブルデモに入るときカメラワークを止める**手直しを `ShowPlayableDemo` に入れた(Day 51 からの不備。検証の途中で分かったこと 3)。
こちらは **Day 51 まで遡って同じ場所・同じ文面で直した**ので、Day 54b と今日の差分には出ない。
...
10. Render/PostProcess.cs          TAA のすぐ後ろに1段を置く(5・9 を使う)
11. Program.cs                     全部を繋ぐ
12. Day55a.csproj                  Day54b.csproj からのリネームのみ
```

**依存順に並べてある**。C# のビルドが通るのは 1・5・9〜11 の順で、シェーダ(2〜4・6〜8)は実行時に読むのでビルドには効かない——
ただし<b>11 より前に写しておく</b>こと。シェーダは起動時に読む(`PostProcess` のコンストラクタが2つのクラスを作る)ので、
写し忘れると被写界深度を入れる前、起動の時点で落ちる。

`BlurField.cs` の頭にある `CameraFrame` は、`DepthOfField` と `PostProcess` が使う小さな構造体。
`DepthOfField.cs` は静的なレンズの式(`FocalLength` → `Aperture` → `LensScale` → `CircleOfConfusion`)を先に読み、それから `Render` を読む。

`Program.cs` の中は次の順で読むとよい。

```
  フィールド(FNumberSteps / FocusSteps / _focusScale)
    → OnRender の PrepareCameraBlur                              **今日の1行**(RenderMotionVectors の直後)
    → DrawOverlayInfo の CameraBlurLabel                          HUD
    → Day 55 の節の頭(UseDepthOfField / FocusDistance / PrepareCameraBlur / CameraBlurLabel / DescribeCameraBlur)
    → RunCameraBlurCheck と道具(UploadDepth / CameraBlurSettings)
    → BuildDebugMenu の末尾(メニュー 1 ページ)と RunDebugMenuCheck の 224
```

## 設計書

**層は今日も増えていない**。`Render/` に型が4つ(`BlurField` / `DepthOfField` / `CameraFrame` と列挙1つ)増えて、
`PostProcess` の口が広がっただけ。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Render/BlurField` | `Render/` | 奥行きと錯乱円の地図、升目、近所の最大。**Day 55b のモーションブラーも同じものを読む** |
...
| `Render/DofGather` | `Render/` | ボケの集め方の列挙 |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Render/PostProcess` | `DepthOfField` / `BlurField` / `Frame`、TAA の後ろの1段 | +87 / -2 |
| `Render/Shader` | `SetInt2` | +10 / -0 |
| `Program.cs` | `PrepareCameraBlur`、メニュー、自己チェック | +663 / -1 |
...
flowchart TD
    ...
    TQ -->|"No"| CQ
    TR --> CQ{"カメラの数字が挿さっているか(Day 55a)<br/>被写界深度を入れているか"}
    CQ -->|"Yes"| BF["ぼけの地図<br/>奥行きと錯乱円 → 升目の横 → 縦 → 近所<br/>4パス"]
    BF --> DF["被写界深度<br/>半分の大きさで2層を集め、原寸で重ねる<br/>3パス"]
    DF --> IN["後ろの段が読む絵"]
    CQ -->|"No"| IN
    IN --> BL["明部 → ぼかし(Day 31)"]
...
**TAA と FXAA の間に、HDR のまま差し込んだ**。Day 54a で「TAA は入口、FXAA は出口」と置いた並びの、入口側に1段が続く形。
被写界深度は「近所の光の量を混ぜる」処理なので、光の量として足し算が効く HDR のうちに置く(要点7)。

| | 被写界深度 |
|---|---|
| カメラの何をやめるか | 穴が点(ピンホール) |
| 手がかり | 奥行き(深度の写しから) |
| 1つの点の広がり | 円(錯乱円) |
| 大きさの決まり方 | L × (1/S − 1/z) |
| 近所の升目で知りたいこと | いちばん手前の錯乱円(手前のボケは外へ広がる) |
| 大きさ | 半分(48 点) |
| 全画面パス | 地図 4 + 被写界深度 3 |
| 持つバッファ(960x640) | 地図 + 升目 3枚 = 2.7MB、半分 3枚 + 原寸 1枚 = 8.2MB |
| GPU の時間(VSync で休みながら) | 地図 0.08〜0.18ms、被写界深度 0.45〜0.58ms |

**地図を被写界深度から切り離してある**のは、Day 55b のモーションブラーも同じ地図を読むから。
升目が「近所でいちばん手前の錯乱円」と「近所でいちばん速い速度」を一緒に持つので、明日は地図を作る4パスを1回で2人ぶんにできる。
...
flowchart LR
    ...
    PP -- "Build(速度は渡さない)" --> BF["BlurField<br/>奥行き・錯乱円・近所の升目"]
...
**`DepthOfField` は `Camera` を知らない**。数字は `CameraFrame` で届く。
`TemporalAA` がずらしを `Vector2` で返して `Camera` を知らずに済ませたのと同じ線で、自己チェックは 128x64 の作り物の絵に
「ピント 2m、無限遠のボケ 8 画素」のような数字を直接並べて、ボケの式を確かめられる。

**`CameraFrame` は1フレームの時間も持っている**が、今日は誰も読まない。Day 55b のモーションブラーがシャッターを秒で決めるときに読む。
`PostProcess.Frame` は `Velocity` と同じ「このフレームだけ挿す」口で、今日は被写界深度だけがこれを見る。
...
flowchart LR
    ...
    V["速度バッファ RG16F<br/>(Day 55b から読む)"] -.-> S
...
**速度の口は今日は閉じてある**(`uHasVelocity = 0`)。升目は速度の欄を 0 のまま書く。
速度を長さで比べて向きごと覚える処理(`blur-tiles.frag` / `blur-max.frag`)は、Day 55b で速度が流れ込んだときに初めて効く。
...
**1つ目: 深度を写す場所が3つになった**。ソフトパーティクル(Day 50)のために粒の前で1回、速度(Day 54a)のために `_post.End` の前でもう1回、
そして今日、TAA を入れずに被写界深度だけを入れたときに `PrepareCameraBlur` で1回。中身はどれも同じ(粒は深度を書かない)。
Day 54a の歪みの2つ目が1つ増えた形で、「フレームに1回、不透明を描き終えたところで写す」持ち主を1つ決めれば片付く。

**2つ目: 半透明は奥行きを持たない**。粒・トレイルは深度を書かないので、
被写界深度では**後ろにある物のボケ方を借りる**(ピントの合った粒でも、後ろの壁がぼけていれば一緒にぼける)。
Day 54b の歪みの3つ目(速度を持たないもの)と同じ種類の話。本物は半透明を別の層に描いて、ボケを後から重ねる。
...
**4つ目: 原寸の RGBA16F が段ごとに1枚ずつ増える**。TAA の履歴2枚に、今日は被写界深度の出力が1枚足された。
後処理の VRAM は FXAA と被写界深度で 24.9MB。段ごとに「自分の出力を持つ」形のほうが、どの段を切っても他が壊れないので、今日は分かりやすさを取った
(Day 55b でモーションブラーの出力がもう1枚増える)。

**Day 54a・54b の歪みのうち、今日動いたもの**:

| Day 54a・54b の歪み | 今日 |
|---|---|
| ずらしを知らない場所が3つ(Day 54a の1つ目) | そのまま。TAA を入れているときは、錯乱円の地図が「ずらして描いた深度」から作られる(半画素以内なので絵には出ない) |
| 深度を2回写している(Day 54a の2つ目) | **1つ増えた**(今日の歪みの1つ目) |
| 記録の鍵が「メッシュと何回目か」(Day 54b の1つ目) | そのまま |
| スキニングの式が4か所(Day 54b の2つ目) | そのまま |
| 速度を持たないものが残っている(Day 54b の3つ目) | そのまま。同じ種類の歪みが被写界深度にも出た(今日の歪みの2つ目) |

Day 54b の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。変わった図は次の4つ。

| 図 | 何が変わったか |
|---|---|
| `Render` のクラス図 | **`BlurField` / `DepthOfField` / `CameraFrame` と列挙1つが増えた**。`PostProcess` に `DepthOfField` / `BlurField` / `Frame`、`Shader` に `SetInt2` |
| `Demo` のクラス図 | ページが 22 枚から **23 枚**へ(`Demo/` のコードは1行も変わっていない)。カメラワークとプレイアブルデモの「排他」の記述を直した |
| `1フレームの流れ` | `_post.End` の前に `PrepareCameraBlur` |
| `HDR パイプラインの中身` | TAA の後ろに被写界深度 |

新しく足した図が4つ(この節の上に並べたもの)。Day 54a・54b の設計書の冒頭にあった図(FXAA と TAA の設計比較、速度バッファができるまで、など)は
TAA の話なので、今日は外した(Day 54a・54b の計画書を見ること)。

| 図 | 何を描いたか |
|---|---|
| 後処理の並び | TAA の後ろ、ブルームの前 |
| 今日足したものが繋がった場所 | `DepthOfField` が `Camera` を知らないこと |
| ぼけの地図ができるまで | 地図・横・縦・近所 |
...
**Day 55a でも層は増えていない**。増えたのは `Render/` の中の型4つで、矢印は1本も足していない——
`BlurField` / `DepthOfField` を `PostProcess` が持つのも、`CameraFrame` が `Camera` から数字を抜くのも、全部 `Render/` の中の話。
...
### Render — OpenGL の薄い皮(Day 55a で `BlurField` と `DepthOfField` が増えた)

**`DepthOfField` は後処理の中の部品**(Day 55a)。`TemporalAA` と同じく `PostProcess` が持ち、
`Dispose` も後処理と一緒に回る(大きさは描くときに自分で合わせる)。知っているのは `Framebuffer` と `Texture` と `Shader` と `GpuTimer`、
それに地図(`BlurField`)と数字(`CameraFrame`)だけで、**`Camera` を知らない**——
自己チェックが 128x64 の作り物の絵にピントの数字を直接並べて、ボケの式を確かめられるのはこのおかげ。

**`BlurField` は被写界深度の下ごしらえ**。奥行き・錯乱円・近所の升目を作り、被写界深度はそれを読む。
奥行きに戻す式が1か所で済む(`blur-field.frag`)。**速度を読む口も持っている**が、今日は渡さない(`velocity` は null、升目の速度の欄は 0)。
Day 55b のモーションブラーが同じ地図を読むときに、この口を開ける。
...
**`GpuTimer` を持つクラスが5つになった**(`TemporalAA` / `MotionVectors` / `PostProcess` の FXAA / `BlurField` / `DepthOfField`)。
...
classDiagram
    ...
    PostProcess *-- BlurField : ぼけの地図を作る(Day 55a)
...
**Day 55a でも `Demo/` は1行も変わっていない**。メニューのページが1枚増えた(23 枚)が、ページを組むのは `Program.BuildDebugMenu` の側。
キツネ(`PlayableDemo`)は今日から被写界深度の的にもなるが、`PlayableDemo` はそれを知らない——
ピントは追従カメラの注視点から `Program` が決める。
...
いまは `ShowPlayableDemo` が `_cameraPlaying` も落とす(Day 51〜55a の同じ場所)。
...
flowchart TD
    ...
    RMV --> PCB["PrepareCameraBlur()(Day 55a)<br/>被写界深度なら<br/>このフレームのカメラの数字を _post.Frame へ<br/>TAA でなければ深度をここで写す"]
    PCB --> PE["_post.End(幅, 高さ)<br/>TAA → <b>被写界深度</b> → 明部・ぼかし<br/>→ 合成 → LDR → FXAA → 画面"]
...
**Day 55a で尻にもう1段増えた**。`RenderMotionVectors` の直後の `PrepareCameraBlur` で、被写界深度を入れているときだけ
カメラの数字(`CameraFrame`)を後処理に挿す。ピントは注視点までの距離、1フレームの時間は `OnRender` の間隔。
TAA を入れていなければ速度を描かないので、深度の写しもここで作る。
その間の分岐(`Render3D` 以下)は今日も1行も変わっていない——ボケは描き終えた深度から作るので、図の分岐のどれを通っても掛かる。
切っていれば、条件を見て何もせずに帰る(`_post.Frame` は挿さらず、`_post.End` は Day 54b のまま)。
...
### HDR パイプラインの中身 — 11 回のフルスクリーンパス(Day 55a で TAA の後ろに被写界深度が入った)
...
flowchart TD
    ...
    BFD --> DOF["被写界深度 dof-prepare / dof-gather / dof-combine<br/>3パス"]
    DOF --> IN["後ろの段が読む絵"]
    CQ -->|"No"| IN
...
**Day 55a で TAA の後ろに 7 段増えた**。ぼけの地図(4)・被写界深度(3)。どれも HDR のまま、ブルームより前(Day 55a の要点7)。
FXAA を入れたまま掛けると **11 + 4 + 3 = 18**、TAA となら **1(TAA)+ 7 + 1(明部)+ 8(ぼかし)+ 1(合成)= 18**。
地図を被写界深度の外に置いたのは、Day 55b のモーションブラーも同じ地図を読むから——別々に作ると、地図の4パスが2回になる。
...
| 升目の横・升目・近所(Day 55) | 横 1/16 x 縦 原寸 → 1/16 | RGBA16F | なし | 速度(2成分。Day 55b から読む)といちばん手前の錯乱円といちばん大きい錯乱円 |
| 被写界深度の下ごしらえ・2層(Day 55) | 画面の 1/2 | RGBA16F | なし | **ぼけたものは縮めても分からない**(ブルームと同じ)。2層は MRT で1回に書く |
| 被写界深度の出力(Day 55) | 画面と同じ | RGBA16F | なし | ピントの帯は原寸の細部が要る |
...
`dotnet run --project reference/Day55a -c Release` で起動する。
先に「デモ v1」ページ(8 枚目)の `1`(決めの構図)でデモ v1 を出し、`Space` で一時停止する。
メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「被写界深度」(23 枚目)が開く。
...
重ねる段は半径 1 画素未満のボケを原寸の絵のまま出す(要点2・6)。**壊れているのではなく、これが正しい絵**。

### 2. 『ゴミ箱の金属』で F 値を回す

「カメラワークと機能」ページ(9 枚目)の `F4`(次のショットへ)で、2番目のショット『ゴミ箱の金属』へ飛ぶ(一時停止のままでよい)。
「被写界深度」ページに戻ると、HUD は `ピント:3.20m(注視点) 無限遠のボケ:4.3px`。
...
f/0.7 で `無限遠のボケ:8.6px`。壁のレンガの目地も柔らかくなる。HUD を消して OFF と比べた差は、f/1.4 で平均 0.53%、f/0.7 で平均 1.27%。
...
差は缶の輪郭のまわりにしか出ない(平均 0.17%)が、拡大すると一目で分かる——
「手前のボケの縁が硬い」のは、後処理の被写界深度でいちばんよく見かける失敗。
もう一度 `F5` で戻し、`F4` でピントを注視点に戻す。

### 6. キツネを歩かせる

「プレイアブルデモ」ページ(19 枚目)の `F2` でキツネを出し、矢印キーの上で歩かせる。カメラはキツネの後ろに付く。

- ピントは追従カメラの注視点(キツネの少し上、2.8m)。決めの構図から入ると HUD `無限遠のボケ:3.7px`(画角は直前のショットのものを引き継ぐので、入る前の構図で少し変わる)。奥の突き当たりの壁と、画面の端の缶がぼける
- 「TAA」ページの `F2` で TAA も入れると、2つが同時に掛かる(全画面 18 パス: TAA 1 + 地図 4 + 被写界深度 3 + 明部 1 + ぼかし 8 + 合成 1)
...
--- 被写界深度の内訳 ---
  ...
  VRAM: 地図 2.7MB / 被写界深度 8.2MB  後処理の合計 24.9MB
  GPU の時間(タイマークエリの移動平均): 地図 0.090ms + 被写界深度 0.538ms
```

(『ゴミ箱の金属』で被写界深度を入れたときのもの。)**レンズの数字を順にたどる**のが眼目で、画角 → 焦点距離 → 口径 → ピント面の高さ → 錯乱円の、
どの段で数字が小さくなっているかが分かれば「ボケない」の理由が分かる。「半分の手前」と「無限遠」が同じ大きさ(符号だけ逆)なのが要点2の表。
**GPU の時間は走行ごとにぶれる**(VSync で休みながら測るので、GPU のクロックが上下する。Day 54a と同じ注意)。

### 8. `F8`: 自己チェックが 14 項目すべて合格

```
=== Day 55 自己チェック(錯乱円・ボケの集め方・後処理への差し込み)===
  [OK] **錯乱円はピントで 0、奥は L/S で頭打ち、手前は際限なく大きい**(ピントの半分の奥行きは無限遠と同じ大きさ)  ...
  ...
  すべて合格(錯乱円の式・ボケの集め方・後処理への差し込みが仕様どおり)
```

**GL を使わない項目が 4 つ**(レンズの式)、**128x64 の作り物の絵で走る項目が 4 つ**(ボケの集め方)。
`DepthOfField` が `Camera` を知らず、地図とテクスチャを受け取るだけの部品なので、ボケの式は本物のシーンを描かずに入力を作って確かめられる。

### 9. Day 38〜54b の絵と自己チェックが1つも壊れていない

**既定は OFF** なので、起動した直後の絵は Day 54b とまったく同じ。決めの構図と『ゴミ箱の金属』を同じ手順で撮った2枚ずつを比べ、
**違う画素は 0**(被写界深度を入れて切ったあとの絵も同じ)。
Day 48 の自己チェック(`Alt+F1`)は **224 項目**で通る。**被写界深度を入れたままでも**、
Day 54 の TAA(`RunTaaCheck`)、Day 53 の Forward+、Day 52 のディファード、Day 38 の FXAA(「SSAO」ページの「自己チェック(AA)」)の自己チェックは全部通る——
被写界深度は「このフレームのカメラの数字」が挿さっているときだけ掛かるので、キーで呼ぶ自己チェックには割り込まない(要点7)。
...
本物のレンズは、画面の端へ行くほど斜めに入る光が鏡筒の縁で削られ、丸いボケが**レモン形(猫目)**に欠ける(口径食)。
点のボケの形を「半径 rs の円」と「同じ半径の円を、画面の中心の向きに `k × (画面の中心からの距離) × rs` だけずらしたもの」の重なりにする。

`dof-gather.frag` の「相手の円が自分まで届くか」(`Cover`)を、「自分が相手の形(2つの円の重なり)の中にいるか」に置き換える。
**判定するのは相手の形**なので、ずらす向きと量は自分ではなく相手の画面の位置で決めること(散らしを集めるで書いた裏返しがここでも効く。要点4)。
円盤に撒く点の範囲は今日のままでよい(重なりは元の円より小さいので、探す半径は足りている)。

確かめ方は2つ。`k = 0` で今日の絵と1画素も変わらないこと。課題1と同じ手順で光を撒いてピントを手前に外すと、
**画面の真ん中のボケは丸いまま、四隅のボケが中心を囲むように細くなる**こと(レモンの長い向きが、中心からの向きと直角になる)。
...
| 解像度 | 960 x 640 |
| 確認の条件 | **VSync を入れて**確認した。計測(`glFinish` を挟んで連続で描く)は今日も無く、GPU の時間はタイマークエリで普段のフレームの中で測った |
...
| 素朴に集める(『ゴミ箱の金属』、ピントを奥に外す) | 散らしとして集めたときとの差 平均 0.17% |
| 全画面パス | FXAA だけ 11 / FXAA + 被写界深度 18 |
| VRAM | 地図 2.7MB / 被写界深度 8.2MB。後処理の合計は FXAA と被写界深度で 24.9MB |
| GPU の時間 | 地図 0.08〜0.18ms / 被写界深度 0.45〜0.58ms(走行ごとにぶれる) |
| 升目を縦横に分けた効き目(同じ構図) | 地図 0.35ms → 0.09ms |
| Day 54b との一致(OFF) | 決めの構図・『ゴミ箱の金属』とも、違う画素 0 |
...
=== Day 55 自己チェック(錯乱円・ボケの集め方・後処理への差し込み)===
  ...
  [OK] **深度から戻した奥行きと錯乱円が、CPU で計った値と一致**(地図は1回作って、被写界深度が読む)  箱の上面 4.41m / -0.32px(期待 4.43 / -0.31)  手前の床 4.79m / +0.18px(期待 4.79 / +0.19)  奥の床 6.86m / +2.00px(期待 6.86 / +2.00)
  [OK] **カメラの数字を挿したフレームだけ掛かる**(自己チェックが後処理を単独で回しても割り込まない)  挿さない 1 パス / 挿す 8 パス(地図 4 + 被写界深度 3)
  [OK] 代償: 地図は原寸の RG16F と升目3枚(横・縦・近所)、被写界深度は半分の大きさ3枚と原寸1枚  地図 2.7MB / 被写界深度 8.2MB(TAA の履歴は 9.4MB)
  [OK] 錯乱円の式が C# と GLSL で同じ形(L × (1/S − 1/z))
  [OK] **奥行きに戻すのは blur-field.frag の1か所だけ**(被写界深度は地図を読む。Day 50 の particle.frag にもう1つある)  今日のシェーダ 6 本のうち blur-field.frag
  [OK] 撒く点の数が C# とシェーダで同じ  被写界深度 48 点
  すべて合格(錯乱円の式・ボケの集め方・後処理への差し込みが仕様どおり)
```
...
`_cameraPlaying = false` の1行を足し、**Day 51〜54b の reference にも同じ場所・同じ文面で入れた**(`Mesh.ReadIndices` を Day 35 まで遡って直したのと同じ扱い)。
文面を揃えてあるので、Day 54b と今日の差分には出ない。すでに Day 51〜54b を写経していれば、その1行を足し直すこと。
...
比べる相手を「流し込んだ絵を読み返したもの」にすると差は 0 になった。**確かめたいものの手前で、入力が丸まっていないか**——Day 54a で深度テストを切ったまま描いていた話と同じ種類の、検査側の穴。
