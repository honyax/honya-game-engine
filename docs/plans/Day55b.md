# Day 55b: モーションブラー(シャッターの時間を、カメラに戻す)

Day 55 の2日目で、reference は Day 55a の完全コピー + 差分。
Day 55a でカメラの穴を点でなくした(被写界深度)。今日はもう1つの嘘、**シャッターが一瞬**をやめる。

| | Day 55a までのカメラ | Day 55b のカメラ |
|---|---|---|
| 光を溜める時間 | 一瞬。動いていても写らない | シャッターが開いている間。**動いた道のりに沿って線(尾)に伸びる** |
| 描き方 | 1回描いた絵を、深度を手がかりにぼかす | さらに**速度を手がかりに**、動いた向きへ流す(後処理の近似) |
| 手がかり | 深度(ぼけの地図) | **Day 54 の速度バッファをそのまま**。ぼけの地図と升目は Day 55a のものを読む |
| 払うもの | 全画面 7 パス(地図 4 + 被写界深度 3) | ブラーは全画面 1 パス + 原寸 1枚(4.7MB)。ブラーだけなら地図の 4 パスも要る |

**新しく作るものは少ない**。ぼけの地図(`BlurField`)は Day 55a で速度の口を空けたまま作ってあり、升目は「近所でいちばん速い速度」を
すでに記録できる形になっている。速度バッファは Day 54 からある。今日足すのは、それを読んで尾を集める1パス(`MotionBlur`)と、
**速度を描く条件の1行**だけ。

> **今日いちばん意外だったのは、ゆっくりのカメラワークではモーションブラーが目で拾えなかったこと**
> カメラワークの移動中に撮った ON と OFF は、見比べてもほとんど同じだった。升目を見ると尾の半分の長さは 1〜2 画素で、
> 60fps・180° では「1秒に 60 画素動く物の尾は 0.5 画素」にしかならない。
> **モーションブラーが見えるのは、1フレームに何画素も動くとき**(カメラを素早く振る、走る)。
> 30fps のゲームで効き目が大きいのは、1フレームの動きが倍になるから(要点1、検証の途中で分かったこと 1)。

## 今日のゴール

**メニューの「被写界深度とモーションブラー」ページで `F7` を押してモーションブラーを入れ、カメラを素早く振ると、
画面が振った向きに流れ、縦の目地だけがにじむ。**

| キー(「被写界深度とモーションブラー」ページ) | 何が起きるか |
|---|---|
| `F2`〜`F6` | 被写界深度・F 値・ピント・ボケの集め方・錯乱円を見る(Day 55a のまま) |
| `F7` | モーションブラー ON / OFF。**今日の到達点** |
...
ページの名前が「被写界深度」から「被写界深度とモーションブラー」に変わり、**内訳と自己チェックは `F7`・`F8` から `F12`・`1` へ移った**
(今日の5項目を被写界深度の後ろに差し込んだため)。
**既定はどちらも OFF** なので、起動した直後の絵は Day 55a と1ビットも変わらない(完成条件7 で画素ごとに確かめた)。

### 今日いちばん大事な2つ

**1つ目は「Day 54 の速度バッファを、そのまま読む」**。TAA のために作った「この画素は前のフレームでどこに居たか」は、
...
尾の長さは「1フレームの動き × シャッターが開いている割合」。割合をシャッター角で決めると、フレームレートが落ちたときに尾が伸びる(要点1)。

**2つ目は「尾も、散らすものを集めるで書く」**。Day 55a のボケと同じく、尾も本来は「1つの点が道のりに沿って光を配る」処理。
裏返して集めるとき、**読む向きを自分の速度ではなく、近所の升目でいちばん速い速度にする**のが要——
止まった背景の画素にも、すぐ隣を横切る物の尾は伸びてくるから。Day 55a の升目が速度も記録していたのはこのため(要点2)。

### 今日も Render とシェーダが主役

差分は `Render/` に新規1本(`MotionBlur`)、シェーダ1本(`motion-blur.frag`)、`PostProcess` のもう1段、そして `Program.cs`。
**`BlurField` とぼけの地図のシェーダ3本は1行も変わっていない**——Day 55a で空けておいた速度の口に、速度を挿すだけ。
...
- [Morgan McGuire, Padraic Hennessy, Michael Bukowski, Brian Osman, "A Reconstruction Filter for Plausible Motion Blur"(I3D 2012)](https://casual-effects.com/research/McGuire2012Blur/McGuire12Blur.pdf)
  — **今日のモーションブラーの一次資料**。升目の最大(TileMax)と近所の最大(NeighborMax)、手前・奥・両方の3つの場合で混ぜる重み(要点2)。
...
- [Jorge Jimenez, "Next Generation Post Processing in Call of Duty: Advanced Warfare"(SIGGRAPH 2014, Advances in Real-Time Rendering)](https://www.iryoku.com/next-generation-post-processing-in-call-of-duty-advanced-warfare)
  — **被写界深度とモーションブラーを同じ枠組みで扱った資料**(Day 55a でも読んだ)。
  画素ごとに読む位置をずらすノイズ(interleaved gradient noise)の出どころ(要点2)
- [Gilberto Rosado, "Motion Blur as a Post-Processing Effect"(GPU Gems 3 第 27 章)](https://developer.nvidia.com/gpugems/gpugems3/part-iv-image-effects/chapter-27-motion-blur-post-processing-effect)
  — **一世代前の定番**。深度からカメラの動きだけで速度を作るモーションブラー(Day 54a の速度と同じ発想)。
  今日のものと比べると、升目と「手前・奥の3つの場合」が無いぶん何を取りこぼすかが見える
- **Day 54a の計画書 要点4**・**Day 54b の計画書 要点1** — 速度バッファの中身(向きと単位)、自分で動いた物の速度
- **Day 55a の計画書 要点4・5** — 散らすものを集めるで書くこと、升目

## 理論の要点

### 1. モーションブラー — 速度バッファを読み、シャッターが開いている割合を掛ける

Day 54 の速度バッファは「この画素は前のフレームで画面のどこに居たか」(UV、前 → 今)。これはそのまま**1フレームの間の動き**でもある。

```
  尾の長さ [画素] = 1フレームの動き [画素] × シャッターが開いている割合
  尾は今の位置を真ん中にして前後に伸ばす(片側は半分)。半分の長さの上限は 16 画素(升目の大きさ。Day 55a の要点5)
```
...
### 2. 尾の混ぜ方 — 手前・奥・両方の3つの場合(McGuire 2012)

尾も「点が道のりに沿って光を配る」処理なので、Day 55a のボケと同じく裏返して集める(Day 55a の要点4)。読む向きは**近所の升目でいちばん速い速度**。
...
### 3. 並び — 被写界深度の後ろ、ブルームの前、HDR のまま

モーションブラーは Day 55a の被写界深度のすぐ後ろに置く。TAA の後ろ・ブルームの前・HDR のままという理由は被写界深度と同じ(Day 55a の要点7)で、
尾も「近所の光の量を混ぜる」処理なので、明るい点はまず線に伸ばしてからにじませる。

2つの並びは 被写界深度 → モーションブラー。本物のカメラでは同じ露光の中で同時に起きることなので、どちらを先にしても近似になる
(どちらの段も、ぼける前の輪郭について測った奥行きと速度を読んでいる。今日残した歪みの3つ目)。

**地図は2人ぶんを1回だけ作る**。被写界深度だけのフレームは速度を読まず(Day 55a のまま)、ブラーだけのフレームはレンズの係数を 0 にする(錯乱円が全部 0)。
両方なら、速度もレンズの係数も入れて1回。別々に作ると、地図の4パスが2回になる。

**モーションブラーも「このフレームのカメラの数字」と速度が挿さっているときだけ掛かる**(`PostProcess.Frame` と `Velocity`)。
自己チェックが後処理を単独で回しても割り込まない(挿さない 1 パス / 挿す 9 パス)ので、Day 38〜55a の自己チェックは入れたままでも全部通る。
...
| 半透明の尾 | 粒・トレイルは速度を持たないので、後ろの物の尾を借りる(火花が横切っても火花自身は流れない) | 今日残した歪みの1つ目 |
| 露出との連動 | 本物のカメラは F 値とシャッターで明るさも変わる。今日は尾だけで、明るさは露出(Day 31)が別に決める | 改造課題2(物理カメラの EV100) |
...
| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Render/PostProcess.cs` | `MotionBlur` を持ち、被写界深度の後ろで掛ける。地図はどちらか片方でも作る(要らない側を 0 に) | +54 / -26 |
| `Render/DepthOfField.cs` | コメントのページ名(「被写界深度」→「被写界深度とモーションブラー」)と、後ろの段の名前 | +6 / -6 |
| `Program.cs` | 速度を描く条件、`PrepareCameraBlur` の条件、HUD、メニュー 5 項目、内訳のシャッター、自己チェック 5 項目 | +294 / -57 |

**`Program.cs` の差分の4割は自己チェック**(尾の4項目とシャッターの式で 120 行ほど)。
本番の流れで変わったのは、`PrepareCameraBlur` の条件と、速度を描く2つの条件だけ——**何を描くかは今日も1行も書き換えていない**。

Day 55a のコードに入った手直し(差分の -89 行はこれで、どれも「被写界深度だけ」だった条件や文面に「モーションブラー」を足したもの)。
...
| カメラの数字を挿す条件 | `PrepareCameraBlur` | ブラーだけでも挿す。深度を写すのは「TAA でもブラーでもない」ときだけ(どちらかなら `RenderMotionVectors` が写してある) |
| 地図を作る条件 | `PostProcess.EndCore` | ブラーだけでも地図を作る(レンズの係数を 0 に)。速度の口に `Velocity` を挿す |
| ページの名前、内訳と自己チェックの席、件数 224 → 229 | `BuildDebugMenu` / `RunDebugMenuCheck` | 今日の5項目を被写界深度の後ろに差し込んだ |
| 自己チェックの「挿す」パスの数 8 → 9、控えるつまみ | `RunCameraBlurCheck` / `CameraBlurSettings` | ブラーの1パスとシャッターのつまみ |

### 写経する順番

```
 1. shaders/motion-blur.frag       尾を集める(新規。2 が読む)
 2. Render/MotionBlur.cs           モーションブラー(新規。Day 55a の BlurField を使う)
 3. Render/PostProcess.cs          被写界深度の後ろにもう1段を置く(2 を使う)
 4. Render/DepthOfField.cs         コメントのページ名だけ
 5. Program.cs                     全部を繋ぐ
 6. Day55b.csproj                  Day55a.csproj からのリネームのみ
```

**依存順に並べてある**。シェーダ(1)は実行時に読むのでビルドには効かないが、<b>5 より前に写しておく</b>こと
(`PostProcess` のコンストラクタが `MotionBlur` を作るときに読むので、写し忘れると起動の時点で落ちる)。
`MotionBlur.cs` は `OpenFraction`(シャッターが開いている割合)が要。

`Program.cs` の中は次の順で読むとよい。

```
  フィールド(ShutterAngleSteps / _renderSeconds)
    → BeginTemporalFrame と RenderMotionVectors の条件           速度を読むのが2人になった
    → Day 55 の節の頭(UseMotionBlur / ShutterLabel / PrepareCameraBlur の条件)
    → CameraBlurLabel と DescribeCameraBlur のブラーの行          HUD と内訳
    → RunCameraBlurCheck の「尾」の節とシャッターの式             自己チェック
    → BuildDebugMenu の末尾(5 項目を差し込む)と RunDebugMenuCheck の 229
```

## 設計書

**層は今日も増えていない**。`Render/` に型が2つ(`MotionBlur` と列挙1つ)増えて、`PostProcess` の段が1つ増えただけ。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Render/MotionBlur` | `Render/` | シャッターと1パス。**`MotionVectors` を知らない**(速度はテクスチャで受け取る) |
| `Render/ShutterMode` | `Render/` | シャッターの決め方の列挙 |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Render/PostProcess` | `MotionBlur`、被写界深度の後ろの1段、地図を片方でも作る | +54 / -26 |
| `Render/DepthOfField` | コメントのページ名 | +6 / -6 |
| `Program.cs` | 速度を描く条件、`PrepareCameraBlur` の条件、メニュー、自己チェック | +294 / -57 |
...
**TAA と FXAA の間に、HDR のまま差し込んだ**。Day 54a で「TAA は入口、FXAA は出口」と置いた並びの、入口側に2段が続く形(被写界深度は Day 55a、ブラーは今日)。
被写界深度もブラーも「近所の光の量を混ぜる」処理なので、光の量として足し算が効く HDR のうちに置く(要点3)。
...
それを TAA も読み、ブラーも読む。Day 54a の設計書に「速度を作る側を1行も変えずに、読む側を足すだけで済む形になっている」と書いた、その形になった。
...
「ピント 2m、無限遠のボケ 8 画素」「1 フレーム 16 画素で動く棒」のような数字を直接並べて、ボケと尾の式を確かめられる。

**`BlurField` の地図は、2人が1回作ったものを読む**(Day 55a で速度の口を空けておいたのがここで効く)。被写界深度だけのフレームは速度を読まず(`uHasVelocity = 0`)、
...
**今日から速度が流れ込む**(Day 55a では `uHasVelocity = 0` で閉じていた口)。**速度は長さで比べて、向きごと覚える**。x と y を別々に最大にすると、どの物も動いていない斜めの向きができあがる(`blur-tiles.frag` / `blur-max.frag`)。

**同じ `blur-max.frag` を2回使う**。「窓の中の最大」を、窓の大きさと歩幅を uniform で変えて、升目の縦の段((1, 16) の窓)と近所の段((3, 3) の窓)に使い回す。
これが Day 55a で `Shader.SetInt2` を足した理由。
...
**1つ目: 半透明は速度も持たない**。粒・トレイルは深度を書かず、`Program.Draw` を通らないので記録もされない。
被写界深度で後ろにある物のボケ方を借りていた(Day 55a の歪みの2つ目)のと同じく、
モーションブラーでは**後ろにある物の尾を借りる**(火花が横切っても火花自身は流れない)。Day 54b の歪みの3つ目の効き目が増えた。
本物は半透明を別の層に描いて、ボケと尾を後から重ねる。

**2つ目: 原寸の RGBA16F がもう1枚増えた**。TAA の履歴2枚、被写界深度の出力、ブラーの出力で、TAA と両方を入れると後処理の VRAM が 39.0MB になった
(Day 54 は TAA で 23.4MB)。被写界深度の出力とブラーの出力は同時に要らないので、使い回せば1枚減る。
段ごとに「自分の出力を持つ」形のほうが、どの段を切っても他が壊れないので、今日も分かりやすさを取った。

**3つ目: 2つの段は、ぼける前の輪郭で測った奥行きと速度を読む**。被写界深度のあとの絵では、手前の物のボケが輪郭の外の背景に被さっているが、
その画素の速度は背景の速度のまま。モーションブラーはそこを背景の動きで流す。逆の並びでも別の食い違いが出るので、
後処理を2段に分けた以上は避けられない(本物のカメラでは同じ露光の中で同時に起きる)。

**Day 55a・54b の歪みのうち、今日動いたもの**:

| 歪み | 今日 |
|---|---|
| 深度を写す場所が3つ(Day 55a の1つ目) | そのまま。ブラーを入れていれば `RenderMotionVectors` が写すので、`PrepareCameraBlur` は写さない |
| 半透明は奥行きを持たない(Day 55a の2つ目) | **尾も借りるようになった**(今日の歪みの1つ目) |
| ピントの手がかりが注視点だけ(Day 55a の3つ目) | そのまま |
| 原寸の RGBA16F が段ごとに増える(Day 55a の4つ目) | **もう1枚増えた**(今日の歪みの2つ目) |
| 記録の鍵が「メッシュと何回目か」(Day 54b の1つ目) | そのまま。**モーションブラーでは、嘘の速度が1フレームだけ尾として見えうる**(TAA では疑いが隠していた) |

Day 55a の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。変わった図は次の4つ。

| 図 | 何が変わったか |
|---|---|
| `Render` のクラス図 | **`MotionBlur` と列挙 `ShutterMode` が増えた**。`PostProcess` に `MotionBlur` / `MotionBlurApplied` |
| `Demo` のクラス図 | 図は変わっていない(ページは 23 枚のまま)。キツネがモーションブラーの的にもなることを書き足した |
| `1フレームの流れ` | 速度を描く条件と記録を始める条件にモーションブラー。`PrepareCameraBlur` はブラーでも挿す |
| `HDR パイプラインの中身` | 被写界深度の後ろにブラー |

設計書の冒頭の図は、Day 55a の4つのうち3つを描き直し、1つを足した。

| 図 | 何を描いたか |
|---|---|
| 後処理の並び | 被写界深度の後ろにブラー。2つを並べた表 |
| 今日足したものが繋がった場所 | `MotionBlur` が `MotionVectors` を、2人とも `Camera` を知らないこと |
| ぼけの地図ができるまで | 速度が升目に流れ込む |
| 1画素の尾が決まるまで(新しく足した) | 手前・奥・両方 |

「1画素のボケが決まるまで」は Day 55a のまま(Day 55a の計画書を見ること)。
...
### Render — OpenGL の薄い皮(Day 55b で `MotionBlur` が増えた)
...
### HDR パイプラインの中身 — 11 回のフルスクリーンパス(Day 55b で被写界深度の後ろにブラーが入った)
...
`dotnet run --project reference/Day55b -c Release` で起動する。
...
カメラワーク(「カメラワークと機能」の `F2`)は動きがゆっくりなので、流れは細かいところでしか分からない(検証の途中で分かったこと 1)。
...
### 3. `F9` と L キー: シャッターの決め方

2 のキツネのまま(升目は ON に戻す)、メニューを閉じて L を3回押す(1ステップ 20ms の負荷。1フレーム 161ms、6.2fps まで落ちる)。
...
- `F2` で被写界深度も入れると、奥の突き当たりの壁と、画面の端の缶がぼける(Day 55a の完成条件6)
...
(『ゴミ箱の金属』で被写界深度だけを入れたときのもの。ブラーの行はまだ作っていないので 0。)
Day 55a の内訳に、**シャッターの2行**が増えた。「いまのフレーム」は `OnRender` の間隔で、開いている割合と露光の時間がそこから出る——
角度で決めるときは割合が固定で露光の時間がフレームに連れて伸び、秒で決めるときは露光の時間が固定で割合が動く(要点1)。

### 6. `1`: 自己チェックが 19 項目すべて合格
...
Day 55a の 14 項目に、**GL を使わない項目が 1 つ**(シャッターとフレームレート)、**作り物の絵で走る項目が 4 つ**(尾)増えた。
`MotionBlur` が `Camera` も `MotionVectors` も知らず、速度をテクスチャで受け取るだけの部品なので、尾の式も本物のシーンを描かずに入力を作って確かめられる
(1 フレーム 16 画素で右へ動く幅 4 画素の棒を、速度バッファに直接書く)。

### 7. Day 38〜55a の絵と自己チェックが1つも壊れていない

**既定はどちらも OFF** なので、起動した直後の絵は Day 55a とまったく同じ。決めの構図と『ゴミ箱の金属』を同じ手順で撮った2枚ずつを比べ、
**違う画素は 0**(被写界深度だけを入れた絵も同じ)。
...
2つの段は「このフレームのカメラの数字」が挿さっているときだけ掛かるので、キーで呼ぶ自己チェックには割り込まない(要点3)。

**IBL の自己チェック(Day 36)はデモ v1 を出す前に走らせる**こと(Day 52〜55a と同じ注意)。

## 改造課題

### 課題1(易): 読む位置のずらしを切って、縞を見る

`motion-blur.frag` は尾に沿って 15 点を読むとき、読む位置を画素ごとに少しずらしている(interleaved gradient noise。要点2)。
**このずらし(`jitter`)を 0 にする**と、明るい物の尾が 15 枚の写しになって縞に見える。
カメラを素早く振り、壁のレンガの明るい目地や、「ディファード」ページで撒いた光(`F4` で夜、`F3` で点光源 64 個)の尾を見比べる。
ずらしは縞を細かいざらつきに変えているだけで、**光の量は変えていない**——ON と OFF で絵全体の平均がほとんど変わらないことも確かめる。

### 課題2(中): 露出を F 値とシャッターから決める(物理カメラ)

いまの明るさは露出(Day 31)が別に決めていて、F 値を絞っても、シャッターを速くしても絵は暗くならない。
本物のカメラは、**F 値の2乗に反比例し、露光の時間に比例して**光を溜める。写真の露出値 EV100 は

```
  EV100 = log2(F 値² / 露光の時間[秒])        (感度 ISO 100 のとき)
```

で、露出を `1 / (1.2 × 2^EV100)` にすると、F 値とシャッターから明るさが決まる(Frostbite の物理カメラの式)。
`MotionBlur.OpenFraction` × フレームの時間が露光の時間なので、それと `DepthOfField.FNumber` から露出を出して合成の段に渡す。
確かめ方: F 値を 1.4 → 2.8 にすると絵が 1/4 の明るさになり、シャッター角を 180° → 360° にすると 2 倍になる。
**「よくボケる絵は明るく、よく流れる絵も明るい」**——被写界深度とモーションブラーが、露出という同じ財布から払っていることが見える。
...
カメラの動きだけの速度は、Day 54a の速度(`camera-velocity.frag`)と同じ式——深度から戻した点を前のフレームの行列で写し直したもの。
`MotionVectors` に前のフレームのビュー射影を読む口を足し、`CameraFrame` に再投影の行列を入れて、`motion-blur.frag` で同じ計算をして引く
...
| Day 55a との一致(両方 OFF、被写界深度だけ ON) | 決めの構図・『ゴミ箱の金属』とも、違う画素 0 |
...
**1. ゆっくりのカメラワークでは、モーションブラーが目で拾えなかった**(今日の冒頭の話)。カメラワークの移動中に撮った ON と OFF は、見比べてもほとんど同じだった。
升目を見ると尾の半分の長さは 1〜2 画素で、60fps・180° では「1秒に 60 画素動く物の尾は 0.5 画素」にしかならない。
**モーションブラーが見えるのは、1フレームに何画素も動くとき**(カメラを素早く振る、走る)。30fps のゲームで効き目が大きいのは、1フレームの動きが倍になるから(要点1)。
