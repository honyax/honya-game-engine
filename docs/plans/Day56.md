...
ただし今日の点光源の強さは「距離 0 のときの明るさ」で、分母に `+ 1` の入った嘘の単位(Day 52 の要点)なので、この式に乗らない。
...
「デモ v1」の決めの構図(止まった1枚)は過焦点距離で動きも無いので(Day 55 の検証の途中で分かったこと 1・自己チェック「止まった絵は1画素も変わらない」)、
...
| 光る面がまわりを照らす | 看板の桃色は、看板の手前に置いた点光源が出している。発光そのものは何も照らさない | 動的 GI(Lumen・DDGI。教養編 Day 66 の講読)、パストレーサ(Day 60) |
...
```
 ...
 3. Program.cs               全部を繋ぐ
 4. Day56.csproj             Day55.csproj からのリネームのみ
```
...
| Day 52-1 / Day 54-3 | 同じ BRDF が3か所、スキニングの式が4か所 | そのまま | シェーダの読み込みに `#include` を足す(エンジンが文字列を差し込むだけ) |
| Day 52-2 | 影のパスだけが描くものを手で書き写している | そのまま | 描くものの一覧を1つ持ち、パスごとにシェーダだけ変える |
| Day 52-3 | 位置の精度が 16F(ディファード) | そのまま(完成版は Forward+ なので効かない) | 深度バッファから位置を戻す(Day 52 の改造課題2) |
| Day 52-4 | 光1個 = ドローコール1回(ディファード) | そのまま(完成版は Forward+ なので効かない) | インスタンシング(Day 52 の改造課題3) |
| Day 52-5 | ディファードに乗れない絵がある | そのまま | 完成版は Forward+ を選んで避けた(要点5) |
...
| Day 54-1 | ずらしを知らない場所が3つ | そのまま(半画素以内) | 射影の非対角も送る(Day 54 の改造課題3) |
| Day 54-2 | 記録の鍵が「メッシュと何回目か」 | そのまま。光る板は `Items` の末尾に足すので、**灯りを足しても前に並ぶ物の鍵はずれない** | 物ごとに消えない番号を持つ |
| Day 54-4 / Day 55-1 | 深度を写す場所が3つ | そのまま | 不透明を描き終えたところで1回写す持ち主を決める |
| Day 54-5 / Day 55-2 | 半透明は奥行きも速度も持たない | そのまま | 半透明を別の層に描いて、ボケと尾を後から重ねる |
| Day 55-3 | ピントの手がかりが注視点だけ | そのまま(追従カメラはキツネの上を見るので合う) | オートフォーカス(Day 55 の改造課題2) |
| Day 55-4 | 原寸の RGBA16F が段ごとに1枚ずつ増える | そのまま(後処理 39.0MB) | 被写界深度とブラーの出力を使い回す |
| Day 55-5 | 被写界深度とブラーが、ぼける前の輪郭で測った奥行きと速度を読む | そのまま(後処理を2段に分けた以上は避けられない) | 何枚も描いて平均する(オフラインの手法) |
...
Day 55 の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。変わった図は次の2つ。
...
新しく足した図が3つ(この節の上に並べたもの)。Day 55 の設計書の冒頭にあった5つの図(後処理の並び、ぼけの地図ができるまで、など)は
被写界深度とブラーの話なので、今日は外した(Day 55 の計画書を見ること)。
...
| 消火栓と灯りのボケ | 画角 30 度・ピント 2.6m。**消火栓がくっきり、奥の缶と蛍光灯が丸いボケ**(Day 55 の要点3: 望遠ほどボケる) |
...
**効いていたのは空(IBL)の明るさ**だった。TAA は明るさで重みを付けて混ぜる(Day 54 の要点8)ので、明るい縁ほど 64 点の答え(光の量の平均)から離れる。
