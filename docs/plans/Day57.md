# Day 57: コンピュートシェーダ入門(GPU に 100 万粒を動かさせる)

**教養編(Day 57〜67)の初日**。ロードマップ本編(Day 1〜56)は昨日で終わっている。
ここから先はデモに載せない題材を、**理解のためだけに**手で確かめる。置き場所も別で、エンジン本体ではなく `Sandbox/` に置く。

今日やることは1つ。**粒の配列を VRAM に置きっぱなしにして、CPU が1フレームに「ディスパッチ3回と描画1回」しか出さない状態を作る**。

| | Day 49 の粒(CPU) | Day 57 の粒(GPU) |
|---|---|---|
| 粒の住まい | C# の `Particle[]`(主記憶) | **SSBO**(VRAM)。生まれてから死ぬまで動かない |
| 進める | CPU が全部を舐める。1万粒で 0.1ms | **`glDispatchCompute` 1回**。100 万粒で **0.2ms** |
| 撒く | `ParticleEmitter` が空きを探して詰める | **死んだ粒が自分で生まれ直す**(空き探しが要らない) |
| 描く | CPU が四隅を作って 960KB を毎フレーム送る | **1バイトも送らない**。同じバッファを頂点として読む |
| 数を数える | `foreach` で足す | **共有メモリで畳んでから `atomicAdd`** |
| 出せる数 | 1万粒(それ以上は CPU が保たない) | **100 万粒で 60fps**(GPU の時間は合計 0.7ms) |
| 乱数 | `Random` を1つ持って順に引く | **番号を混ぜて散らす**(状態を持てない) |

今日の差分は `Sandbox/` に新規1本、シェーダ4本、そして `Shader` / `RenderResources` / `Program.cs`。
**`Render/` の描画パス(Day 52〜56)には1本も線が伸びていない**——教養編の題材はエンジン本体から切り離しておく。

> **今日いちばん意外だったのは、「共有メモリで畳んだほうが速い」が、このカードでは出なかったこと**
> 100 万個の `atomicAdd` を2つの番地にぶつける素朴な書き方と、共有メモリで畳んでから 4096 回だけぶつける書き方を、
> **同じフレームの中で続けて** 7 通りの条件で測ったら、差は **±7% の中**に収まり、**どちらが速いかも測るたびに入れ替わった**。
> VRAM を叩く回数は 256 倍違うのに、である。理由は2つあって、
> (1) 最近の GPU はワープの中の atomic をまとめてから1回だけ発行する、
> (2) そもそも**どちらも 32MB を読むところで頭打ち**になっている(実測 373〜399GB/s。RTX 3070 の帯域は 448GB/s)。
> しかも**最初は「共有 1.22ms ↔ atomic 0.63ms で2倍」という数字を出していた**——
> 片方ずつ切り替えて測っていて、その間に GPU のクロックが変わっていた(検証の途中で分かったこと 1)。
> 畳み込みの値打ちは「速いから」ではなく「**VRAM を叩く回数を設計で決められるから**」のほうにある(要点6)。

## 今日のゴール

**メニューの「コンピュート」ページで `F2` を押すと、床の上に 100 万粒の噴水が立つ。
`F3` で粒を 100 万(既定は 26 万)にしても **fps は 60 のまま**。
`F4` で引力と渦に切り替わり、`F5` でワークグループの大きさを変えても**絵は1ピクセルも変わらない**。
`F8` を押すと、同じ 100 万個を C# で1ステップ回した時間(4.6ms)が出て、GPU の 23 倍だと分かる。**

| キー(「コンピュート」ページ) | 何が起きるか |
|---|---|
| `F2` | GPU パーティクル ON / OFF。**今日の到達点**。VRAM に粒を置いて、毎フレーム dispatch する |
| `F3` | 粒の数(26万 → 100万 → 6.5万 → …。既定は 26 万)。**CPU 側の仕事は1つも増えない** |
| `F4` | 筋書き(噴水 / 引力 / 渦)。3つとも同じ1本のシェーダの中の分岐 |
| `F5` | ワークグループの大きさ(64 / 128 / 256 / 1024)。**シェーダを焼き直す**(`#define`) |
| `F6` | 数え上げ(両方 / 共有メモリだけ / atomic だけ)。**既定は両方**——同じフレームで測らないと比べられない |
| `F7` | 毎フレーム読み戻す。**8 バイト読むだけで並走が切れる** |
| `F8` | CPU で同じ数を1ステップ回して時間を測る(1回だけ) |
| `F9` | 内訳(実行モデルの上限・バッファ・ディスパッチ・GPU の時間・集計) |
| `F10` | 今日の自己チェック(10 項目) |

メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「コンピュート」(25 枚目)が直接開く。
**GPU パーティクルを出さない限り、絵は Day 56 と1ビットも変わらない**
(起動直後・デモ v1・決めの構図・完成版の4枚を画素で突き合わせた。完成条件 8)。

### 今日いちばん大事な3つ

**1つ目は「コンピュートは、出口の無いシェーダ」**。頂点シェーダには「次の段へ渡す頂点」、画素シェーダには「フレームバッファの画素」という
決まった出口があった。コンピュートシェーダには**出口が無い**。代わりに、自分で書いた場所がそのまま結果になる。
だから GL は「どこに何を書いたか」を知らず、**書いたものを別の経路で読む前に `glMemoryBarrier` が要る**(要点5)。

```glsl
// particle-sim.comp。**この1本が 100 万回走る**
layout (local_size_x = 256) in;                    // ← 塊の大きさ(コンパイル時に決まる)
layout (std430, binding = 0) buffer Particles { Particle particles[]; };

void main()
{
    uint index = gl_GlobalInvocationID.x;          // 自分は通しで何番目か
    if (index >= uCount) { return; }               // ← 切り上げたぶんの番人(要点2)
    ...
    particles[index] = p;                          // ← 出口はこれだけ
}
```

**2つ目は「バッファは型を持たない」**。OpenGL のバッファオブジェクトは**ただのバイト列**で、何であるかは挿した口が決める。
`GL_SHADER_STORAGE_BUFFER` に挿せば SSBO、`GL_ARRAY_BUFFER` に挿せば頂点バッファ。
**同じ1本を両方に挿せる**ので、コンピュートが書いた結果を、写し替え無しでそのまま描ける(要点4)。
これが無いと 100 万粒 = **32MB を毎フレーム CPU へ往復**することになり、GPU で計算した意味がそこで消える。

**3つ目は「速いのは計算。高いのは持って帰ること」**。100 万粒を進めるのが 0.18ms、数えるのが 0.09ms なのに対して、
**その結果の 8 バイトを CPU が読むと 0.1〜3.5ms 止まる**(要点7)。理由はバイト数ではなく、
`glGetBufferSubData` が「その時点の中身」を約束するために**GPU が追いつくまで CPU を待たせる**から。
Day 54 の `GpuTimer` がクエリを輪にして「数フレーム前の結果」を拾っていたのと、まったく同じ形の問題になる。

### 今日は Sandbox とシェーダが主役

差分は `Sandbox/` に新規1本(`GpuParticles`)、`shaders/` に新規4本、そして `Render/Shader.cs` の作り直しと `Program.cs`。

| どこ | 何をしたか |
|---|---|
| `Render/Shader.cs` | **段を配列にした**(頂点+画素の2本固定をやめた)。コンピュート用の入口、`#define` の差し込み、`Dispatch` / `Barrier` |
| `Render/RenderResources.cs` | `LoadComputeShader`。**キーに `#define` を混ぜる**(同じファイルの違うバリアントを別物として数える) |
| `Sandbox/GpuParticles.cs` | SSBO・VAO・3本のシェーダ・タイマー4本。**GL を触るのはここだけ** |
| `Program.cs` | 文脈を **4.3** へ、ページ1枚、1フレームに3行、内訳と自己チェック |

**`Render/` の描画パスもシェーダも、既存のものは1行も変わっていない**。
GPU パーティクルは `_post.Begin` と `_post.End` の間に自分で描くだけで、G-Buffer にも影にも SSAO にも関係しない。

## 事前に読む資料

- [OpenGL Wiki: Compute Shader](https://www.khronos.org/opengl/wiki/Compute_Shader)
  — **今日の要点1・2の一次資料**。`local_size` と `gl_GlobalInvocationID` の定義、`glDispatchCompute` の引数の意味、
  `shared` と `barrier()` の制約(「ワークグループの全員が同じ回数通ること」)
- [OpenGL Wiki: Shader Storage Buffer Object](https://www.khronos.org/opengl/wiki/Shader_Storage_Buffer_Object)
  — 要点3・4。`std430` の詰め方(**`vec3` は 16 バイトに整列する**)と、末尾の可変長配列
- [OpenGL Wiki: Memory Model](https://www.khronos.org/opengl/wiki/Memory_Model)
  — 要点5。`glMemoryBarrier` のビットが何と何の間の順序を保証するのか。
  **「書いた」と「他の経路から読める」が別である**という前提そのものがここに書いてある
- [Mark Harris, "Optimizing Parallel Reduction in CUDA"](https://developer.download.nvidia.com/assets/cuda/files/reduction.pdf)
  — 要点6の教科書。共有メモリで木のように畳む形と、その段階的な速度改善。CUDA だが考え方はそのまま GLSL に載る
- [NVIDIA, "CUDA Pro Tip: Optimized Filtering with Warp-Aggregated Atomics"](https://developer.nvidia.com/blog/cuda-pro-tip-optimized-filtering-warp-aggregated-atomics/)
  — **今日の測定結果の答え合わせ**。ワープの中の atomic をハードウェアとコンパイラがまとめる話。
  「素朴な atomicAdd が思ったより遅くない」のはこれが理由
- **Day 49 の計画書**(CPU のパーティクル)・**Day 53 の計画書 要点**(TBO を選び、SSBO を選ばなかった理由)・
  **Day 54 の計画書**(`GpuTimer` と、GPU の時間を測る作法) — 今日はこの3つの続きになる

## 理論の要点

### 1. コンピュートシェーダ — 出口の無いシェーダと、ワークグループ

描画のシェーダには**決まった入口と出口**がある。頂点シェーダは頂点属性を受け取って `gl_Position` を返し、
画素シェーダは補間された値を受け取ってフレームバッファへ色を返す。間の段(ラスタライズ・深度テスト・ブレンド)は GPU の固定機能。

コンピュートシェーダには、その入口も出口も無い。**あるのは「自分が何番目か」だけ**。

| | 頂点 / 画素 | コンピュート |
|---|---|---|
| 入口 | 頂点属性 / 補間された値 | **番号(`gl_GlobalInvocationID`)だけ** |
| 出口 | `gl_Position` / `FragColor` | **無い**。自分で書いた場所が結果 |
| 何個走るか | 頂点の数 / 画素の数(GPU が決める) | **`glDispatchCompute` で人が決める** |
| 走る単位 | ワープ/ウェーブ(見えない) | **ワークグループ**(見える。大きさを決められる) |

呼び出しは「ワークグループ」という塊に分かれて走る。

| | 決まるところ | 今日の値 |
|---|---|---|
| ワークグループの大きさ | `layout (local_size_x = N) in;`。**コンパイル時の定数** | 64 / 128 / 256 / 1024(`F5`) |
| ワークグループの数 | `glDispatchCompute(x, y, z)`。**実行時** | 100 万 ÷ 256 = 4096 |
| 呼び出しの総数 | 大きさ × 数 | 1,048,576 |

塊に分かれているのは**GPU の作りがそうだから**。1つのワークグループは必ず1つの演算ユニット
(NVIDIA なら SM、AMD なら CU)に載り、**その中でだけ共有メモリと `barrier()` が使える**。
別のワークグループどうしは、走る順番も、同時に走るかどうかも決まっていない——だから**グループをまたいで待ち合わせる手段は無い**。

**大きさを uniform にできない**のは、`shared` 配列の大きさとして使うから。
配列の大きさはコンパイル時に決まっていなければならないので、64 と 256 を見比べるには**焼き直すしかない**。
そこで `#version` の次の行に `#define LOCAL_SIZE 64` を差し込んでコンパイルし直す(`Shader.InjectDefines`)。
本物のエンジンが「シェーダバリアント」と呼んでいるものの、いちばん小さい形になる。

### 2. 切り上げと番人 — ディスパッチの数え違いは静かに壊れる

`glDispatchCompute` に渡すのは**ワークグループの数**であって、呼び出しの数ではない。
100 万粒を 256 ずつ処理するなら、渡すのは 3907(= ⌈1,000,000 / 256⌉)。

**ワークグループは丸ごとしか走らせられない**ので、3907 × 256 = 1,000,192 呼び出しになり、**192 個が余る**。
余ったぶんが `particles[1000000]` 以降を書かないように、シェーダの先頭で弾く。

```glsl
uint index = gl_GlobalInvocationID.x;
if (index >= uCount) { return; }     // ← これが無いと配列の外を書く
```

書き忘れたときの壊れ方が厄介で、**SSBO の範囲外アクセスは落ちない**。仕様は「書き込みが捨てられるか、
バッファの中のどこかに落ちる」と決めていて、後者なら**関係ない粒が壊れる**。
落ちてくれないぶん、原因が「粒が数個だけ変な方向に飛ぶ」という形でしか出てこない。

逆方向の間違い——**粒の数をそのままディスパッチに渡す**——もよく起きる。
これは 256 倍のワークグループが走るだけで**答えは合う**ので、絵を見ても気づけない。
今日の数(6.5万・26万・100万)はどれも 256 の倍数なので余りが 0 になる。
自己チェックが 1,000,000 という割り切れない数をわざわざ持ち出しているのはそのため(完成条件 7 の3項目め)。

### 3. SSBO と std430 — C# の構造体と1バイトずつ合わせる

SSBO(シェーダストレージバッファ)は **OpenGL 4.3 で入った、シェーダから読み書きできる大きなバッファ**。
Day 53 で光の一覧を渡すのに TBO(テクスチャバッファ)を選んだのは、あのときのエンジンが 3.3 だったからで、
**4.3 に上げた今は SSBO がそのまま使える**(Day 53 の計画書の表の3行目)。

| | UBO | TBO(Day 53) | SSBO(今日) |
|---|---|---|---|
| 入った版 | 3.1 | 3.1 | **4.3** |
| 大きさ | 16KB(保証) | 65536 テクセル(保証) | **実質 VRAM いっぱい** |
| 書けるか | 読むだけ | 読むだけ | **読み書きできる** |
| 詰め方 | std140 | テクセル形式 | **std430** |

`std430` は詰め方の決まりで、**std140 との違いが落とし穴になる**。std140 は配列の要素を必ず 16 バイト境界に丸めるので、
`float` の配列が4倍に膨らむ。std430 はそれをしないので、C# の構造体とそのまま突き合わせられる。

ただし **`vec3` が 16 バイトに整列する**(12 ではない)という規則は std430 にも残る。今日の粒はそこを踏まえて組んである。

```
   0    4    8   12   16   20   24   28   32
   +----+----+----+----+----+----+----+----+
   |  position (vec3)  |age |  velocity (vec3) |life|
   +----+----+----+----+----+----+----+----+
```

`vec3` のうしろに `float` を置いて 16 バイトぴったりにする、という組み方。
**`age` を抜くと `velocity` の前に 12 バイトの穴が空いて、C# 側と 12 バイトずれる**。
ずれると「速度を位置として読む」ことになり、粒が明後日の方向へ飛ぶだけで**エラーは1つも出ない**。
自己チェックが `Marshal.OffsetOf` で 12 / 16 / 28 を確かめているのはこのため。

### 4. バッファは型を持たない — SSBO のまま頂点として読む

**今日いちばん効く1手**。OpenGL のバッファオブジェクトは中身に型を持たない**ただのバイト列**で、
「何であるか」は挿した口(ターゲット)が決める。

```csharp
// GpuParticles.CreateVertexArray。**同じ _particleBuffer を GL_ARRAY_BUFFER へ挿す**
_gl.BindVertexArray(array);
_gl.BindBuffer(BufferTargetARB.ArrayBuffer, _particleBuffer);   // ← SSBO と同じ1本

_gl.VertexAttribPointer(0, 3, Float, false, 32, (void*)0);      // position
_gl.VertexAttribPointer(1, 1, Float, false, 32, (void*)12);     // age
_gl.VertexAttribPointer(2, 3, Float, false, 32, (void*)16);     // velocity
_gl.VertexAttribPointer(3, 1, Float, false, 32, (void*)28);     // lifetime
```

`std430` の並びが、そのまま**歩幅 32 バイトの頂点配列**として読める。写し替えは1バイトも要らない。
もしこれが無ければ、コンピュートの結果を CPU へ戻して頂点バッファへ詰め直すことになる——
**100 万粒で毎フレーム 32MB の往復**(実測で片道 10.3ms)。GPU で 0.2ms かけて計算したものを、
運ぶのに 50 倍かけることになる。

描くのは `GL_POINTS`。**粒1つにつき頂点1つ**で、四角形を張らない。

| | Day 49(四角形) | Day 57(点) |
|---|---|---|
| 1粒あたりの頂点 | 4(+ 索引6) | **1** |
| ビルボードの向き | CPU がカメラの右と上で焼き込む | **不要**(点は常に画面に平行) |
| 大きさ | 四隅の位置で決める | `gl_PointSize`(画素数)。`GL_PROGRAM_POINT_SIZE` を有効にする |
| UV | 頂点属性 | **`gl_PointCoord`**(ラスタライザがくれる) |

代償は2つ。**点は中心で視錐台判定される**(大きな点は中心が画面外に出た瞬間まるごと消える)ことと、
`gl_PointSize` に実装依存の上限があること(今日は 64 画素で自分から頭打ちにしてある)。
Day 49 の四角形はどちらの制限も受けないので、**点が四角形の上位互換というわけではない**。

### 5. メモリバリア — 「書いた」と「読める」は別

コンピュートが SSBO へ書いた値を、そのあとの描画が頂点属性として読む。この2つの間には**順序の保証が無い**。

GL は命令を積んだ順に実行するが、**シェーダが自分で書いたメモリが他の経路から見えるようになる時期**までは約束していない。
画素シェーダの出力のような「決まった出口」は GL が面倒を見てくれるが(要点1の裏返し)、
コンピュートが `particles[i] = p;` と書いた場所は**書いた本人にしか分からない**ので、面倒の見ようがない。
...
// **このあと誰が読むか**をビットで伝える
shader.Barrier(
    MemoryBarrierMask.ShaderStorageBarrierBit          // 次のコンピュートが SSBO として読む
    | MemoryBarrierMask.VertexAttribArrayBarrierBit);  // 描画が頂点属性として読む
```

**外しても動いてしまうことが多い**のが、この手の間違いのいちばん厄介なところ。
GPU もドライバも世代で違うので、手元で動いたことは何の保証にもならない。
「絵が出ているから合っている」が通じない種類の間違いで、Day 34 の接空間や Day 54 のずらしとは質が違う——
あちらは間違えれば必ず絵に出た。

### 6. 共有メモリとアトミック — 畳み込みと、測ってみたら差がほとんど無かった話

「生きている粒が何個か」を GPU に数えさせる。C# なら `foreach` で `sum++` するだけだが、
GPU では 100 万個が同時に走るので、その `sum++` が全部ぶつかる。
「読んで、足して、書く」の間に割り込まれると、**足したはずの値が消える**。

割り込まれない足し算が `atomicAdd`。ただし**同じ番地への atomic は順番待ちになる**ので、
100 万回ぶつけると、そこが1本の行列になって並列性が消える……**というのが教科書の話**。

そこで2段にするのが定番(リダクション)。

```glsl
shared uint sAlive[LOCAL_SIZE];        // ワークグループの中だけで見えるメモリ

sAlive[local] = alive;
barrier();                             // 全員が書き終わるまで待つ

for (uint stride = LOCAL_SIZE / 2u; stride > 0u; stride >>= 1u)
{
    if (local < stride) { sAlive[local] += sAlive[local + stride]; }
    barrier();                         // ← if の外。**全員が同じ回数通る**
}

if (local == 0u) { atomicAdd(aliveCount, sAlive[0]); }   // グループにつき1回
```

**`barrier()` の置き場所が制約そのもの**になっている。`barrier()` はワークグループの全員が
同じ回数通ることが条件なので、`if` の中に入れてはいけない。同じ理由で、
このシェーダは**範囲外の呼び出しを早期 `return` させられない**——帰ってしまうと残りが永久に待つ。
だから「帰る」のではなく「0 を持って参加する」と書いてある(要点2の番人とは書き方が変わる)。

もう1つの制約は **GLSL 4.3 の `atomicAdd` が整数にしか無い**こと(`float` 版は `NV_shader_atomic_float` などの拡張)。
高さの合計は `uint(clamp(y, 0, 20) * 100)` と固定小数にしてから足している。
上限を決めてあるのは桁あふれのためで、1個あたり最大 2000 × 100 万個 = 2.0e9 が `uint` の 4.29e9 に収まる。
**外すと静かに一周する**(エラーは出ない)。

#### 測ってみた

同じフレームの中で続けて走らせ、粒の数とワークグループの大きさを変えて 7 通り測った(単位 ms)。

| 条件 | 共有メモリで畳む | 全部 `atomicAdd` | 比 |
|---|---|---|---|
| 26 万粒 / WG 256 | 0.305 | 0.311 | 1.02 |
| 100 万粒 / WG 256(クロックが低い) | 0.968 | 1.026 | 1.06 |
| 100 万粒 / WG 1024 | 0.128 | 0.125 | 0.98 |
| 100 万粒 / WG 64 | 0.131 | 0.125 | 0.95 |
| 100 万粒 / WG 256 | 0.134 | 0.127 | 0.95 |
| 100 万粒 / WG 256(毎フレーム読み戻し中) | **0.090** | **0.084** | 0.93 |
| 100 万粒 / WG 256(最後) | 0.293 | 0.280 | 0.96 |

**VRAM を叩く回数が 256 倍違うのに、時間は ±7% の中に収まる**。しかも向きが一定しない。理由は2つ。

1. **ワープの中の atomic はまとめられる**。最近の GPU とコンパイラは、32 呼び出しぶんの `atomicAdd` を
   レジスタで足してから1回だけ発行する(warp-aggregated atomics)。
   「100 万回」は実際には 3 万回ほどしか発行されていない
2. **そもそも両方とも 32MB を読むところで頭打ち**。いちばん速い行(0.084ms)で
   33.5MB ÷ 0.084ms = **399 GB/s**。RTX 3070 のメモリ帯域 448 GB/s の **89%** で、
   もう読むこと以外にはほとんど時間を使っていない。
   共有メモリの木(8段の `barrier()`)と atomic の発行の差は、その残り 1 割の中の話になる

**同じ表の3行目と6行目を見比べる**と、条件を揃えることの大事さが分かる。どちらも 100 万粒・WG 256 なのに 0.134 と 0.090——
**VSync で休んでいる間に GPU のクロックが落ちる**ので、同じ処理でも 1.5 倍ぶれる。
だから既定を「同じフレームで両方走らせる」にしてある(この揺れは両方に等しく乗るので、比のほうは信用できる)。

**ここで大事なのは「じゃあ畳み込みは要らない」ではない**。
測り方を間違えると逆の結論も簡単に出る——実際、最初は**片方ずつ切り替えて**測っていて、
そのときは「共有 1.22ms ↔ atomic 0.63ms」と**2倍の差**が出ていた。
クロックの違いを実力の違いと読み違えた形で、**そのまま計画書に「共有メモリのほうが速い」と書きかねなかった**
(検証の途中で分かったこと 1)。

畳み込みの値打ちは速さではなく、**VRAM を叩く回数を設計で決められること**にある。
出力の番地が1つしかない場合、入力がもっと多い場合、ワープをまたいで値を共有したい場合(接頭辞和・ソート・ビットニックなど)には、
共有メモリでしか書けない。今日ここで書いた形は、その入口になる。

### 7. 読み戻しが同期を呼ぶ — 8 バイトで数ミリ秒

集計は `uint` 2つ、**たった 8 バイト**。それを CPU が読むのに **0.1〜3.5ms** かかる。

```csharp
_gl.GetBufferSubData(BufferTargetARB.ShaderStorageBuffer, 0, 8, pointer);
```

`glGetBufferSubData` は「その時点の中身」を返す約束なので、**GPU が積まれた命令を全部こなすまで CPU が待つ**。
高いのはバイト数ではなく、**CPU と GPU の並走が切れること**そのもの。
GPU は普段 CPU より1〜2フレーム後ろを走っているので、ここで待つと**そのぶん丸ごと止まる**。

だから既定では読み戻さない(`F9` の内訳を出したときだけ)。HUD の「生存」に `(古)` と付くのはそのためで、
**止まっている数字を新しい数字だと思わせない**ための印になる。

本物のエンジンは、ここを「数フレーム前の結果を拾う」形にして避ける。
**Day 54 の `GpuTimer` がクエリを4本の輪にしていたのとまったく同じ解決**で、
`glGetQueryObject` に `GL_QUERY_RESULT_AVAILABLE` を先に聞いていたあの作法を、バッファでもやればよい
(OpenGL ならフェンス `glFenceSync` とピクセルバッファ。改造課題2)。

### 8. 並列にすると設計が変わる — 4つの作り直し

今日いちばん学びが濃いのはここ。**同じ「粒を撒いて進めて捨てる」でも、並列だと書き方が変わる**。

| Day 49(CPU) | Day 57(GPU) | なぜ変わるか |
|---|---|---|
| 死んだ粒を末尾と入れ替えて前に詰める(swap-remove) | **番号は最初から最後まで変わらない。死んだらその場で生まれ直す** | 「空きを探して詰める」は**取り合い**になる。並列に書けない |
| `Random` を1つ持って順に引く | **番号を混ぜて散らす**(整数ハッシュ) | 100 万個が同時に走るので「次の値」という概念が無い |
| `EmitRate` と端数の持ち越しで撒く | **撒く仕事そのものが消えた** | 撒く = 空きを探すこと。上と同じ理由 |
| 明るさは1粒いくらで決める | **群れ全体でいくら、を個数で割って配る** | 加算合成は重なったぶん足し算になる。個数を 16 倍すると絵が 16 倍明るくなる |

いちばん効いたのは1行目。**「前に詰める」をやめた瞬間に、撒く側のコードが丸ごと消えた**。
`ParticleEmitter` の 400 行のうち、GPU 側に写ったのは「どう撒くか」の数式だけで、
プールの管理(`Count` / `Alive` / `_carry`)は1行も要らなくなった。
代わりに払っているのが「死んだ粒も配列に居座る」ことで、
**生きている粒が半分でも 100 万個ぶんのメモリと 100 万回の呼び出しを払う**。

4行目は絵の話だが、同じくらい大事になる。粒を 16 倍にしたのに同じ絵に見えるのは、
1粒の明るさを 1/16 にしているから(`GpuParticles.ParticleIntensity`)。
本物のエンジンもエフェクトの明るさを「1粒いくら」ではなく「この炎ぜんぶでいくら」で決めていて、理由はこれ。

もう1つ、**切り替えた直後に定常の形を出す**工夫がある。全員 `age = 0` で撒くと、
(1) 寿命ぶんの時間ごとに全員が同時に生まれ直して脈打ち、(2) 形になるまで数秒待たされる。
寿命の途中の齢を配り、**そのぶん先へ進めた位置**から始めればどちらも消える
(噴水は放物運動の式で厳密に、渦は上向きのぶんだけ、引力は殻がそのまま定常なので送らない)。
...
| ファイル | 役割 |
|---|---|
| `Sandbox/GpuParticles.cs` | **今日の主役**。SSBO 2本・VAO 1本・シェーダ3本・タイマー4本。進める / 数える / 読み戻す / 描く。`GpuParticle`(32 バイト)と `ReductionMode` もここ |
| `shaders/particle-sim.comp` | 粒を1フレーム進めるコンピュート。撒き直し・整数ハッシュ・3つの筋書き |
| `shaders/particle-reduce.comp` | 数え上げのコンピュート。共有メモリの畳み込みと、比べるための素朴な `atomicAdd` |
| `shaders/gpu-particle.vert` | SSBO をそのまま頂点として読み、`gl_PointSize` を決める |
| `shaders/gpu-particle.frag` | `gl_PointCoord` で点を丸くする |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Render/Shader.cs` | **段を配列に**。コンピュート用の入口、`#define` の差し込み、`Dispatch` / `Barrier` / `SetDefines` / `SetUInt` | +243 / −30 |
| `Render/RenderResources.cs` | `LoadComputeShader`(キーに `#define` を混ぜる) | +27 / −0 |
| `Program.cs` | 文脈を 4.3 へ、フィールド、`OnLoad` の上限の問い合わせ、`OnRender` の3行、HUD、ページ1枚、内訳、自己チェック、既存 `Show*` に1行ずつ | +813 / −2 |
| `Day57.csproj` | `Day56.csproj` からのリネームのみ | — |

### 写経する順番

依存の向きに沿って並べてある。**この順に写せば、途中でビルドが通らない状態を作らずに済む**。

1. **`Render/Shader.cs`** — 今日の土台。`_vertexPath` / `_fragmentPath` の2本を `_stages` の配列にし、
   コンピュート用のコンストラクタ(`Shader(gl, computePath, params defines)`)を足す。
   `TryCreateProgram` は段の数だけ回る形に。`InjectDefines`(`#version` の次の行 + `#line 2`)、
   `Dispatch` / `Barrier` / `SetDefines` / `SetUInt` / `IsCompute` もここ。
   **`GpuParticles` がこれを全部使う**ので、いちばん先に置く
2. **`Render/RenderResources.cs`** — `LoadComputeShader`。`LoadShader` のすぐ下に置く
3. **`shaders/particle-sim.comp`**(新規) — 粒を進めるコンピュート。**`struct Particle` の並びが今日の要**
4. **`shaders/particle-reduce.comp`**(新規) — 数え上げ。`struct Particle` は 3 と同じものを書く
   (`#include` が無いので写す。設計書「今日残した歪み」の1つ目)
5. **`shaders/gpu-particle.vert`**(新規) — 3 の並びと**1バイトずつ同じ**頂点属性
6. **`shaders/gpu-particle.frag`**(新規) — `gl_PointCoord` で丸くする
7. **`Sandbox/GpuParticles.cs`**(新規) — 3〜6 を使う。`GpuParticle` の並びは 3 と揃える
8. **`Program.cs`** — 7 を使う。順に、
   (a) `Main` の `new APIVersion(3, 3)` を **`(4, 3)`** へ、
   (b) 「Day 57」のフィールド群(`_gpuParticles` / `_gpuDemo` / `_computeLimits` / `_computeAvailable` ほか)、
   (c) `OnLoad` の `ApplyTrailSettings()` の直後(版の確認 → 上限の問い合わせ → 生成)、
   (d) `OnRender` の `RenderParticles()` の直後に `RenderGpuParticles(deltaSeconds)`、
   (e) `DrawOverlayInfo` に HUD の1行、
   (f) `BuildMenu` の末尾に「コンピュート」ページ(9項目)、
   (g) 末尾に今日の関数一式(`RequireCompute` / `ShowGpuParticles` / `HideGpuParticles` / `ApplyGpuParticleScene` /
   `RenderGpuParticles` / `GpuParticleLabel` / `ReductionName` / `GpuModeName` / `GpuModeNote` /
   `MeasureCpuParticleStep` / `DescribeGpuParticles` / `RunComputeCheck`)、
   (h) `OnClosing` に `_gpuParticles?.Dispose()`、
   (i) **既存の `Show*` 5か所**(`ShowPhysicsDemo` / `ShowCharacterDemo` / `ShowParticleDemo` / `ShowEffectDemo` /
   `ShowPlayableDemo`)に `HideGpuParticles();` を1行ずつ、
   (j) `RunDebugMenuCheck` の項目数を **237 → 246** へ
9. **`Day57.csproj`** — `Day56.csproj` をリネームするだけ(中身は同じ)

## 設計書

**層が1つ増えた**。`Sandbox/` ——教養編の実験台。`Demo/` や `Game/` と同じ「エンジンを使う側」の位置に、3つ目の箱が並んだ形になる。
Phase 6 の初日(Day 39)に `Demo/` を作って以来の構造の変化で、本編が終わってから増えた最初の層でもある。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Sandbox/GpuParticles` | `Sandbox/` | SSBO 2本・VAO 1本・シェーダ3本・タイマー4本。進める / 数える / 読み戻す / 描く |
| `Sandbox/GpuParticle` | `Sandbox/`(`GpuParticles.cs`) | 粒1つ。**32 バイト**。シェーダの `struct Particle` と1バイトずつ同じ |
| `Sandbox/GpuParticleMode` | `Sandbox/`(`GpuParticles.cs`) | 筋書き(噴水 / 引力 / 渦)。シェーダの `uMode` と同じ番号 |
| `Sandbox/ReductionMode` | `Sandbox/`(`GpuParticles.cs`) | 数え上げのやり方(共有 / atomic / 両方) |
| `Sandbox/GpuParticles.ComputeLimits` | `Sandbox/`(`GpuParticles.cs`) | ドライバに聞いたコンピュートの上限 |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Render/Shader` | **段が配列になった**。コンピュート用の入口、`#define` の差し込み、`Dispatch` / `Barrier` / `SetDefines` / `SetUInt` | +243 / −30 |
| `Render/RenderResources` | `LoadComputeShader` | +27 / −0 |
| `Program.cs` | 文脈を 4.3 へ、ページ1枚、1フレームに3行、内訳と自己チェック | +813 / −2 |

### 粒が1フレーム進むまで — CPU が出す4つの命令

```mermaid
flowchart TD
    subgraph CPU["CPU(Program.RenderGpuParticles)"]
        A["_gpuTime += dt"] --> B["Simulate&lpar;dt, time&rpar;<br/>uniform 10 本 + BindBufferBase"]
        B --> C["Reduce&lpar;&rpar;<br/>集計を 0 に戻す + uniform 2 本"]
        C --> D["Draw&lpar;camera, 高さ&rpar;<br/>uniform 6 本 + 状態"]
    end

    B --> D1["glDispatchCompute&lpar;4096&rpar;<br/>particle-sim.comp"]
    D1 --> MB1["glMemoryBarrier<br/>SHADER_STORAGE | VERTEX_ATTRIB_ARRAY"]

    C --> D2["glDispatchCompute&lpar;4096&rpar;<br/>particle-reduce.comp&lpar;共有&rpar;"]
    D2 --> D3["glDispatchCompute&lpar;4096&rpar;<br/>particle-reduce.comp&lpar;atomic&rpar;"]
    D3 --> MB2["glMemoryBarrier<br/>BUFFER_UPDATE | SHADER_STORAGE"]

    D --> DR["glDrawArrays&lpar;GL_POINTS, 0, 1048576&rpar;"]

    MB1 --> SSBO[("粒の SSBO<br/>1,048,576 x 32 バイト = 32MB")]
    MB2 --> STAT[("集計の SSBO<br/>uint 2 個 = 8 バイト")]
    SSBO --> D2
    SSBO --> DR
    DR --> FB["_post.Scene&lpar;RGBA16F&rpar;<br/>加算合成。1.0 超はブルームへ"]

    STAT -.->|"F7 が ON のときだけ"| RB["glGetBufferSubData&lpar;8 バイト&rpar;<br/><b>ここで CPU が止まる</b>"]
```

**CPU が出しているのは 4 つの GL 命令と uniform だけ**で、粒の数に比例するものが1つも無い。
6.5 万でも 100 万でも、この図はまったく同じ形になる——変わるのは `glDispatchCompute` の引数と `glDrawArrays` の個数。
Day 49 の CPU パーティクルは、この位置に「全部を舐める `for`」と「頂点バッファへの 960KB の転送」があった。

**`Reduce` が2回ディスパッチしているのが既定**。同じ答えを2通りの書き方で出して、時間だけを見比べる(要点6)。
片方だけにすると、切り替えの前後で GPU のクロックが変わって比べられなくなる。

**破線が1本だけある**のが今日の要点7。`glGetBufferSubData` は「その時点の中身」を約束するので、
この線を引いた瞬間に CPU が GPU を待つ。だから既定では引かない。

### 数え上げが1つの数になるまで — 2段の畳み込み

```mermaid
flowchart TD
    subgraph WG["1つのワークグループ(256 呼び出し)"]
        L0["各呼び出し<br/>alive = 0 か 1<br/>height = uint&lpar;clamp&lpar;y,0,20&rpar; * 100&rpar;"]
        L0 --> SH["shared sAlive[256] / sHeight[256]<br/>自分の番地へ書く"]
        SH --> B0["barrier&lpar;&rpar;<br/><b>全員が書き終わるまで待つ</b>"]
        B0 --> T["stride = 128, 64, 32, ... 1<br/>if &lpar;local &lt; stride&rpar; 足す<br/>barrier&lpar;&rpar; は if の外"]
        T --> Z{"local == 0 か"}
        Z -->|Yes| AT["atomicAdd&lpar;aliveCount, sAlive[0]&rpar;<br/>atomicAdd&lpar;heightFixed, sHeight[0]&rpar;"]
        Z -->|No| END["何もしない"]
    end

    AT --> G[("集計の SSBO<br/>aliveCount / heightFixed")]

    subgraph NV["比べるための素朴な形(uNaive = 1)"]
        N0["各呼び出しが直接<br/>atomicAdd を2回"]
    end

    N0 --> G
```

| | 共有メモリで畳む | 全部 atomicAdd |
|---|---|---|
| VRAM への atomic | **4,096 回** | 1,048,576 回 |
| `barrier()` | 9 回(1 + log2 256) | 0 回 |
| 共有メモリ | 2KB/グループ | 0 |
| 実測(100 万粒) | 0.090ms | **0.084ms** |

**答えはどちらも同じ**(自己チェックの6項目め)。にもかかわらず**素朴なほうがわずかに速い**のは、
ワープの中の atomic がまとめて発行されることと、両方とも 32MB を読むところで頭打ちになっているため(要点6)。

**`barrier()` が `if` の外にあること**が、この図でいちばん壊れやすい。中に入れると
「ワークグループの全員が同じ回数通る」が破れて未定義になる。同じ理由で、このシェーダの先頭には
要点2のような早期 `return` を置けない——範囲外の呼び出しも **0 を持って畳み込みに参加する**。

### 同じ1本のバッファが、2つの口に挿さっている

```mermaid
graph LR
    BUF[("バッファオブジェクト<br/>1,048,576 x 32 バイト<br/><b>型を持たないバイト列</b>")]

    BUF -->|"BindBufferBase&lpar;SHADER_STORAGE, 0&rpar;"| SSBO["SSBO として<br/>particle-sim.comp が読み書き<br/>particle-reduce.comp が読む"]
    BUF -->|"BindBuffer&lpar;ARRAY_BUFFER&rpar;<br/>+ VertexAttribPointer x4"| VBO["頂点バッファとして<br/>gpu-particle.vert が読む<br/>歩幅 32 / オフセット 0,12,16,28"]

    SSBO -.->|"glMemoryBarrier&lpar;VERTEX_ATTRIB_ARRAY&rpar;"| VBO
```

**写し替えが1バイトも無い**のがこの形の値打ち(要点4)。
`std430` の並びと頂点属性の並びが同じになるように `GpuParticle` を組んであるので、口を変えるだけで読み方が変わる。

**破線がバリア**。順序を保証するのはこの1本だけで、外しても手元では絵が出てしまう(要点5)。

### 今日残した歪み(4つ)

**1つ目: `struct Particle` が3か所にある**。`particle-sim.comp` と `particle-reduce.comp` の GLSL 2つと、C# の `GpuParticle`。
GLSL に `#include` が無いので写すしかなく、**1つ直して2つ忘れると、絵は出るが数が合わない**という壊れ方をする。
行き先は Day 52a の歪みの1つ目(BRDF が3か所)とまったく同じで、**シェーダの読み込みに `#include` を足す**
(エンジンがファイルを読んで文字列を差し込むだけ。今日 `InjectDefines` で `#version` の次の行に割り込む口は作ってある)。

**2つ目: 死んだ粒も配列に居座る**。番号を固定した代償で、**生きている粒が半分でも 100 万個ぶんの呼び出しを払う**(要点8)。
本物の GPU パーティクルは、生きている粒の番号だけを別の SSBO に集め(コンパクション)、
`glDrawArraysIndirect` で「GPU が決めた個数」を描く。Day 65(GPU 駆動レンダリング)の題材そのものなので、そこまで持っていく。

**3つ目: 集計を読み戻すのが同期式**。`glGetBufferSubData` は GPU が追いつくまで CPU を止める(要点7)。
Day 54 の `GpuTimer` が輪を作って避けたのと同じことを、バッファでもやればよい(`glFenceSync` とピクセルバッファ。改造課題2)。
今日は「代償を見るための項目」(`F7`)として、わざと同期のまま残してある。

**4つ目: Day 53 のライトカリングは CPU のまま**。ロードマップは Day 57 に「Day 53 のライトカリングの GPU 化もここで」と書いている。
今日はそこまでやらなかった——升目の振り分けを移すには、升目の式を GLSL へ写し、TBO を SSBO へ替え、
`ClusteredLighting` の3人の受け取り手を全部差し替えることになり、1日の差分に収まらない。
**今日作った道具はそのまま使える**(SSBO・ディスパッチ・切り上げ・バリア)ので、改造課題3に置いた。

### 引き継いだ図

Day 56 の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。変わった図は次の3つ。

| 図 | 何が変わったか |
|---|---|
| `全体構成` | **`Sandbox/` の箱が増えた**。`Program` から呼ばれ、`Render` と `Core` を使い、誰からも知られていない |
| `Render` のクラス図 | `Shader` に **`IsCompute` / `Dispatch` / `Barrier` / `SetDefines` / `SetUInt`**、`RenderResources` に `LoadComputeShader` |
| `1フレームの流れ` | `RenderParticles` の直後に **`RenderGpuParticles`**。`_post.Begin` と `_post.End` の間 |

新しく足した図が4つ(この節の上の3つと、下の `Sandbox` のクラス図)。
Day 56 の設計書の冒頭にあった3つの図(灯りがシーンに入るまで、灯りが1フレームで動くまで、完成版の1フレーム)は
灯りの話なので、今日は外した(Day 56 の計画書を見ること)。
...
今日それが守られていることは、Day 56 との画素の突き合わせ(完成条件 8)で確かめた。
...
`dotnet run --project reference/Day57 -c Release` で起動する。
起動直後のコンソールに、今日から1行増えている。

```
GL_RENDERER : NVIDIA GeForce RTX 3070/PCIe/SSE2
GL_VERSION  : 4.3.0 NVIDIA 596.49            ← **3.3 ではなく 4.3**
...
コンピュート: OpenGL 4.3  1グループ最大 1024 呼び出し / x の上限 1024 / グループ数の上限 2,147,483,647 / 共有メモリ 48KB
```

メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「コンピュート」(25 枚目)が開く。
...
```
GPU パーティクル: **ON**  262,144 個 / 8.0MB を VRAM に置きっぱなし。1フレームの CPU の仕事は **dispatch 3 回 + draw 1 回**(進める1回 + 数える2回)
```

Day 31 の床と立方体の上に、**オレンジ色の噴水が立つ**。空は消してある(加算合成の粒は明るい背景の上では見えない)。
HUD に今日の行が出る。

```
GPU粒 262,144  噴水  WG 256x1,024  GPU 進 0.55ms / 数 共有 0.31 / atomic 0.31ms / 描 0.37ms  生存 ?
```

**生存が `?` なのは、まだ一度も読み戻していないから**(要点7)。`F9` を押すと数字が入り、以後は `(古)` が付く。

### 2. `F3`: 粒を 100 万にする

既定は 26 万なので、**1回押すと 100 万**(もう1回で 6.5 万、その次で 26 万に戻る)。**fps は 60 のまま落ちない**。

```
粒: **1,048,576 個**  32.0MB  ディスパッチ 4,096 グループ x 256 (余分な呼び出し 0 個)
```

絵の明るさはほとんど変わらない——**粒が 4 倍になっても明るさは同じ**(1粒の明るさを個数で割っている。要点8)。
変わるのは粒の細かさで、6.5 万だと粒が1つずつ見え、100 万だと連続した霧に見える。

**CPU 側の HUD(`更新:0.00ms`)は動かない**。粒を進めているのは GPU なので、CPU の仕事は uniform を送ることだけ。

### 3. `F4`: 筋書きを回す

- **噴水** … 床から噴き上がり、床で跳ねる。上ほど赤く、生まれたては白い
- **引力** … 中心から 3〜9m の殻を回る球。速い粒(近い粒)が白く光る。**中心はゆっくり回る**
- **渦** … 床から立ち上がる青緑の柱。位置から速度を作るので、粒が同じ流れに乗って筋になる

切り替えた瞬間から定常の形が出る(寿命の途中の齢を配って、そのぶん先へ進めた位置から始める。要点8)。

### 4. `F5`: ワークグループの大きさを変える

`[リロード成功]` が2回(進めるシェーダと数えるシェーダ)出て、次の行が出る。

```
ワークグループ: **1024 呼び出し**  ディスパッチ 1,024 グループ(余分 0 個)  共有メモリ 8.0KB / 48KB
ワークグループ: **64 呼び出し**  ディスパッチ 16,384 グループ(余分 0 個)  共有メモリ 0.5KB / 48KB
```

**絵は1ピクセルも変わらない**。HUD の `WG 256x4,096` が `WG 64x16,384` に変わるだけ。
GPU の時間もほとんど動かない(100 万粒で 0.125〜0.134ms の間)——
**この規模では、ワークグループの大きさは速度をほとんど決めていない**。

### 5. `F6` → `F9`: 数え上げの2通りを見比べる

`F9`(内訳)を押すと、同じフレームで測った2つが並ぶ。

```
  共有メモリの使用: 2048 バイト/グループ (uint 2本 x 256)  数え上げ: **両方**(同じフレームで続けて)  VRAM を叩く回数 共有 4,096 回 ↔ atomic 1,048,576 回
  GPU(タイマークエリ): 進める 0.178ms / 数える 共有 0.090ms ↔ atomic 0.084ms(**0.93 倍**) / 描く 0.389ms
```

**VRAM を叩く回数が 256 倍違うのに、時間はほとんど同じ**(要点6)。
`F6` で片方だけに切り替えると、走らせていないほうの数字が古いまま止まる。

### 6. `F7` → `F8`: 読み戻しと、CPU との勝負

`F7` で毎フレーム読み戻すと、HUD の「生存」から `(古)` が消えて毎フレーム動く。
`F9` の内訳で `読み戻し 0.15ms` が出る(**8 バイトを読むのに、粒を進める時間に近い時間がかかる**)。

`F7` でもう一度切ってから `F8`。

```
--- CPU で1ステップ(1,048,576 個)---
  CPU(C# / 1スレッド): **4.59ms**  1個あたり 4.4ns  寿命切れ 21,143 個は進めていない
  GPU(タイマークエリ): **0.203ms**  **23 倍**
  参考: 粒を読み戻すのに 10.3ms (32.0MB)。**CPU で回すなら毎フレームこれを往復する**
  ※ C# 側は撒き直し(寿命が尽きた粒を作り直す)を省いてあるので、わずかに CPU に有利
```

**「23 倍」より「10.3ms」のほうが効く**。仮に CPU が GPU と同じ速さで計算できたとしても、
粒を運ぶだけで 60fps の予算(16.6ms)の 6 割が消える。

### 7. `F10`: 自己チェックが 10 項目すべて合格

`F2` を押す前に `F10` を押すと、**GL を使わない5項目だけ**が走る(構造体の並び・切り上げ・上限)。
GPU パーティクルを出してから押すと 10 項目全部。**押したあと、筋書きと粒の数は元に戻っている**(粒は撒き直される)。

### 8. Day 38〜56 の絵と自己チェックが1つも壊れていない

文脈を 3.3 から 4.3 に上げたので、**既存の絵が変わっていないことを画素で確かめる**。
Day 56 と Day 57 に同じ台本を流して4枚を突き合わせた。
...
| 最終デモ(完成版。Day 56 の到達点) | **バイト単位で一致** |

自己チェックも、Day 56 の 227 項目がすべて同じ数字で通る。
違うのは**メニューの枚数と項目数だけ**(24 枚 237 項目 → **25 枚 246 項目**)で、
これは今日ページを1枚足したぶん(`RunDebugMenuCheck` の期待値も 246 に直してある)。

```
  [OK] ページがある  25 枚
  [OK] **Day 31 以降のスイッチが全部入っている**  246 項目
  [OK] 最後のページまで送れる  25 枚目
```

## 改造課題

### 課題1(易): 番人を外して、何が起きるか見る

`particle-sim.comp` の先頭の `if (index >= uCount) { return; }` をコメントにして、粒の数を **1,000,000**
(`GpuParticles.CountSteps` に足す)にしてみる。`1,000,000 / 256 = 3907 グループ = 1,000,192 呼び出し`で、**192 個が余る**。

- 絵はどうなるか。**何も起きないように見えるはず**(SSBO の範囲外は、仕様上「捨てられるか、バッファの中のどこかに落ちる」)
- `GpuParticles.Count` の上限(`CountSteps[^1]`)を 1,000,000 にして、確保するバイト数も 1,000,000 x 32 に縮めてみる。
  **今度は余った 192 個が確保の外を書く**。手元の GPU で何が起きるか
- 自己チェックの「切り上げ」の項目が、なぜ 1,000,000 という割り切れない数をわざわざ持ち出しているのか

**落ちない間違い**をひとつ手で作っておくのが目的。次に同じことをしたとき、絵ではなく数から疑えるようになる。

### 課題2(中): 読み戻しを非同期にする

要点7の代償(`glGetBufferSubData` が CPU を止める)を、Day 54 の `GpuTimer` と同じ手で避ける。

1. 集計用の SSBO を**3本の輪**にする(`_statsBuffer[3]`)
2. 書き終えたら `glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0)` でしるしを置く
3. 次のフレーム以降、`glClientWaitSync(sync, 0, 0)` で**待たずに**聞き、`GL_ALREADY_SIGNALED` か
   `GL_CONDITION_SATISFIED` が返ったものだけ `glGetBufferSubData` で読む

`F7` を ON にしたまま `F9` の `読み戻し` が **0.00ms** になり、生存の数は2〜3フレーム遅れる。
**「遅れた正しい数」と「今の数を待って取る」のどちらが要るか**を決めるのが、この手の設計の分かれ目になる。

### 課題3(難): Day 53 のライトカリングを GPU へ移す

ロードマップが Day 57 に挙げているもう1つの題材(設計書「今日残した歪み」の4つ目)。

1. `ClusterGrid.Assign`(CPU)の升目の式を `light-cull.comp` へ写す。
   **ワークグループの割り当ては升目1つにつき1グループ**(`glDispatchCompute(15, 10, 16)`)にすると、
   `gl_WorkGroupID.xyz` がそのまま升目の座標になる——今日の1次元とは違う使い方が見られる
2. 光の一覧を SSBO に置き、升目ごとの番号の並びも SSBO に書く。
   TBO(Day 53)を3本まとめて SSBO 2本にできる
3. 画素シェーダ側(`textured.frag` の `PointLightContribution`)の `texelFetch` を SSBO の添字に替える
4. **絵が1ピクセルも変わらないこと**を、Day 56 の「フォワードと Forward+ で同じ絵」の自己チェックで確かめる

見どころは**升目に入る光の数を数える `atomicAdd`** で、今日の数え上げと同じ形が出てくる。
「1升に入る光に上限が無い」(Day 53 の歪みの3つ目)も、GPU 側なら `atomicMin` で明るい順に切れる。
...
| .NET | 10.0 |
| GPU | NVIDIA GeForce RTX 3070(メモリ帯域 448 GB/s) |
| OpenGL | **4.3 コアプロファイル**(ドライバは 4.6 に対応。文脈で頼んだ版で決まる) |
| 解像度 | 960 x 640 |
...
| 項目 | 値 |
|---|---|
| 粒 | 1,048,576 個 x 32 バイト = **32.0MB**(確保は常に最大数ぶん) |
| ディスパッチ | 4,096 グループ x 256 = 1,048,576 呼び出し(余分 0 個) |
| CPU が出す GL 命令 | **ディスパッチ3回 + 描画1回**(粒の数に比例するものは無い) |
| ドライバの上限 | 1グループ 1024 呼び出し / x の上限 1024 / グループ数 2,147,483,647 / 共有メモリ 48KB |
| GPU の時間(進める) | 0.18〜2.13ms(**VSync でクロックが上下するので走行ごとに1桁ぶれる**) |
| GPU の時間(数える) | 共有 0.090〜0.968 / atomic 0.084〜1.026ms。**同じフレームで測った比は 0.93〜1.06** |
| GPU の時間(描く) | 0.37〜1.70ms(カメラの寄り方で変わる。点の面積そのもの) |
| 集計の読み戻し | 0.07〜3.47ms(**8 バイト**。バイト数ではなく同期の代償) |
| 粒の読み戻し | 10.3ms(32.0MB) |
| CPU で1ステップ | 4.59ms(1個あたり 4.4ns。C# / 1スレッド。撒き直しは省く) |
| fps | **60(VSync)**。100 万粒・3つの筋書きのどれでも落ちない |
| Day 56 との一致 | 違う画素 **0**(起動直後・デモ v1・決めの構図・完成版の4枚) |
| GL のエラー | 0(毎フレーム `glGetError` を見て確かめた) |

### 自己チェック(10 項目すべて合格)

```
=== Day 57 自己チェック(構造体の並び・切り上げ・畳み込み・CPU との一致)===
  [OK] **OpenGL 4.3 以上の文脈が取れている**(コンピュートと SSBO の下限)  4.3.0 NVIDIA 596.49
  [OK] **C# の構造体が std430 の並びと一致する**(32 バイト、穴なし)  32 バイト  age 12 / velocity 16 / lifetime 28
  [OK] **切り上げが全部の組み合わせで足りていて、1グループも余らない**  1,000,000 / 256 → 3,907 グループ = 1,000,192 呼び出し(余分 192)
  [OK] いま使っているワークグループが、このドライバの上限に収まっている  256 呼び出し x 4,096 グループ
  [OK] **共有メモリが仕様の保証(32KB)に収まっている**(別の GPU でも動く)  最大 8KB(uint 2本 x 1024)
  [OK] **共有メモリで畳んでも、全部 atomicAdd しても、答えは同じ**(道筋が違うだけ)  共有 1,048,576 個 / 平均 1.7586m  ↔  atomic 1,048,576 個 / 平均 1.7586m
  [OK] 撒き直した直後は**全部の粒が生きている**(寿命の途中からばらけて生まれる)  1,048,576 / 1,048,576 個
  [OK] **ワークグループを 256 → 64 に変えても答えが変わらない**(絵も数も同じ。時間だけ違う)  1,048,576 個 / 平均 1.7586m  グループ 16,384(さっきは 4,096)
  [OK] **GPU の1ステップと C# の1ステップが同じ答えになる**(積分も床の跳ね返りも)  65,108 個を比べて、いちばん離れた粒で 4.8E-007m
  [OK] **GPU が畳んだ合計と、C# が素直に舐めた合計が一致する**  GPU 65,536 個 / 1.7605m  ↔  CPU 65,536 個 / 1.7605m
  すべて合格(並び・切り上げ・畳み込み・CPU との一致が仕様どおり)
  ※ 自己チェックは粒を撒き直す(筋書きと数は元に戻してある)
```

**GL を使わない項目が5つ**(構造体の並び・切り上げ・上限2つ、と版の確認)。
**残り5つは GPU に走らせないと確かめられない**——ここが Day 56 までの自己チェックといちばん違うところで、
コンピュートの答えは「シェーダを走らせて読み戻す」以外に知る方法が無い。

いちばん値打ちがあるのは下から2つめ。**同じ1ステップを GPU と C# で回して位置を比べる**(いちばん離れた粒で 4.8e-7m)。
GLSL の四則演算は IEEE の丸めが決まっているので、足し算と掛け算だけで書かれた積分は**C# と同じ答えになる**。
`normalize` や `length` を通した式は精度の保証が緩いので、比べるのは噴水(重力と床の跳ね返りだけ)に限ってある。

### 検証の途中で分かったこと

**1. 「共有メモリのほうが2倍速い」という嘘の数字を、危うくそのまま書くところだった**(今日の冒頭の話)。
最初は `F6` で片方ずつ切り替えて測っていて、次の数字が出た。

| 測った順 | 数え上げ | GPU の時間 | そのときの「進める」 |
|---|---|---|---|
| 1回目 | 共有メモリ | 1.221ms | 2.175ms |
| 2回目 | 全部 atomicAdd | 0.633ms | **1.158ms** |

「共有メモリのほうが2倍遅い」と読めるが、**同じフレームで測っていない**。
右端の「進める」(どちらのときもまったく同じ処理)が 2.175 → 1.158 と半分になっていることが、そのしるしになる。
VSync を入れていると GPU は1フレームのうち 1ms しか働かないので深く休み、**負荷が続くとクロックが上がる**。
切り替えの前後で、GPU が別の速さで走っていた。

直し方は「**同じフレームの中で、続けて両方走らせる**」。クロックの揺れは両方に等しく乗るので、比のほうは信用できる。
`ReductionMode.Both` を既定にして、タイマーも2本に分けた。測り直すと差は ±7% の中に収まり、向きも一定しない(要点6)。

**Day 54 の計画書が「VSync 中はクロックが上下して走行ごとに倍以上ぶれる」と書いていたのと同じ罠**だが、
あのときは「絶対値がぶれる」話だった。今日は**2つを比べる**話で、ぶれが片方にだけ乗ると**結論そのものが逆になる**。

**2. 加算合成は、粒の数をそのまま明るさにする**。最初は1粒の明るさを固定していて、
26 万粒で既に画面の半分が真っ白、100 万粒では床も立方体も見えない白い塊になった。
加算合成は重なったぶんが足し算なので、**個数を4倍にすると絵が4倍明るくなる**——考えれば当たり前だが、
Day 49 の1万粒では気づけない(重なりが浅いので飽和しない)。
直し方は「**群れ全体の明るさを決めて、個数で割って配る**」(`GpuParticles.ParticleIntensity`)。
基準を 65,536 個に置くと、6.5 万・26 万・100 万のどれで見ても同じ明るさになり、変わるのは粒の細かさだけになる。
本物のエンジンがエフェクトの明るさを「1粒いくら」ではなく「この炎ぜんぶでいくら」で持っているのは、これが理由。

**3. 筋書きを切り替えた直後、数秒間だけ形が違った**。渦に切り替えると、最初の数秒は**床に貼り付いた平たい円盤**で、
柱になるのは 6 秒後(寿命ぶん)だった。撒き直し(`uReset`)が全員を撒き口に置いていたため。
**寿命の途中の齢を配るだけでは足りない**——齢を配っても位置は撒き口のままなので、脈打ちは消えても形は出ない。
そこで**そのぶん先へ進めた位置**から始めるようにした(噴水は放物運動の式で厳密に、渦は上向きのぶんだけ、
引力は殻がそのまま定常なので送らない)。3つで扱いが違うのは、**「だいたい定常の形」で始められればよい**から。

**4. 引力の粒が、全部まとめて中心の1点に落ちた**。最初は接線方向へ「適当な速さ(2〜3.5 m/s)」で蹴り出していて、
1秒後には直径 1m ほどの真っ白な玉になっていた。円軌道の速さは距離で決まる(`v = sqrt(GM / r)`)ので、
半径 3m の粒には 4.5 m/s が要る——遅い粒は全部まっすぐ落ちる。
**距離から速さを出す**ようにし(0.8〜1.3 倍に散らして楕円にする)、減速も 0.15/s から 0.02/s に下げたら、殻のまま回り続けるようになった。
**物理を並列にしても物理は物理**で、初期値の与え方を間違えれば同じように壊れる。

**5. `#define` を差し込むと、コンパイルエラーの行番号がずれる**(先に潰した)。
`InjectDefines` は `#version` の次の行に割り込むので、素直に書くとログの行番号がファイルより
差し込んだ行数ぶん大きくなる。Day 14 で「ログの行番号はファイルの行番号と一致するので、
ファイル名を添えておくとエディタからそのまま飛べる」と書いた性質が、そこで壊れる。
**差し込みの最後に `#line 2` を置いて数え直させる**と一致したまま保てる
(GLSL の `#line N` は「次の行は N 行目」の意味)。
ワークグループを切り替えるたびに焼き直す作りなので、**ずれたまま運用すると効いてくる**のは間違いない——
シェーダを書き換えている最中に、ログの行番号だけが1つずれているのがいちばん気づきにくい。
