# Day 60a: CPU レイトレーサ(2) — パストレーシングとモンテカルロ積分(木をやめて道にする)
...
| | 何をするか |
|---|---|
| **Day 60a(今日)** | **パストレーシング**。面に当たったときに枝分かれするのをやめ、**行き先を乱数で1つだけ引いて、何千本も平均する**(Kajiya, 1986 のレンダリング方程式を、モンテカルロ積分で解く) |
| Day 60b | **BVH**。形が 466 個になって総当たりが秒単位になるので、形を箱の木に入れて「通らない箱の中身を丸ごと飛ばす」 |

| | Whitted(Day 59) | パストレーシング(Day 60a) |
|---|---|---|
...
今日の差分は**新規 5 ファイル**(842 行、うちコード 399 行)と**変更 12 ファイル**(+1,012 / −292 行)。
場面への問い合わせは Day 59 と同じ**総当たり**のまま(形は多くて 11 個なので、まだ困らない)。
...
**起動すると、赤と緑の壁のコーネルボックスが出る。天井の光る球1つだけで照らされていて、20 秒ほどで 150 サンプルほどが重なる。
白い玉の赤い壁側がうっすら赤みを帯び(色移り)、影の縁がぼけ、鏡の玉に光る球そのものが映る。
`A` を押して Whitted に切り替えると、同じ場面が「のっぺりした灰色 + 真っ白な円」になる。**

| キー | 何が起きるか |
|---|---|
| `1`〜`5` | 場面(Whitted 1980 / 屈折率と金属 / 合わせ鏡 / **コーネルボックス** / **ガラスの集光**) |
...
| `PageUp` / `PageDown` | 下の欄をめくる(自己チェックは 32 行ある) |
| `S` `F` `E` `J` | Day 59 のまま(影 / フレネル / 始点を浮かせる / 画素内のずらし) |
| 左ドラッグ / ホイール / `R` | カメラを回す / 寄る / 戻す |
| `C` / `P` / `X` / `H` / `Esc` | 自己チェック(16 項目)/ PNG 保存 / 下の欄を消す / 文字を消す / 終了 |
...
**3つ目は「絵ではなく数で確かめる」**(要点7)。パストレーシングの答えはノイズに埋もれているので、
...
- [Peter Shirley ほか「Ray Tracing in One Weekend」](https://raytracing.github.io/books/RayTracingInOneWeekend.html) の
  **「Diffuse Materials」**(Day 59 では読み飛ばした章)
  — 乱数で跳ね返す拡散。今日の `PathTracer` の拡散面はここから始まる(ただし本書は重点サンプリングも NEE も使っていない)
- [同 「Ray Tracing: The Rest of Your Life」](https://raytracing.github.io/books/RayTracingTheRestOfYourLife.html)
  — **今日の一次資料**。モンテカルロ積分・pdf・重点サンプリング・光源のサンプリングを、
...
- [PBRT 4th edition(全文が無料で読める)](https://pbr-book.org/4ed/contents) の
  **13章 Light Transport I: Surface Reflection**(パストレーシングと NEE)と **A.5 Russian Roulette**
...
**ノイズを半分にするには4倍の時間が要る**——これがパストレーシングの絵がなかなか綺麗にならない理由(要点7)。
...
この規則のおかげで、**「拡散 → ガラス → ガラス → 光源」の集光の経路は正しく拾える**(直前が鏡面なので足す)。場面5がそれ。

> 点光源は大きさ 0 なので、散乱の光線が当たることは決して無い。**NEE を切ると点光源の光が丸ごと消える**——
> 場面1〜3 で `N` を切ると、空の光だけになる。「点光源は NEE 無しでは扱えない」が目で分かる。
...
|---|---|---|---|
| 6 | 8.70 本 / 70ms | 12.56 本 / 117ms | 0.5508 / 0.5511 |
| 8 | 8.80 本 / 71ms | 15.87 本 / 142ms | 0.5528 / 0.5532 |
| 16 | **8.85 本 / 71ms** | 28.16 本 / 328ms | 0.5537 / 0.5542 |
| 32 | **8.85 本 / 71ms** | 48.67 本 / 558ms | 0.5537 / 0.5542 |

**RR ありは 8.85 本で頭打ち**(暗くなった道は勝手に切れるので、上限をいくら上げても道が伸びない)。
なしは上限に比例して増え、深さ 32 では **5.5 倍の光線と 8 倍の時間**を使って、**同じ明るさ**にたどり着く。
自己チェック16 でも、平均が **1.0313 → 1.0316(差 0.0%)**のまま光線が **0.69 倍**になることを測っている。

下限 0.05 を入れてあるのは、q を 0 まで許すと生き残ったときの重み 1/q が跳ね上がり、
**たまに1画素だけ極端に明るい点**(firefly)が出るから(要点7)。
...
|---|---|---|
| **BVH**(形を箱の木に入れて、通らない箱の中身を丸ごと飛ばす) | 場面への問い合わせは総当たりのまま。光線1本につき、場面の全部の形に聞く。今日の場面は多くて 11 個なので困らないが、形が数百個になると1パスが秒単位になる | **Day 60b** |
| 光源を明るさで選ぶ | NEE は光源を一様に1つ選ぶ。暗い光源にも明るい光源と同じだけ影の光線を使う | 改造課題2 |
| firefly を潰す | たまに1画素だけ極端に明るい点が出る(不偏だが目に付く) | 改造課題3(偏りと引き換え) |
| ざらざらした金属(GGX) | 金属は完全な鏡だけ。鏡面と拡散の中間が無い | Day 61 の GPU 版で向き合う(今日残した歪みの3つ目) |
| 空への NEE | 空は無限に遠い面光源として働くが、向きを引く手立てが無い。空からの光は「散乱した先で偶然空に抜けた」経路だけが拾う | 環境マップの重点サンプリング(オフラインの定番) |
...
| `Geometry/RayCounters.cs` | 42(18) | Day 59 の `WhittedTracer.cs` から**移動**。散乱の光線を追加 |
| `Tracing/Tracer.cs` | 212(106) | **追い方の共通部分**(Day 59 の歪み2・3 を直したもの)。`TraceLog` / `SurfaceHit` もここへ移動 |
...
| ファイル | 差分 | 何をした |
|---|---|---|
| `Day60a.csproj` | +4 −3 | コメントだけ(Day 59 のコピーであること、高DPIの理由) |
| `Program.cs` | +8 −9 | コメントだけ(今日の主題の説明) |
| `Geometry/Shape.cs` | +34 −2 | `TrySampleDirection`(面光源用)を追加 |
| `Geometry/Sphere.cs` | +49 −0 | 上を実装。球へ向かう**円錐一様サンプリング**(要点4) |
| `Shading/Material.cs` | +50 −0 | `Emission` / `IsEmissive` / `IsSpecular` と `Light()` |
| `Tracing/Scene.cs` | +43 −9 | `Prepare`(光る形を集める)と `AreaLights`。問い合わせは総当たりのまま |
| `Tracing/SceneLibrary.cs` | +113 −2 | 場面4・5(コーネルボックス / ガラスの集光) |
| `Tracing/RenderSettings.cs` | +90 −11 | `TraceAlgorithm` / NEE / ロシアンルーレット / 乱数の相関 |
| `Tracing/WhittedTracer.cs` | +36 −216 | `Tracer` を継ぐ形に。共通部分を親へ移し、発光を足せるようにした。**木の追い方は Day 59 のまま** |
| `Tracing/ProgressiveRenderer.cs` | +17 −7 | `Tracer` を引数で受け取る。画素ごとに `Rng` を作る |
| `App/SelfCheck.cs` | +474 −8 | 項目 11〜16 を追加(要点7) |
| `App/ViewerWindow.cs` | +94 −25 | 新しいキー、HUD、下の欄のページめくり、場面 1〜5 |

**今日の量は多い**(自己チェックを除いてもコード 600 行ほど)。区切り方は下の「写経する順番」の末尾に書いた。
...
**6 から 15 までは一続き**になる。エラーの数が減っていくのを目印に進める。

1. **`Sampling/Rng.cs`** — 新規。どこにも依存しない。要点6
2. **`Sampling/Sampler.cs`** — 新規。1 を使う。要点3・4の式
3. **`Shading/Material.cs`** — `Emission` / `IsEmissive` / `IsSpecular` / `Light()` を足す
4. **`Geometry/Shape.cs`** — `TrySampleDirection` を足す(1 を使う)。
   `<see cref="WhittedTracer"/>` を `Tracer` に直した箇所もある(コメントだけの変更)
5. **`Geometry/Sphere.cs`** — 4 を実装。球へ向かう円錐サンプリング(要点4)
   — **ここまでは足すだけなので、5 を写し終えるとビルドが通る**
6. **`Geometry/RayCounters.cs`** — 新規。`Tracing/WhittedTracer.cs` の先頭にあった `RayCounters` を**そのまま移して**、
   `Scatter` を足したもの(移した理由は設計書と、ファイルのコメントに書いてある)。
   **10 で元のほうを消すまで二重定義になる。ここから 15 までビルドは通らない**
7. **`Tracing/Scene.cs`** — 3 を使う。`Prepare` / `AreaLights`
8. **`Tracing/RenderSettings.cs`** — つまみの追加
9. **`Tracing/Tracer.cs`** — 新規。1・6・7・8 を使う。`TraceLog` / `SurfaceHit` は
   `WhittedTracer.cs` から**そのまま移動**(`SurfaceHit` に `Distance` が1つ増えている)
10. **`Tracing/WhittedTracer.cs`** — 9 を継ぐ形に書き換える。**Day 59 から消える行のほうが多い**
    (`RayCounters` / `TraceLog` / `SurfaceHit` / `Sample` / `TracePixel` / `SpawnRay` / `HeatColor` などが 6・9 へ移った)
11. **`Tracing/PathTracer.cs`** — 新規。2・9 を使う。**今日の主役**。これで `Tracer.Create` の両方が揃う
12. **`Tracing/ProgressiveRenderer.cs`** — 9 を使う。`Start` の引数と、画素ごとの `Rng`
13. **`Tracing/SceneLibrary.cs`** — 3・5・7 を使う。場面4・5
14. **`App/SelfCheck.cs`** — 上の全部を使う。項目 11〜16
15. **`App/ViewerWindow.cs`** — 12・13・14 を使う。キーと HUD — **ここでビルドが戻る**
16. **`Program.cs`** — コメントのみ。差分0にしたいなら合わせておく
17. **`CpuRayTracer.csproj`**(reference では `Day60a.csproj`) — コメントのみ。同上

**区切るなら 5 まで**(足すだけで、ビルドが通る状態で止められる)。6 以降を1回で終える自信が無ければ、
**14(`App/SelfCheck.cs`)だけ先に reference からコピーしておく**と、自分で書く量が 500 行ほど減る。
...
- **`RayCounters` が `Tracing/WhittedTracer.cs` から `Geometry/` へ移った**(追い方が2つになったので。下で説明する)
...
graph TD
    ...
    Tracing --> Shading
    Tracing --> Sampling
    Geometry["Geometry/ 光線・形"] --> Shading
    Geometry --> Sampling
...
| `Geometry/` | Shading・Sampling | 場面・窓 | `Ray` / `RayCounters` / `Shape` / `Sphere` / `Plane` |
...
(そのおかげで、検証では窓を開かないコンソールのプログラムから `Tracing/` を呼んで 5 つの場面を PNG にできた)。

**`RayCounters` を `Geometry/` へ移した理由**。Day 59 の場所は `Tracing/WhittedTracer.cs` の先頭で、Whitted 法しか使わなかったからそこでよかった。
今日は `PathTracer` と、2人の親の `Tracer` も数えるので、**どちらか一方の追い方のファイルに置いておく理由が無くなった**。
数えているのは「光線」で、`Geometry/` の言葉(`Ray` の隣)——カメラ・影・反射といった**名前**を付けているのが `Tracing/` 側、という切り分けにした。
(Day 60b で BVH が「箱を何回見たか」「形を何回調べたか」を数えるようになると、`Geometry/` から使う側が現れて、この置き場所が効いてくる。)
...
`Shape` に増えた **`TrySampleDirection`** は、既定で false を返す仮想メソッド。**球だけが実装している**。
平面は無限に広いので、その上に一様な点を引くこと自体ができない——無限に広い面光源が要るなら、
空(`Scene.Sky`)のように「当たらなかったときの色」で表すほうが素直になる。
抽象メソッドにしなかったのは、**光源になれない形に「なれない」と書かせる手間を省く**ため。
...
flowchart TD
    ...
    B --> C["scene.Prepare()<br/>光る形(面光源)を集める<br/>(ここで場面を書き換えるので、先に止めておく)"]
...
**2. `Scene.Prepare` が場面を書き換える**。光る形の一覧を作るのに、不変のはずの `Scene` を後から書き換えている。
いまは「`ViewerWindow.Restart` が `Stop()` してから呼ぶ」という**約束**でしか守られておらず、型では守られていない。
Day 60b で BVH を組むのもここになるので、書き換えるものはさらに増える。
...
`dotnet run --project reference/Day60a -c Release` で起動する(**Debug は使わない**。パストレーシングは1パスが約 10 倍遅くなる)。
...
```
場面 4/5: コーネルボックス    サンプル 154 / 4096
1パス 0.112 秒   合計 20.6 秒   毎秒 4081 万本
...
(1パスの時間は機械とそのときの混み具合で変わる。上は 12 スレッドの CPU で、描画に 11 本を使ったとき。
窓を開かないハーネスで同じ場面を測ると 1パス 70〜90ms——窓では UI スレッドが1コア使うぶん遅く出る)
...
- HUD の `1画素あたり` が **8.80 → 1.04 本**。1パスが **0.07 → 0.007 秒**ほど(窓を開かないハーネスでの実測)
...
**場面1〜3 で `N` を切ると、点光源の光が丸ごと消える**(要点4)。場面1は空の光だけのぼんやりした絵になる。
...
- 深さ 8(既定): **8.80 → 15.87 本**、1パス **0.071 → 0.142 秒**
- `]` を8回押して深さ 16: **8.85 → 28.16 本**、1パス **0.071 → 0.328 秒**
- さらに深さ 32: **8.85 → 48.67 本**、1パス **0.071 → 0.558 秒**。**それでも明るさは同じ**
...
深さ 4 でもう 98% に届いている。**閉じた白い箱でこれ**なので、開けた場面ならもっと浅くて足りる。
...
### 9. `C`: 自己チェック(16 項目すべて合格、1.8 秒)

1〜10 は Day 59 のまま。11〜16 が今日ぶん。`PageUp` / `PageDown` でめくる。

```
自己チェック: 16 項目中 16 項目 OK
...
- **`V` で光線の数**: 1画素に何本使ったかを色で見る(Day 59 と同じ。紺が 1 本、白が 64 本以上)
- **`P`**: `reference/Day60a/bin/Release/net10.0-windows/captures/day60-scene4-512spp-….png` に保存。HUD は写らない
...
**3. コーネルボックスの最初の構図では、天井の光る球が HUD に隠れていた**。
カメラを 3.0m → 3.6m に引き、見下ろす角度を少し付けて収めた。ついでに白い玉を赤い壁へ 8cm 寄せて、色移りをはっきりさせた。

## 改造課題

### 課題1(易): コサイン重点をやめて、半球に一様に引く

`PathTracer` の拡散面の散乱は `Sampler.CosineHemisphere` で向きを引き、重みは `albedo` の1つだけ(要点3)。
これを `Sampler.UniformHemisphere` に替えて、重みを `albedo × 2cosθ`(`BRDF × cosθ / pdf` を pdf = 1/(2π) で計算したもの)にする。

1. 場面4を 16 / 64 / 256 サンプルで撮り、コサイン重点のときと**ノイズの量**を見比べる
2. **明るさの平均は変わらない**ことを確かめる(引き方を変えても不偏)。変わったら、重みの式か pdf がずれている
3. 自己チェック14(白い炉)が**通ったまま**であることを確かめる。通らなければ、2 と同じく式のずれ

自己チェック13 が「同じ N で重点は一様の 0.52 倍の誤差」と言っているものを、絵で見る課題。
**重みを pdf で割り戻せば、どんな引き方でも答えは同じ**で、違うのはノイズの量だけ——要点2の手口の確かめ直しになる。

### 課題2(中): 光源を明るさに比例して選ぶ

`PathTracer.SampleDirectLight` は光源(点光源と面光源)を**一様に**1つ選び、光源の数を掛けて割り戻している(設計書「NEE の中身」)。
明るい光源と暗い光源が混ざっている場面では、暗い光源にも同じだけ影の光線を使うので無駄が多い。

1. 場面1(Whitted 1980)か場面2に、明るさの違う点光源をもう1つ足す(例えば 20 倍暗いもの)
2. 光源を**明るさ(色 × 強さの成分の合計)に比例した確率** p で選び、寄与を `1 / p` で割り戻す(光源の数を掛ける代わり)
3. 同じサンプル数で、一様に選んだときより**ノイズが減り、平均は変わらない**ことを確かめる

面光源の「明るさ」は、発光 × 見かけの大きさで変わる(遠い光源は小さく見える)。**点から見た明るさで選ぶ**ところまで踏み込むと、
「どの光源を選ぶか」自体を重点サンプリングすることになる(実務の「光源の木」の入り口)。

### 課題3(難): firefly を潰す — 偏りと引き換えに

要点7の firefly(たまに1画素だけ極端に明るい点)を、実務でよく使われる方法で潰してみる。
場面5(ガラスの集光)がいちばん分かりやすい。

1. `PathTracer.Trace` の最後に、1サンプルの戻り値を上限 C で切る(`Vector3.Min(radiance, new Vector3(C))`)。C は 5 くらいから
2. 場面5を 512 サンプルで、クランプあり / なしで撮り、**明るい点の周りのざらつきがどれだけ減るか**を見る
