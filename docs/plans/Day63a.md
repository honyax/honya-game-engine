# Day 63a: ジオメトリシェーダ(図形を丸ごと作り直す段)

**教養編の3日目**。Day 57(コンピュート)・Day 58(レイマーチング)と同じく `Sandbox/` に置くが、
**今日は `Render/` も3か所だけ触る**——パイプラインに段が1つ増えるのは、エンジンの形そのものの話だから。

今日やることは1つ。**頂点シェーダと画素シェーダの間に、図形を丸ごと受け取って作り直す段を挟む**。
この段(ジオメトリシェーダ)は、三角形1枚を受け取って線を4本出したり、点1つを受け取って板を1枚出したり、
**隣の三角形まで見て、表と裏の境目の辺だけを選んで出したり**できる。

| | 頂点シェーダ(Day 14〜) | ジオメトリシェーダ(Day 63a) |
|---|---|---|
| 受け取るもの | 頂点1つ | **図形1つ**(点1・線2・三角形3・隣接付き三角形6) |
| 出すもの | 頂点1つ(**必ず1つ**) | **0 個から max_vertices 個まで** |
| 出す図形の種類 | 入れたものと同じ | **別でよい**(三角形を入れて線を出せる) |
| 隣の頂点 | 見えない | **同じ図形の全部が見える**(だから面法線が作れる) |
| 隣の三角形 | 見えない | **隣接付きで入れれば見える**(だから輪郭が出せる) |
| 何枚目の図形か | 分からない | `gl_PrimitiveIDIn` で分かる |
| 値段 | 1入れて1出るので、GPU は素直に並べられる | **素通しでも 4.89 倍**(下の枠) |

今日の差分は `Sandbox/` に新規3本、シェーダ9本、`Render/` に3か所、そして `Program.cs`。

> **今日いちばん意外だったのは、「何もしないジオメトリシェーダ」が 4.89 倍になったこと**
> 三角形 24,576 枚の球を 64 回描いて、1回ぶんの GPU の時間を測った。
> **GS 無し 44.3us に対し、受け取った三角形をそのまま1枚出すだけの GS を挟むと 216.4us**。
> 絵は1成分も変わっていない(同じ `.vert` と同じ `.frag` で、自己チェックが 245 万成分を突き合わせて確かめている)。
> 増えたのは<b>「図形を1枚この段に通す」ことそのものの値段</b>だけ。
> 理由は2つあって、**出る数が実行してみるまで分からない**ので出力を溜める場所が要ることと、
> **出た順を守らなければならない**(GL は描いた順にラスタライズすると約束している)ので待ち合わせが要ること。
> 頂点シェーダはどちらも要らない——だから「1入れて1出る」に固定されている。
> Day 64 のメッシュシェーダが「GS をやめて作り直した」のは、まさにこの2つを外すため。

## 今日のゴール

**メニューの「ジオメトリシェーダ」ページで `F2` を押すと、ゆっくり回る球の三角形すべてから法線の線が生える。
青い線が頂点法線3本、橙の線が面法線1本で、三角形 384 枚から線が 1,536 本出る。
`F3` で題材が変わり、面が弾けて三角形ごとに色が変わり、頂点が板になり、最後に**輪郭の線**だけが残る。
`F7` で溶接を切ると、立方体の辺が1本残らず輪郭になる。**

| キー(「ジオメトリシェーダ」ページ) | 何が起きるか |
|---|---|
| `F2` | ジオメトリシェーダ ON / OFF。**今日の到達点**。他のデモは全部下ろす |
| `F3` | 題材(法線を見せる / 面を押し出す / 点を板にする / **輪郭を出す**) |
| `F4` | 形(球 16x12 / 立方体 / 細かい球 128x96)。**立方体は 8 隅を 24 頂点で持っている** |
| `F5` | 量(法線の長さ・押し出す距離・板の大きさ。題材ごとに効き先が変わる) |
| `F6` | 下地(素通しの GS で元の形も描く)。切ると線だけになる |
| `F7` | 溶接する。**輪郭の題材で効く**。切ると隣が見つからず、辺が全部輪郭になる |
| `F8` | 面ごとの色。押し出しで `gl_PrimitiveIDIn` を色にする |
| `F9` | GS の値段(GS 無し / 素通しの GS / 今の題材)。**細かい球で押すこと** |
| `F10` | 内訳(溶接・辺・隣接の大きさ・出した図形の数・GL の上限) |
| `F11` | 今日の自己チェック(11 項目) |

メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「ジオメトリシェーダ」(27 枚目)が直接開く。
**実験台を出さない限り、絵は Day 58 と1ビットも変わらない**(`Render/` に足したのは口だけで、既存の呼び出しは1つも書き換えていない)。

### 今日いちばん大事な3つ

**1つ目は「この段にしか見えないものがある」**。頂点シェーダは自分の頂点1個しか見えないので、
「この三角形はどっちを向いているか」を知る手段が無い——隣の2点が分からないと外積が取れないから。
面法線・面ごとの番号・隣の三角形は、全部この段で初めて見える(要点2・3)。

**2つ目は「隣接は CPU が組む」**。`GL_TRIANGLES_ADJACENCY` はインデックスを6つずつ束ねて読むだけで、
**「隣」を探してくれるわけではない**。誰が隣かは CPU 側が組んで渡す。そして素直に組むと、
**たいてい隣が見つからない**——立方体は 8 隅を 24 頂点で持っているし、UV 球は継ぎ目が2重だから(要点4)。

**3つ目は「出る数が決まらないことの値段」**。この段が遅いのは計算が重いからではなく、
**何個出るか実行するまで分からない**から。出力を溜める場所と、順番を守るための待ち合わせが要る(要点6)。

## 事前に読む資料

- [ゲームグラフィックス特論 B-15「ジオメトリシェーダ」](https://tokoik.github.io/gg/)
  — **今日の一次資料**。入出力の型、`max_vertices`、`EmitVertex` / `EndPrimitive`、隣接付きの並び。
  講義は C++ / GLFW だが、GLSL はそのまま読める
- **西川善司「ゲーム制作者になるための3Dグラフィックス技術 改訂3版」Ch5**
  — ジオメトリシェーダが何のために入り、何に使われ、**なぜ期待ほど使われなかったか**。今日の要点6の背景
- [OpenGL Wiki: Geometry Shader](https://www.khronos.org/opengl/wiki/Geometry_Shader)
  — 入出力の対応表(`points` / `lines` / `triangles` / `lines_adjacency` / `triangles_adjacency`)と、
  `gl_PrimitiveIDIn` / `gl_Layer` / `invocations`。**隣接付き三角形の6頂点の並び方の図**はここがいちばん分かりやすい
- [OpenGL Wiki: Interface Block (GLSL)](https://www.khronos.org/opengl/wiki/Interface_Block_(GLSL))
  — 要点1。**段をまたぐ受け渡しをブロックにする理由**。今日の `Varying` ブロックがそれで、
  これのおかげで画素シェーダが「GS があるかどうか」を知らずに済む
- [LearnOpenGL: Geometry Shader](https://learnopengl.com/Advanced-OpenGL/Geometry-Shader)
  — 法線の可視化と「爆発」の2題。今日の題材のうち2つと同じ
- **Day 33 の計画書 要点4**(シャドウマップの深度バイアス) — 今日の線が面に埋もれないよう
  クリップ空間の z をずらす話は、あれとまったく同じ「深度の刻みの細かさ」の問題
- **Day 57 の計画書 要点1・8**(`#define` の差し込みと、VSync でクロックが上下する罠) —
  今日の「GS の値段」も同じ読み方をする

## 理論の要点

### 1. 段が1つ増えると、段の間の受け渡しが問題になる

ジオメトリシェーダは**入口が配列**になる。`triangles` を受けるなら3つ、`triangles_adjacency` なら6つ。
そして**出口は画素シェーダへの1つ**。つまりこの段は、同じ名前の変数を `in` と `out` の両方で宣言したくなる。

```glsl
in  vec3 vWorldPosition[];   // 手前の段から(配列)
out vec3 vWorldPosition;     // ← 同じ名前は再宣言エラー
```

これを避けるために名前を変える(`gWorldPosition` にする)と、今度は**画素シェーダの入口の名前が変わる**。
GS を挟むかどうかで画素シェーダを2本用意することになり、「GS を挟むと何 ms 増えるか」を
**同じ画素シェーダで**比べられなくなる。
...
```glsl
// gs.vert
out Varying { vec3 worldPosition; vec3 normal; vec4 color; vec2 texCoord; } OUT;
// gs-passthrough.geom
in  Varying { ... } IN[];
out Varying { ... } OUT;
// gs-surface.frag
...
段どうしの突き合わせは<b>ブロック名(`Varying`)と中身の並び</b>で決まり、手前に付ける名前(`IN` / `OUT`)は
**そのシェーダの中だけの呼び名**になる。おかげで `in` と `out` に同じ `Varying` を使えて、
**画素シェーダは GS があるかどうかを知らずに済む**。今日の「GS の値段」が成り立つのはこの1点のおかげで、
自己チェックの10項目めが「素通しの GS を挟んでも 245 万成分が1つも変わらない」ことを確かめている。

### 2. 面法線は、この段でしか作れない

`cross(b - a, c - a)` は3点が要る。頂点シェーダには1点しか来ないので、どうやっても書けない。

| どこで作るか | 何ができるか | できないこと |
|---|---|---|
| CPU | 面ごとに頂点を分ければ持てる | **頂点が3倍に増える**(立方体が 8 → 24 頂点になるのと同じ話) |
| 頂点シェーダ | 頂点法線で代用 | 丸い形だと面がばらけず、ただ膨らむ |
| **ジオメトリシェーダ** | **3点から作る**。押し出す向きにも、平らな陰影にも使える | — |
| 画素シェーダ | `dFdx` / `dFdy` で近い値 | あれは**画面の上での傾き**。面の向きそのものではない |

平らな陰影だけが目的なら、今は `flat` 補間子(provoking vertex の値をそのまま配る)のほうがずっと安い。
**GS が要るのは、頂点を動かすほう**——今日の「面を押し出す」は、面法線の向きへ三角形ごと動かしている。

`gl_PrimitiveIDIn`(何枚目の三角形か)も同じ性質を持つ。頂点シェーダには「何枚目の三角形か」という概念が無い
(同じ頂点が何枚もの三角形に使われるので、決めようがない)。押し出しの題材で三角形ごとに色が変わるのは、この番号を使っている。

### 3. 隣接付き三角形 — 6つで1枚

`GL_TRIANGLES_ADJACENCY` はインデックスを**6つずつ**束ねて1枚の三角形として読む。
...
隣の三角形は (0,1,2) / (2,3,4) / (4,5,0) になる。**共有する辺を逆向きに辿る並び**で、
これは「隣り合う2枚は辺を逆向きに共有する」= 巻き順が揃っている、という約束そのもの。

**この並びは GL が作ってくれるものではない**。誰が隣かは CPU 側が組んで渡す(要点4)。
インデックスバッファは三角形1枚につき6つになるので、**ふつうの索引の2倍**の大きさになる
(細かい球なら 72,960 個 → 145,920 個 = 570KB)。

### 4. 隣を見つけるには、先に位置で溶接する

素直に組むと、たいてい隣が見つからない。頂点が重複しているから。

| 形 | 頂点 | なぜ重複するか |
|---|---|---|
| 立方体 | 8 隅を **24 頂点** | 面ごとに法線が違う。1つの頂点は1つの法線しか持てない |
| UV 球(16x12) | 221 頂点 | 継ぎ目の1列が2重(U が 0 と 1 で違う)。極は 16 個が同じ場所 |

インデックスだけを突き合わせると、隣り合う面が「別の頂点」を使っているので**辺が共有されていないことになる**。
だから先に**位置で溶接**して、その上で辺をたどる。

```
                溶接あり            溶接なし
  立方体   8 頂点 / 18 辺 / 境界 0    24 頂点 / 30 辺 / **境界 24**
  球       178 頂点 / 528 辺 / 境界 0  221 頂点 / 604 辺 / **境界 56**
```

**ぴったり一致は見ない**。位置は計算で作られるので、UV 球の継ぎ目は φ = 0 と φ = 2π で作られ、
`sin(0) = 0` に対して `sin(2π)` は -2.4e-7 になる——同じ点のつもりでも同じ float にならない。
0.1mm の升目で丸めてから突き合わせる(`MeshAdjacency.WeldGrid`)。

**溶接すると潰れる三角形が出る**。UV 球の極に接する四角形は、2枚のうち1枚が「3頂点のうち2つが極」になるため
(16x12 の球で 32 枚)。面積 0 なので絵には出ないが、辺として数えると帳尻が合わなくなるので、組むときに外す。

**隣が無い辺**は、その辺の始点をもう一度書く。すると隣の三角形が潰れて面積 0 になり、
シェーダ側の外積が 0 ベクトルになる——つまり**必ず「裏を向いている」と判定される**ので、
表の面から見た端の辺は必ず輪郭として出る。「隣が無い」を番兵の番号で表さないのは、
**シェーダ側に分岐を足したくない**から。

### 5. 輪郭は「表と裏の境目」。だから毎フレーム変わる

三角形が表を向いているかは、巻き順から作った法線と「面からカメラへ向かうベクトル」の内積の符号で決まる
(Day 10 の背面カリングと同じ式)。**輪郭(シルエット)の辺とは、片側が表・片側が裏の辺**のこと。

```glsl
if (!isFrontFacing(v0, v2, v4)) return;        // 真ん中が裏なら何も出さない
if (!isFrontFacing(v0, v1, v2)) emitEdge(v0, v2);
if (!isFrontFacing(v2, v3, v4)) emitEdge(v2, v4);
if (!isFrontFacing(v4, v5, v0)) emitEdge(v4, v0);
```

**表の側からだけ出す**のが要点。輪郭の辺は必ず「表の三角形」と「裏の三角形」で共有されているので、
表の側からだけ出せば<b>1本の辺がちょうど1回</b>出る。両側から出すと同じ線を2回描くことになる。

カメラが動けば表裏が変わるので、**輪郭は事前に焼いておけない**。これが「見る側で決まる量を GPU で出す」
という、この段のいちばん正当な使い道になる。シャドウボリューム(特論 B-12 の前半)は、
この輪郭の辺を光と反対へ引き伸ばして「影の柱」を作る技法で、GS が入った頃いちばん期待されていた使い道だった。

**出る本数はごくわずか**。細かい球(24,320 枚)で輪郭になるのは 36,480 本の辺のうち **270 本(0.7%)**。
三角形が細かくなるほど、輪郭の辺の割合は下がっていく(輪郭は形の「縁」なので、面の数の平方根でしか増えない)。

### 6. この段が遅い理由は、計算量ではない

「何もしない GS」が 4.89 倍になる(下の表)。計算は `gl_Position` を写すだけなのに。

| | 頂点シェーダ | ジオメトリシェーダ |
|---|---|---|
| 出る数 | **必ず1つ** | 0 〜 `max_vertices` |
| 出力の置き場所 | 要らない(そのまま次へ流せる) | **溜める場所が要る**(何個来るか分からないので) |
| 順番 | 入力の順がそのまま出力の順 | **出た順を守らせる待ち合わせが要る** |

GL は「描いた順にラスタライズされる」と約束しているので、あとの三角形が先に終わっても先へ出せない。
並列に走らせながら順番を保つには待ち合わせが要り、そのぶん並列度が落ちる。

`max_vertices` の上限にもこれが出ている。

| GL に聞いた上限(RTX 3070) | 値 |
|---|---|
| 1回に出せる頂点 | 1024 |
| 1頂点あたりの成分 | 128 |
| **総成分** | **1024** |
...
**総成分が「1回に出せる頂点数 x 1頂点の成分数」に掛かる**。今日の `Varying` は 12 成分 + `gl_Position` の4成分 = 16 成分なので、
出せるのは 1024 / 16 = 64 頂点まで。**`max_vertices` を大きく書くほど、1頂点に持てる情報が減る**——
出力を溜める場所が要る、という性質がそのまま数字に出ている。

### 7. GS を測るには、画素の仕事を揃える

今日の値段の表は3行あるが、**値段として読めるのは上2行の差だけ**。

| 測ったもの(細かい球 24,576 枚 x 64 回) | GPU の時間 | 出した図形 |
|---|---|---|
| GS 無し | 44.3us | 24,576 |
| 素通しの GS | **216.4us(4.89 倍)** | 24,576 |
| 輪郭を出す | 44.0us | 270 |

上2行は**同じ `.vert` と同じ `.frag`** で、絵も1成分も変わらない(自己チェックの10項目め)。
だから差は段そのものの値段になる。

3行目は**画素の仕事が違う**。線は面よりずっと塗る画素が少ないので、GS を通っているのに GS 無しと同じ時間になる。
「GS を通すと必ず遅くなる」ではなく、**「図形を1枚通すのが高い」のであって、出す図形が減れば取り返せる**、
というのが正しい読み方。輪郭のように「入力 24,576 → 出力 270」なら、段の値段よりラスタライズの節約のほうが大きい。

VSync を入れたまま測っているので GPU のクロックは上下する。順番を入れ替えて2周してから平均を取っているが、
それでも走行ごとにぶれる(Day 57 の要点8)。**同じ条件で何度か押して、落ち着いた値を読むこと**。

### 8. 線は面に埋もれる。クリップ空間で手前へ引く

法線の線の根元も、輪郭の線も、**面のちょうど上**にある。そのまま描くと深度が同じ値の取り合いになり、
線が虫食いになる。Day 33 のシャドウアクネとまったく同じ「深度の刻みの細かさ」の問題。

```glsl
clip.z -= uDepthBias * clip.w;
```

`w` を掛けるのは、クリップ空間の z が w に比例する量だから。こうしておくと、遠くでも近くでも
「画面の奥行きで」同じだけ引ける。今日は 0.0012 にしてある。

線の太さも同じく実装任せの部分がある。**コアプロファイルでは 1.0 しか保証されていない**ので、
`GL_ALIASED_LINE_WIDTH_RANGE` を聞いてから、その範囲に収めて渡す(超えた値を渡すと `GL_INVALID_VALUE` になり、
**絵は出るので気付きにくい**)。
...
| ファイル | 役割 |
|---|---|
| `Sandbox/MeshAdjacency.cs` | **今日の C# 側の主役**。位置で溶接し、隣接付きインデックス(三角形1枚につき6つ)を組む。`Report`(溶接・辺・境界・オイラーの式)と、輪郭を数える C# の鏡 |
| `Sandbox/PrimitiveCounter.cs` | `GL_PRIMITIVES_GENERATED` の薄い皮。**GS が何個の図形を出したかを GL に数えさせる**。毎フレーム用(待たない)と自己チェック用(待つ)の2通り |
| `Sandbox/GeometryLab.cs` | **GL の側**。`GeometryTopic` / `GeometryShape` / `GeometryPass`。シェーダ6本・形3つ(それぞれ三角形/溶接あり隣接/溶接なし隣接の3メッシュ)・タイマー・カウンタ |
| `shaders/gs.vert` | 今日の5通りに共通の頂点シェーダ。**`Varying` ブロック**でワールド位置を後ろへ渡す |
| `shaders/gs-passthrough.geom` | 素通し。受け取った三角形をそのまま1枚出すだけ。**値段を測るためのもの** |
| `shaders/gs-normals.geom` | 三角形 → 線分4本(頂点法線3 + 面法線1)。`max_vertices = 8` |
| `shaders/gs-explode.geom` | 三角形 → 三角形。面法線の向きへ押し出し、`gl_PrimitiveIDIn` で色を変える |
| `shaders/gs-billboard.geom` | 点 → 四角形。ビュー行列の行からカメラの右と上を取る |
| `shaders/gs-silhouette.geom` | **今日の本題**。隣接付き三角形 → 線分0〜3本。表と裏の境目の辺だけを出す |
| `shaders/gs-surface.frag` | 面を塗る(ランバート + 環境光)。**GS の有無を知らない** |
| `shaders/gs-line.frag` | 線を塗る。陰影を付けない |
| `shaders/gs-sprite.frag` | 板を丸く抜く。四隅に入れた 0〜1 の座標で判定 |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Render/Shader.cs` | **頂点 + ジオメトリ + 画素**の3段のコンストラクタ。中身は1行も要らなかった(Day 57 で段が配列になっているので) | +28 |
| `Render/RenderResources.cs` | `LoadShader(vert, geom, frag)`。キーに3本目のパスが入る | +26 |
| `Render/Mesh.cs` | `Draw(PrimitiveType)`(`Points` / `TrianglesAdjacency` で描く)と `DrawVertices(PrimitiveType)`(索引を使わない) | +45 |
| `Program.cs` | フィールド2つ、今日の関数一式、`OnLoad` で `GeometryLab` を作る、`OnRender` に1行、HUD に1行、ページ1枚(10 項目)、`OnClosing`、既存 `Show*` 7か所に1行ずつ、`RunDebugMenuCheck` の項目数 | +783 / −1 |
| `Day63a.csproj` | `Day58.csproj` からのリネームのみ | — |

**今日の差分は Day 58 と同じくらい**(新規 1,188 行 + `Program.cs` 783 行 + `Render/` 99 行。どちらもコメント込み)。
1日で重ければ、下の順番の **18 まで**(絵が出るまで)と **19 から**(値段・内訳・自己チェック)で2回に分けるとよい。
18 の (d) でメニューの F9〜F11 の3行を飛ばせば、そこまででビルドが通り `F2`〜`F8` が全部動く。

### 写経する順番

依存の向きに沿って並べてある。**この順に写せば、途中でビルドが通らない状態を作らずに済む**。

1. **`Render/Shader.cs`**(変更) — `Shader(gl, computePath, defines)` の手前に、3段のコンストラクタを足す。
   `_stages` は Day 57 のまま。**中身は1行も足さない**
2. **`Render/RenderResources.cs`**(変更) — `LoadComputeShader` の手前に `LoadShader(vert, geom, frag)`
3. **`Render/Mesh.cs`**(変更) — `Draw()` を `Draw(PrimitiveType)` へのくぐり戸にし、`DrawVertices(PrimitiveType)` を `Dispose` の手前に
4. **`Sandbox/MeshAdjacency.cs`**(新規) — `Vertex` しか使わない。GL を1行も知らない
5. **`Sandbox/PrimitiveCounter.cs`**(新規) — GL のクエリだけ。`GpuTimer` と同じ作り
6. **`shaders/gs.vert`**(新規) — `Varying` ブロックを決める。以下の8本はこの中身と1文字ずつ揃える。
   シェーダの読み込みは実行時なので、ビルドの順番には効かない
7. **`shaders/gs-surface.frag`**(新規) — 面を塗る。**GS の有無を知らない**のが要点1
8. **`shaders/gs-line.frag`**(新規) — 線を塗る。陰影を付けない
9. **`shaders/gs-sprite.frag`**(新規) — 板を丸く抜く
10. **`shaders/gs-passthrough.geom`**(新規) — 素通し。**この段の型を掴むならここから**
11. **`shaders/gs-normals.geom`**(新規) — 三角形 → 線分4本
12. **`shaders/gs-explode.geom`**(新規) — 三角形 → 三角形。**`centroid` は GLSL の予約語**なので変数名に使えない
13. **`shaders/gs-billboard.geom`**(新規) — 点 → 四角形
14. **`shaders/gs-silhouette.geom`**(新規) — 隣接付き三角形 → 線分0〜3本。**いちばん難しいので最後に**
15. **`Sandbox/GeometryLab.cs`**(新規) — 1〜14 の全部を使う
16. **`Program.cs` のフィールド** — 「Day 58」のフィールドの直後に「Day 63a」の2つ(`_geometryLab` / `_geometryDemo`)
17. **`Program.cs` の今日の関数(前半)** — ファイル末尾の `RunRaymarchCheck` の直後に、
    `ShowGeometryLab` / `HideGeometryLab` / `GeometryLabVisible` / `RenderGeometryLab` / `GeometryLabLabel` /
    `GeometryTopicName` / `GeometryTopicNote` / `GeometryExpectedText` / `GeometryShapeName`
18. **`Program.cs` の組み込み** — 順に、
    (a) `OnLoad` の `_raymarcher = new Raymarcher(...)` の直後に `_geometryLab = new GeometryLab(...)`、
    (b) `OnRender` の `RenderRaymarch(deltaSeconds);` の直後に `RenderGeometryLab(deltaSeconds);`、
    (c) `DrawOverlayInfo` のレイマーチングの行の直後に HUD の1行、
    (d) `BuildMenu` の「レイマーチング」ページの直後に「ジオメトリシェーダ」ページ(10 項目。
    **このページの F9〜F11 の3項目は 19 で足す関数を指すので、分けて写すなら後回しにする**)、
    (e) `OnClosing` の `_raymarcher.Dispose();` の直後に `_geometryLab.Dispose();`、
    (f) **既存の `Show*` 7か所**(`HideRaymarch();` を書いた6か所の直後と、`ShowRaymarch` の `HideGpuParticles();` の直後)に
    `HideGeometryLab();` を1行ずつ、
    (g) `RunDebugMenuCheck` の項目数を **255 → 265** へ
19. **`Program.cs` の今日の関数(後半)** — 17 の続きに `DescribeGeometryLab` / `MeasureGeometryCost` /
    `RunGeometryLabCheck` / `ReadScenePixels`
20. **`Day63a.csproj`** — `Day58.csproj` をリネームするだけ(中身は同じ)

## 設計書

**層は増えていない**。Day 57 で作った `Sandbox/` にクラスが3つ増えた。
ただし**`Render/` が久しぶりに変わった**——Day 56 から Day 58 まで3日続けて1行も触らずに済んでいたが、
今日は**パイプラインに段が1つ増える**話なので、`Shader` に3段の入口、`RenderResources` に窓口、
`Mesh` に「図形の種類を指定して描く」口が要った。どれも既にある形の上に口を1つ足しただけで、中身は書き換えていない。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Sandbox/MeshAdjacency` | `Sandbox/MeshAdjacency.cs` | 溶接と隣接付きインデックス。静的クラス。**`Vertex` と `System.Numerics` しか使わない** |
| `Sandbox/MeshAdjacency.Report` | `Sandbox/MeshAdjacency.cs` | 溶接・辺・境界・潰れた三角形・オイラーの式 |
| `Sandbox/PrimitiveCounter` | `Sandbox/PrimitiveCounter.cs` | `GL_PRIMITIVES_GENERATED` の薄い皮。`GpuTimer` と同じ輪 + 待つ口が1つ |
| `Sandbox/GeometryLab` | `Sandbox/GeometryLab.cs` | GL の側。シェーダ6本・形3つ・タイマー・カウンタ |
| `Sandbox/GeometryTopic` / `GeometryShape` / `GeometryPass` | `Sandbox/GeometryLab.cs` | 題材 / 形 / 値段を比べる3通り |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Render/Shader` | 3段(頂点 + ジオメトリ + 画素)のコンストラクタ | +28 |
| `Render/RenderResources` | `LoadShader(vert, geom, frag)` | +26 |
| `Render/Mesh` | `Draw(PrimitiveType)` と `DrawVertices(PrimitiveType)` | +45 |
| `Program.cs` | ページ1枚、`OnRender` に1行、HUD に1行、出す/下ろす、値段・内訳・自己チェック | +783 / −1 |

### 三角形1枚が線4本になるまで — この段の実行モデル

```mermaid
flowchart TD
    IB["インデックスバッファ<br/>3つずつ束ねる"] --> VS["gs.vert<br/><b>頂点ごとに1回</b>。3回走る<br/>ワールド位置・法線・色を Varying へ<br/>gl_Position も入れておく"]
    VS --> ASM["プリミティブ組み立て<br/>3つの出力を1つの三角形にまとめる"]
    ASM --> GS["gs-normals.geom<br/><b>三角形ごとに1回</b>。1回だけ走る<br/>gl_in[0..2] と IN[0..2] が見える"]
    GS --> E1["頂点法線3本<br/>emitLine で 2 頂点ずつ<br/>EndPrimitive で1本ずつ切る"]
    GS --> E2["面法線1本<br/>cross&lpar;b-a, c-a&rpar; を重心から<br/><b>この段でしか作れない</b>"]
    E1 --> BUF["出力を溜める場所<br/><b>何個来るか分からない</b>ので必要<br/>max_vertices = 8 が上限"]
    E2 --> BUF
    BUF --> ORD["順番の待ち合わせ<br/><b>描いた順を守る</b>という GL の約束<br/>あとの三角形が先に終わっても出せない"]
    ORD --> RAS["ラスタライズ<br/>線分4本 x 三角形 384 枚 = 1,536 本"]
    RAS --> FS["gs-line.frag<br/>色をそのまま出す"]
```

**`EmitVertex` は「いま `OUT` に入っている値で頂点を1つ出す」**。代入の時点では何も起きず、
この行で初めて1つ確定する。出し終えた `OUT` の中身は**未定義**になるので、次の頂点で全部入れ直す。
`EndPrimitive` は「ここでひと繋がりを切る」で、`line_strip` なら切らずに出した頂点が1本の折れ線になる——
**法線の線で切り忘れると、法線の先から次の頂点へ戻る線が余分に描かれる**。

`max_vertices` を小さく書くと、超えたぶんは<b>エラーも警告も無く捨てられる</b>。
絵は「なんとなく足りない」だけなので睨んでも気付けない。だから `PrimitiveCounter` で数える
(自己チェックの7項目め: 三角形 384 枚 → 線分 1,536 本)。

**出力を溜める場所と順番の待ち合わせが、この段の値段の正体**(要点6)。素通しの GS を挟むだけで 4.89 倍になるのは、
計算が増えたからではなく、この2つが要るから。

### 隣接付きインデックスが組み上がるまで — 2周して、辺の表を1つ作る

```mermaid
flowchart TD
    V["Mesh.ReadVertices&lpar;&rpar; / ReadIndices&lpar;&rpar;<br/>GPU から読み返す<br/><b>起動時に1回だけ</b>"] --> W{"溶接するか"}
    W -->|Yes| Q["位置を 0.1mm の升目で丸めて鍵にする<br/>同じ鍵の頂点は<b>最初に出てきた番号</b>を代表にする<br/>立方体 24 → 8 / 球 221 → 178"]
    W -->|No| ID["remap[i] = i<br/><b>何もしない</b>"]
    Q --> L1
    ID --> L1
    L1["1周目: 向きのある辺の表<br/>三角形 a,b,c から<br/>a→b の向かい = c<br/>b→c の向かい = a<br/>c→a の向かい = b"]
    L1 --> DG{"溶接して潰れたか<br/>a==b or b==c or c==a"}
    DG -->|Yes| SKIP["数えて捨てる<br/><b>UV 球の極に 32 枚</b>"]
    DG -->|No| ADD["表に足す。辺の使用回数も数える"]
    ADD --> L2["2周目: 6つ組を書き出す<br/>偶数番 = <b>元の番号のまま</b>(法線と UV が要るので)<br/>奇数番 = 逆向きの辺を引いた答え"]
    SKIP --> L2
    L2 --> NB{"逆向きの辺が表にあるか"}
    NB -->|Yes| OK["その向かい側の頂点を書く"]
    NB -->|No| BND["<b>辺の始点をもう一度書く</b><br/>隣が潰れる → 外積が 0<br/>→ 必ず裏と判定される"]
    OK --> OUT["uint[] 三角形 x 6<br/>+ Report&lpar;溶接・辺・境界・オイラー&rpar;"]
    BND --> OUT
```

**偶数番を元の番号のままにする**のが効きどころ。ここを代表に置き換えると、
継ぎ目の反対側の法線と UV がやってきて、**球の継ぎ目に縦の筋が出る**。
奇数番は位置しか読まれない(`gs-silhouette.geom` が `IN[1].worldPosition` しか見ない)ので、代表でよい。

**向きを付けて持つ**のも効きどころ。「隣り合う2枚は辺を逆向きに共有する」= 巻き順が揃っている、
という約束がそのまま検査になる。向きを無視して持つと、裏返った面が混じっていても気付けない。

検算はオイラーの多面体定理に任せてある。穴の無い閉じた面なら **V - E + F = 2**。
細かさにも形にもよらないので、**絵を見ずに「ちゃんと閉じたか」を1つの数で言える**(自己チェックの4項目め)。

| | V | E | F | V - E + F |
|---|---|---|---|---|
| 立方体(溶接あり) | 8 | 18 | 12 | **2** |
| 立方体(溶接なし) | 24 | 30 | 12 | 6 |
| 球(溶接あり) | 178 | 528 | 352 | **2** |
| 球(溶接なし) | 221 | 604 | 384 | 1 |
| 細かい球(溶接あり) | 12,162 | 36,480 | 24,320 | **2** |

### 輪郭の辺が選ばれるまで — 表の側から1回だけ

```mermaid
flowchart TD
    IN["gl_in[0..5]<br/>真ん中 = 0,2,4<br/>隣の向かい = 1,3,5"] --> M{"isFrontFacing&lpar;v0, v2, v4&rpar;<br/>真ん中は表か"}
    M -->|No| STOP["何も出さない<br/><b>裏からは出さない</b>"]
    M -->|Yes| E0{"isFrontFacing&lpar;v0, v1, v2&rpar;<br/>辺 0-2 の隣は表か"}
    E0 -->|No| O0["辺 v0-v2 を出す"]
    E0 -->|Yes| E1
    O0 --> E1{"isFrontFacing&lpar;v2, v3, v4&rpar;"}
    E1 -->|No| O1["辺 v2-v4 を出す"]
    E1 -->|Yes| E2
    O1 --> E2{"isFrontFacing&lpar;v4, v5, v0&rpar;"}
    E2 -->|No| O2["辺 v4-v0 を出す"]
    E2 -->|Yes| END["おわり"]
    O2 --> END
```

**表の側からだけ出すので、1本の辺がちょうど1回出る**。輪郭の辺は必ず「表の三角形」と「裏の三角形」で
共有されているから。両側から出すと同じ線を2回描くうえに、裏側から見ると隣の判定が逆になって辻褄が合わなくなる。

**隣が無い辺は必ず出る**。隣接を組むときに潰れた三角形を書いてあるので、外積が 0 ベクトルになり、
`dot(0, eye - a) > 0` が偽 = 裏、と判定されるため。これが「溶接を切ると立方体の辺が全部出る」の正体
(`F7`。12 枚の三角形のうち表の6枚が、自分の3辺を全部出して 12 本になる)。

| 見ているもの | 辺の総数 | 輪郭になる辺 | 割合 |
|---|---|---|---|
| 立方体(溶接あり) | 18 | 6 | 33.3% |
| 立方体(溶接なし) | 30 | 12 | 40.0% |
| 球(溶接あり) | 528 | 34 | 6.4% |
| 細かい球(溶接あり) | 36,480 | 270 | **0.7%** |

**細かくするほど割合が下がる**。輪郭は形の「縁」なので、面の数の平方根でしか増えないため。
これが要点7の「入力 24,576 → 出力 270 なら、段の値段よりラスタライズの節約のほうが大きい」の中身になる。

同じ式を C# にも写してある(`MeshAdjacency.CountSilhouetteEdges`)。
GPU に数えさせた本数(`PrimitiveCounter`)と突き合わせると、**両方が同じ辺を選んでいる**ことまで言える
(自己チェックの9項目め。姿勢を4通り変えて 34:34 / 32:32 / 27:27 / 34:34)。
Day 58 の `SdfScene` と同じ「C# の鏡」の作法で、**今日は鏡のほうが本物より短い**(30 行ほど)。

### 今日残した歪み(3つ)

**1つ目: 同じ頂点バッファが3重に置かれている**。`Mesh<TVertex>` は VBO と EBO を組で持つ形なので、
「同じ頂点を別の索引で読む」ができない。形1つにつき三角形用・溶接あり隣接用・溶接なし隣接用の3つを作っていて、
細かい球なら 12,513 頂点 x 96 バイト x 3 = 3.4MB を余分に持っている。
本来は VAO だけ差し替えて VBO を共有したいところ(改造課題3)。

**2つ目: 板の題材だけ `DrawVertices` を使っている**。索引を使わずに頂点を順に描くので、
「板1枚 = 頂点1つ」の対応が保たれて数えられる——が、**メッシュの意味では正しくない**。
UV 球の極の 16 頂点は同じ位置にあるので、そこだけ板が 16 枚重なっている。
溶接した頂点の配列を別に持てばきれいになるが、今日の主題ではないので置いた。

**3つ目: `MeshAdjacency` の溶接が升目の丸めだけ**。升目の境目をまたぐ2点(0.09mm しか離れていないのに別の升)は
溶接されない。自分で作ったメッシュには効くが、外から読んだモデル(Day 39 の glTF)では取りこぼしうる。
本気でやるなら格子の 27 近傍を見るか、空間分割で近傍を探す。

### 引き継いだ図

Day 58 の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。変わった図は次の3つ。

| 図 | 何が変わったか |
|---|---|
| `全体構成` | `Sandbox/` → `Render/` の矢印のラベルに **`Mesh` / `Primitives` / `Vertex`** が加わった。今日の段落を足した |
| `Render` のクラス図 | **`Shader` の3段のコンストラクタ、`RenderResources.LoadShader(vert, geom, frag)`、`Mesh.Draw(mode)` と `DrawVertices(mode)`** |
| `Sandbox` のクラス図 | 3枚目を足した。**`GeometryLab` / `MeshAdjacency` / `PrimitiveCounter` と、`GeometryTopic` / `GeometryShape` / `GeometryPass` / `Report`** |
| `1フレームの流れ` | `RenderRaymarch` と `RenderParticles` の間に **`RenderGeometryLab`** |

新しく足した図が3つ(この節の上の3つ)。
Day 58 の設計書の冒頭にあった4つの図(1画素の色が決まるまで、球面追跡の3つの出口、深度が橋になる、同じ式が2か所にある)は
レイマーチングの話なので、今日は外した(Day 58 の計画書を見ること)。

`Model/`・`Scene/`・`Ecs/`・`Physics/`・`Text/`・`Audio/`・`Game/`・`Demo/`・`Core/` の図は**昨日のまま**で、今日は1つも触っていない。

### 全体構成 — 9つの層と、その上のゲーム・デモ・実験台(Day 63a で `Sandbox/` の矢印が太くなった)
...
### Render — OpenGL の薄い皮(Day 63a で段を1つ挟めるようになった)

**Day 63a で3つの口が増えた**。どれも既にある形の上に足しただけで、中身は1行も書き換えていない。
...
### Sandbox — 教養編の実験台(Day 63a でジオメトリシェーダの3つが増えた)

Day 57 のコンピュート(`GpuParticles`)、Day 58 のレイマーチング(`Raymarcher` / `SdfScene` / `Sdf`)、
Day 63a のジオメトリシェーダ(`GeometryLab` / `MeshAdjacency` / `PrimitiveCounter`)は**互いを知らない**ので、図を3枚に分けてある。
**題材が増えても互いに線が伸びない**のが `Sandbox/` の値打ちで、Day 61・64 もここに並ぶ予定。
...
`dotnet run --project reference/Day63a -c Release` で起動する。起動直後のコンソールは Day 58 と同じ(今日は起動時に何も足していない)。
メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「ジオメトリシェーダ」(27 枚目)が開く。
...
```
ジオメトリシェーダ: **ON**  法線を見せる  球(16 x 12)  頂点 221 / 三角形 384  → この段が出す図形は 1,536 個
```

Day 31 の床と箱が消え、暗い背景の真ん中に**ゆっくり回る球**が出る。球の表面は薄い灰色に塗られ(素通しの GS で描いた下地)、
そこから線が**針山のように**生えている。

- **青 → 水色の線が3本ずつ** … 頂点法線。球なので放射状にまっすぐ外を向く
- **橙 → 黄の線が1本ずつ** … 面法線。三角形の重心から生え、**頂点法線より少し寝ている**(平らな面の向きなので)

HUD に今日の行が出る。

```
GS 法線を見せる  球(16 x 12)  三角形 384  出した図形 1,536 (見積もり 1,536)  GPU 0.03ms
```

**「出した図形」と「見積もり」が一致すること**が今日の1つ目の関門。出した図形は `GL_PRIMITIVES_GENERATED` で
GL に数えさせた実測、見積もりは C# が「三角形 x 4」で出した数。
**`F6` で下地を切っても数が変わらない**(下地は数えていない)。

### 2. `F3`: 題材を回す

- **面を押し出す** … 球の表面が三角形ごとにばらけて外へ膨らみ、**息をするように出入りする**。
  三角形は `gl_PrimitiveIDIn` で色が変わるので、384 枚が全部違う色になる。
  中に下地の球が残っているので、どれだけ離れたかが見える。出した図形は **384**(1枚入れて1枚出る)
- **点を板にする** … 球の頂点 221 個が、それぞれカメラを向いた丸い板になる。
  色は法線から作っているので、球の向きに沿って色が回る。出した図形は **442**(頂点 x 2 枚)。
  **極では板が 16 枚重なっている**(UV 球の極は 16 頂点が同じ位置。設計書の「今日残した歪み」2つ目)
- **輪郭を出す** … 線が全部消え、**球の縁に1本の輪**だけが残る。回っても輪の位置は変わらない(球なので)。
  出した図形は 30〜40 本で、HUD の見積もりは「カメラ次第」になる

### 3. `F4` と `F7`: 立方体で溶接を切る

`F3` を押して「輪郭を出す」にし、`F4` で立方体にする。

- **溶接 ON** … 立方体の**外周6辺**だけに線が出る。見えている3面の境目には線が出ない
- **`F7` で溶接 OFF** … **見えている3面の境目にも線が出て、12 本になる**

これが今日の2つ目の関門。立方体は 8 隅を 24 頂点で持っているので、隣り合う面が別の頂点を使っており、
インデックスだけでは辺が共有されていないことになる(要点4)。コンソールにその数が出る。

```
溶接する: **OFF**  頂点 24 → 24  **隣が無い辺 24 / 30**  隣が見つからない辺は必ず輪郭として出る。**立方体は全部の辺が出る**
```

`F4` をもう一度押して**細かい球**(128 x 96)にすると、輪郭が滑らかな1本の円になる。
このとき出している線は **270 本**で、辺 36,480 本の 0.7% しかない。

### 4. `F10`: 内訳

```
--- ジオメトリシェーダの内訳 ---
  題材: 輪郭を出す  形: 細かい球(128 x 96)  出している: ON
  形: 頂点 12513 / 三角形 24576
  溶接あり: 頂点 12513 → **12162**  (潰れた三角形 256 を除いて 24320 枚)  辺 36480  **隣が無い辺 0**  V - E + F = 2
  溶接なし: 頂点 12513(そのまま)  三角形 24576 枚  辺 37088  **隣が無い辺 448**  V - E + F = 1
  隣接付きインデックス: 145,920 個 = 570.0KB (ふつうの索引の **2 倍**。72,960 個 → 145,920 個)
  この段が出した図形(GL_PRIMITIVES_GENERATED): **270**
  いまのカメラでの輪郭(C# の鏡): **270 本**  表を向いている三角形 9574 / 24320  → 辺のうち **0.7%** だけが輪郭
  この GL が許す上限: 1回に出せる頂点 1024  1頂点あたりの成分 128  **総成分 1024**  同時実行 32
```

**GL に数えさせた値と C# の鏡の値が一致する**のが今日の3つ目の関門。
溶接あり側の **V - E + F = 2**(オイラーの多面体定理)も見どころで、これが 2 でなければ形が閉じていない。

### 5. `F9`: GS の値段

**細かい球にしてから押すこと**(三角形が少ないと描画1回ぶんの固定費に埋もれる。押すと注意が出る)。

```
--- GS の値段(細かい球(128 x 96)  三角形 24,576 枚を 64 回 x 2 周)---
  GS 無し                        44.3us  出した図形   24,576  GS 無しの **1.00 倍**
  素通しの GS                     216.4us  出した図形   24,576  GS 無しの **4.89 倍**
  今の題材(輪郭を出す)                  44.0us  出した図形      270  GS 無しの **0.99 倍**
```

**上2行は絵が1成分も変わらない**(同じ `.vert` と同じ `.frag`)。それでも 4.89 倍になるのが今日の到達点で、
差は「図形を1枚この段に通す」ことそのものの値段(要点6)。
3行目は画素の仕事が違うので、倍率は段の値段として読まないこと(要点7)。

VSync を入れたまま測っているので、走行ごとに 4.3〜5.0 倍の間でぶれる。何度か押して落ち着いた値を読む。
...
```
=== Day 63a 自己チェック(溶接・隣接・出した図形の数・素通しで絵が変わらないこと)===
  [OK] **位置で溶接すると頂点が減る**(立方体 24 → 8。面ごとに法線を分けたぶんが戻る)  立方体 24 → 8 / 球 221 → 178
  [OK] **溶接すれば隣が全部見つかり、溶接しなければ1つも見つからない**(立方体)  立方体 溶接あり 0 本 / 溶接なし 24 本(全 30 辺)
  [OK] **球も溶接すれば閉じる**(継ぎ目と極のぶんが残らない)  球 溶接あり 0 本 / 溶接なし 56 本(全 604 辺)
  [OK] **V - E + F = 2**(球も立方体も、溶接したあとなら成り立つ)  立方体 8 - 18 + 12 = 2 / 球 178 - 528 + 352 = 2
  [OK] **隣接の奇数番が、本当に隣の三角形を指している**(球なら隣り合う面のなす角は 23° 以下)  1,056 組で、面どうしのなす角は最大 22.31°
  [OK] **素通しの GS は、入れた三角形の数をそのまま出す**  三角形 384 枚 → 出した図形 384
  [OK] **法線の題材は、三角形1枚につき線分4本**  三角形 384 枚 → 線分 1536(見積もり 1536)
  [OK] **板の題材は、頂点1つにつき三角形2枚**  頂点 221 個 → 三角形 442(見積もり 442)
  [OK] **GPU が出した輪郭の本数と、C# の鏡が数えた本数が一致する**  GPU 34 : C# 34 / GPU 32 : C# 32 / GPU 27 : C# 27 / GPU 34 : C# 34
  [OK] **素通しの GS を挟んでも、絵が1成分も変わらない**  違った成分 0 / 2,457,600(最大の差 0.0E+000)
  [OK] **いちばん多く出す題材(法線の8頂点)が、この GL の上限に収まっている**  上限 頂点 1024 / 1頂点の成分 128 / 総成分 1024
```

前半5項目は**窓を開かずに走る**(`MeshAdjacency` が GL を1行も知らないので)。
後半6項目はシーンのバッファを借りて描き、`GL_PRIMITIVES_GENERATED` と `glReadPixels` で確かめる。

### 7. メニューの自己チェック

`Alt+F1` で Day 48 の割り当て表のチェックが走り、**27 ページ 265 項目**になっている。

### 8. 起動して何もしなければ、絵は Day 58 のまま

`F2` を押さない限り、`GeometryLab` は1回も描かない。`Render/` に足したのは口だけで、
既存の呼び出し(`LoadShader(vert, frag)` / `Draw()`)は1つも書き換えていない
(`Draw()` は `Draw(PrimitiveType.Triangles)` へのくぐり戸になった)。

## 改造課題

### 課題1(易): 面ごとの番号を画素シェーダで受け取る

`gl_PrimitiveIDIn`(ジオメトリシェーダの入口)と `gl_PrimitiveID`(画素シェーダの入口)は別物。
**ジオメトリシェーダがある場合、画素シェーダの `gl_PrimitiveID` は「この段が `gl_PrimitiveID` に入れた値」になる**。

1. `gs-explode.geom` で `gl_PrimitiveID = gl_PrimitiveIDIn;` を書く(`EmitVertex` の前)
2. `gs-surface.frag` で `gl_PrimitiveID` を読み、色を作る(`primitiveColor` を画素シェーダへ写す)
3. `.geom` の代入を**消して**みる。何色になるか。なぜそうなるか
4. `F3` で素通しの下地(`ShowBase`)を見る。下地の三角形にも番号が付いているか

**「段を挟むと、組み込み変数の出どころが変わる」**ことの手触りを掴むのが目的。
3 で何が起きるかは実装によって違う(未定義)ので、**手元の答えが仕様ではない**ことも確かめておくとよい。

### 課題2(中): シャドウボリュームの「柱」を出す

今日の輪郭は線だが、**輪郭の辺を光と反対へ引き伸ばすと影の柱になる**(特論 B-12 の前半)。

1. `gs-silhouette.geom` を写して `gs-shadowvolume.geom` を作る。
   出すものを `line_strip` から `triangle_strip` へ、`max_vertices` を 6 から 12 へ
2. 輪郭の辺 `a`-`b` を出す代わりに、4点 `a` / `b` / `a + L*d` / `b + L*d` で四角形を1枚出す
   (`d` は光の向き、`L` は 20m ほど)。**巻き順に注意**——`triangle_strip` の Z 字の順に並べること
3. 半透明(`glEnable(GL_BLEND)`、アルファ 0.15)で描き、深度は書かない。
   カメラを回すと、球から光と反対へ伸びる**影の柱**が見える
4. `F7` で溶接を切る。柱がどう壊れるか(隣が無い辺も柱を出すので、**全部の辺から柱が生える**)

見どころは**「柱の側面だけでは閉じていない」**こと。本物のシャドウボリュームは
前面(表の三角形をそのまま)と後面(裏の三角形を L だけ伸ばしたもの)も要る。
なぜ要るのかは、柱の中にカメラを入れてみれば分かる。

### 課題3(難): 頂点バッファを3重に持つのをやめる

設計書の「今日残した歪み」の1つ目。形1つにつき `Mesh` を3つ作っているので、
細かい球で 3.4MB を余分に持っている。索引だけ差し替えられるようにする。

1. `Mesh<TVertex>` に、**VAO と EBO だけを追加で作る**口を足す
   (`CreateIndexView(ReadOnlySpan<uint> indices)`。VBO は使い回し、頂点属性の設定も同じものを繰り返す)
2. 返り値は「VAO とインデックス数」を持つ小さな型にする。`Mesh` 本体の `Dispose` で一緒に畳む
3. `GeometryLab.Shape` の `WeldedAdjacency` / `RawAdjacency` をこれに置き換える
4. `F10` の内訳に、頂点バッファのバイト数を出して 1/3 になったことを確かめる

見どころは**「VAO はインデックスバッファの結び付けを記録する」**こと(Day 13 の要点3)。
VAO を差し替えれば EBO も一緒に切り替わるので、描く側は `BindVertexArray` を変えるだけでよい。
ここでつまずくなら、`Mesh` のコンストラクタの最後で **VAO を先に外している**理由を読み直すこと——
同じ罠がここでも待っている。
...
| OpenGL | 4.3 コアプロファイル(ドライバ 596.49)。**ジオメトリシェーダは 3.2 の機能**(`#version 430 core` で書いてあるが、330 でも動く) |
| 解像度 | 960 x 640 |
| 確認の条件 | `-c Release`、**VSync を入れて**確認した。値段の測定は 64 回 x 2 周をタイマークエリで挟んだもので、`glFinish` の連続描画(計測)は今日も無い |

形ごとの数字。

| | 球(16x12) | 立方体 | 細かい球(128x96) |
|---|---|---|---|
| 頂点 | 221 | 24 | 12,513 |
| 三角形 | 384 | 12 | 24,576 |
| 溶接後の頂点 | **178** | **8** | **12,162** |
| 潰れた三角形 | 32 | 0 | 256 |
| 辺(溶接あり) | 528 | 18 | 36,480 |
| 隣が無い辺(溶接なし) | 56 | 24 | 448 |
| V - E + F(溶接あり) | 2 | 2 | 2 |
| 隣接付きインデックス | 2,112 個 / 8.2KB | 72 個 / 0.3KB | 145,920 個 / 570KB |

題材ごとの数字(球 16x12、下地 ON、960x640)。

| 題材 | 出した図形 | GPU の時間 |
|---|---|---|
| 法線を見せる | 1,536(三角形 x 4) | 0.03ms |
| 面を押し出す | 384(三角形 x 1) | 0.05ms |
| 点を板にする | 442(頂点 x 2) | 0.04ms |
| 輪郭を出す | 27〜40(カメラ次第) | 0.03ms |
| 輪郭を出す(細かい球) | 270 | 0.35ms |

GS の値段(細かい球 24,576 枚 x 64 回 x 2 周)。

| | GPU の時間 | 比 |
|---|---|---|
| GS 無し | 44.3us | 1.00 |
| 素通しの GS | **216.4us** | **4.89**(走行ごとに 4.3〜5.0 でぶれる) |
| 輪郭を出す | 44.0us | 0.99(**画素の仕事が違うので段の値段ではない**) |
