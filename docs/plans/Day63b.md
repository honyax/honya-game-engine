# Day 63b: テッセレーション(4隅だけ送って、GPU に割らせる)

**教養編の4日目**。Day 63a の続きで、同じく `Sandbox/` に置き、`Render/` は2か所だけ触る。

今日やることは1つ。**頂点シェーダの後ろに段を2つ挟んで、四角形1枚を GPU の中で何千枚にも割る**。
CPU から送るのは<b>パッチの4隅だけ</b>で、割った点がどこへ行くかは評価シェーダの式が決める。
...
| | ジオメトリシェーダ(Day 63a) | テッセレーション(Day 63b) |
|---|---|---|
| 増やすのは誰か | **自分**(`EmitVertex` を呼ぶ) | **固定機能の回路**(6つの数を渡すだけ) |
| 出せる量の上限 | 1回に 1024 頂点(総成分 1024) | **1パッチから三角形 8,192 枚**(レベル 64) |
| 何を決めるか | 出す図形そのもの | <b>どれだけ細かく割るか</b>と、<b>割った点の居場所</b> |
| 隣のパッチ | 隣接付きで渡せば見える | **見えない**。だから「隣と同じ答えになる決め方」が要る(要点5) |
| 値段 | 素通しでも 4.89 倍 | レベル 1 なら三角形1万枚あたり 573us、レベル 64 なら **6.8us** |
| 今の立ち位置 | メッシュシェーダに置き換わりつつある | **地形・水面・曲面で現役**(西川本 Ch6) |

今日の差分は `Sandbox/` に新規2本、シェーダ3本、`Render/` に2か所、そして `Program.cs`。
**画素シェーダは Day 63a の `gs-surface.frag` をそのまま使う**——`Varying` ブロックを揃えてあるので、
間に何段挟まっていても同じ1本で受けられる(Day 63a の要点1がここで効く)。

> **今日いちばん意外だったのは、割れば割るほど1枚あたりが安くなったこと**
> パッチ 64 枚を 32 回描いて、三角形1万枚あたりの GPU の時間を測った。
> レベル 1(三角形 128 枚)で **573us**、レベル 64(524,288 枚)で **6.8us**。**84 倍の開き**がある。
> 三角形の数はレベルの2乗で増えるのに、時間はそこまで増えない——
> パッチあたりの固定費(制御シェーダ4回・テッセレータのセットアップ)が薄まるから。
> <b>「細かく割るのは高い」より「パッチを細かく分けるのが高い」</b>ほうが実態に近い。
> 同じ三角形の数なら、<b>パッチを減らしてレベルを上げる</b>ほうが安い、ということでもある。

## 今日のゴール

**メニューの「テッセレーション」ページで `F2` を押すと、8x8 のパッチでできた波打つ地形が出る。
CPU が送った頂点は 256 個(24KB)だけで、GPU が作った三角形は 8,192 枚。
`F9` で線にすると、四角形1枚が細かい格子に割られているのが見える。
`F5` で距離に応じて割り方を変えると、手前が細かく奥が粗くなる。
`F7` で辺を隣と揃えるのをやめると、パッチの継ぎ目に**亀裂**が走る。**

| キー(「テッセレーション」ページ) | 何が起きるか |
|---|---|
| `F2` | テッセレーション ON / OFF。**今日の到達点**。他のデモは全部下ろす |
| `F3` | 題材(平らな格子に高さを付ける / 球(立方体を膨らませる)) |
| `F4` | レベル(1 → 2 → 4 → 8 → 16 → 32 → 64)。**2倍にすると三角形は4倍** |
| `F5` | 距離で決める。近いパッチほど細かく。**2 の冪へ落とす**ので隣と2倍違いうる |
| `F6` | 割り方(equal / fractional_odd / fractional_even)。**uniform にできないので3本焼いてある** |
| `F7` | 辺を隣と揃える(既定 ON)。**切ると亀裂が出る** |
| `F8` | 変位の強さ。**0 にすると平らなまま**(割っても絵が1画素も変わらない) |
| `F9` | 線で描く。**今日はこれが主な見方** |
| `F10` | パッチごとの色(`gl_PrimitiveID`)。どこまでが1パッチかが見える |
| `F11` | 値段(レベルごとの三角形の数と GPU の時間・距離 LOD で何%に減るか) |
| `F12` | 内訳(送った頂点と出た三角形の比・辺の合意・GL の上限) |
| `1` | 今日の自己チェック(12 項目) |

メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「テッセレーション」(28 枚目)が直接開く。

### 今日いちばん大事な3つ

**1つ目は「割るのは自分ではない」**。この段が書くのは <b>`gl_TessLevelOuter[4]` と `gl_TessLevelInner[2]` の6つの数</b>だけ。
実際に点と三角形を作るのは、プログラムできない回路(テッセレータ)。
Day 63a のジオメトリシェーダが「出力を溜める場所」の制約を抱えていたのは自分で出すからで、
回路に任せればその制約が消える——**レベル 64 なら1パッチから 8,192 枚**(GS の上限は 1024 頂点)。

**2つ目は「隣が見えない」**。パッチは互いを知らないので、共有する辺のレベルが食い違うと**そこに隙間が開く**(亀裂)。
直し方は<b>「共有しているものから決める」</b>の一言——辺の中点は両側で同じ点なので、
そこから決めれば必ず同じ値になる(要点5)。

**3つ目は「割っただけでは何も変わらない」**。レベルを 64 にしても、評価シェーダが
4隅を双一次補間するだけなら<b>絵は1画素も変わらない</b>(平らな四角形が細かい平らな四角形になるだけ)。
細かくする意味は、**変位**(高さを付ける)や**曲面へ写す**(球にする)といった
「割った点を動かす」式があって初めて出る(`F8` で変位を 0 にすると手触りが分かる)。

## 事前に読む資料

- [ゲームグラフィックス特論](https://tokoik.github.io/gg/)
  — B-15 の続きとしてのテッセレーション。**パッチ・制御点・レベル**の言葉遣いをここで揃える
- **西川善司「ゲーム制作者になるための3Dグラフィックス技術 改訂3版」Ch6**
  — 「DirectX 11 のテッセレーション」。**何のために入り、どこで現役なのか**(地形・水面・キャラクタの曲面化)。
  今日の題材2つはここから取っている
- [OpenGL Wiki: Tessellation](https://www.khronos.org/opengl/wiki/Tessellation)
  — 要点2・3・4の一次資料。<b>`quads` / `triangles` / `isolines` それぞれの `gl_TessCoord` の意味</b>と、
  outer / inner レベルがどの辺に対応するかの図。**ここを読み違えると特定の向きの辺にだけ亀裂が出る**
- [OpenGL Wiki: Tessellation Control Shader](https://www.khronos.org/opengl/wiki/Tessellation_Control_Shader) と
  [Tessellation Evaluation Shader](https://www.khronos.org/opengl/wiki/Tessellation_Evaluation_Shader)
  — `gl_InvocationID` / `barrier()` / `gl_TessCoord` / `gl_PatchVerticesIn` の扱い
- [LearnOpenGL: Tessellation(CPU tessellation / GPU tessellation)](https://learnopengl.com/Guest-Articles/2021/Tessellation/Height-map)
  — 高さマップから地形を作る実例。今日の「変位」と同じ筋道
- **Day 63a の計画書 要点1**(インターフェースブロック) — 今日の `Varying` ブロックがそのまま効いている。
  **Day 63a の要点6・7**(段の値段と、測るときに画素の仕事を揃える話)も今日そのまま使う
- **Day 33 の計画書 要点2**(シャドウマップの解像度と、見えているものに合わせる話) —
  距離 LOD の考え方は「見えている大きさに合わせて解像度を配る」で、あれと同じ

## 理論の要点

### 1. パッチは三角形ではない

`GL_PATCHES` で描くと、GL は<b>インデックスを n 個ずつ束ねて1つのパッチ</b>にする。
n は `glPatchParameteri(GL_PATCH_VERTICES, n)` で決める GL 側の状態で、**シェーダの layout ではない**。

```csharp
_gl.PatchParameter(GLEnum.PatchVertices, 4);   // 4つで1パッチ
mesh.Draw(PrimitiveType.Patches);
```

| | 三角形(Day 14〜) | パッチ(Day 63b) |
|---|---|---|
| インデックスの束ね方 | 3つ | **n 個**(今日は4。上限は 32 以上) |
| 頂点の意味 | **形の点**。そのまま画面に出る | **形を決める数**。そのままでは画面に出ない |
| 頂点シェーダ | 出した `gl_Position` がラスタライザへ | 出した値は<b>制御シェーダが読むだけ</b> |
| 描いたあと | ラスタライズ | **制御 → テッセレータ → 評価**を通ってからラスタライズ |

`glPatchParameteri` の数と、制御シェーダの `layout (vertices = 4) out;` は**別のもの**。
前者は<b>入れる</b>制御点の数、後者は<b>出す</b>制御点の数で、
食い違うと `GL_INVALID_OPERATION` になるが——**絵が出ないだけで理由は言ってくれない**。

### 2. 制御シェーダが書くのは6つの数だけ

```glsl
gl_TessLevelOuter[0..3]   // 4つの辺を、それぞれ何等分するか
gl_TessLevelInner[0..1]   // 内側を、横と縦に何等分するか
```

外側の4つが<b>どの辺に対応するか</b>は GL が決めている。四角パッチなら:
...
**ここを1つでも取り違えると、特定の向きの辺にだけ亀裂が出る**——
「なぜか縦の継ぎ目だけ割れる」という、原因の見当が付きにくい壊れ方をする。

内側は外側からはみ出さない値にしておく。辺に置く点の数は外側で決まっているので、
内側だけ細かくしても「中だけ細かい歪んだ割り方」になるだけ。

**この段はパッチの制御点1つにつき1回走る**(四角パッチなら4回)。
レベルを決めるのは1回でよいので、`gl_InvocationID == 0` の回だけが書く。
4回走るのは「制御点を作り変える」使い方(ベジエの制御点を計算する等)のためで、今日は使わない。

### 3. 評価シェーダは「升目の (u,v) を、形の上の点に翻訳する」

テッセレータが 0〜1 の升目の上に点を作り、その点1つにつき1回、評価シェーダが呼ばれる。
受け取るのは <b>`gl_TessCoord`</b>(四角パッチなら (u, v)、三角パッチなら重心座標)で、
<b>制御点は `gl_in[]` で全部見える</b>。

```glsl
vec3 base = mix(mix(p00, p10, u), mix(p01, p11, u), v);   // 双一次補間
world = base + up * height(base.xz) * uDisplacement;      // ここが変位
```

**レベル 64 の四角パッチなら 65 x 65 = 4,225 回**走る。CPU が送った頂点は4つだけ。
投影(`uViewProjection`)を掛けるのはこの段——割る前の制御点は画面に出ないので、
クリップ空間へ写すのはいちばん後ろで1回でよい。

**割っただけでは絵が変わらない**。双一次補間しかしないなら、
平らな四角形が細かい平らな四角形になるだけで1画素も変わらない。
細かくする意味は<b>割った点を動かす式</b>(変位・球面への写像)があって初めて出る。

### 4. 割り方は3つ。**uniform にできない**

```glsl
layout (quads, fractional_odd_spacing, ccw) in;
```

割り方は **layout 修飾子**なので、コンパイル時に決まる。
3通りを見比べるには<b>同じソースを3回コンパイルする</b>しかない——
Day 57 でコンピュートのワークグループの大きさに使った `#define` の差し込みが、そのまま効く。

| 割り方 | 丸め方 | レベル 4.3 → | 特徴 |
|---|---|---|---|
| `equal_spacing` | 整数へ切り上げ | 5 | **1 刻みでしか変われない**。変わる瞬間に面全体の頂点が一斉に動く(ポッピング) |
| `fractional_odd_spacing` | 奇数へ切り上げ | 5 | 端の2本だけを伸び縮みさせる。**1 から連続に変えられる**。LOD の定石 |
| `fractional_even_spacing` | 偶数へ切り上げ | 6 | 同じく連続。**レベル 1 が作れない**(最低 2) |

`fractional_odd` は**レベル 4 でも 5 等分**になる(奇数へ切り上げるので)。
自己チェックの10項目めがこれを GPU に数えさせている(レベル 4 も 5 も 3,200 枚で同じ)。

### 5. 亀裂 — パッチは隣を知らない

隣り合う2枚のパッチは辺を共有している。その辺に置く点の数が両側で違うと、
**辺の上の点が一致せず、隙間が開く**。これがテッセレーションでいちばん有名な落とし穴。

直し方は<b>「共有しているものから決める」</b>の一言に尽きる。

| 決め方 | 隣と一致するか | 理由 |
|---|---|---|
| **辺の中点から** | **必ずする** | 中点は両側で**同じ点**。同じ入力に同じ式なら同じ答え |
| パッチの中心から | しない | 中心は両側で**違う点**。距離が違えばレベルも違う |

今日は距離レベルを<b>2 の冪へ落として</b>いる(1, 2, 4, 8, 16, 32, 64)。実務でよくやる形で、
段を粗く刻むと**カメラが少し動いただけでレベルが揺れない**。
代わりに<b>隣り合うパッチのレベルが 2 倍違いうる</b>ので、揃えなかったときの亀裂がはっきり出る。

**「亀裂が出ないこと」は絵では言えない**——カメラを動かせばいつか出るかもしれない、を否定できない。
だから C# の鏡で、**すべての内部の辺(112 本)を4通りのカメラで調べて食い違い 0** を数で言う(自己チェック5・6)。

隙間の大きさは、辺の等分の数の食い違いで決まる。

| 等分の組 | 辺の上の点のずれ(辺の長さに対して) |
|---|---|
| 4 と 5 | **10.0%** |
| 8 と 16 | **0.0%**(割り切れるので、片方の点が全部もう片方に含まれる) |

直線の辺なら点がずれても同じ線の上に乗るので隙間は開かない。
**開くのは変位したあと**——辺の上の違う場所の高さを取るので、片方が山の上、片方が谷の途中になる。

### 6. 距離 LOD — 「画面での三角形の大きさ」を揃える

```glsl
level = clamp(uLevel * uLodDistance / distance, 1.0, 64.0);
```

距離に反比例させるのは、画面に写る大きさが距離に反比例するから。
「割ったあとの三角形が、画面でだいたい同じ大きさになる」ようにしたい。

値打ちは<b>何と比べるか</b>で決まる。

| | 三角形 | 備考 |
|---|---|---|
| 全体をレベル 32 で固定 | 131,072 枚 | **いちばん細かいパッチに合わせた場合** |
| 距離で決める | **48,176 枚(36.8%)** | 遠いパッチが粗くなるぶん |

基準のレベル(8m でレベル 16)とそのまま比べると、**距離で決めたほうが多くなる**——
近いパッチはそれより細かくなるので当然。
「同じ細かさを全体に配るより、距離で配るほうが少なくて済む」が正しい言い方になる。

### 7. 値段は「三角形の数」ではなく「パッチの数」で決まる

パッチ 64 枚を 32 回描いた測定。

| レベル | 三角形 | GPU | 三角形1万枚あたり |
|---|---|---|---|
| 1 | 128 | 7.3us | **573us** |
| 4 | 2,048 | 12.7us | 61.9us |
| 8 | 8,192 | 22.6us | 27.5us |
| 16 | 32,768 | 54.8us | 16.7us |
| 32 | 131,072 | 129.4us | 9.9us |
| 64 | 524,288 | 355.5us | **6.8us** |

三角形の数はレベルの**2乗**で増えるのに、時間は**2乗まで増えない**。
パッチあたりの固定費(制御シェーダ4回・テッセレータのセットアップ)が薄まるから。

**同じ三角形の数なら、パッチを減らしてレベルを上げるほうが安い**、ということでもある。
「テッセレーションは細かくすると高い」より、**「パッチを細かく分けると高い」**ほうが実態に近い。

なお実際のゲームでは、**細かすぎる三角形はラスタライザの側で高くつく**
(1画素より小さい三角形は 2x2 の画素の塊で処理されるので、4倍近く無駄になる)。
これが Nanite(Day 65)や、そもそもメッシュシェーダ(Day 64)が解こうとしている問題の1つ。

### 8. `Varying` ブロックは段が増えても効く

今日の画素シェーダは **Day 63a の `gs-surface.frag` をそのまま使っている**。
ブロック名(`Varying`)と中身の並びが揃っていれば、間に何段挟まっていても繋がるから。

```
  gs.vert   →                                      → gs-surface.frag   (Day 63a・GS 無し)
  gs.vert   → gs-passthrough.geom                  → gs-surface.frag   (Day 63a)
  tess.vert → tess.tesc → [回路] → tess.tese       → gs-surface.frag   (Day 63b)
```

Day 63a の要点1で「画素シェーダが GS の有無を知らずに済む」と書いた仕掛けが、
**段が2つ増えてもそのまま効いた**。`tess.vert` → `tess.tesc` の間だけブロック名を `Control` に分けてあるのは、
あそこだけ<b>制御点の受け渡し</b>という別の役割だから(画素シェーダへは行かない)。
...
| ファイル | 役割 |
|---|---|
| `Sandbox/TessellationMath.cs` | **今日の C# の鏡**。高さの関数・双一次補間・変位と法線・距離からレベル・割り方の丸め・辺のずれ |
| `Sandbox/TessellationLab.cs` | **GL の側**。`TessellationTopic` / `TessellationSpacing`。シェーダ3本(割り方ごと)・パッチの並び2つ・タイマー・カウンタ。辺の合意を調べる鏡も持つ |
| `shaders/tess.vert` | 制御点をワールドへ持ち上げるだけ。**投影しない** |
| `shaders/tess.tesc` | **制御シェーダ**。6つのレベルを決める。距離 LOD と、亀裂の出る/出ない決め方 |
| `shaders/tess.tese` | **評価シェーダ**。双一次補間 → 変位 or 球面へ。ここで初めてクリップ空間へ |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Render/Shader.cs` | **4段(頂点 + 制御 + 評価 + 画素)のコンストラクタ**。`#define` も受け取る | +38 |
| `Render/RenderResources.cs` | `LoadShader(vert, tesc, tese, frag, defines)`。キーに4本のパスと `#define` | +34 |
| `Program.cs` | フィールド2つ、今日の関数一式、`OnLoad` で `TessellationLab` を作る、`OnRender` に1行、HUD に1行、ページ1枚(12 項目)、`OnClosing`、既存 `Show*` 8か所に1行ずつ、`RunDebugMenuCheck` の項目数 | +781 / −1 |
| `Day63b.csproj` | `Day63a.csproj` からのリネームのみ | — |

**今日の差分は Day 63a より少し軽い**(新規 1,106 行 + `Program.cs` 781 行 + `Render/` 72 行。どちらもコメント込み)。
画素シェーダを1本も書かずに済んだのと、`Render/` に足したのが口2つだけなのが効いている。
1日で重ければ、下の順番の **10 まで**(絵が出るまで)と **11 から**(値段・内訳・自己チェック)で2回に分けるとよい。
10 の (d) でメニューの F11・F12・`1` の3行を飛ばせば、そこまででビルドが通り `F2`〜`F10` が全部動く。

### 写経する順番

依存の向きに沿って並べてある。**この順に写せば、途中でビルドが通らない状態を作らずに済む**。

1. **`Render/Shader.cs`**(変更) — Day 63a の3段のコンストラクタの隣に、4段のものを足す。
   **`params string[] defines` を受け取る**のがここだけの違い(割り方が uniform にできないため)
2. **`Render/RenderResources.cs`**(変更) — Day 63a の `LoadShader(vert, geom, frag)` の隣に、
   `LoadShader(vert, tesc, tese, frag, defines)`。**キーに `#define` を混ぜ忘れない**
3. **`Sandbox/TessellationMath.cs`**(新規) — `System.Numerics` しか使わない。
   `TessellationSpacing` を参照するので、**4 を先に写しても、5 を先に写してもよい**(同じアセンブリ内)
4. **`shaders/tess.vert`**(新規) — `Control` ブロックを決める。5・6 はこの中身と1文字ずつ揃える
5. **`shaders/tess.tesc`**(新規) — レベルを決める6つの数。**Outer の番号と辺の対応に注意**(要点2)
6. **`shaders/tess.tese`**(新規) — 双一次補間 → 変位 or 球面。**出口は Day 63a の `Varying` ブロック**
7. **`Sandbox/TessellationLab.cs`**(新規) — 1〜6 の全部を使う。パッチの並びを作るのもここ
8. **`Program.cs` のフィールド** — 「Day 63a」のフィールドの直後に「Day 63b」の2つ
   (`_tessellation` / `_tessellationDemo`)
9. **`Program.cs` の今日の関数(前半)** — ファイル末尾の `RunGeometryLabCheck` の直後に、
   `ShowTessellationLab` / `HideTessellationLab` / `ApplyTessellationCamera` / `TessellationVisible` /
   `RenderTessellationLab` / `TessellationLabLabel` / `TessellationTopicName` / `TessellationSpacingName` /
   `TessellationSpacingNote` / `TessellationExpectedText`
10. **`Program.cs` の組み込み** — 順に、
    (a) `OnLoad` の `_geometryLab = new GeometryLab(...)` の直後に `_tessellation = new TessellationLab(...)`、
    (b) `OnRender` の `RenderGeometryLab(deltaSeconds);` の直後に `RenderTessellationLab(deltaSeconds);`、
    (c) `DrawOverlayInfo` のジオメトリシェーダの行の直後に HUD の1行、
    (d) `BuildMenu` の「ジオメトリシェーダ」ページの直後に「テッセレーション」ページ(12 項目。
    **F11・F12・`1` の3項目は 11 で足す関数を指すので、分けて写すなら後回しにする**)、
    (e) `OnClosing` の `_geometryLab.Dispose();` の直後に `_tessellation.Dispose();`、
    (f) **既存の `Show*` 8か所**(`HideGeometryLab();` を書いた7か所の直後と、`ShowGeometryLab` の `HideRaymarch();` の直後)に
    `HideTessellationLab();` を1行ずつ、
    (g) `RunDebugMenuCheck` の項目数を **265 → 277** へ
11. **`Program.cs` の今日の関数(後半)** — 9 の続きに `DescribeTessellationLab` / `MeasureTessellationCost` /
    `RunTessellationCheck`
12. **`Day63b.csproj`** — `Day63a.csproj` をリネームするだけ(中身は同じ)

## 設計書

**層は増えていない**。`Sandbox/` にクラスが2つ増え、`Render/` に口が2つ増えた。
Day 63a が「段が1つ増える」話だったのに対し、今日は<b>段が2つと固定機能の回路が1つ</b>増える。
足したものの形は昨日とまったく同じ(コンストラクタと窓口)で、**中身は1行も書き換えていない**。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Sandbox/TessellationMath` | `Sandbox/TessellationMath.cs` | C# の鏡。静的クラス。**`System.Numerics` しか使わない** |
| `Sandbox/TessellationLab` | `Sandbox/TessellationLab.cs` | GL の側。シェーダ3本・パッチの並び2つ・タイマー・カウンタ |
| `Sandbox/TessellationTopic` / `TessellationSpacing` | `Sandbox/TessellationLab.cs` | 題材 / 割り方 |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Render/Shader` | 4段(頂点 + 制御 + 評価 + 画素)のコンストラクタ。`#define` 付き | +38 |
| `Render/RenderResources` | `LoadShader(vert, tesc, tese, frag, defines)` | +34 |
| `Program.cs` | ページ1枚、`OnRender` に1行、HUD に1行、出す/下ろす、値段・内訳・自己チェック | +781 / −1 |

### 四角形1枚が三角形 8,192 枚になるまで — 段2つと回路1つ

```mermaid
flowchart TD
    IB["インデックスバッファ<br/><b>4つずつ</b>束ねる<br/>glPatchParameteri&lpar;GL_PATCH_VERTICES, 4&rpar;"] --> VS["tess.vert<br/><b>制御点ごとに1回</b>。4回走る<br/>ワールドへ持ち上げるだけ<br/><b>投影しない</b>"]
    VS --> TCS["tess.tesc<br/><b>出す制御点ごとに1回</b>。4回走る<br/>gl_InvocationID == 0 の回だけが<br/>6つのレベルを書く"]
    TCS --> LV["gl_TessLevelOuter[0..3]<br/>gl_TessLevelInner[0..1]<br/><b>この6つだけがこの段の成果</b>"]
    LV --> TG["テッセレータ<br/><b>固定機能。プログラムできない</b><br/>0〜1 の升目に点と三角形を作る<br/>割り方&lpar;equal / odd / even&rpar;で丸める"]
    TG --> TES["tess.tese<br/><b>割った点ごとに1回</b>。レベル 64 なら 4,225 回<br/>gl_TessCoord&lpar;u,v&rpar;と gl_in[] が見える"]
    TES --> D["双一次補間 → 変位 or 球面へ<br/><b>ここで初めてクリップ空間へ</b>"]
    D --> RAS["ラスタライズ<br/>レベル 64 なら三角形 8,192 枚"]
    RAS --> FS["gs-surface.frag<br/><b>Day 63a のものをそのまま</b>"]
```

**CPU が送るのは 4 隅だけ**。パッチ 64 枚なら制御点 256 個(24KB)で、そこから三角形が 8,192 枚出る
(レベル 8 のとき。レベル 64 なら 524,288 枚)。
同じ形を頂点バッファで持つなら、**その3倍の頂点を送って VRAM に置く**ことになる。

**割るのは回路**というのが Day 63a との決定的な違い。ジオメトリシェーダは
「何個出すか」を自分で決めて自分で出すので、<b>出力を溜める場所</b>と<b>順番の待ち合わせ</b>が要った。
テッセレータは 6 つの数を見て決まったやり方で割るだけなので、そのどちらも要らない——
だから GS の上限(1回に 1024 頂点)をはるかに超えた量を出せる。

**制御シェーダが4回走るのに、レベルを書くのは1回**。
4回走るのは「制御点を作り変える」使い方(ベジエの制御点を計算する等)のためで、今日は使わない。
`gl_InvocationID != 0` で早く返るのは、**1回でよいものを4回書いている**と読まれないため。

### 亀裂が出るまで / 出ないまで — 「共有しているものから決める」

```mermaid
flowchart TD
    Q{"辺のレベルを<br/>何から決めるか"}
    Q -->|"辺の中点"| M1["パッチ A から見た中点<br/>= パッチ B から見た中点<br/><b>同じ点</b>"]
    Q -->|"パッチの中心"| C1["パッチ A の中心<br/>≠ パッチ B の中心<br/><b>違う点</b>"]
    M1 --> M2["同じ入力 → 同じ距離<br/>→ 同じレベル"]
    C1 --> C2["違う距離 → 違うレベル<br/><b>2 の冪へ落としてあるので<br/>2 倍違いうる</b>"]
    M2 --> M3["辺の上の点が一致する<br/><b>隙間が開かない</b>"]
    C2 --> C3["片方が 8 等分・もう片方が 16 等分<br/>→ 辺の上の点が食い違う"]
    C3 --> C4["<b>変位すると、そこが穴になる</b><br/>直線のままなら同じ線に乗るので開かない"]
```

**「共有しているものから決める」の一言に尽きる**のがこの問題の面白いところ。
パッチは隣を知らないし、知る手段も無い(隣接付き三角形のような仕掛けはパッチには無い)。
知らないまま同じ答えを出すには、**両側から見て同じものを入力にする**しかない。

**「亀裂が出ないこと」は絵では言えない**。カメラを動かせばいつか出るかもしれない、を否定できないから。
C# の鏡で、内部の辺 112 本をカメラ4通りで調べて**食い違い 0** を数で言う(自己チェック5)。
同じ条件で中心から決めると **149 本**食い違う(同6)。

隙間の大きさは、辺の等分の数の食い違いで決まる。

| 等分の組 | 辺の上の点のずれ | なぜ |
|---|---|---|
| 4 と 5 | **10.0%** | 互いに素なので、いちばん離れた点が 1/(4x5) x 2 ぶんずれる |
| 8 と 16 | **0.0%** | 割り切れるので、8 等分の点が全部 16 等分の点に含まれる |

**2 の冪へ落としているのは実務どおり**。段を粗く刻むと、カメラが少し動いただけでレベルが揺れない
(連続だと毎フレーム 12.3 → 12.4 と変わり、`equal_spacing` ではそのたびに頂点が一斉に動いてチラつく)。
代わりに隣との差が 2 倍になりうるので、**辺を揃えることがいっそう効く**。

### 値段はパッチの数で決まる

```mermaid
flowchart LR
    P["パッチ 1 枚"] --> F["固定費<br/>制御シェーダ 4 回<br/>テッセレータのセットアップ<br/><b>レベルによらず一定</b>"]
    P --> V["レベルに比例する費用<br/>評価シェーダ &lpar;n+1&rpar;^2 回<br/>三角形 2n^2 枚のラスタライズ"]
    F --> T["1 パッチの時間"]
    V --> T
    T --> R["三角形 1 枚あたりの時間<br/><b>レベルを上げるほど下がる</b><br/>固定費が薄まるので"]
```

| レベル | 三角形 | GPU(パッチ 64 枚) | 三角形1万枚あたり |
|---|---|---|---|
| 1 | 128 | 7.3us | **573us** |
| 8 | 8,192 | 22.6us | 27.5us |
| 64 | 524,288 | 355.5us | **6.8us** |

**同じ三角形の数なら、パッチを減らしてレベルを上げるほうが安い**。
「テッセレーションは細かくすると高い」より、**「パッチを細かく分けると高い」**ほうが実態に近い。

ただし実際のゲームでは、**細かすぎる三角形はラスタライザの側で高くつく**
(1画素より小さい三角形は 2x2 の画素の塊で処理されるので、4倍近く無駄になる)。
この表は三角形がまだ十分大きい範囲での話で、**レベルを上げ続ければどこかで折り返す**。
Day 64(メッシュシェーダ)と Day 65(GPU 駆動レンダリング)が解こうとしている問題の1つがこれ。

### 今日残した歪み(3つ)

**1つ目: 割り方が3本のプログラムになっている**。`equal` / `odd` / `even` を見比べるために
同じソースを3回コンパイルしていて、起動時に 3 本ぶんのリンクを払う。
実際のエンジンは「シェーダバリアント」として何百通りも焼くので、これはその最小形だが、
<b>切り替えのたびに焼き直す</b>(`SetDefines`)形にすれば1本で済む。どちらが良いかは焼く時間と本数次第。

**2つ目: パッチの並びが専用のメッシュになっている**。`TessellationLab` が自分で格子と立方体を作っていて、
`Primitives` には足していない。理由は<b>並びの約束が違う</b>から——
パッチは (0,0) (1,0) (0,1) (1,1) の順で4つ、三角形は3つ。
同じ `Mesh<Vertex>` に両方入れると、「この索引は何個ずつ読むのか」が型から読めなくなる。
本来は `PatchMesh` のような別の型にしたいところ(改造課題3)。

**3つ目: 距離 LOD が世界の距離だけを見ている**。画面での大きさを揃えたいなら、
本当は**画角とパッチの世界での大きさ**も要る(近くの小さいパッチと遠くの大きいパッチが同じ扱いになる)。
今日は格子が均一なので問題にならないが、大きさの違うパッチが混じると破綻する。

### 引き継いだ図

Day 63a の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。変わった図は次の4つ。

| 図 | 何が変わったか |
|---|---|
| `全体構成` | **図は同じ**。今日の段落を足した(`Sandbox/` → `Render/` の矢印のラベルは Day 63a のまま) |
| `Render` のクラス図 | **`Shader` の4段のコンストラクタと、`RenderResources.LoadShader(vert, tesc, tese, frag, defines)`** |
| `Sandbox` のクラス図 | 4枚目を足した。**`TessellationLab` / `TessellationMath` と、`TessellationTopic` / `TessellationSpacing`** |
| `1フレームの流れ` | `RenderGeometryLab` と `RenderParticles` の間に **`RenderTessellationLab`** |

新しく足した図が3つ(この節の上の3つ)。
Day 63a の設計書の冒頭にあった3つの図(三角形1枚が線4本になるまで、隣接付きインデックスが組み上がるまで、輪郭の辺が選ばれるまで)は
ジオメトリシェーダの話なので、今日は外した(Day 63a の計画書を見ること)。

`Model/`・`Scene/`・`Ecs/`・`Physics/`・`Text/`・`Audio/`・`Game/`・`Demo/`・`Core/` の図は**昨日のまま**で、今日は1つも触っていない。

### 全体構成 — 9つの層と、その上のゲーム・デモ・実験台(Day 63b でも形は変わらない)
...
### Render — OpenGL の薄い皮(Day 63b で段を2つ挟めるようになった)
...
### Sandbox — 教養編の実験台(Day 63b でテッセレーションの2つが増えた)
...
`dotnet run --project reference/Day63b -c Release` で起動する。起動直後のコンソールは Day 63a と同じ。
メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「テッセレーション」(28 枚目)が開く。
...
```
テッセレーション: **ON**  平らな格子に高さを付ける  パッチ 64 枚(制御点 256 個)  レベル 8  equal  → 出る三角形は 8,192 枚(1パッチ 128 枚)
```

Day 31 の床と箱が消え、**波打つ地形**が画面いっぱいに広がる。目の高さは約 1.8m で、
手前のパッチが 2.5m・奥が 8.5m くらいのところにある。HUD に今日の行が出る。

```
TESS 平らな格子に高さを付ける  パッチ 64  レベル 8  equal  三角形 8,192 (見積もり 8,192)  辺を揃える ON  GPU 0.05ms
```

**「三角形」と「見積もり」が一致すること**が今日の1つ目の関門。
三角形は `GL_PRIMITIVES_GENERATED` で GL に数えさせた実測、見積もりは `2 x レベル^2 x パッチ数`。

### 2. `F9`: 線で描く

四角形1枚が細かい格子に割られているのが見える。**1つのパッチの中で、対角線が全部同じ向きを向いている**のが
テッセレータの割り方で、外周の1列だけ三角形の形が違う(内側の升目と外側の辺をつなぐ帯)。

`F4` でレベルを上げると格子が細かくなり、下げると粗くなる。**レベル 1 で三角形2枚**(ただの四角形2分割)。

### 3. `F5`: 距離で決める

手前が細かく、奥が粗くなる。**パッチの境目で細かさが段になっている**のが分かる——
距離レベルを 2 の冪(1, 2, 4, 8, 16, 32, 64)に落としているため。

```
距離で決める: **ON**  8m でレベル 8、近いほど細かい。**2 の冪へ落とす**ので隣と 2 倍違いうる
```

### 4. `F7`: 辺を隣と揃えるのをやめる(**今日の見どころ**)

線で描いたまま切ると、**パッチの継ぎ目に T 字の頂点**ができる(片方が 8 等分、もう片方が 16 等分)。
`F9` で面に戻すと、**継ぎ目に点線状の亀裂**が走る。

```
辺を隣と揃える: **OFF**  パッチの**中心**からレベルを決める。中心は両側で違う点なので、**食い違って隙間が開く**  → 内部の辺 112 本のうち食い違い **31 本**(最大の差 16)
```

隙間が点線状になるのは正しい姿で、**辺を共有している点では一致し、その間で最大にずれる**から。
`F8` で変位を 0 にすると亀裂が消える——**直線の辺なら点がずれても同じ線の上に乗る**ので隙間が開かない。

### 5. `F3`: 球

立方体の6面を 2x2 に割った **24 枚のパッチ**が、評価シェーダの `normalize` だけで球になる。
`F10` でパッチごとに色を付けると、24 枚の区画が見える。レベルを上げると滑らかになるが、
**制御点は 96 個のまま**(24 パッチ x 4 隅)。

### 6. `F12`: 内訳

```
--- テッセレーションの内訳 ---
  題材: 平らな格子に高さを付ける  出している: ON
  割り方: **equal**  レベルを**整数へ切り上げ**て等分。1 刻みでしか変われないので、**変わる瞬間に面全体の頂点が一斉に動く**
  CPU が送った頂点: **256 個**(パッチ 64 枚 x 4 隅)  = 24.0KB。**割る前の数しか送らない**
  テッセレータが作った三角形(GL_PRIMITIVES_GENERATED): **8,192 枚**  見積もり 8,192(レベル 8 → 丸めて 8、1パッチ 2 x 8^2)
  → 送った頂点1つにつき三角形 **32.0 枚**。同じ形を頂点バッファで持つなら、その3倍の頂点を送ることになる
  辺のレベル(C# の鏡。**中点から**決めている): 内部の辺 112 本のうち**食い違い 0 本**
  この GL が許す上限: レベル 64  1パッチの制御点 32  制御シェーダの出力成分 128  評価シェーダの出力成分 128
  レベル 64 の四角パッチ1枚は三角形 8,192 枚。**ジオメトリシェーダの上限(1回に 1024 頂点)ではまったく届かない**。
```

### 7. `F11`: 値段

```
--- テッセレーションの値段(平らな格子に高さを付ける  パッチ 64 枚を 32 回  equal)---
    レベル     三角形      GPU        1万枚あたり
         1          128       7.3us    572.50us
         2          512       7.9us    153.75us
         4        2,048      12.7us     61.87us
         8        8,192      22.6us     27.54us
        16       32,768      54.8us     16.74us
        32      131,072     129.4us      9.87us
        64      524,288     355.5us      6.78us
  距離で決める(基準 8m でレベル 16。**いちばん細かいパッチはレベル 32**): 三角形 48,176 / 全体をレベル 32 で固定すると 131,072  → **36.8%**  GPU 63.3us / 129.9us
```

**三角形1万枚あたりが 573us から 6.8us へ、84 倍下がる**のが今日の到達点(要点7)。
...
```
=== Day 63b 自己チェック(パッチ・双一次補間・変位の法線・割り方・辺の合意・三角形の数)===
  [OK] **インデックスは4つで1パッチ**(三角形の3つではない)  インデックス 256 個 / パッチ 64 枚 = 4.0 個/枚
  [OK] **双一次補間が四隅を再現する**  256 隅で、ずれは最大 0.0E+000m
  [OK] **変位のあとの法線が、面の上の向きと直交する**  2,000 点 x 2 方向で、内積の絶対値は最大 0.0144
  [OK] **割り方ごとの丸め**  equal 4.3→5 / odd 4.0→5 / even 1.0→2
  [OK] **辺の中点からレベルを決めると、隣り合うパッチが必ず一致する**  内部の辺 112 本 x カメラ4通りで、食い違い 0 本
  [OK] **パッチの中心から決めると食い違う**(これが亀裂の正体)  同じ条件で食い違い 149 本
  [OK] **等分の数が違うと、辺の上の点がずれる**  4 と 5 → 辺の長さの 10.0% / 8 と 16 → 0.0%
  [OK] **レベル 1 の四角パッチは三角形2枚**  パッチ 64 枚 → 三角形 128(見積もり 128)
  [OK] **レベル n の四角パッチは三角形 2n^2 枚**  n=2: 512 / n=4: 2,048 / n=8: 8,192 / n=16: 32,768
  [OK] **fractional_odd はレベル 4 でも 5 等分になる**  レベル 4 → 3,200 / レベル 5 → 3,200
  [OK] **距離で決めると、いちばん細かいパッチに合わせるより少なくて済む**  レベル 32 で固定すると 184,832 枚 / 距離で決めると 54,940 枚(29.7%)
  [OK] **この GL の上限が仕様の保証(レベル 64・制御点 32)を満たしている**  レベル 64 / 1パッチの制御点 32
```

前半7項目は**窓を開かずに走る**(`TessellationMath` が GL を1行も知らないので)。
後半5項目はシーンのバッファを借りて描き、`GL_PRIMITIVES_GENERATED` で数える。

### 9. メニューの自己チェック

`Alt+F1` で Day 48 の割り当て表のチェックが走り、**28 ページ 277 項目**になっている。

## 改造課題

### 課題1(易): 変位を 0 にして、割る意味を確かめる

`F8` で変位を 0 にし、`F4` でレベルを 1 から 64 まで上げてみる。

1. **絵が1画素も変わらない**ことを確かめる(平らな四角形が細かい平らな四角形になるだけ)
2. `F11` で値段を測る。レベル 64 は 355us 払っているのに、絵は何も変わっていない
3. 球のほうで同じことをする。**こちらは変わる**——なぜか
4. `F9` で線にすれば違いは見える。**「絵が変わる」と「三角形が増える」は別**

見どころは 3 で、球は評価シェーダが `normalize` して<b>割った点を動かしている</b>から変わる。
「割ること」自体には意味が無く、**割った点をどう動かすかに意味がある**というのが要点3。

### 課題2(中): 三角パッチにする

四角パッチ(`quads`)を三角パッチ(`triangles`)に変えると、`gl_TessCoord` が
**(u, v) から重心座標 (a, b, c)** に変わる。

1. `TessellationLab` に、三角形のパッチを作る `CreateTriangleGrid` を足す(制御点は3つ)。
   `glPatchParameteri(GL_PATCH_VERTICES, 3)` も忘れずに
2. `tess.tesc` の `layout (vertices = 3) out;`、外側のレベルは**3つ**(`Outer[0..2]`)、内側は**1つ**(`Inner[0]`)
3. `tess.tese` の `layout (triangles, ...)`。位置は
   `p0 * gl_TessCoord.x + p1 * gl_TessCoord.y + p2 * gl_TessCoord.z`
4. `F11` で三角形の数を測る。**レベル n の三角パッチは三角形 n^2 枚**(四角の半分)。合うか

見どころは**外側のレベルがどの辺に対応するか**。四角と三角で約束が違う
(三角は `Outer[i]` が「`gl_TessCoord[i]` が 0 になる辺」)。
ここを間違えると、やはり特定の向きの辺にだけ亀裂が出る。

### 課題3(難): パッチを型として分ける

設計書の「今日残した歪み」の2つ目。`Mesh<Vertex>` に三角形の索引とパッチの索引の両方が入るので、
「この索引は何個ずつ読むのか」が型から読めない。

1. `Render/` に `PatchMesh<TVertex>` を足す。**コンストラクタで制御点の数を受け取り**、
   `Draw()` の中で `glPatchParameteri` も自分で呼ぶ
2. `Mesh<TVertex>` から `Draw(PrimitiveType.Patches)` を呼べないようにする
   (`PrimitiveType` を受ける `Draw` から `Patches` を弾く、あるいは専用の列挙型にする)
3. `TessellationLab` の `PatchMesh` を、この新しい型に置き換える
4. Day 63a の隣接付きインデックス(`TrianglesAdjacency`)も同じ問題を抱えている。
   **あちらは 6 つずつ**。同じ型で扱えるか、別の型が要るかを考える

見どころは**「索引の束ね方」は誰の責任か**という線引き。GL は描くときの引数で決める形にしているが、
バッファを作った側がいちばんよく知っている。4 を考えると、
「束ね方をメッシュが持つ」のが素直だと分かるはず。
...
| OpenGL | 4.3 コアプロファイル(ドライバ 596.49)。**テッセレーションは 4.0 の機能** |
| 解像度 | 960 x 640 |
| 確認の条件 | `-c Release`、**VSync を入れて**確認した。値段の測定は 32 回ぶんをタイマークエリで挟んだもので、`glFinish` の連続描画(計測)は今日も無い |

題材ごとの数字。

| | 平らな格子 | 球(立方体) |
|---|---|---|
| パッチ | 64(8 x 8) | 24(6 面 x 2 x 2) |
| 制御点 | 256 個(24.0KB) | 96 個 |
| 内部の辺 | 112 | — |
| レベル 8 の三角形 | 8,192 | 3,072 |
| レベル 64 の三角形 | 524,288 | 196,608 |
...
距離 LOD(基準 8m でレベル 16、いちばん細かいパッチはレベル 32)。

| | 三角形 | GPU |
|---|---|---|
| 全体をレベル 32 で固定 | 131,072 | 129.9us |
| 距離で決める | **48,176(36.8%)** | 63.3us |
