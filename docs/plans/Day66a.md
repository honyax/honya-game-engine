# Day 66a: 焼いたイラディアンスプローブ(照り返しを格子に焼いて、回り込ませる)

**教養編で初めて、最終デモの絵そのものに手を入れる日**。Day 57〜63b は `Sandbox/` の実験台だったが、
今日のプローブは `Render/` に置き、`textured.frag` と `deferred.frag` の IBL の1行を差し替える。

やることは1つ。**夜の裏通りに点を格子状に並べ、各点でシーンを6方向に撮り、届いている光を9個の数に縮めて持つ**。
描くときは、画素の位置を囲む8点の9個を混ぜて、その向きの放射照度を引く。

```
  焼く(キーを押したとき・1回)                                   引く(毎フレーム・画素ごと)
  448点 x 6面を撮る → 読み返す → SH へ射影 → 余弦を掛ける → 9 x 448 のテクスチャ     位置 → 囲む8点 → 9係数 x 法線 → 混ぜる
  1.3 秒(跳ね返り2回)                                                              texelFetch 72 回
```

| | IBL(Day 36) | 焼いたプローブ(Day 66a) |
|---|---|---|
| 持っているもの | 放射照度マップ1枚(32x32 x 6面) | **448 点 x 9 係数**(1点 72 バイト) |
| どこから見た光か | **世界に1か所**。どこに居ても同じ空 | **格子の点ごと**。その点から見えたもの |
| 何が写っているか | 空だけ | 空 + 灯りに照らされた路面・壁・小物 |
| 画素が引くもの | 法線だけ(`texture(uIrradianceMap, N)`) | **位置と法線**(囲む8点 x 9 係数) |
| 苦手なもの | 壁の陰でも空が見えていることになる | 格子の間の壁(光漏れ)。**動く光**(焼き直すまで付いてこない) |
| 立ち位置 | 屋外の環境光の定番 | **ライトマップと並ぶ AAA の静的 GI の主力** |

> **今日いちばん意外だったのは、最初に「よく効いている」と見えた照り返しの4割が、空に残った1つの点だったこと**
> 焼いたプローブの平均の明るさは、はじめ 0.051 だった。自己チェックで「空だけを撮ったプローブ」と空の係数を突き合わせると、
> 真横(+X)から来る光が **2.47 倍**になっていた。太陽を抜いたあとの空にも輝度 3,200 ほどの点が残っていて、
> 256x256 の空を 32x32 で撮ると、その点を拾った1画素が 64 画素ぶんの面積を代表してしまう。
> 撮る画素と同じ大きさまで縮めた段(3段目)で空を引くと、平均は **0.031** に下がった(要点4)。
> 残った照り返しを最終の絵で見ると、差は**暗がりで +2〜17%**。成分表示(放射照度)にすると一目で分かる——
> 空だけなら青一色だった路面が、灯りのまわりで暖色に、看板の前で桃色に染まる。

## 今日のゴール

**メニューの「プローブGI」ページで `F2` を押すと、完成版の夜の裏通りが出て、448 個のプローブが焼かれる(1.3 秒)。
拡散の環境光が「空1枚」から「その場所から見えたシーン」に変わり、灯りの当たらない面に、照らされた路面や壁の色が回り込む。
`F8` で放射照度を見ると、空だけ(青一色)とプローブ(灯りのまわりが暖色・看板の前が桃色)の違いが一目で分かる。
`F9` で壁の上から見下ろすと、壁の外の地面に通りの照り返しが漏れている。`F10` で灯りを消しても、焼いた照り返しは残る。**

| キー(「プローブGI」ページ) | 何が起きるか |
|---|---|
| `F2` | 焼いたプローブ ON / OFF。**今日の到達点**。完成版が出ていなければ出して(決めの構図で止めて)焼く |
| `F3` | 焼き直す。今の条件(灯り・時間帯)で焼く。**焼いたものは条件が変わっても付いてこない** |
| `F4` | 跳ね返りの回数(1 → 2 → 3)。焼き直す。**増え方は回を追うごとに減る** |
| `F5` | 格子の間隔(3.0 → 1.5 → 1.0m)。焼き直す。**壁越しの漏れは、列が壁のどこに立つかで変わる** |
| `F6` | 光る材質も焼く(既定 OFF)。焼き直す。**同じ灯りを二重に数える** |
| `F7` | プローブを見る。各点を球で出し、**白い面がそこで受ける照り返し**の色で塗る |
| `F8` | 放射照度を見る(成分表示 18。Day 36)。**`F2` で空だけ ↔ プローブを見比べる**。今日の主な見方 |
| `F9` | 壁の上から見下ろす。左の壁の外の地面を見る(`F8` と合わせて光漏れを見る) |
| `F10` | 灯りを消す(焼き直さない)。直接の光は消えるが、**焼いた照り返しは残る** |
| `F11` | 内訳(格子・焼いた条件・跳ね返りごとの明るさ・場所ごとの照り返し) |
| `F12` | 値段(焼く時間の内訳と、毎フレーム引く時間) |
| `1` | 今日の自己チェック(15 項目) |

メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「プローブGI」(29 枚目)が直接開く。
**プローブを入れない限り、絵は Day 63b と1ビットも変わらない**(起動直後・デモ v1・完成版・ディファードの6つの場面で、画素ごとに確かめた。完成条件 10)。

### 今日いちばん大事な3つ

**1つ目は「放射照度は滑らかなので、9個で足りる」**。放射照度は、入ってくる光に余弦のこぶ(N・L)を掛けて半球で積分したもの。
余弦のこぶはとても滑らかなので、球面調和(SH)の次数ごとに見ると<b>高い次数ほどほとんど残らない</b>(要点1)。
2次(9個)で打ち切っても、「上半分だけ光る空」なら誤差 0.02% で再現できる。縮める手順は、次数ごとに掛け算するだけ。

```csharp
// SphericalHarmonics.ToIrradiance(Day 66a)。放射輝度の係数 → 放射照度 / π の係数
...
**2つ目は「焼くのは、その場所から見えたもの全部」**。撮るのは本描画と<b>同じ `textured.frag`</b> で、違うのはカメラと4つの uniform だけ
(SSAO を切る・定数の環境光を 0・IBL の鏡面を 0・プローブは前の回のもの)。
1回目は環境光なしで撮るので、写るのは空と、灯りと月に**直接**照らされた面——それを縮めたものが「1回跳ねた光」になる。
**跳ね返りを重ねるには、同じ関数をもう一度呼ぶだけ**(要点3・6)。

**3つ目は「補間は壁を知らない」**。画素は囲む8点を三線形の重みで混ぜるが、重みは<b>距離だけ</b>で決まる。
壁の向こうの点も、近ければ同じだけ効く——これが光漏れの正体(要点5)。
Day 66b で読む DDGI は、この8点の重みに「その点からここが見えるか」を掛けたもの。
**重みをシェーダの中で自分で持つ**(3D テクスチャの補間に任せない)のは、その一歩を後から足せるようにするため。

### 今日は Render とシェーダと Program

差分は `Render/` に新規2本(`SphericalHarmonics` / `IrradianceProbes`)、シェーダに新規2本(球で見る `probe.vert` / `probe.frag`)、
**`textured.frag` と `deferred.frag` に同じ 72 行**(uniform と関数2つ。IBL の拡散の1行を差し替え)、
`EnvironmentMap.DrawSkybox` に引数1つ、そして `Program.cs`。

**`Demo/` も `assets/` も1行も変わっていない**。格子を置く範囲は `Program.cs` に書いた(今日残した歪みの1つ目)。
灯りの置き場所と色は Day 56 の `SceneLights.Fixture` がそのまま持っているので、焼く側はそれを読むだけで済んだ。

## 事前に読む資料

- [Ramamoorthi & Hanrahan, "An Efficient Representation for Irradiance Environment Maps"(SIGGRAPH 2001)](https://graphics.stanford.edu/papers/envmap/)
  — **今日の要点1・2の一次資料**。放射照度を球面調和の9係数で表す原典。**なぜ2次で打ち切ってよいのか**
  (余弦のこぶを畳み込むと、次数ごとに決まった割合しか残らない)と、その割合 Â0 = π、Â1 = 2π/3、Â2 = π/4 の出どころ。
  シェーダで9係数から放射照度を組み立てる式も載っている
- [Peter-Pike Sloan, "Stupid Spherical Harmonics (SH) Tricks"(GDC 2008)](https://www.ppsloan.org/publications/StupidSH36.pdf)
  — **実装の手引き**。基底の定数(0.282095 など)・キューブマップからの射影・次数ごとの畳み込み。
  **リンギング**(2次で打ち切ると、明暗の急な境目の反対側で値が 0 の下へはみ出す)と、その抑え方(窓掛け)も
- **西川善司「ゲーム制作者になるための3Dグラフィックス技術 改訂3版」Ch10**
  — 「大局照明技術」。**ライトマップ・イラディアンスボリューム(今日のプローブの格子)・その先の動的 GI** が並んだ地図。
  今日がそのどこに座っているかを先に見ておく
- **Day 36 の計画書 要点**(放射照度マップと、1/π を焼き込む約束)と**設計書「IBL が焼き上がるまで」** —
  今日の係数も 1/π 済みで持ち、使う側の式を IBL と同じ1行にしている。**事前フィルタのミップの話**(1サンプルが担当する立体角に見合った段から引く)は、
  今日の要点4でもう一度出てくる
- **Day 56 の計画書 要点1**(灯りは照らす側と見える側の2つでできている) — 光る材質を焼かない理由(要点3)
- **Day 52 の計画書**(点光源は影を持たない) — 今日の絵の差が小さい理由(要点8)と、壁の外が直接光で明るい理由

## 理論の要点

### 1. 放射照度は「余弦のこぶで均した光」なので、9個で足りる

ある点のある向きの面が受ける光(放射照度)は、半球から来る光に N・L を掛けて足したもの。

```
  E(n) = ∫ L(ω) max(0, n・ω) dω      (半球の積分。Day 36 の irradiance.frag が画素ごとにやっていたこと)
```

`L(ω)`(どの向きからどれだけ来るか)は細かい模様を持ちうるが、`max(0, n・ω)`(余弦のこぶ)は**とても滑らか**なので、
掛けて積分すると模様の細かい成分はほとんど消える。球面調和(SH)は球の上のフーリエ級数で、
次数 l が上がるほど細かい模様を表す。**余弦のこぶと畳み込むと、次数ごとに決まった割合しか残らない**。

| 次数 l | 係数の数 | 残る割合 Â_l / π | 何を表すか |
|---|---|---|---|
| 0 | 1 | **1** | 向きによらない平均 |
| 1 | 3 | **2/3** | どちらから多く来るか(x・y・z の傾き) |
| 2 | 5 | **1/4** | 上下から来て横からは来ない、のような偏り |
| 3 | 7 | **0** | ちょうど消える(3 以上の奇数次はすべて 0) |
| 4 | 9 | −1/24 | ほぼ消える |

だから**2次まで(1 + 3 + 5 = 9 個)で打ち切っても、放射照度はほとんど変わらない**。
自己チェックの「上半分だけ光る空」は、0次と奇数次しか持たない関数なので、3次以上がちょうど消えて**9個で厳密に**再現できる
(上 1.0002 / 横 0.5000 / 下 −0.0002)。

**9個 x RGB = 27 個の数**。同じものを Day 36 の放射照度マップ(32x32 x 6面 x RGB)で持つと 18,432 個になる。
今日はそれを 448 点ぶん持つので、差は **683 分の1**(21.0MB と 31.5KB)。

### 2. 射影は「その基底とどれだけ似ているか」を足すだけ — ただし立体角で重みを付ける

キューブマップの各テクセルは、ある向きから来る光の明るさ。これを SH の係数にするのが**射影**:

```
  L_i = Σ(テクセル)  明るさ x Y_i(向き) x 立体角
```

**立体角を忘れると壊れる**。面の真ん中のテクセルは正面から見ているので大きく、隅のテクセルは斜めから見ているので小さい。
32x32 の面では**真ん中と隅で 4.86 倍**違う。等しい重みで足すと隅(立方体の角の向き)を5倍近く重く数え、
面の境目の向きから来る光を大きく見積もる。

| 立体角の求め方 | 6面ぶん足すと | 使うところ |
|---|---|---|
| 厳密な式(`atan2(xy, √(x²+y²+1))` を4隅で足し引き) | **ちょうど 4π**(12.56637) | 今日(`SphericalHarmonics.TexelSolidAngle`) |
| 近似 `4 / (n² (1+u²+v²)^1.5)` | 4π に近い | 広く使われている |

射影が済んだら、次数ごとに 1 / 2/3 / 1/4 を掛ける(`ToIrradiance`)。**Day 36 では画素ごとに半球を刻んで積分していた**
(0.025 ラジアン刻みで、1テクセルあたり 1 万 6 千回ほど読む)ところが、**9回の掛け算**になる。これが SH で持ついちばんの御利益。

向きの表(どのテクセルがどの向きか)は OpenGL の仕様の表(Table 8.19)をそのまま写す。Day 36 の `Program.CubeTexelDirection` と同じもので、
**覚えるものではない**。撮るときの6面のカメラ(`IrradianceProbes.Faces`)も Day 36 の `CubeMap.FaceViews` と同じ表で、
この2つが食い違うと、その面の光が別の向きから来たことになる——自己チェックが「空だけを撮ったプローブ」で突き合わせる(6面とも 1.00)。

### 3. 焼くのは「その場所から見えたもの全部」。変えるのは4つだけ

撮るのは本描画と同じ `textured.frag` で、**違うのはカメラ(90 度・1:1)と、`PrepareProbeCapture` が変える4つの uniform だけ**。

| 変えるもの | なぜ |
|---|---|
| SSAO を切る | 遮蔽率のテクスチャは本描画のカメラで作った画面のもの。撮るカメラとは画素が対応しない |
| 定数の環境光を 0 | 撮った光に、手で決めた光が混ざらないように |
| IBL の鏡面を 0(`uIblIntensity = 0`) | プローブが運ぶのは拡散だけ。**鏡面は見る場所で変わる**ので、プローブの位置から見た鏡面は当てにならない |
| プローブ | 1回目は使わない(環境光なし)。2回目からは前の回のもの(要点6) |

光の渡し方は**フォワード**(uniform の配列)。Forward+ の升目は本描画のカメラで切ったもので、撮るカメラの画面とは合わない。

**焼かないものが2つある**。

1つ目は**光る材質**(窓の板・街灯のガラス・看板)。Day 56 で、灯りは「照らす側(点光源)」と「見える側(光る材質)」の
2つでできていると書いた。窓の照らす側は板の 40cm 手前からもう壁と路面を照らしているので、板まで焼き込むと
**同じ窓の光を、点光源として1回、板の照り返しとしてもう1回**数えることになる。`F6` で焼くと、平均の明るさが 0.0336 → 0.0417(**+24%**)になる。

2つ目は**揺らぎ**。灯りは**揺らぎの無い満点**で焼く。焼くのは一度きりなので、押した瞬間の揺らぎを焼き込むと、押すたびに明るさが変わる。
代わりに、ランタンが揺らいでも照り返しは付いてこない(要点7)。

**空は撮る**。壁の切れ目から見える空も「その場所から見えたもの」の一部で、夜は 7% の明るさで写る。
つまりプローブは、**IBL の拡散(空)を丸ごと置き換える**。空が壁に隠れている場所では、空からの光も減る。

### 4. SH が苦手なもの — 小さく明るい光(と、それを 32x32 で撮ること)

9個では**小さな光を小さく持てない**。真上の半径 10 度の円だけが光る空を射影すると、

| | 本当の値 | 9個で持つと |
|---|---|---|
| 上向きの面 | 0.0302 | 0.0307 |
| 下向きの面 | **0** | **0.0017(上の 5.6%)** |

ぼやけて広がり、**真裏にまで漏れる**。**灯りを点光源のまま当てている**のはこのためで、
灯りの直接の光を SH に入れると、灯りの真裏の面まで光ってしまう。プローブが運ぶのは、灯りに照らされた**面**からの光
(広がりのある光)だけにしてある。

同じ弱点が**空の側**にもあった。夜の空(HDRI)には、太陽を抜いたあとにも輝度 3,200 ほどの点が残っている。

- **9個に縮めると、その点を向いた面の値がずれる**。Day 36 の放射照度マップと比べると、点に背を向けた面では最大 8.3% の一致なのに、
  向いた面では最大 **86%** ずれる(自己チェック7)。これは要点1の「9個で足りる」の例外で、打ち切りの限界
- **32x32 で撮ると、点を拾うかどうかで空全体の明るさが変わる**。こちらは打ち切りではなく<b>標本化</b>の問題。
  256x256 の空を8画素おきに読むと、点を拾った1画素が 64 画素ぶんの面積を代表する。撮った空の係数は +X で **2.47 倍**になった

2つ目は直せる。**撮る画素と同じ大きさまで縮めた段で空を引けばよい**(256 / 32 = 8 なので3段目)。
そうすれば 8x8 の平均を読むことになり、明るい点は面積に見合った重さで入る。
Day 36 の事前フィルタで「1サンプルが担当する立体角に見合った大きさのミップから引く」と書いたのと**同じ理屈**で、
今日はそのために `EnvironmentMap.DrawSkybox` に引数を1つ足した(画面に描くときは 0 段目のまま)。

### 5. 格子と三線形補間 — 重みは壁を知らない

プローブは 1.5m おきに 7 x 4 x 16 = **448 個**。画素は自分を囲む8点を、3軸の一次補間の積で混ぜる。

```glsl
// textured.frag / deferred.frag の ProbeIrradiance(Day 66a)
...
**重みは距離だけで決まる**。壁の向こうの点も、近ければ同じだけ効く。格子は左の壁(x = −4)の外に1列(x = −5)はみ出させてあり、
壁から 50cm 外の地面は、**8点の重みの 33% を壁の内側のプローブから**受け取る。

| 格子の間隔 | 壁をまたぐ升 | 壁から 50cm 外の地面が内側から受け取る重み | 放射照度 / π(空だけなら 0.0249) |
|---|---|---|---|
| 3.0m | x = −5 〜 −2 | 17% | 0.0248(1.0 倍) |
| 1.5m(既定) | x = −5 〜 −3.5 | **33%** | 0.0316(**1.3 倍**) |
| 1.0m | なし(x = −4 の列がちょうど壁の面に乗る) | **0%** | 0.0228(0.9 倍) |

**漏れの量は格子の粗さでは決まらない**。3.0m のほうが 1.5m より漏れが少ないのは、壁をまたぐ升の中で壁が内側の列から遠く
(重み 17%)、しかも内側の列(x = −2)が街灯から離れていて照り返しを多く見ていないから。
**壁をまたぐ升のどこに壁があるか**と、**内側の列が何を見ているか**の2つで決まる。
間隔を細かくすれば升は小さくなるが、列の置き場所を選ばない限り、漏れる升は必ずどこかに残る。

**格子の外は、端のプローブで塗る**(位置を格子の中へ寄せる)。壁からずっと離れた地面は x = −5 の列の値になる。
もう1つ、**片面の壁は裏から透けて見える**。壁は1枚の板で、裏面は間引いて描いているので、壁の外のプローブからは壁の向こうの通りが見えている。
Day 66b で読む DDGI は、この2つ(重みが壁を知らない・点が壁の中や裏に居る)を<b>「その点から、ここが見えるか」</b>(深度)と
「点が裏面ばかり見ていたら使わない」で防ぐ。

**3D テクスチャの補間に任せない**理由は2つ。1つは**サンプラの数**——27 個の数を RGBA の 3D テクスチャに入れると7枚要り、
`textured.frag` はすでに 14 枚使っている。もう1つが上の話で、**8点の重みがシェーダの中で見えていれば、
「この点は壁の向こうだから 0 にする」を後から足せる**(改造課題2)。係数は横 9 列・縦 448 行の2D テクスチャに入れ、`texelFetch` で1つずつ読む。

### 6. 跳ね返りを重ねる — 同じ関数を、前の回の結果で照らして呼ぶ

1回目は環境光なしで撮る。写るのは**直接**照らされた面なので、縮めたものは「1回跳ねた光」。
2回目は**1回目の係数で照らして**撮る。照り返しに照らされた面が写るので、跳ねる回数が1つ増える。

| 跳ね返り | 全プローブの平均の明るさ | 増えた量 |
|---|---|---|
| 1回 | 0.0312 | — |
| 2回 | 0.0336 | **+8%** |
| 3回 | 0.0336 | **+0%**(+0.0001) |

**増え方が急に減る**のは、面が光の一部しか返さないから(路面やレンガのアルベドは 0.2〜0.4)。1回跳ねるたびに数分の1になるので、
足し算がすぐ収まる。既定を2回にしたのはこのため(3回目は焼く時間が 1.3 倍になるのに、平均は 0.0001 しか増えない)。

**差し替えるのは全部撮り終えてから**(`IrradianceProbes.Bake`)。途中で差し替えると、先に焼いたプローブの光を後のプローブが拾い、
**並べた順で明るさが変わる**(格子の片側だけ1回多く跳ねる)。

### 7. 焼いたものは付いてこない — 静的 GI

焼くのは一度きりなので、**あとから条件が変わっても照り返しは変わらない**。

| 変えたもの | 直接の光 | 照り返し(焼き直すまで) |
|---|---|---|
| ランタンが揺らぐ | 揺らぐ | **揺らがない**(満点で焼いてある) |
| 灯りを消す(`F10`) | 消える | **残る**。壁と路面がぼんやり光ったまま |
| 夕暮れにする(「最終デモ」の `F4`) | 太陽が戻る | **夜の照り返しのまま** |

HUD は焼いたときの条件を覚えていて、今と違えば「**焼いた条件と違う(灯りを点けて焼いた)**」と出す。
灯りを消して焼き直すと(`F3`)、光だまりを見下ろす面の照り返しは 0.0600 → 0.0113(**19%**)まで落ちる(残りは月に照らされた路面)。

これが「静的 GI」の意味で、**灯りが動かない静的なシーンだから焼いて済む**。ライトマップ(面ごとに焼く)と並ぶ AAA の主力で、
どちらも<b>動く光と動く物には付いてこない</b>。付いてこさせたければ毎フレーム焼き直すしかなく、それを実時間でやるのが DDGI(Day 66b)。

### 8. 絵の差が小さい理由と、値段

最終の絵でプローブを入れると、暗いところが **中央値で +2.2%、上位 1% で +10%、上位 0.1% で +17%** 明るくなる。
成分表示(放射照度)では一目で分かるのに、最終の絵で控えめなのは、**このシーンに暗がりがほとんど無い**から。

- 灯り(点光源)は**影を持たない**(Day 52)。届く距離の中なら、壁の裏でも物の陰でも直接の光が当たる
- 照り返しは直接の光の数分の1(アルベド x 見えている面積)。直接の光が当たっている面では、足しても目立たない
- **照り返しが主役になるのは、直接の光が届かないところ**——いちばん効いているのは突き当りの壁の右側とバリアの裏

プローブの値打ちは**色**にも出る。灯りのまわりは暖色、看板の前は桃色に染まる(空だけなら、どこも同じ青)。
Day 56 の課題3(街灯に影を付ける)をやると、影の中は照り返しだけになり、今日の差がはっきり出るはず。

| | 値段 |
|---|---|
| 焼く(跳ね返り2回・448 点) | **1.3 秒**。1回ぶん: 撮る 136ms / 読み返す 321ms / 縮める 161ms |
| 焼く1回の描画 | 448 点 x 6面 = 2,688 面 x シーンの物 35 個 = **94,080 回**の描く命令 |
| 引く(毎フレーム) | シーンを描く時間が 0.379ms → **0.515ms(+0.14ms)**。画素1つにつき `texelFetch` 72 回 |
| 係数 | **31.5KB**(RGBA16F 9 x 448)。撮った面のまま持つと 21.0MB |

**読み返しがいちばん重い**。撮るのは CPU が描く命令を積むだけで、GPU が撮り終えるのを待つのは `glReadPixels` の側に入る。
1面ずつ読むと待ちが6倍になるので、**16 点 x 6面を1枚(192x512)に並べて撮り、まとめて1回で読む**(28 回)。
...
| ファイル | 役割 |
|---|---|
| `Render/SphericalHarmonics.cs` | **今日の数学の全部**。基底・立体角・キューブの向き・射影・余弦の畳み込み・評価。**`System.Numerics` しか使わない** |
| `Render/IrradianceProbes.cs` | **格子と焼き**。格子を決める・6面を撮る・まとめて読む・縮める・テクスチャへ・C# の鏡の補間・球で見る |
| `shaders/probe.vert` | 球をプローブの位置へ置くだけ |
| `shaders/probe.frag` | 1個のプローブの9係数を、球の法線で評価して塗る |

### 変更ファイル

| ファイル | 何が変わったか | 差分 |
|---|---|---|
| `Render/EnvironmentMap.cs` | `DrawSkybox` に `environmentMip`(既定 0)。プローブを撮るときは3段目で空を引く | +10 / −2 |
| `shaders/textured.frag` | uniform 5つと `ProbeSh` / `ProbeIrradiance`。IBL の拡散の1行を差し替え | +72 / −1 |
| `shaders/deferred.frag` | **`textured.frag` と1文字も違わない**同じもの(位置の変数名だけ `worldPos`) | +72 / −1 |
| `Program.cs` | フィールド、今日の関数一式、`OnLoad`・`ApplyLighting`・`OnRender`・HUD に1か所ずつ、ページ1枚(12 項目)、`OnClosing`、`RunDebugMenuCheck` の項目数 | +1,317 / −1 |
| `Day66a.csproj` | `Day63b.csproj` からのリネームのみ | — |

**今日の差分は Day 63b より重い**(新規 909 行 + シェーダ 144 行 + `Program.cs` 1,317 行。どれもコメント込み)。
`Program.cs` の今日の関数は、**絵が出るまでの前半が約 410 行、内訳・値段・自己チェックの後半が約 760 行**。
1日で重ければ、下の順番の **10 まで**(絵が出るまで)と **11 から**(内訳・値段・自己チェック)で2回に分けるとよい。
10 の (e) でメニューの `F11`・`F12`・`1` の3行を飛ばせば、そこまででビルドが通り `F2`〜`F10` が全部動く。

### 写経する順番

依存の向きに沿って並べてある。**この順に写せば、途中でビルドが通らない状態を作らずに済む**。

1. **`Render/SphericalHarmonics.cs`**(新規) — `System.Numerics` しか使わない。**今日の数学はここに全部ある**。
   基底の定数は Sloan の資料から写す(覚えるものではない)。`CubeTexelDirection` は Day 36 の `Program.CubeTexelDirection` と同じ表
2. **`Render/EnvironmentMap.cs`**(変更) — `DrawSkybox` に `float environmentMip = 0.0f` を足し、`Environment` を引く枝の `uMip` をそれにする。
   **既定値を付けるので、既存の呼び出しは1つも書き換えない**
3. **`shaders/probe.vert`**(新規) — 球を置くだけ
4. **`shaders/probe.frag`**(新規) — 9係数の評価。**基底の定数は 1 と同じ5つ**
5. **`Render/IrradianceProbes.cs`**(新規) — 1〜4 を全部使う。`Faces` の表は `CubeMap.FaceViews` と同じ並び
6. **`shaders/textured.frag`**(変更) — `uniform int uIblPrefilter;` の直後に uniform 5つと `ProbeSh` / `ProbeIrradiance`。
   IBL の枝の `vec3 irradiance = texture(uIrradianceMap, normal).rgb * uIblIntensity;` を3行の三項演算子に
7. **`shaders/deferred.frag`**(変更) — 6 と同じ場所に同じもの。**`ProbeSh` から `ProbeIrradiance` の `return` の手前まで、1文字も違えない**
   (自己チェック 15 が突き合わせる)。差し替える1行の位置の変数だけ `worldPos`
8. **`Program.cs` のフィールド** — 「Day 63b」のフィールドの直後に「Day 66a」の 12 個
   (`_probes` / `_probeGi` / `_probeBounces` / `ProbeSpacings` / `_probeSpacingIndex` / `ProbeGridMin` / `ProbeGridMax` /
   `_probeBakeEmissive` / `_showProbes` / `_probeLeakView` / `_probeBakedWith` / `_probeBounceMeans`)。
   `_probeBakedWith` の型(`ProbeBakeConditions`)は 9 で足すので、**8 と 9 は続けて写す**
9. **`Program.cs` の今日の関数(前半)** — ファイル末尾の `RunTessellationCheck` の直後に、
   `ProbeBakeConditions` / `ProbesActive` / `CurrentProbeConditions` / `ProbesStale` / `ToggleProbeGi` / `BakeProbes` /
   `BakeLights` / `SetGlowForBake` / `RestoreGlow` / `PrepareProbeCapture` / `DrawProbeCapture` / `ProbeSkyMip` /
   `MeanProbeLuminance` / `ProbeBounceMeansText` / `RenderProbeSpheres` / `ToggleProbeLeakView` / `ProbeLeakPoint` /
   `LeakWeights` / `ProbeGiLabel`
10. **`Program.cs` の組み込み** — 順に、
    (a) `OnLoad` の `_tessellation = new TessellationLab(...)` の直後に `_probes = new IrradianceProbes(...)` と `Configure`、
    (b) `ApplyLighting` の `_env.Apply(shader);` の直後に `_probes.Apply(shader, ProbesActive());`、
    (c) `OnRender` の `RenderRaymarch(deltaSeconds);` の直前に `RenderProbeSpheres();`、
    (d) `DrawOverlayInfo` の灯りの行(`SceneLightsLabel`)の直後に HUD の1行、
    (e) `BuildMenu` の「テッセレーション」ページの直後に「プローブGI」ページ(12 項目。
    **F11・F12・`1` の3項目は 11 で足す関数を指すので、分けて写すなら後回しにする**)、
    (f) `OnClosing` の `_tessellation.Dispose();` の直後に `_probes.Dispose();`、
    (g) `RunDebugMenuCheck` の項目数を **277 → 289** へ
11. **`Program.cs` の今日の関数(後半)** — 9 の続きに `EnvironmentCoefficients` / `BrightestDirection` / `LampPoolPoint` /
    `LampPoolLuminance` / `ProbeSamplePoints` / `DescribeProbeGi` / `MeasureProbeCost` / `MeasureSceneDraw` /
    `ReadProbeIrradianceOnGpu` / `RunProbeGiCheck`
12. **`Day66a.csproj`** — `Day63b.csproj` をリネームするだけ(中身は同じ)

## 設計書

**層は増えていない**。`Render/` にクラスが2つ増え、`EnvironmentMap` に引数が1つ増えた。
教養編の Day 57〜63b は `Sandbox/`(エンジンを使う側)に閉じていたが、**今日は `Render/`(エンジンの側)に置いた**。
プローブは実験台ではなく、完成版の絵を描く道の一部——`ApplyLighting` から毎フレーム渡され、`textured.frag` の IBL の1行を差し替える。

| 増えたもの | どこに | 何をするか |
|---|---|---|
| `Render/SphericalHarmonics` | `Render/SphericalHarmonics.cs` | 静的クラス。**`System.Numerics` しか使わない**(`Pbr` と同じ置き方) |
| `Render/IrradianceProbes` | `Render/IrradianceProbes.cs` | 格子・焼き・テクスチャ・C# の鏡・球で見る。**シーンを知らない**(描く関数を受け取る) |

| 変わったもの | 何が変わったか | 差分 |
|---|---|---|
| `Render/EnvironmentMap` | `DrawSkybox(camera, restoreDepthTest, restoreCulling, environmentMip)` | +10 / −2 |
| `Program.cs` | ページ1枚、`ApplyLighting` に1行、`OnRender` に1行、HUD に1行、焼く・撮る・見る・内訳・値段・自己チェック | +1,317 / −1 |

### プローブ 448 個が焼けるまで — 16 個ずつ撮って、まとめて読む

```mermaid
flowchart TD
    CF["IrradianceProbes.Configure&lpar;min, max, 1.5&rpar;<br/>7 x 4 x 16 = 448 個<br/><b>焼いてあったものは捨てる</b>"] --> B1
    B1["16 個ずつ束ねる(28 回)<br/>撮る先 192x512 を消す"] --> F6
    F6["1点 x 6面ぶん<br/>ビューポートを 32x32 の枠へ<br/>カメラを 90 度・その面の向きへ"] --> DS
    DS["Program.DrawProbeCapture&lpar;camera&rpar;<br/>空(<b>3段目</b>)→ シーンの物 35 個<br/>本描画と同じ textured.frag"] --> Q{"16 個ぶん撮ったか"}
    Q -->|"まだ"| F6
    Q -->|"撮った"| RB["glReadPixels 1回<br/><b>ここで GPU を待つ</b>"]
    RB --> PJ["Parallel.For で1点ずつ<br/>CopyTile x 6 → ProjectFace<br/>明るさ x 基底 x 立体角 を足す"]
    PJ --> TI["ToIrradiance<br/>次数ごとに 1 / 2/3 / 1/4"]
    TI --> MORE{"全部の点を撮ったか"}
    MORE -->|"まだ"| B1
    MORE -->|"撮った"| UP["Upload<br/>RGBA16F 9 x 448<br/><b>ここで初めて差し替える</b>"]
    UP --> BC["Bounces++"]
```

**`IrradianceProbes` はシーンを知らない**。何を描くかは `Bake(Action<Camera> drawScene)` に渡される関数が決め、
このクラスが決めるのは「どこから・どの向きへ撮るか」と「撮ったものをどう縮めるか」だけ。
`EnvironmentMap`(何を映すかは太陽の向きだけで決まる)や `DeferredLighting.Render`(光の uniform を `Action<Shader>` で受け取る)と同じ線の引き方で、
**灯りを点けるか・何回跳ねるか・光る材質を焼くかは、全部呼ぶ側(`Program`)の都合**で決められる。

**読み返しは束ごとに1回**。`glReadPixels` は積まれた描画が全部終わるまで返らないので、1面ずつ読むと GPU の空き待ちが 6 倍になる。
撮る先を 192x512 にして 16 点 x 6面を並べ、28 回の読み返しで済ませている。**撮る 136ms に対して読み返し 321ms**——
読み返しの中身はほとんどが GPU の描画の待ち。

**縮めるのは CPU**(`Parallel.For`)。GPU のコンピュートでやれば読み返しも要らないが、C# で書いておけば
自己チェックが**同じ関数**に答えの分かる空を通せる(一様・上半分・小さな円)。Day 36 で BRDF の表を CPU で焼いた判断と同じ。

### 跳ね返りが重なるまで — 前の回の係数で照らして、もう一度撮る

```mermaid
flowchart LR
    B0["撮る前<br/>Configure で係数を捨てる"] --> C1["1回目を撮る<br/>環境光 0・IBL の鏡面 0<br/>写るのは空と<b>直接</b>照らされた面"]
    C1 --> T1["係数 A<br/>1回跳ねた光<br/>平均 0.0312"]
    T1 --> C2["2回目を撮る<br/>拡散の環境光 = A<br/>A に照らされた面も写る"]
    C2 --> T2["係数 B<br/>2回跳ねた光まで<br/>平均 0.0336(+8%)"]
    T2 --> C3["3回目を撮る<br/>拡散の環境光 = B"]
    C3 --> T3["係数 C<br/>平均 0.0336(+0%)"]
```

**跳ね返りのために書き足したコードは無い**。`BakeProbes` のループが `_probes.Bake` を回数だけ呼び、
2回目からは `PrepareProbeCapture` が「プローブを使う」を立てるだけ。撮る側のシェーダは本描画と同じなので、
**本描画がプローブで照らされるなら、撮る絵もプローブで照らされる**——跳ね返りはそれを繰り返しているにすぎない。

**撮っている間は、テクスチャは前の回のまま**(`Bake` が差し替えるのは最後)。だから1回の焼きの中では、どのプローブも同じ回数だけ跳ねた光を見る。

### 1画素が照り返しを受け取るまで — 8点を混ぜる、壁を見ずに

```mermaid
flowchart TD
    P["画素の世界の位置 vWorldPos<br/>と法線 N"] --> CELL["cell = &lpar;位置 - 格子の角&rpar; / 1.5m<br/><b>格子の中へ寄せる</b>(外は端の値)"]
    CELL --> BASE["base = 切り捨て<br/>t = 端数(0〜1)<br/>いちばん上の点では1つ手前の升へ"]
    BASE --> LOOP["8 つの角ごとに<br/>重み = 3軸の一次補間の積<br/>番号 = x + 7 &lpar;y + 4 z&rpar;"]
    LOOP --> SH["ProbeSh&lpar;番号, N&rpar;<br/>texelFetch 9 回 x 基底"]
    SH --> SUM["重みを掛けて足す<br/><b>壁があるかは見ていない</b>"]
    SUM --> CL["max&lpar;0&rpar;<br/>2次で打ち切った波で負になる向きを止める"]
    CL --> AMB["拡散の環境光 = kD x これ x albedo x AO<br/><b>IBL の放射照度マップの1行を差し替えただけ</b>"]
```

**C# の鏡(`IrradianceProbes.Irradiance`)が同じ手順を持っている**。自己チェックは小さな板を置いて成分表示 18 で描かせ、
画面の真ん中の画素を C# の値と突き合わせる(ずれ 0.05%)。テクスチャの並び・番号の振り方・重み・基底の定数のどれが食い違っていても落ちる。

**各点で評価してから混ぜても、係数を混ぜてから評価しても同じ**(どちらも一次式)。シェーダは読みやすさを取って前者にした。
後者なら基底の計算が8回から1回に減るが、`texelFetch` の 72 回は変わらない——値段の大半はそちら。

### 今日残した歪み(4つ)

**1つ目: 格子の範囲が `Program.cs` に直書きされている**(`ProbeGridMin` / `ProbeGridMax`)。
完成版の裏通りの形(左の壁 x = −4、突き当り z = −13)を知っているのは本来シーンなので、Day 56 の灯りや夜と同じく
`demo-final.json` に書くのが筋。新しいキーなら古い Day は黙って飛ばすので、書き足しても壊れない(改造課題3)。

**2つ目: 焼いた係数を保存しない**。起動して `F2` を押すたびに 1.3 秒焼く。本物のエンジンはライトマップと同じく**ビルドのときに焼いてファイルに持つ**
(ゲームの実行中に焼くことはまずない)。係数は 31.5KB なので、書き出すのは難しくない(改造課題3)。

**3つ目: 鏡面は空のまま**。プローブが運ぶのは拡散だけで、金属のゴミ箱には今日も空(7%)が映る。
場所ごとの映り込みには**反射プローブ**(点ごとにキューブマップを持ち、粗さでぼかす)が別に要る。
9係数では鏡面の細かさを持てない(要点4の「小さく明るい光」と同じ理由)ので、拡散のプローブを流用することもできない。

**4つ目: 壁の外のプローブが壁の裏から通りを覗いている**。撮るときも本描画と同じく裏面を間引くので、
片面の板の壁は裏から見ると透ける。壁の外の列(x = −5)は、壁の向こうの通りの照り返しを受け取っている。
裏面を描いても、裏面は表の法線で照らされる(`textured.frag` は `gl_FrontFacing` を見ていない)ので、やはり明るく写る。
DDGI(Day 66b)は「裏面ばかり見ている点は使わない」でこれを防ぐ。

### 引き継いだ図

Day 63b の設計書を丸ごと引き継ぎ、差分の当たった図にだけ手を入れてある。変わった図は次の4つ。

| 図 | 何が変わったか |
|---|---|
| `全体構成` | **図は同じ**。今日の段落を足した(プローブは `Render/` の中。`Render/` から外への矢印は増えていない) |
| `Render` のクラス図 | **`IrradianceProbes` / `SphericalHarmonics` を足し、`EnvironmentMap.DrawSkybox` に `environmentMip`** |
| `1フレームの流れ` | 分岐の合流点と `RenderRaymarch` の間に **`RenderProbeSpheres`** |
| `1画素の色が決まるまで` | IBL の拡散の段に「**焼いてあればプローブ**」 |

新しく足した図が3つ(この節の上の3つ)。
Day 63b の設計書の冒頭にあった3つの図(四角形1枚が三角形 8,192 枚になるまで、亀裂が出るまで、値段はパッチの数で決まる)は
テッセレーションの話なので、今日は外した(Day 63b の計画書を見ること)。
「IBL が焼き上がるまで」には段落を1つ足した(図は同じ)。

`Model/`・`Scene/`・`Ecs/`・`Physics/`・`Text/`・`Audio/`・`Game/`・`Demo/`・`Core/`・`Sandbox/` の図は**昨日のまま**で、今日は1つも触っていない。

### 全体構成 — 9つの層と、その上のゲーム・デモ・実験台(Day 66a でも形は変わらない)
...
graph TD
    ...
    SB -->|"Shader / RenderResources / Camera / GpuTimer / Mesh / Primitives / Vertex(Day 63a)"| R
...
**Day 66a でも層も矢印も増えていない**。今日の2つ(`IrradianceProbes` / `SphericalHarmonics`)は `Render/` の中に入り、
借りるのも `Render/` の中の型(`Framebuffer` / `Camera` / `Texture` / `Mesh` / `Primitives` / `Shader` / `RenderResources`)と、
`Core/` の `Handle` だけ。**教養編で初めて `Sandbox/` ではなく `Render/` に置いた**——プローブは実験台ではなく、完成版の絵を描く道の一部だから。

`Program` は `ApplyLighting` から毎フレーム `_probes.Apply` を呼び、焼くときは描く関数(`DrawProbeCapture`)を渡す。
**`Render/` が `Demo/`(完成版のシーン)を知らずに済んでいる**のはこの渡し方のおかげで、
Day 36 の `EnvironmentMap`(何を映すかは外から渡す太陽の向きだけで決まる)と同じ向きの線になっている。

**Day 63b では層も矢印も増えていない**。`Sandbox/` → `Render/` のラベルは Day 63a の7つのまま——
今日足した `TessellationLab` が借りるのも、ちょうどこの7つだった
(`Shader` / `RenderResources` / `Camera` / `GpuTimer` / `Mesh` / `Vertex`。`Primitives` は使わず、
パッチの並びは自分で作っている)。

**`Render/` は2か所だけ変わった**。`Shader` に4段のコンストラクタ、`RenderResources` に窓口。
どちらも Day 63a で足したものの隣に並ぶ形で、**中身は1行も書き換えていない**。
教養編が `Render/` に触るのはこれで2日目になるが、触り方は同じ——
<b>エンジンが持っていない GL の機能を使うための口を足すだけで、既にある道は曲げない</b>。

**Day 63a では層は増えていないが、矢印が太くなった**。`Sandbox/` → `Render/` のラベルが4つから7つになる——
`Mesh` / `Primitives` / `Vertex` が加わった。Day 57・58 はシェーダとフルスクリーンの三角形だけで済んでいたが、
今日は**メッシュを段に通す**のが主題なので、形を作る側と頂点の並びを知る必要が出た。

**そして Day 56 から3日ぶりに `Render/` が変わった**。足したのは3つの口だけ
(`Shader` の3段のコンストラクタ、`RenderResources.LoadShader(vert, geom, frag)`、`Mesh.Draw(mode)` と `DrawVertices(mode)`)で、
**既存の呼び出しは1つも書き換えていない**。Day 57・58 が `Render/` を1行も触らずに済んだのは、
どちらも**パイプラインの外(コンピュート)か、既にある段の中(画素)**だったため。
今日は段そのものが増えるので、段の数を受け取る口が要った。
教養編が `Render/` に触るのはこれが初めてで、**触り方の目安**にもなる——
「エンジンが持っていない GL の機能を使う」なら口を足してよいが、既にある道を曲げてはいけない。
...
| `Sandbox/` | **`Render` の7つだけ**(`Shader` / `RenderResources` / `Camera` / `GpuTimer` / `Mesh` / `Primitives` / `Vertex`。後ろ3つは Day 63a) | それ以外の全部 |
...
### Render — OpenGL の薄い皮(Day 66a でプローブの2つが増えた)

**Day 66a で2つのクラスと、1つの口が増えた**。

| 足したもの | 何のために |
|---|---|
| `SphericalHarmonics`(静的) | 基底・立体角・キューブの向き・射影・余弦の畳み込み・評価。**`System.Numerics` しか使わない** |
| `IrradianceProbes` | 格子・6面を撮る・まとめて読む・縮める・テクスチャ・C# の鏡の補間・球で見る |
| `EnvironmentMap.DrawSkybox(…, environmentMip)` | プローブを撮るときは空を3段目で引く(既定 0。画面に描くときは今までどおり) |

**`SphericalHarmonics` は `Pbr` と同じ置き方**(どこにも矢印を出さない)。シェーダ3本(`textured.frag` / `deferred.frag` / `probe.frag`)に
同じ定数があり、こちらは定義と検算。自己チェックの前半6項目が窓を開かずに走るのはこのおかげ。

**`IrradianceProbes` は「自分でバッファを持ち、シェーダは借りる」形**(`PostProcess` / `ShadowMap` / `EnvironmentMap` / `Ssao` / `GBuffer` と同じ)。
違うのは**描くものを関数で受け取る**こと(`Bake(Action<Camera> drawScene)`)。`DeferredLighting.Render` が光の uniform を
`Action<Shader>` で受け取るのと同じ形で、`Render/` が `Demo/` を知らずに完成版のシーンを撮れる。
`Camera` を1つ自分で持つ(画角 90 度・1:1)のもここだけ。

**`Framebuffer` は Day 31 の形(カラー1枚 + 深度)のまま、ビューポートで 96 の枠に切って使う**。
Day 36 の `EnvironmentMap` は生の FBO にキューブの面を挿していたが、今日は読み返して CPU で縮めるので、キューブマップの形で持つ必要が無かった
(2D の枠から `glReadPixels` で読んだ画素の並びは、キューブの面を `glGetTexImage` で読んだときと同じ)。

**Day 63b で2つの口が増えた**。昨日と同じく、既にある形の上に足しただけ。

| 足したもの | 何のために |
|---|---|
| `Shader(gl, vert, tesc, tese, frag, defines)` | 頂点 + 制御 + 評価 + 画素の4段。**`#define` も受け取る** |
| `RenderResources.LoadShader(vert, tesc, tese, frag, defines)` | 窓口。キーに4本のパスと `#define` |

**`#define` を受け取るのが Day 63a の3段との違い**。テッセレーションの割り方
(`equal_spacing` / `fractional_odd_spacing` / `fractional_even_spacing`)は **layout 修飾子**で、
uniform にできない。3通りを見比べるには<b>同じソースを3回コンパイルする</b>しかない。

**そして Day 57 の `InjectDefines` がそのまま効いた**。あれはコンピュートのワークグループの大きさのために書いたもので、
<b>差し込みは段の種類を見ていない</b>ので、描画用のプログラムにも書き足すものが無かった。
Day 63a で「段の配列が6日後に別の用途で使えた」と書いたのと同じことが、今度は `#define` で起きている。

**`Mesh` は今日は変わっていない**。Day 63a で足した `Draw(PrimitiveType)` に
`PrimitiveType.Patches` を渡すだけで済んだ——**約束が変わるだけで、バッファの中身は同じバイト列**。

**Day 63a で3つの口が増えた**。
どれも既にある形の上に足しただけで、中身は1行も書き換えていない。

| 足したもの | 何のために |
|---|---|
| `Shader(gl, vert, geom, frag)` | 頂点 + ジオメトリ + 画素の3段。**中身は1行も要らなかった** |
| `RenderResources.LoadShader(vert, geom, frag)` | 窓口。キーに3本目のパスが入る |
| `Mesh.Draw(PrimitiveType)` | `Points` や `TrianglesAdjacency` で描く。`Draw()` はこれへのくぐり戸になった |
| `Mesh.DrawVertices(PrimitiveType)` | `glDrawArrays`。索引を使わず頂点を順に並べる |

**`Shader` に中身を足さずに済んだのが今日いちばんの収穫**。Day 57 がコンピュートを入れるために
段の並びを配列(`_stages`)にしてあったので、「2本」でも「1本」でも「3本」でも同じ道を通る。
<b>拡張のために書いたのではなく、コンピュートを入れるために仕方なく配列にした</b>ものが、
6日後に別の用途でそのまま使えた形になる。ホットリロードも uniform のキャッシュも、書き足さずにそのまま効く。

**`Mesh.Draw` の固定が破れた**。Day 14 からここは `PrimitiveType.Triangles` で固定で、
「インデックスを3つずつ束ねて三角形にする」以外の読み方をエンジンが必要としなかった。
ジオメトリシェーダは同じ頂点バッファを `Points`(1つずつ)や `TrianglesAdjacency`(**6つずつ**)と読み替える——
**並べ方の約束が変わるだけで、バッファの中身は同じバイト列**(Day 13 の「バッファは型を持たない」)。

**`DrawVertices` を別に足したのは、数を合わせるため**。三角形の索引のまま点を描くと、
何枚もの三角形に使われている頂点がその回数だけ点になり(球なら 221 頂点に対して点が 1,152 個)、
「板1枚 = 頂点1つ」の対応が崩れて検算にならない。

**Day 57 で `Shader` が作り直された**。
昨日までは頂点と画素の2本で固定(`_vertexPath` / `_fragmentPath`)だったが、
...
classDiagram
    ...
    class Shader {
        +Shader(gl, vert, geom, frag)
        +Shader(gl, vert, tesc, tese, frag, defines)
        +Use()
        +SetInt(name, value)
        +SetInt3(name, x, y, z)
        +SetInt2(name, x, y)
    ...
    class RenderResources {
        ...
        +LoadShader(vert, geom, frag) Handle
        +LoadShader(vert, tesc, tese, frag, defines) Handle
    ...
    class EnvironmentMap {
        ...
        +Apply(shader)
        +DrawSkybox(camera, restoreDepthTest, restoreCulling, environmentMip)
    }
    class SphericalHarmonics {
        ...
        +float Band2Zonal
        +float Band2Sectoral
        +BandFactor(coefficient) float
        +Basis(n, y)
        +CubeTexelDirection(face, s, t) Vector3
        +TexelSolidAngle(x, y, size) float
        +ProjectFace(pixels, stride, size, face, coefficients)
        +ToIrradiance(coefficients)
        +Evaluate(coefficients, n) Vector3
        +ProjectFunction(radiance, size) Vector3[]
    }
    class IrradianceProbes {
        ...
        +double TotalBakeMilliseconds
        +int FacesRendered
        +long ByteSize
        +Configure(min, max, spacing)
        +Clear()
        +Index(x, y, z) int
        +ProbePosition(x, y, z) Vector3
        +Coefficients(index) ReadOnlySpan
        +Bake(drawScene)
        +CaptureOne(position, drawScene) Vector3[]
        +Apply(shader, enabled)
        +Irradiance(position, normal) Vector3
        +Corners(position) Corner[]
        +ReadTexture() float[]
        +DrawProbes(camera, radius)
        -CopyTile(pixels, width, left, bottom, tile)
        -Upload(coefficients)
    }
    ...
    IrradianceProbes *-- Framebuffer : 撮る先(横6面 x 縦16点)
    IrradianceProbes *-- Texture : 係数(9列 x プローブの数)
    IrradianceProbes *-- Mesh : 見るための球
    IrradianceProbes *-- Camera : 撮るカメラ(90度・1対1)
    IrradianceProbes ..> SphericalHarmonics : 縮める / 引く
    IrradianceProbes ..> RenderResources : シェーダとプレースホルダを借りる
    IrradianceProbes ..> Primitives : 球を作ってもらう
    IrradianceProbes ..> Shader : 係数を刺す / 球を描く
    IrradianceProbes ..> RenderTargetFormat : Rgba16F
...
### Sandbox — 教養編の実験台(Day 63b でテッセレーションの2つが増えた。Day 66a では変わっていない)

Day 57 のコンピュート(`GpuParticles`)、Day 58 のレイマーチング(`Raymarcher` / `SdfScene` / `Sdf`)、
Day 63a のジオメトリシェーダ(`GeometryLab` / `MeshAdjacency` / `PrimitiveCounter`)、
Day 63b のテッセレーション(`TessellationLab` / `TessellationMath`)は**互いを知らない**ので、図を4枚に分けてある。

**ただし今日はじめて、題材どうしが部品を貸し借りした**。`TessellationLab` が
Day 63a の `PrimitiveCounter` をそのまま使い、画素シェーダも `gs-surface.frag` を借りている。
`Sandbox/` の中の横の線はこれが初めてで、**そろそろ「実験台どうしの共通部品」をどこに置くかを決める時期**になる
(いまは Day 63a のファイルに置いたままなので、Day 63a を消すと Day 63b が壊れる)。
...
classDiagram
    class GeometryLab {
        +GeometryTopic Topic
        +GeometryShape ShapeKind
        +bool Weld
        +bool ShowBase
        +bool ColorByPrimitive
        +float Time
        +float Amount
        +double GpuMilliseconds
        +ulong EmittedPrimitives
        +string ShapeName
        +Vertex[] Vertices
        +uint[] Indices
        ...
        +float BillboardSize
        +Draw(camera, lightDir, lightColor, ambient)
        +MeasurePass(camera, lightDir, lightColor, ambient, pass, repeats, ms) ulong
        +DrawTopicOnly(camera, lightDir, lightColor, ambient)
        +Limits() Vertices_Components_Total_Invocations
        +Dispose()
        -float _lineWidth
        -Shape _sphere
        -Shape _cube
    ...
    class MeshAdjacency {
        +float WeldGrid$
        +Weld(vertices, weldedCount)$ int[]
        +Build(vertices, indices, weld, report)$ uint[]
        +CountSilhouetteEdges(vertices, adjacency, model, eye, frontFacing)$ int
        +IsFrontFacing(a, b, c, eye)$ bool
        -AddEdge(opposite, edgeUse, from, to, third)$
        -Neighbor(opposite, from, to, fallback, boundary)$ uint
        -Quantize(position)$ X_Y_Z
    }
    class Report {
        +int VertexCount
    ...
    GeometryLab *-- Shape : 3つ
    GeometryLab *-- GpuTimer : 1本
    GeometryLab *-- PrimitiveCounter : 1本
    GeometryLab o-- RenderResources : シェーダの窓口
    GeometryLab ..> Shader : uniform
    GeometryLab ..> Camera : Draw の引数
    GeometryLab ..> GeometryTopic
    GeometryLab ..> GeometryShape
    GeometryLab ..> GeometryPass
    GeometryLab ..> Primitives : 形を借りる
    GeometryLab ..> MeshAdjacency : 隣接を組む
    Shape *-- Mesh : 3つ
    Shape ..> Report
    MeshAdjacency ..> Report
    MeshAdjacency ..> Vertex : 位置だけ
```

**線がやはり2つの塊に分かれている**(Day 58 の `Raymarcher` / `SdfScene` と同じ形)。
上の `GeometryLab` は `Render/` の型を借りる GL の側、下の `MeshAdjacency` は `Vertex` と `System.Numerics` しか使わない。
つなぐのは `Build`(隣接を組む)と `CountSilhouetteEdges`(輪郭を数える鏡)の2本だけで、
**自己チェックの前半5項目が GL を1行も通らずに走る**のはこのおかげ。

| 持ち方 | 何を | なぜ |
|---|---|---|
| `*--`(所有) | `Shape` 3つ・`GpuTimer` 1本・`PrimitiveCounter` 1本 | メッシュもクエリも GPU のリソース。`Dispose` で自分で返す |
| `o--`(集約) | `RenderResources` | シェーダの寿命は窓口が持つ(Day 31 の約束) |
| `..>`(使う) | `Shader` / `Camera` / `Primitives` / `MeshAdjacency` | 状態を持たない使い方 |

**`Sandbox/` が `Render/` から借りる型が4つから7つに増えた**(`Mesh` / `Primitives` / `Vertex` が加わった)。
Day 57・58 はシェーダとフルスクリーンの三角形だけで済んでいたが、今日は**メッシュを段に通す**のが主題なので、
形を作る側と頂点の並びを知る必要が出た。`Primitives.CreateSphere` / `CreateCube` をそのまま借り、
`Mesh.ReadVertices` / `ReadIndices`(Day 34・35 で自己チェック用に足した窓口)で中身を取り出している。

**`GeometryLab` は形の中身を3重に持っている**。`Shape` 1つにつき `Mesh` が3つ(三角形用・溶接あり隣接用・溶接なし隣接用)。
`Mesh<TVertex>` が VBO と EBO を組で持つ形なので、「同じ頂点を別の索引で読む」ができないため
(設計書の冒頭「今日残した歪み」の1つ目)。

**`PrimitiveCounter` は `GpuTimer` の形をそのまま写した**。輪にして待たずに拾うのも、
「同じ種類のクエリは入れ子にできない」制約も同じ。違うのは<b>待つ口(`Measure`)を1つ持っている</b>ことで、
自己チェックと値段の測定はその場で正確な数が要るから。
なお**時間と図形の数は同時に走らせられる**(入れ子にできないのは同じ種類どうしだけ)ので、
`MeasurePass` は `TIME_ELAPSED` を `PRIMITIVES_GENERATED` の内側に置いて両方を測っている。

#### テッセレーション(Day 63b)

```mermaid
classDiagram
    class TessellationLab {
        +int PatchVertices$
        +TessellationTopic Topic
        ...
        +float LodDistance
        +double GpuMilliseconds
        +ulong GeneratedPrimitives
        +string TopicName
        +int PatchCount
        +Vertex[] Vertices
        +uint[] Indices
        +float Radius
        +int ExpectedPrimitivesPerPatch
        +int ExpectedPrimitives
        +Draw(camera, lightDir, lightColor, ambient)
        +MeasurePass(camera, lightDir, lightColor, ambient, repeats, ms) ulong
        +CheckEdgeAgreement(eye, matchEdges, internalEdges, mismatched) float
        +MaxRoundedLevel(eye) float
        +Limits() GenLevel_PatchVertices_Control_Evaluation
        +Dispose()
    }
    class PatchMesh {
        +string Name
    ...
    class TessellationTopic {
        <<enumeration>>
        Terrain
        Sphere
    }
    class TessellationSpacing {
        <<enumeration>>
        Equal
        FractionalOdd
        FractionalEven
    }
    class TessellationMath {
        +float NormalStep$
        +float MaxLevel$
        +Height(p, time)$ float
        +Bilinear(p00, p10, p01, p11, uv)$ Vector3
        +Displace(basePosition, time, displacement)$ Vector3
        +DisplacedNormal(basePosition, time, displacement)$ Vector3
        +LevelAt(worldPosition, eye, level, lodDistance)$ float
        +Round(level, spacing)$ float
        +EdgeSample(segments, index)$ float
        +WorstEdgeMismatch(a, b)$ float
    }
    class Shader
    class GpuTimer
    class PrimitiveCounter
    ...
    TessellationLab *-- PatchMesh : 2つ
    TessellationLab *-- GpuTimer : 1本
    TessellationLab *-- PrimitiveCounter : 1本
    TessellationLab o-- RenderResources : シェーダの窓口
    TessellationLab ..> Shader : uniform
    TessellationLab ..> Camera : Draw の引数
    TessellationLab ..> TessellationTopic
    TessellationLab ..> TessellationSpacing
    TessellationLab ..> TessellationMath : レベルと丸め
    PatchMesh *-- Mesh : 1つ
    TessellationMath ..> TessellationSpacing
    TessellationMath ..> Vertex : 位置だけ
```

**線がやはり2つの塊に分かれている**(Day 58・63a と同じ形)。
上の `TessellationLab` は `Render/` の型を借りる GL の側、下の `TessellationMath` は
`System.Numerics` しか使わない。つなぐのは `LevelAt` / `Round`(レベルの決め方)と
`Bilinear` / `Displace` / `DisplacedNormal`(評価シェーダの式)だけで、
**自己チェックの前半7項目が GL を1行も通らずに走る**。

| 持ち方 | 何を | なぜ |
|---|---|---|
| `*--`(所有) | `PatchMesh` 2つ・`GpuTimer` 1本・`PrimitiveCounter` 1本 | GPU のリソース。`Dispose` で自分で返す |
| `o--`(集約) | `RenderResources` | シェーダの寿命は窓口が持つ(Day 31 の約束) |
| `..>`(使う) | `Shader` / `Camera` / `TessellationMath` | 状態を持たない使い方 |

**`PrimitiveCounter` を Day 63a から借りている**。あれは「GS が出した図形の数」を数えるために作ったが、
`GL_PRIMITIVES_GENERATED` は<b>ラスタライザへ渡る直前の図形の数</b>なので、
テッセレータが作った三角形もそのまま数えられる。1文字も足していない。

**`TessellationLab` はシェーダを3本持っている**。割り方が layout 修飾子で uniform にできないため
(設計書の冒頭「今日残した歪み」の1つ目)。`RenderResources` のキーに `#define` が入っているので、
3本が別物として数えられる——ここを混ぜ忘れると**3通りが全部 `equal_spacing` になる**うえ、
細かさは同じなので絵を見ても気付けない。

**パッチの並びは `Primitives` に足さず、`TessellationLab` が自分で作っている**。
三角形とパッチでは「索引を何個ずつ読むか」の約束が違うので、同じ `Mesh<Vertex>` に
両方入れると型から読めなくなるため(同2つ目)。
...
flowchart TD
    ...
    RG --> PRB
    RS2 --> PRB
    PRB["RenderProbeSpheres&lpar;&rpar;&lpar;Day 66a&rpar;<br/>プローブを球で出す(F7)<br/><b>深度を書く</b>。壁の向こうの球は隠れる<br/>出していなければ素通り"]
    PRB --> RMC
    RMC["RenderRaymarch&lpar;dt&rpar;&lpar;Day 58&rpar;<br/>三角形1枚で距離関数の場面を描く<br/><b>gl_FragDepth を書く</b>(LEQUAL)<br/>出していなければ素通り"]
    RMC --> GLB
    GLB["RenderGeometryLab&lpar;dt&rpar;&lpar;Day 63a&rpar;<br/>下地 → 今日の題材<br/><b>図形の数を GL に数えさせる</b><br/>出していなければ素通り"]
    GLB --> TSL
    TSL["RenderTessellationLab&lpar;dt&rpar;&lpar;Day 63b&rpar;<br/>パッチを 4 つずつ束ねて描く<br/><b>作った三角形を GL に数えさせる</b><br/>出していなければ素通り"]
    TSL --> PT
...
**Day 66a で `OnRender` にもう1段増えた**。分岐の合流点と `RenderRaymarch` の間の `RenderProbeSpheres`(`F7` の球)。
不透明なものの後ろ、粒の手前——球は深度を書くので、壁の向こうのプローブは壁に隠れる。
完成版が出ていて、焼いてあって、`F7` を押したときだけ描く。

**プローブの本体は図に出てこない**。照り返しは `ApplyLighting`(図の `Render3D` と `RenderDeferredLighting` の中)が
`_probes.Apply` でテクスチャと格子を渡すだけで、描く段は1つも増えていない——**IBL の拡散の1行が差し替わっただけ**なので、
フォワード・Forward+・ディファードのどれで描いても同じように効く。**焼くのはキーを押したときで、フレームの外**。

**`OnUpdate` は今日も1行も変わっていない**。

**Day 63b で `OnRender` にもう1段増えた**。`RenderGeometryLab` と `RenderParticles` の間の `RenderTessellationLab`。
昨日と同じ場所(不透明なものの最後、粒の手前)に置いてある——
**割った三角形はふつうの三角形**なので、深度も速度も後ろの段から見れば区別が付かない。
中で `TessellationVisible` を見て、ほかの「それだけを見る絵」が出ているときは何もしない。

**`OnUpdate` は今日も1行も変わっていない**。波の時計は描画の側で進め、一時停止(`_paused`)だけは見る。
**止めてからカメラだけ動かすと、距離 LOD が「カメラで決まる」ことがはっきりする**。

**Day 63a で `OnRender` にもう1段増えた**。`RenderRaymarch` と `RenderParticles` の間の `RenderGeometryLab`。
レイマーチングと同じ場所(**不透明なものの最後、粒の手前**)に置いたのは、押し出した面も輪郭の線も深度を書くから——
あとの粒・速度・被写界深度から見れば、ふつうのラスタの形と区別が付かない。
中で `GeometryLabVisible` を見て、デモ v1・モデル・材質グリッド・材質テストの板が出ているときは何もしない
(`RaymarchVisible` と同じ優先順位)。出しているあいだは `_draw3D` を切っているので、
Day 31 の床と箱も空も描かれない——線を見る絵なので、背景は暗いほど読みやすい。

**`OnUpdate` は今日も1行も変わっていない**。形の回転も押し出しの息づかいも描画の側(`RenderGeometryLab`)で進め、
一時停止(`_paused`)だけは見る。**止めてからカメラだけ回すと、輪郭が「見る側で決まる」ことがはっきりする**。
...
**Day 66a で `DrawSkybox` に段の引数が1つ増えた**(`environmentMip`、既定 0)。プローブを撮るときは 32x32 の面に 256x256 の空を描くので、
0 段目のままだと8画素おきに読むことになり、太陽を抜いたあとに残った明るい点を拾うかどうかで、撮った空の明るさが向きによって 2.47 倍にもなった。
**3段目(32x32 相当)を引けば 8x8 の平均を読む**。上の「ミップ生成が真ん中に挟まる」と同じ理屈——
**1サンプルが担当する立体角に見合った大きさの段から引く**——が、焼く側ではなく撮る側でもう一度出てきた。
ミップは Day 36 から作ってあった(`GenerateMipmaps`)ので、足したのは引数だけ。
...
flowchart TD
    ...
    DIR --> IBL{"uIblEnabled ?"}
    IBL -->|"1"| AMB["**IBL**(Day 36)<br/>拡散 = kD × <b>放射照度</b> × albedo × <b>AO×SSAO</b><br/>放射照度 = 焼いてあれば <b>ProbeIrradiance&lpar;位置, N&rpar;</b>(Day 66a)<br/>なければ irradiance(N)<br/>鏡面 = prefiltered(R, 粗さ) × (kS×A + B) × <b>AO×SSAO</b>"]
...
**Day 66a で差し替わったのは IBL の拡散の1行**。`texture(uIrradianceMap, N) * uIblIntensity` が、
焼いてあれば `ProbeIrradiance(位置, N)` になる。**鏡面(事前フィルタ)は IBL のまま**で、`kD`・albedo・AO の掛け方も変わらない。
入力に**位置**が増えたのが唯一の違いで、フォワードは `vWorldPos`、ディファードは G-Buffer から戻した `worldPos` を渡す——
どちらも Day 52 までに揃っていたので、足す口が無かった。
...
`dotnet run --project reference/Day66a -c Release` で起動する。起動直後のコンソールは Day 63b と同じ。
メニューを閉じた状態で `Shift+F1` を押すと、最後のページ「プローブGI」(29 枚目)が開く。

### 1. `F2`: 焼いたプローブ

完成版の夜の裏通りが決めの構図で出て(カメラワークは止まっている)、コンソールに次の行が出る。

```
プローブを焼いた: 448 個(7x4x16  間隔 1.5m)  跳ね返り 2 回  面 5,376 枚  1350ms  係数 32KB
  灯り: 7 個(揺らぎの無い満点)  光る材質: 焼かない  時間帯: 夜
  跳ね返りごとの平均の明るさ: 1回 0.0312 → 2回 0.0336(+8%)
焼いたプローブ: **ON**  拡散の環境光を「空1枚」から「その場所から見えたシーン」に差し替えた。灯りの当たらない面に、照らされた路面や壁の色が回り込む
```

焼いている 1.3 秒ほど、画面が止まる。HUD に今日の行が出る。

```
プローブGI:ON 448個(7x4x16) 間隔1.5m  跳ね返り2回  焼き1350ms  係数32KB
```

`F2` を何度か押して ON / OFF を見比べる。**差は控えめ**(要点8)で、いちばん分かりやすいのは**突き当りの壁の右側**と**バリアの裏**。
ON にすると、そこがわずかに明るく、赤みを帯びる。

### 2. `F8`: 放射照度を見る(**今日の見どころ**)

成分表示 18(Day 36 の「放射照度」)に切り替わる。画素の色が「その面が受けている環境光」になる。

- **プローブ ON**: 灯りのまわりの路面と壁が暖色に、突き当りの壁の看板の前が桃色に、ランタンの下が橙色に染まる。
  通りの真ん中や、灯りから離れた路面は青みがかる(空が主)。**場所によって色も明るさも違う**
- **`F2` で OFF(空だけ)**: 路面は青一色、壁は向きごとに一様な色。**向きだけで決まり、場所によらない**——
  Day 36 の放射照度マップは世界に1枚しかないので

この2枚の違いが、今日足したものの全部。

### 3. `F7`: プローブを見る

1.5m おきに 448 個の球が浮かぶ(半径 12cm)。球の各点は「その向きを向いた白い面が、そこで受ける照り返し」。
ほとんどの球は**暖かい灰色**で、**看板の前の球だけが桃色**になる(画素の色でおよそ (95, 78, 55) と (140, 76, 81))。
灯りのそばの球でも、灯りの側だけが光ることはない——プローブが運ぶのは灯りの直接の光ではなく、照らされた面からの光だから。
照り返しの明るさは白い面でも 0.03〜0.1 ほどしかないので、色の違いは `F8` の放射照度のほうが読みやすい。

### 4. `F9`: 壁の上から見下ろす(`F8` と一緒に)

真上近く(仰角 86 度)から、左の壁と、その外の地面を見る。画面の下が通り、上が壁の外。

```
壁の上から見下ろす: **ON**  画面の下が通り、上が壁の外。**F8(放射照度を見る)で見ること**。壁の外の地面は、そもそも街灯に直接照らされている(点光源は影を持たないので、光が壁を突き抜ける)。照り返しの漏れは、直接の光を外した放射照度で初めて見える(壁の外に 70cm ほどの暖色の帯)
  壁から 50cm 外の地面 (-4.5, -4.0): 8点の重みのうち **33% が壁の内側のプローブ**、67% が外側。三線形補間は壁を見ていない
```

ふつうの表示では、壁の外の地面も街灯に照らされて明るい(点光源は壁を突き抜ける)。`F8` で放射照度にすると、
**壁の線の外側に、幅 70cm ほどの暖色の帯**が見える。帯の外(x < −5)は、格子の外なので端の列の値で一様に塗られる。
`F5` で間隔を 1.0m にすると、帯が消える。3.0m にすると、帯はあるが 1.5m より薄い(要点5の表)。

### 5. `F10`: 灯りを消す(焼き直さない)

直接の光が全部消えるが、**壁と路面がぼんやり光ったまま**になる。HUD に「焼いた条件と違う」が出る。

```
灯り: **消灯**  **焼いた照り返しは残っている**。灯りが消えたのに、壁と路面がぼんやり光ったまま(F3 で焼き直すと、灯りのぶんが消える)
```

```
プローブGI:ON 448個(7x4x16) 間隔1.5m  跳ね返り2回  焼き1350ms  係数32KB  **焼いた条件と違う(灯りを点けて焼いた)**
```

`F3` で焼き直すと、灯りのぶんの照り返しが消える(光だまりを見下ろす面で 0.0600 → 0.0113)。もう一度 `F10` で点け、`F3` で焼き直して元に戻す。

### 6. `F4` / `F5` / `F6`: 焼く条件を変える

どれも押すと焼き直す。

| キー | 押したあと | 焼く時間 | 平均の明るさ |
|---|---|---|---|
| `F4`(2 → 3 回) | 跳ね返り3回 | 1.77 秒 | 0.0312 → 0.0336 → **0.0336**(3回目は +0.0001) |
| `F4`(3 → 1 回) | 跳ね返り1回 | 0.54 秒 | 0.0312 |
| `F5`(1.5 → 1.0m) | 1,440 個(10x6x24) | 1.7 秒(1回) | 0.0305 |
| `F5`(1.0 → 3.0m) | 108 個(4x3x9) | 0.13 秒(1回) | 0.0285 |
| `F6` | 光る材質も焼く | 1.1〜1.3 秒 | 0.0336 → 0.0417(**+24%**。二重に数えたぶん) |

**間隔で平均が変わる**のは、点の置き場所が変わるため。平均は点ごとの値を足したものなので、
空を広く見る上の段や壁の外の列がどれだけ混ざるかで動く(絵の明るさの平均ではない)。
3.0m では「街灯の下で路面を見下ろす面」が 0.0600 → 0.0471 に下がる——いちばん近い点が光だまりから遠くなるため。

### 7. `F11`: 内訳

```
--- プローブGI の内訳 ---
  格子: 7 x 4 x 16 = **448 個**  間隔 1.5m  (-5.0, 0.3, -14.0) から (4.0, 4.8, 8.5)
    左の壁(x = -4)と突き当りの壁(z = -13)の外に1列ずつはみ出させてある(光漏れを見るため)
  係数: 1個 9 x RGBA16F = 72 バイト(うち A の 18 バイトは使っていない)→ 全部で **31.5KB**  撮った 32x32 x 6面のまま持つと 21.0MB(**683 分の1**)
  焼いた条件: 夜 / 灯り 点灯 / 跳ね返り 2 回 / 光る材質 焼かない  → 今と同じ
  跳ね返りごとの平均の明るさ: 1回 0.0312 → 2回 0.0336(+8%)
  焼いた時間: 合計 1300ms(最後の1回: 撮る 136ms / 読み返す 321ms / 縮める 161ms)

  場所ごとの放射照度 / π(輝度)。空だけ = IBL、プローブ = 今日
    場所                      空だけ     プローブ    倍率   色(プローブ)
    街灯の足元の路面               0.0249     0.0433     1.7倍   (0.059, 0.040, 0.031)
    街灯の横の壁                 0.0507     0.0655     1.3倍   (0.085, 0.062, 0.041)
    看板の下の壁                 0.0313     0.0429     1.4倍   (0.059, 0.039, 0.030)
    街灯の下で路面を見下ろす面          0.0050     0.0600    11.9倍   (0.109, 0.050, 0.018)
    通りの真ん中の路面              0.0249     0.0275     1.1倍   (0.031, 0.026, 0.029)
    手前のバリアの陰               0.0249     0.0263     1.1倍   (0.029, 0.025, 0.028)
    壁の外の地面(光漏れ)            0.0249     0.0316     1.3倍   (0.034, 0.031, 0.033)
    壁の外の地面・遠く              0.0249     0.0235     0.9倍   (0.018, 0.024, 0.034)
  光漏れ: 壁の外 50cm の地面は、8点の重みのうち **33% を壁の内側のプローブ**から受け取っている(外側 67%)。遠くの地面は格子の外なので、端(x = -5.0)の列で塗られる
```

**「街灯の下で路面を見下ろす面」が 11.9 倍**になっているのが今日の核心。空だけなら下を向いた面は地面(空の下半分)しか見ておらず暗いが、
プローブは**灯りに照らされた路面**を見ている。上を向いた面(路面そのもの)は空を見ているので、倍率は小さい(1.1〜1.7 倍)。
「空だけ」の列は Day 36 の空を9係数に縮めたもの x 強さ(0.07)で、本描画の `texture(uIrradianceMap, n) * uIblIntensity` とほぼ同じ値。

### 8. `F12`: 値段

```
--- プローブGI の値段 ---
  焼く(1回ぶん): プローブ 448 個 x 6面 = **2,688 面**、シーンの物 35 個なので 描く命令 94,080 回
    撮る 137ms(1面 50.9us) / 読み返す 305ms(28 回) / 縮める 163ms  → 跳ね返り 2 回で合計 **1392ms**
    ※ 撮るのは CPU が描く命令を積む時間で、GPU が撮り終えるのを待つのは読み返しの側に入る
  引く(毎フレーム・シーンを 8 回描いた平均): 空だけ 0.379ms / プローブ **0.515ms**  差 +0.136ms
    画素1つにつき texelFetch 72 回(8点 x 9係数)。画面 614,400 画素なら **44M 回**。キューブマップを1回引くのと比べれば多いが、読む場所が9列 x 数百行に収まっているのでキャッシュに乗る
```

VSync を入れたまま測っているので走行ごとにぶれる。何度か押して落ち着いた値を読む。

### 9. `1`: 自己チェック(15 項目)

全部 `[OK]` になる。**完成版を出す前に押すと前半6項目だけが走り**、「ここから先は完成版のシーンが要る」と出る。

```
=== Day 66a 自己チェック(SH の数学・撮る向き・補間・跳ね返り・灯り・光る材質)===
  [OK] **6面のテクセルの立体角を足すと 4π**(真ん中と隅で大きさが違う)  12.56637(4π = 12.56637)  真ん中 / 隅 = 4.86 倍
  [OK] **9つの基底は正規直交**(自分とは 1、ほかとは 0。定数を1つ写し間違えると崩れる)  いちばん大きいずれ 0.00040
  [OK] 一様な空(どこからも 1)→ どの向きも 1(**半球の余弦の積分は π、1/π 済みなので 1**)  上 1.0000 / 横 1.0000
  [OK] **上半分だけ光る空 → 上 1 / 横 0.5 / 下 0**(3次以上の奇数次は余弦で消えるので、9個で足りる)  上 1.0002 / 横 0.5000 / 下 -0.0002
  [OK] **小さく明るい光(半径 10 度)は、真裏に上向きの 5% ほど漏れる**(2次で打ち切った限界。本当は 0)  上 0.0307(本当は 0.0302)/ 下 0.0017 = 上の 5.6%
  [OK] 三線形の重みは足すと 1(格子の外でも)。**プローブの上ではそのプローブが 1**(いちばん上の角でも)  ランダムな 200 点で合計 1: はい  (2,1,3) の上 → 1.000 / いちばん上の角 → 1.000
  [OK] **明るい点に背を向けた面では、空の9係数が Day 36 の放射照度マップ(画素ごとの半球積分)と一致**  背を向けた 189 点のいちばん大きいずれ 8.3% / 向いた 195 点では 86%(全体の平均 9.0%。ずれは平均の明るさ 0.323 に対して)
  [OK] **空だけを撮ったプローブが、空の係数 x 強さと一致**(撮る6面の向きの表が GL のキューブと同じ)  いちばん大きいずれ 0.2%  撮った / 期待(+X -X +Y -Y +Z -Z): 1.00 1.00 1.00 1.00 1.00 1.00
  [OK] テクスチャの中身が CPU の係数と同じ(**1行が1プローブ・9列が係数**。RGBA16F に丸めたぶんだけずれる)  108 行 x 9 列 / いちばん大きいずれ 0.04%(0 次に対して)
  [OK] **GPU が引いた放射照度が C# の鏡と一致**(並び・番号・重み・基底のどれかが違えば落ちる)  いちばん大きいずれ 0.05%  GPU/C#: 街灯の足元の路面 0.0333/0.0333  街灯の横の壁 0.0628/0.0628  看板の下の壁 0.0410/0.0410
  [OK] **跳ね返るたびに明るくなり、増え方は減っていく**(面は光の一部しか返さないので、足し算が収まる)  1回 0.0285 → 2回 0.0305(+0.0020)→ 3回 0.0306(+0.0001)
  [OK] **灯りを消して焼き直すと、光だまりを見下ろす面の照り返しはほとんど消える**(焼いているのは灯りの光)  灯りあり 0.0423 → なし 0.0048(11%。残りは月に照らされた路面)  格子全体の平均では 0.0285 → 0.0230(81%。**空を見ているぶんは消えない**)
  [OK] **光る材質も焼くと明るくなる**(同じ灯りを、点光源と光る板の照り返しで二重に数える)  焼かない 0.0285 → 焼く 0.0326(+14%)
  [OK] **焼いても、プローブを切っていれば絵は1ビットも変わらない**(撮るときに借りたものを全部返している)  614,400 画素のうち違うもの 0
  [OK] **基底の定数が C# とシェーダ3本で同じ**、補間の本体が textured.frag と deferred.frag で1文字も違わない  定数 0.282095 / 0.488603 / 1.092548 / 0.315392 / 0.546274
```

前半6項目は**窓を開かずに走る**(`SphericalHarmonics` が GL を1行も知らないので)。
後半は**いちばん粗い格子(3.0m)で焼き直して**確かめ、終わったら元の条件で焼き直す(自己チェックの所要はおよそ 2 秒)。
**夕暮れで押しても、キツネで歩いている最中に押しても同じ数字**になる(焼くところだけ夜にそろえている)。

### 10. プローブを入れない限り、絵は Day 63b と1ビットも変わらない

同じ手順を Day 63b と Day 66a で踏み、画面を画素ごとに突き合わせた。
...
| デモ v1 の決めの構図 | 0 |
| 最終デモ・完成版で遊ぶ | 0 |
| 最終デモ・完成版を眺める(決めの構図) | 0 |
| 同じ構図をディファードで | 0 |

`uProbeEnabled` が 0 のときは `texture(uIrradianceMap, normal)` を引くので、式が同じならば値も同じになる。
**焼いたあとでも**、プローブを切っていれば同じ(自己チェック14)。

### 11. メニューの自己チェック

`Alt+F1` で Day 48 の割り当て表のチェックが走り、**29 ページ 289 項目**になっている。
「最終デモ」の自己チェック(21 項目)も、完成版で遊んでいる状態で全部通る。

## 改造課題

### 課題1(易): 格子の間隔と、光漏れの幅

`F9` で壁の上から見下ろし、`F8` で放射照度にしたまま、`F5` で間隔を 3.0 / 1.5 / 1.0m と変える。

1. それぞれで `F11` の「光漏れ」の行を読む。壁から 50cm 外の地面が、壁の内側のプローブから何 % 受け取っているか
2. 3.0m のとき、プローブの列は x = −5, −2, 1, 4。**壁の内側の路面(x = −3.5 など)は、外の列(x = −5)から何 % 受け取るか**を計算する。
   絵で、壁の内側の足元が暗くなっているか(外の暗さが内側へ漏れる「影漏れ」)
3. 1.0m のとき、x = −4 の列が壁の面にちょうど乗る。この列のプローブは何を見ているか(`F7` の球で見る)
4. `ProbeGridMin` の x を −5.25 にずらすと、1.0m のときの列はどこに来るか。光漏れの幅はどう変わるか

見どころは 2 と 4。光漏れは**「間隔を細かくすれば減る」とは限らない**(3.0m のほうが 1.5m より少なかった。要点5)。
**列が壁のどちら側にどれだけ離れて立つか**で決まるので、実際のエンジンは「プローブを壁から押し出す」(置き場所を自動で直す)。

### 課題2(中): 法線の側の点を重くする

DDGI(Day 66b)の重みの1つ、**「面の裏側にある点を軽くする」**を足してみる。

1. `ProbeIrradiance`(`textured.frag` と `deferred.frag` の両方)の重みに、次の係数を掛ける。
   ```glsl
   vec3 toProbe = normalize(probePosition - worldPos);
   float facing = (dot(toProbe, n) + 1.0) * 0.5;
   w *= facing * facing + 0.2;
   ```
   プローブの位置は `uProbeGridMin + vec3(p) * uProbeSpacing` で作れる。**重みの合計で割り直す**のを忘れない(8点の重みの和が 1 でなくなる)
2. C# の鏡(`IrradianceProbes.Irradiance` と `Corners`)にも同じ式を入れる(自己チェック 10 が落ちなくなるまで)
3. `F9` + `F8` で光漏れの帯を見る。**減るか**。`F11` の「壁の外の地面(光漏れ)」の値はどうなるか
4. 壁の内側の路面(上向き)にとって、壁の外の列は「裏側」か

見どころは 3 と 4。上向きの路面から見ると、壁の外の列もやや上にあるので**裏側にならない**——
向きの重みでは壁は分からない。**壁があるかどうかは、その点から見えるかどうか(深度)を持たないと分からない**。
それを持たせたのが DDGI で、Day 66b で読む。

### 課題3(難): 格子をシーンに書き、焼いた係数をファイルに持つ

今日残した歪みの1つ目と2つ目。

1. `demo-final.json` に `"probes": { "min": [...], "max": [...], "spacing": 1.5 }` を足し、`DemoScene` で読む
   (Day 56 の `night` と同じ書き方。**書いていないシーンではプローブを使わない**ようにすると、古い Day が壊れない)
2. `IrradianceProbes` に `Save(path)` と `Load(path)` を足す。中身は格子の形(min・間隔・数)と係数(float x 27 x 点の数)
3. 焼いたら保存し、起動して完成版を出したときにファイルがあれば読むだけにする。**格子の形が JSON と違えば読まずに焼き直す**
4. `F10` で灯りを消して `F3` で焼き直したとき、保存したファイルはどうすべきか

見どころは 4。焼いた係数は**条件ごとの答え**なので、条件(灯り・時間帯・跳ね返りの回数)も一緒に保存しないと、
別の条件の答えを黙って読むことになる。HUD の「焼いた条件と違う」を、ファイルの側でもやることになる。
...
| OpenGL | 4.3 コアプロファイル(ドライバ 596.49) |
| 解像度 | 960 x 640 |
| 確認の条件 | `-c Release`、**VSync を入れて**確認した。値段の測定はシーンを 8 回描く区間をタイマークエリで挟んだもので、`glFinish` の連続描画(計測)は今日も無い |
...
|---|---|---|
| 1 回 | 2,688 | 0.54 秒 |
| 2 回(既定) | 5,376 | 1.30〜1.39 秒 |
| 3 回 | 8,064 | 1.74〜1.79 秒 |

格子の間隔(跳ね返り1回)。

| 間隔 | 点の数 | 係数 | 焼く時間 |
|---|---|---|---|
| 3.0m | 108(4 x 3 x 9) | 7.6KB | 0.13 秒 |
| 1.5m | 448(7 x 4 x 16) | 31.5KB | 0.54 秒 |
| 1.0m | 1,440(10 x 6 x 24) | 101.2KB | 1.72 秒 |

最終の絵でプローブを入れたときの明るさの変化(HUD を消し、同じ構図で ON / OFF を撮って画素ごとに比べた)。
...
1. **空に残った1つの点で、照り返しが 1.6 倍に見えていた**。最初は空を 0 段目のまま撮っていて、焼いたプローブの平均は 0.051 だった。
   自己チェックの「空だけを撮ったプローブ」が +X で 2.47 倍・+Z で 2.00 倍・−Z で 0.74 倍と、向きによって上にも下にも外れた。
   太陽を抜いたあとの空に残った輝度 3,200 ほどの点(向き (0.90, 0.14, 0.42))を、32x32 の1画素が拾うかどうかで決まっていた。
   撮る画素と同じ大きさまで縮めた3段目で引くようにすると、6面とも 1.00 になり、平均は 0.031 に下がった。
   **灯りの照り返しだと思っていたものの4割が、空の1点の標本化の誤差だった**(要点4)
2. **Day 36 の放射照度マップとは、最大のずれでは一致を言えない**。はじめ「384 点のいちばん大きいずれが 10% 未満」で確かめたら 86% で落ちた。
   ずれは空の明るい点を向いた面に集まっていて、Day 36 の側も 0.025 ラジアン刻みで点を拾い損ねうる。
   **点に背を向けた面だけで比べると 8.3%** に収まるので、そちらで一致を確かめ、向いた面のずれは数字を出すだけにした
3. **灯りの効き目を格子全体の平均で測ると、2割にしか見えない**。灯りを消して焼き直しても、平均は 0.0285 → 0.0230(81%)にしか下がらなかった。
   上の段や壁の外の列は空を広く見ていて、夜の 7% の空でも灯りの照り返しと同じくらい効く。
   **光だまりを見下ろす面(下向き)で測ると 11%** まで下がる——焼いているのが灯りの光だということは、灯りを見ている面で確かめる
4. **見下ろしのカメラは、仰角 80 度では壁の外が見えなかった**。目が壁から 1.6m 内側に下がり、高さ 5.5m の壁が外の地面を隠す。
   上限の 86 度にすると、目が壁の 35cm 内側に来て外の地面まで届く
5. **壁の外の地面は、プローブを切っていても明るい**。点光源は影を持たない(Day 52)ので、街灯の光は壁を突き抜けて外の地面を直接照らしている。
   プローブの光漏れはその上に乗る程度なので、**直接の光を外した放射照度(`F8`)で見ないと分からない**
6. **最終の絵の差は小さい**(中央値 +2.2%)。灯りが影を持たないので暗がりがほとんど無く、照り返しは直接の光の数分の1にしかならない(要点8)。
   **成分表示で確かめる日**と割り切り、完成条件の見どころを `F8` に置いた
