...
internal static class Program
{
    ...
    private static void RunGameCheck()
    {
        ...
        Check("どの試合でも育って倒せる", alwaysGrows, $"{seeds.Length} 試合 x 2 通り、Lv.5 以上 / 撃破 100 以上");

        // **操作に意味がある**ことの確認。ここが同じなら、
        // 遊ぶ側の判断がスコアに効いていない。
        Check("逃げ回るほうが長く生きる(3試合の平均)", movingAverage > idleAverage,
        ...
        Check("選ぶと反映される", applied, $"「{picked.Title}」");
        Check("選び終わると再開する", pausing.Phase == GamePhase.Playing);
        ...
        foreach (WeaponKind kind in (WeaponKind[])[WeaponKind.Bolt, WeaponKind.Orbit, WeaponKind.Aura])
        {
            ...
            Check($"{Weapons.NameOf(kind)} だけで倒せる", solo.Kills > 0,
        ...
        Check("長く遊んでも武器が育つ", maxed.WeaponCount > 1,
    ...
    private static void RunGltfCheck()
    {
        ...
        // **同じファイルを2回読む**ことになるが、テクスチャは ResourceManager が
        ...
        foreach ((string path, int parts, int triangles, int materials) in expected)
        {
            ...
            Check($"{name}: 読めた", true, GltfLoader.Describe(full));
            Check($"{name}: パーツが {parts} 個以上", model.Parts.Count >= parts, $"実際 {model.Parts.Count}");
            Check($"{name}: 三角形が {triangles:N0} 個以上", model.TriangleCount >= triangles,
                $"実際 {model.TriangleCount:N0}");
            Check($"{name}: マテリアル {materials} 個", model.Materials.Count == materials,
                $"実際 {model.Materials.Count}");
            Check($"{name}: ベースカラーのマップがある",
                model.Materials.All(m => m.MainTexture.IsValid));
            Check($"{name}: 境界箱が有限", float.IsFinite(model.BoundsRadius) && model.BoundsRadius > 0.0f,
        ...
        Check("Lantern: 3つのパーツが別々の位置にある(親子の掛け合わせが効いている)", distinctTransforms);

        // **BoxTextured は matrix 形式**。Z-up を Y-up に直す回転が入っているので、
        // 単位行列のままなら読めていない。
        Model box = loaded[3];
        Check("BoxTextured: matrix 形式のノードが単位行列になっていない",
        ...
        Check("2回目の読み込みでテクスチャが増えない(重複排除が効いている)",
            _resources.TextureCount == texturesAfterFirst,
            $"{texturesAfterFirst} → {_resources.TextureCount}");
        Check("そのぶんキャッシュヒットが増える", _resources.CacheHits > cacheHitsBefore,
        ...
        Check("全部畳んだらテクスチャの数が元に戻る", _resources.TextureCount == texturesBefore,
    ...
    private static void RunHdrCheck()
    {
        ...
        Check("RGBA16F: 暗部 0.25 がそのまま残る", Near(hdr.X, 0.25f), $"実際 {hdr.X:F3}");
        Check("RGBA16F: 1.0 がそのまま残る", Near(hdr.Y, 1.0f), $"実際 {hdr.Y:F3}");
        Check("RGBA16F: **4.0 が 4.0 のまま入る**", Near(hdr.Z, 4.0f), $"実際 {hdr.Z:F3}");

        _post.SceneFormat = RenderTargetFormat.Rgba8;
        Vector4 ldr = ClearAndRead(probe);

        Check("RGBA8: 1.0 までは入る", Near(ldr.Y, 1.0f), $"実際 {ldr.Y:F3}");
        Check("RGBA8: **4.0 は 1.0 に丸められる**", Near(ldr.Z, 1.0f), $"実際 {ldr.Z:F3}");
    ...
    private static void OnClosing()
    {
        ...
        // **ResourceManager より先に畳む**——順番を逆にすると、
