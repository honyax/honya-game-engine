...
///
/// 今日の箱と平面は、必ず平面の面が支える(<see cref="FaceB"/>)。
/// Day 44b で箱どうしが入ると、分離軸定理がどの軸を選んだかで
/// 接触点の作り方がまるで変わるので、**絵がおかしいときに最初に見る値**になる。
/// </summary>
...
/// <summary>
/// 接触マニフォールド。**1組の物体が触れている「面」を、数点で代表したもの**(要点3)。
///
...
internal struct ContactManifold
{
    ...
    ///
    /// 箱が平面に深く沈むと、平面の下に入る頂点が5つ以上になりうる(8つまで)。
    ...
    /// <b>広がりを見て</b>選んでいる。
    /// 今日それを入れなかったのは、箱と平面では頂点が5つ以上沈むほど
    /// めり込むことがまず無く、選び方が問題になる場面が無いため
    /// (箱どうしの面のクリップで点が増える Day 44b で、改めて話題になる)。
    /// </para>
    /// </summary>
