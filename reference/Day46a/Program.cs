...
internal static class Program
{
    ...
    {
        /// <summary>傾きを振った坂。**坂の上限(Alt+X)が効く**(要点2)。</summary>
        Slopes,

        /// <summary>高さを振った階段。**段差の乗り越え(Ctrl+Shift+X)が効く**(要点3)。</summary>
        Steps,
    ...
    /// <summary>地形の上に降らせた体の数。**乱数の種と色の巡回に使う**。</summary>
    ...
    private static void OnLoad()
    {
        ...
        Console.WriteLine("Ctrl+Shift+Alt+7:接触点の解き方(同時 / **順番**。切ると積んだ箱が崩れる)");
        ...
        Console.WriteLine("Ctrl+Shift+Alt+G:今日の自己チェック");
    ...
    private static void ShowPhysicsDemo(PhysicsScene scene)
    {
        ...
        _orbit.Reset();
        _orbit.Target = new Vector3(0.0f, scene == PhysicsScene.BoxStack ? 2.4f : 1.0f, 0.0f);
    ...
    private static void BuildPhysicsScene(PhysicsScene scene)
    {
        ...
        // 囲っておかないと、一度滑り始めた物は画面の外まで行ったきり戻ってこない。
    ...
    ///
    /// <para>
    /// 摩擦が無いので、乗ったものは必ず滑り落ちる。
    /// **止まってほしければ Day 47 まで待つ**。
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// 地形の上に大量に落とす筋書き(Day 46)。**60 個が谷へ集まる**。
    ///
    /// 60 個も置くと、総当たりでは 2,000 組近くを毎ステップ試すことになる。
    /// 今日はまだ総当たりのまま——**この筋書きが Day 46b のブロードフェーズの客**になる。
    /// </summary>
    ...
    ///
    /// <para>
    /// <b>転がる物も少しだけ置く</b>。動く体と地形が同じ世界にいることを見せるため。
    /// </para>
    /// </summary>
    ...
    private static void OnKeyDown(IKeyboard keyboard, Key key, int scancode)
    {
        ...
        switch (key)
        {
            ...
            case Key.Number7 when ctrl && shift && alt:
                ...
                {
                    Console.WriteLine(
                        "  **箱を積む筋書き(Ctrl+Shift+Alt+2)で見ると一目で分かる**"
                        + "——柱がじりじり滑って崩れる");
                }
    ...
    private static void RunRigidBodyCheck()
    {
        ...
        //
        // **倍率のしきい値は Day 44 で 3.0 から 2.0 へ緩めた**。
        // 位置の補正が「印で測り直す」形になって、1周のときのめり込みが
        // 64.4mm から 37.4mm に改善したため(8周のほうは 14.7 → 12.8mm)。
        // **数字が良くなったせいで通らなくなったチェック**で、
        // 中身の主張(反復が足りないと沈む)は変わっていない。
        checks.Check(
            "反復1周だと柱が沈む(**素朴版の限界**)",
            deep1 > deep8 * 2.0f,
    ...
    private static void RunBoxCollisionCheck()
    {
        ...
        checks.Check(
            "1点だと傾き続ける(4点なら傾かない)",
            tilt1 > tilt4 + 0.005f,
            $"1点 {tilt1 * 180.0f / MathF.PI:F2} 度 / 4点 {tilt4 * 180.0f / MathF.PI:F3} 度");

        (_, _, float spinLoose) = DropBox(4, 300, together: false);

        checks.Check(
            "**4点を順番に解くと、床の箱1つでも揺れが止まらない**(要点5)",
            spinLoose > 20.0f * MathF.Max(spin4, 1e-4f),
            $"順番 {spinLoose:F4} rad/s / 同時 {spin4:F5} rad/s");
        ...
        checks.Check(
            ...
            drift < 0.01f && top > 4.70f,
            $"横ずれ {drift * 1000.0f:F2}mm、最上段 {top:F3}m(理想 4.950m)");
        checks.Check(
            "**順番に解くと同じ柱が崩れる**(Day 44a の要点5)",
            looseDrift > 1.0f && looseTop < 2.0f,
            $"横ずれ {looseDrift:F2}m、最上段 {looseTop:F3}m");
        checks.Check(
            "ただし段数ぶん沈む(**蓄積インパルスが無いから**。Day 47 の宿題)",
            deep > 0.02f,
            $"めり込み {deep * 1000.0f:F1}mm(許容 5.0mm)");
    ...
    ///
    /// <para>
    /// <b>今日いちばん大事なのは「三角形の最近接点」と「坂の上限」</b>。
    /// 前者は7つの領域の場合分けを総当たりと突き合わせ、
    /// 後者は地形の上を実際に歩かせて「登れた高さ」を測る。
    /// 絵では「登れていない」のか「引っかかっている」のかが区別できないので、数字で確かめる。
    /// </para>
    /// </summary>
    private static void RunTerrainCheck()
    {
        Console.WriteLine();
        Console.WriteLine("--- Day 46: 地形の自己チェック ---");
        var checks = new CheckList();
        ...
        var settled = BuildTerrainDropWorld();
    ...
    /// <summary>
    /// 自己チェック用の世界を組む。**地形の上へ 200 体を落とす**。同じ種で必ず同じ配置になる。
    ///
    /// 乱数の種を固定し、体を足す順番も固定してある。
    /// 今日はまだ総当たりなので、200 体で 2 万組近くを毎ステップ試している。
    /// </summary>
    private static PhysicsWorld BuildTerrainDropWorld()
    {
        var world = new PhysicsWorld();
