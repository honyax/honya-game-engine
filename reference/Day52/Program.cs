...
internal static class Program
{
    ...
    /// <summary>
    /// ディファードで描くか。**今日の主スイッチ**(「ディファード」の F2)。
    ///
    /// <para>
    /// <b>効くのはデモ v1(とプレイアブルデモ)を出しているときだけ</b>。
    ...
    private static void Render3D()
    {
        ...
        // **点光源はフォワードのぶんだけ**(Day 52)。上限を超えたぶんは切り捨てる。
        // G-Buffer パスで呼ばれたときは光を当てないので、送らない(空を渡す)。
        ApplyForwardLights(shader, UseDeferred() ? ReadOnlySpan<PointLight>.Empty : CurrentLights());
    ...
    private static bool UseDeferred() =>
        _deferred && _demo is not null && !_playing && _debugChannel == 0;
    ...
    private static string DeferredLabel()
    {
        ...
        if (!UseDeferred())
        {
            string reason = _deferred && _debugChannel != 0 ? "(成分表示中はフォワード)" : string.Empty;
    ...
    private static void DescribeDeferred()
    {
        ...
        Console.WriteLine(
            ...
            + (_swarm.Count > PointLight.MaxForward
                ...
                : string.Empty));
        Console.WriteLine(
            $"  いまの描き方: {(UseDeferred() ? "ディファード" : "フォワード")}  "
    ...
    private static void BenchmarkDeferred()
    {
        ...
        foreach (int count in LightSwarm.CountSteps)
        {
            ...
            double forward = MeasureLitScene(deferred: false, Frames);
            double deferred = MeasureLitScene(deferred: true, Frames);
    ...
    private static double MeasureLitScene(bool deferred, int frames)
    {
        _deferred = deferred;
    ...
    /// <summary>シーンのバッファへ、今の描き方で1枚描く(空も影も SSAO も入れない)。</summary>
    ...
    private static void RunDeferredCheck()
    {
        ...
        RenderProbe(deferred: true, [], probeFloor, probeCube, cube);
        ...
        float[] deferredBase = RenderProbe(deferred: true, [], probeFloor, probeCube, cube);
        ...
        float[] belowImage = RenderProbe(deferred: true, below, probeFloor, probeCube, cube);
        ...
        float[] aroundImage = RenderProbe(deferred: true, around, probeFloor, probeCube, cube);
        ...
        float[] forwardImage = RenderProbe(deferred: false, probeLights, probeFloor, probeCube, cube);
        float[] deferredImage = RenderProbe(deferred: true, probeLights, probeFloor, probeCube, cube);
        ...
        float[] forwardBase = RenderProbe(deferred: false, [], probeFloor, probeCube, cube);
        float[] forwardCrowd = RenderProbe(deferred: false, crowd, probeFloor, probeCube, cube);
        float[] deferredCrowd = RenderProbe(deferred: true, crowd, probeFloor, probeCube, cube);
    ...
    private static float[] RenderProbe(
        bool deferred, ReadOnlySpan<PointLight> lights, Material floor, Material cubeMaterial, Matrix4x4 cube)
    {
        int width = _window.FramebufferSize.X;
        int height = _window.FramebufferSize.Y;
        ...
        else
        {
            ...
            shader.Use();
            PrepareProbeShader(shader);
            ApplyForwardLights(shader, lights);
