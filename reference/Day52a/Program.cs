...
internal static class Program
{
    ...
    /// <summary>ライティングパス(太陽・環境光・発光を全画面1枚で)。</summary>
    ...
    private static void OnRender(double deltaSeconds)
    {
        ...
        // **G-Buffer を1枚ずつ全画面に出す**(「ディファード」の F3。Day 52)。
    ...
    private static DebugMenu BuildDebugMenu()
    {
        ...
        DebugMenu.Page deferred = menu.Add(
            "ディファード", "Day 52。G-Buffer に表面を書いて、あとから光を当てる");
        ...
        deferred.Add("内訳", "G-Buffer の4枚と、いまの描き方", DescribeDeferred);

        deferred.Add("自己チェック", "G-Buffer・位置の復元・フォワードとの一致", RunDeferredCheck);
    ...
    private static void OnClosing()
    {
        ...
        // G-Buffer とライティングパスも同じ(Day 52)。4枚 + 深度と、空 VAO を畳む。
        _gbuffer.Dispose();
    ...
    /// <b>あとから描く粒(Day 49)が壁の向こうに透けて見え、ソフトパーティクル(Day 50)も効かない</b>。
    /// Day 52b で足す点光源の球も、塗る画素をこの深度と比べて選ぶので、同じ理由で要る。
    /// LearnOpenGL の Deferred Shading の章が最後に
    ...
    private static void RenderDeferredLighting()
    {
        ...
        _deferredLighting.Render(_camera, _gbuffer, ApplyLighting);

        // 本描画のドローコールは G-Buffer パスで数えてある。足すのは全画面の1回だけ。
        _drawCalls += 1;
    ...
    private static string DeferredLabel()
    {
        ...
        if (!UseDeferred())
        {
            ...
            return $"描画:フォワード{reason}  "
                + $"幾何パス:{shadowPass + (_ssao.Enabled ? 1 : 0) + 1}回";
        }
    ...
    private static void DescribeDeferred()
    {
        ...
        Console.WriteLine(
            ...
            + "(CPU が命令を積んだ時間。GPU の時間は測っていない)");
        Console.WriteLine();
    }

    /// <summary>
    /// **今日の自己チェック**(メニューの F5。Day 52)。
    ///
    /// <para>
    ...
    ///
    /// <list type="number">
    /// <item><b>窓なし</b>: ベースカラーを平方根で詰める効き目</item>
    /// <item><b>G-Buffer の中身</b>: 距離・法線・材質が CPU の答えと一致するか。位置が戻せるか。SSAO が借りられるか</item>
    /// <item><b>フォワードとの一致</b>: 描き方を変えても同じ絵になるか</item>
    /// <item><b>2か所の式</b>: textured.frag と deferred.frag が同じ約束でいるか</item>
    /// </list>
    /// </summary>
    private static void RunDeferredCheck()
    {
        Console.WriteLine();
        Console.WriteLine("=== Day 52 自己チェック(G-Buffer・位置の復元・フォワードとの一致)===");
        var checks = new CheckList();
        ...
        // ============================================================
        //
        // **今日のいちばん大事な1項目**。ディファードは「絵が変わらない」のが正解なので、
        // 同じ太陽・同じ影・同じ IBL・同じ SSAO で描いて、画素まで揃うことを見る。

        float[] forwardImage = RenderProbe(deferred: false, probeFloor, probeCube, cube);
        float[] deferredImage = RenderProbe(deferred: true, probeFloor, probeCube, cube);
        (float meanError, float outliers) = CompareImages(forwardImage, deferredImage, 0.05f);

        checks.Check(
            "**フォワードとディファードで同じ絵**(太陽・環境光・発光・影・IBL・SSAO 込み)",
        ...
        checks.Report("すべて合格(G-Buffer・位置の復元・フォワードとの一致が仕様どおり)");
        Console.WriteLine();
    }
    ...
    private static float[] RenderProbe(
        bool deferred, Material floor, Material cubeMaterial, Matrix4x4 cube)
    {
        int width = _window.FramebufferSize.X;
        int height = _window.FramebufferSize.Y;
