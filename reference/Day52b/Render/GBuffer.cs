...
/// <summary>G-Buffer のどれを直接見るか(「ディファード」の F5)。</summary>
...
internal sealed class GBuffer : IDisposable
{
    ...
    /// <summary>中間結果を直接見る(「ディファード」の F5)。</summary>
    ...
    /// 切れていると最後に描いたものが残り、深度も書かれないので点光源の球が1つも通らない。
    /// </para>
    /// </summary>
    public void Begin()
    {
    ...
    /// <summary>
    /// 1枚を画面いっぱいに出す(「ディファード」の F5)。**後処理の外**で呼ぶこと。
    ///
    /// <para>
