...
internal sealed class Ssao : IDisposable
{
    ...
    /// <summary>
    /// **借りてきた幾何バッファ**(Day 52)。null なら自前の <see cref="_geometry"/> を読む。
    ///
    /// <para>
    /// ディファードでは G-Buffer の2枚目がまったく同じ形(ビュー法線 + 距離)で出来ているので、
    /// 自分で幾何パスを描かずにそれを読む。<b>ジオメトリを描く回数が1回減る</b>のが
    /// Day 37 の設計書に書いた歪みの片付けになる。
    /// </para>
    /// </summary>
    private Texture? _borrowedGeometry;
    ...
    /// <summary>直近の <see cref="Compute"/> が借りた幾何バッファを読んだか(Day 52)。HUD 用。</summary>
    public bool BorrowsGeometry => _borrowedGeometry is not null;

    /// <summary>遮蔽の計算が読む幾何バッファ。借りていればそちら(Day 52)。</summary>
    private Texture GeometrySource => _borrowedGeometry ?? _geometry.Color;
    ...
    /// Day 52 でこの重複を1回にまとめる。
    /// </para>
    ///
    /// <para>
    /// <b>Day 52 でそのとおりになった</b>。ディファードのときはこのメソッドを呼ばず、
    /// <see cref="Compute"/> に G-Buffer の2枚目を渡す。自前の幾何パスが残っているのは、
    /// フォワードの絵(材質グリッドやモデル単体)がまだこれを使うため。
    /// </para>
    ///
    /// <para>
    ...
    /// </summary>
    /// <param name="geometry">
    /// **借りる幾何バッファ**(Day 52)。G-Buffer の2枚目を渡すと、
    /// <see cref="BeginGeometry"/> 〜 <see cref="EndGeometry"/> を飛ばしてよい。
    /// 省略すれば Day 51 までと同じく自前の幾何パスの結果を読む。
    /// </param>
    public void Compute(Camera camera, int screenWidth, int screenHeight, Texture? geometry = null)
    {
        _borrowedGeometry = geometry;

        // **借りるときは幾何パスが走っていない**ので、数え直しもここでやる
        // (普段は BeginGeometry が 0 に戻している)。
    ...
    public void DrawDebug(int screenWidth, int screenHeight, float depthRange)
    {
        ...
        // 借りているときは G-Buffer の2枚目を見せる(Day 52)。
        // 自前のほうはもう描かれていないので、見せると1フレーム前の残骸が出る。
        GeometrySource.Bind(TextureUnit.Texture1);
    ...
    private void ComputeOcclusion(Camera camera)
    {
        ...
        // **自前か借り物か**(Day 52)。どちらも同じ形なので、シェーダは違いを知らない。
        GeometrySource.Bind(TextureUnit.Texture0);
