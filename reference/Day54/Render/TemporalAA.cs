...
/// <summary>履歴の読み方(「TAA」の F6)。</summary>
internal enum TaaHistoryFilter
{
    /// <summary>双線形。**動かすと絵がじわじわ眠くなる**。</summary>
    Bilinear,

    /// <summary>Catmull-Rom(3次)。ぼけずに間を埋める。**既定**。</summary>
    CatmullRom,
}
...
/// <item><b>どう読むか</b> … 双線形で読むと、読み直すたびにぼける(<see cref="HistoryFilter"/>)</item>
/// </list>
///
/// <para>
...
internal sealed class TemporalAA : IDisposable
{
    ...
    /// <summary>履歴の読み方(「TAA」の F6)。</summary>
    public TaaHistoryFilter HistoryFilter { get; set; } = TaaHistoryFilter.CatmullRom;

    /// <summary>混ぜる前に明るさで畳むか(「TAA」の F8)。</summary>
    public bool ToneWeighted { get; set; } = true;

    /// <summary>
    /// 毎フレームずらすか(「TAA」の F9)。**OFF にすると AA は効かなくなる**。
    ///
    /// <para>
    ...
    public Texture Resolve(Texture current, Texture depth, Texture velocity)
    {
        ...
        shader.SetInt("uHistoryFilter", (int)HistoryFilter);
        shader.SetInt("uToneWeighted", ToneWeighted ? 1 : 0);
