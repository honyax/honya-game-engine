...
internal static class Program
{
    ...
    private static DebugMenu BuildDebugMenu()
    {
        ...
        temporal.Add("自己チェック", "ずらし・解決の式・カメラの速度・スーパーサンプリングへの収束", RunTaaCheck);
    ...
    ///
    /// <para>
    /// このフレームのずらしを決めて、カメラに入れる(<see cref="Camera.Jitter"/>)。
    /// ここから <see cref="OnRender"/> の最後までに描くものは、全部同じだけずれる。
    /// 速度バッファにも、前のフレームと繋がっているかを確かめさせる。
    /// </para>
    /// </summary>
    private static void BeginTemporalFrame()
    {
        _camera.Jitter = UseTaa()
    ...
    private static string TaaLabel()
    {
        ...
        return $"TAA:{(_post.TaaApplied ? "ON" : "待機")}  {jitter}  今の絵:{taa.CurrentWeight:P0}  "
            + $"疑い方:{RectifyLabel(taa.Rectify)}  速度:カメラだけ  "
    ...
    private static void DescribeTaa()
    {
        ...
        Console.WriteLine(
            _motion.Target is null
                ? "  速度バッファ: まだ作っていない"
                : $"  速度バッファ: {_motion.Target.Width} x {_motion.Target.Height} RG16F = "
                + $"{_motion.ByteSize / (1024.0 * 1024.0):F1}MB(**カメラの動きだけ**。物の速度は Day 54b)"
    ...
    /// <item><b>解決の式</b>(小さな作り物の絵): 指数移動平均・疑い方3つ・画面の外</item>
    /// <item><b>速度</b>(確認用のシーン): 止まった絵は 0・カメラの動き・カット</item>
    /// <item><b>収束</b>: 止まった絵に TAA を掛け続けると、64 点のスーパーサンプリングに、ずらしの 8 点の平均まで近づく(FXAA と比べて)</item>
    /// <item><b>文字列の約束</b>: 速度の向きと単位、履歴を引く式</item>
    /// </list>
    /// </summary>
    private static void RunTaaCheck()
    {
        Console.WriteLine();
        Console.WriteLine("=== Day 54 自己チェック(ずらし・解決の式・カメラの速度・スーパーサンプリングへの収束)===");
        var checks = new CheckList();
        TemporalAA taa = _post.Taa;
        ...
        TaaSettings saved = TaaSettings.Capture(taa);
        ...
        checks.Check(
            ...
            !_motion.Cut && MaxSpeedPixels(still) < 0.01f,
            $"全画素の最大 {MaxSpeedPixels(still):F4} 画素");
        ...
        checks.Check(
            ...
            Vector2.Distance(floorMeasured, floorExpected) < 0.05f,
            $"({floorMeasured.X:+0.000;-0.000}, {floorMeasured.Y:+0.000;-0.000}) / 期待 "
                + $"({floorExpected.X:+0.000;-0.000}, {floorExpected.Y:+0.000;-0.000}) 画素");

        // --- 動いた箱は、今日の1段では拾えない ---
        //
        // 箱を動かしてもカメラが止まっていれば、速度は 0 のまま。深度に入っているのは「今どこにあるか」だけで、
        // 箱が前のフレームにどこに居たかは書かれていない。**これが Day 54b の出発点**。
        ...
        Vector2 topMeasured = VelocityPixels(boxMoved, WorldToScreenPixel(new Vector3(0.1f, 0.5f, 0.0f), viewProjection));

        checks.Check(
            "**カメラが止まっていれば、動いた箱の速度も 0**(自分で動いた物は今日は拾えない。Day 54b)",
            topMeasured.Length() < 0.01f && MaxSpeedPixels(boxMoved) < 0.01f,
            $"箱の上面 {topMeasured.Length():F4} 画素  全画素の最大 {MaxSpeedPixels(boxMoved):F4} 画素");
        ...
        long velocityBytes = (long)width * height * 4;
        checks.Check(
            "代償: 履歴は画面と同じ大きさの RGBA16F が2枚、速度は RG16F が1枚",
        ...
        checks.Check(
            "速度の向きは「前 → 今」、単位は UV(camera-velocity.frag)",
        ...
        checks.Report("すべて合格(ずらし・解決の式・カメラの速度・スーパーサンプリングへの収束が仕様どおり)");
        Console.WriteLine();
    }
    ...
    {
        public static TaaSettings Capture(TemporalAA taa) => new(
            taa.Enabled, taa.CurrentWeight, taa.Rectify, taa.JitterEnabled, taa.DebugView);

        public void Restore(TemporalAA taa)
        {
            taa.Enabled = Enabled;
