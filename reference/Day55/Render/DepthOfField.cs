...
/// <summary>ボケの集め方(「被写界深度とモーションブラー」の F5)。<c>dof-gather.frag</c> の <c>uScatterAsGather</c> と対応する。</summary>
...
internal sealed class DepthOfField : IDisposable
{
    ...
    /// <summary>重ねた結果(原寸 RGBA16F)。後ろの段(モーションブラー・ブルーム・合成)はこれを読む。</summary>
    ...
    /// <summary>
    /// 被写界深度を掛けるか(「被写界深度とモーションブラー」の F2)。**既定は OFF**——起動した直後の絵は Day 54 と同じ。
    ///
    /// <para>
    ...
    /// <summary>
    /// **F 値**(「被写界深度とモーションブラー」の F3)。焦点距離 ÷ 口径。**小さいほど口径が大きく、よくボケる**。
    /// 1.4 は明るい単焦点レンズの開放。
    /// </summary>
    public float FNumber { get; set; } = 1.4f;

    /// <summary>ボケの集め方(「被写界深度とモーションブラー」の F5)。</summary>
    public DofGather Gather { get; set; } = DofGather.ScatterAsGather;

    /// <summary>錯乱円を色で見る(「被写界深度とモーションブラー」の F6)。手前は青、奥は橙。</summary>
