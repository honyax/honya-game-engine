...
///
/// <para>
/// <b>Day 55 で TAA のすぐ後ろに1段増えた</b>。被写界深度(<see cref="HonyaEngine.DepthOfField"/>)。
/// 「カメラの穴が理想の点ではない」ことを足す段で、ブルームより前・HDR のままで走る。
/// <code>
///   [シーンバッファ] ──▶ TAA ──▶ 被写界深度 ──▶ 明部 → ぼかし / 合成 → … → 画面
///                                  ▲
///                                  └── ぼけの地図   (奥行き・錯乱円・近所の升目)
/// </code>
/// </para>
...
internal sealed class PostProcess : IDisposable
{
    ...
    /// <summary>
    /// **ぼけの地図**(Day 55)。奥行き・錯乱円・近所の升目。被写界深度はこれを読む。
    /// </summary>
    ...
    public PostProcess(GL gl, RenderResources resources, string shaderDirectory, int width, int height)
    {
        ...
        // 被写界深度も、バッファは入れたときに初めて作る(Day 55)。ここで読むのはシェーダだけ。
    ...
    ///
    /// <para>
    /// <b>被写界深度は、これが挿さっているときだけ掛かる</b>(<see cref="Velocity"/> と TAA の関係と同じ)。
    /// 自己チェックが後処理を単独で走らせても(<see cref="EndToTarget"/>)割り込まないので、
    /// Day 38〜54 の自己チェックは、被写界深度を入れたままでもそのまま通る。
    /// </para>
    /// </summary>
    ...
    private void EndCore(Framebuffer? target, int screenWidth, int screenHeight)
    {
        ...
        // **被写界深度は TAA のすぐ後ろ、ブルームの前、HDR のまま**(Day 55)。
        //   - TAA の後ろ … 近所の色を混ぜる処理。ずらして描いた揺れる絵ではなく、TAA で落ち着いた絵を混ぜる。
        //     前に置くと、TAA が「近所の色の範囲」で履歴を疑うときに、ぼかした色の範囲で疑うことになる
        //   - ブルームの前 … 明るい点をまず丸いボケに広げ、それからにじませる。後ろに置くと、
        ...
        if (frame is { } camera && _depthCopy?.Depth is { } depthCopy)
        {
            ...
            if (defocus)
            {
                // 地図は速度を読まない(速度の口は空けておく。null なら升目は錯乱円だけを見る)。
                float lensScale = _depthOfField.LensScale(camera, depthCopy.Height);
                _blurField.Build(depthCopy, null, camera, lensScale);
    ...
    public void Resize(int width, int height)
    {
        ...
        // 被写界深度は、次に描くときに自分で大きさを合わせる(Day 55)。
        // 前のフレームを覚えていないので、捨てるものが無い。
    }
    ...
    public void ReloadShaders()
    {
        ...
        // 被写界深度も(Day 55)。ボケの縁の幅は、絵を見ながら決めるもの。
