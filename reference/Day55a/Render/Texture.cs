...
internal enum RenderTargetFormat
{
    ...
    /// <summary>
    /// **2成分の半精度浮動小数点**(Day 54)。速度バッファの器。
    ///
    /// <para>
    /// 入るのは「この画素は、前のフレームから画面の上でどれだけ動いたか」の (横, 縦) の2つで、
    /// <b>負にもなる</b>(左へ動けば負)。0〜1 しか入らない 8bit には入らないし、
    /// RGBA16F にすると使わない2成分のために帯域が倍になる。
    /// R8(Day 37)と同じ「中身で選ぶ」判断で、今度の中身は「符号付きの小数が2つ」。
    /// </para>
    ///
    /// <para>
    /// 精度は仮数部 10bit。画面の幅を 1 とした単位で持つので、1画素(960 画素なら 0.001)の動きが
    /// 1/1000 画素の細かさで入る。100 画素の動きでも 0.1 画素まで。TAA が履歴を引くには十分。
    /// </para>
    /// </summary>
    Rg16F,
}
...
internal sealed class Texture : IDisposable
{
    ...
    public static unsafe Texture CreateTarget(GL gl, int width, int height, RenderTargetFormat format)
    {
        ...
        (InternalFormat internalFormat, PixelFormat pixelFormat, PixelType pixelType) = format switch
        {
            ...
            RenderTargetFormat.Rg16F => (InternalFormat.RG16f, PixelFormat.RG, PixelType.HalfFloat),
