...
///
/// <para>
/// <b>Day 55 で TAA のすぐ後ろに2段増えた</b>。被写界深度(<see cref="HonyaEngine.DepthOfField"/>)と
/// モーションブラー(<see cref="HonyaEngine.MotionBlur"/>)。どちらも「カメラが理想の点と一瞬ではない」ことを足す段で、
/// ブルームより前・HDR のままで走る。
/// <code>
///   [シーンバッファ] ──▶ TAA ──▶ 被写界深度 ──▶ モーションブラー ──▶ 明部 → ぼかし / 合成 → … → 画面
///                                  ▲                ▲
///                                  └── ぼけの地図 ──┘   (奥行き・錯乱円・近所の升目。2つで1回作る)
/// </code>
/// </para>
/// </summary>
internal sealed class PostProcess : IDisposable
{
    ...
    /// <summary>
    /// **ぼけの地図**(Day 55)。奥行き・錯乱円・近所の升目。被写界深度とモーションブラーが<b>1回作ったものを2人で読む</b>。
    /// </summary>
    private readonly BlurField _blurField;

    /// <summary>被写界深度(Day 55)。レンズの口径が有限であること。</summary>
    private readonly DepthOfField _depthOfField;

    /// <summary>モーションブラー(Day 55)。シャッターが一瞬ではないこと。</summary>
    ...
    public PostProcess(GL gl, RenderResources resources, string shaderDirectory, int width, int height)
    {
        ...
        // 被写界深度とモーションブラーも、バッファは入れたときに初めて作る(Day 55)。ここで読むのはシェーダだけ。
        _blurField = new BlurField(gl, resources, shaderDirectory);
        _depthOfField = new DepthOfField(gl, resources, shaderDirectory);
        _motionBlur = new MotionBlur(gl, resources, shaderDirectory);
    }
    ...
    /// <summary>被写界深度(Day 55)。つまみ(F 値・集め方・表示)はこちらが持つ。</summary>
    public DepthOfField DepthOfField => _depthOfField;

    /// <summary>モーションブラー(Day 55)。つまみ(シャッター・升目・表示)はこちらが持つ。</summary>
    public MotionBlur MotionBlur => _motionBlur;

    /// <summary>ぼけの地図(Day 55)。自己チェックが奥行きと錯乱円を読み返すために公開する。</summary>
    public BlurField BlurField => _blurField;

    /// <summary>
    /// **このフレームのカメラの数字**(Day 55)。Program が <see cref="End"/> の直前に挿し、End が使い切って外す。
    ///
    /// <para>
    /// <b>被写界深度とモーションブラーは、これが挿さっているときだけ掛かる</b>(<see cref="Velocity"/> と TAA の関係と同じ)。
    /// 自己チェックが後処理を単独で走らせても(<see cref="EndToTarget"/>)割り込まないので、
    /// Day 38〜54 の自己チェックは、被写界深度とモーションブラーを入れたままでもそのまま通る。
    /// </para>
    /// </summary>
    public CameraFrame? Frame { get; set; }

    /// <summary>直前のフレームで被写界深度が掛かったか(HUD 用)。</summary>
    public bool DepthOfFieldApplied { get; private set; }

    /// <summary>直前のフレームでモーションブラーが掛かったか(HUD 用)。</summary>
    public bool MotionBlurApplied { get; private set; }

    /// <summary>
    /// パイプラインが抱えている VRAM の推定バイト数。**Day 54 から TAA の履歴も入る**。
    /// **Day 55 から被写界深度とモーションブラーのバッファも入る**(どちらも入れたときに初めて作る)。
    /// </summary>
    public long ByteSize =>
        _scene.ByteSize + _bright.ByteSize + _blurA.ByteSize + _blurB.ByteSize + _ldr.ByteSize
        + (_depthCopy?.ByteSize ?? 0) + _taa.ByteSize
        + _blurField.ByteSize + _depthOfField.ByteSize + _motionBlur.ByteSize;
    ...
    private void EndCore(Framebuffer? target, int screenWidth, int screenHeight)
    {
        ...
        // **被写界深度とモーションブラーは TAA のすぐ後ろ、ブルームの前、HDR のまま**(Day 55)。
        //   - TAA の後ろ … どちらも近所の色を混ぜる処理。ずらして描いた揺れる絵ではなく、TAA で落ち着いた絵を混ぜる。
        //     前に置くと、TAA が「近所の色の範囲」で履歴を疑うときに、ぼかした色の範囲で疑うことになる
        //   - ブルームの前 … 明るい点をまず丸いボケや尾に広げ、それからにじませる。後ろに置くと、
        //     ぼける前の鋭い点から出たにじみが、ぼけた絵の上に浮く
        //   - HDR のまま … 太陽の映り込みのような 50 の点は、半径 8 画素の円に広げても 1 画素あたり 0.25 残る。
        //     1.0 で切ったあとでぼかすと 0.005 になって消える——**明るい点が丸いボケとして見えるのは HDR のおかげ**
        //
        // 2つの並びは 被写界深度 → モーションブラー。本物のカメラでは同じ露光の中で同時に起きることなので、
        // どちらを先にしても近似になる(どちらの段も、ぼける前の輪郭について測った奥行きと速度を読んでいる)。
        CameraFrame? frame = Frame;
        Frame = null;
        DepthOfFieldApplied = false;
        MotionBlurApplied = false;

        if (frame is { } camera && _depthCopy?.Depth is { } depthCopy)
        {
            bool defocus = _depthOfField.Enabled && camera.Perspective;
            Texture? motion = _motionBlur.Enabled ? Velocity : null;

            if (defocus || motion is not null)
            {
                // 地図は1回だけ作る。被写界深度を掛けないフレームはレンズの係数を 0 にする(錯乱円が全部 0)。
                float lensScale = defocus ? _depthOfField.LensScale(camera, depthCopy.Height) : 0.0f;
                _blurField.Build(depthCopy, motion, camera, lensScale);
                _passes += BlurField.PassCount;

                if (defocus)
                {
                    scene = _depthOfField.Render(scene, _blurField);
                    _passes += DepthOfField.PassCount;
                    DepthOfFieldApplied = true;
                }

                if (motion is not null)
                {
                    scene = _motionBlur.Render(scene, motion, _blurField, camera);
                    _passes += MotionBlur.PassCount;
                    MotionBlurApplied = true;
                }
            }
        }

        // 使い切り。次のフレームで Program が挿し直さなければ、TAA もモーションブラーも掛からない。
        Velocity = null;
    ...
    public void Resize(int width, int height)
    {
        ...
        // 被写界深度とモーションブラーは、次に描くときに自分で大きさを合わせる(Day 55)。
        // どちらも前のフレームを覚えていないので、捨てるものが無い。
    }
    ...
    public void ReloadShaders()
    {
        ...
        // 被写界深度とモーションブラーも(Day 55)。ボケの縁の幅や尾の重みは、絵を見ながら決めるもの。
        _blurField.ReloadShaders();
        _depthOfField.ReloadShaders();
        _motionBlur.ReloadShader();
    }
    ...
    public void Dispose()
    {
        ...
        // 被写界深度とモーションブラーも遅延確保(Day 55)。
        _blurField.Dispose();
        _depthOfField.Dispose();
        _motionBlur.Dispose();
