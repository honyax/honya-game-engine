...
internal sealed class ViewerWindow : Form
{
    ...
    public ViewerWindow(int width, int height)
    {
        ...
        Text = "Day59 - CPU レイトレーサ(1): 球と平面、反射・屈折";
    ...
    private void Restart() => _renderer.Start(_scene, CreateCamera(), _settings);
    ...
    private void DrawHud(Graphics g)
    {
        ...
        var lines = new List<string>
        {
            ...
            $"1パスの光線: カメラ {Man(rays.Camera)} / 影 {Man(rays.Shadow)} / 反射 {Man(rays.Reflection)} / 屈折 {Man(rays.Refraction)}"
                + $"   1画素あたり {(double)rays.Total / pixelCount:F2} 本",
            $"深さの上限 {_settings.MaxDepth} ([ ])   影 {OnOff(_settings.Shadows)} (S)   フレネル {FresnelName(_settings.Fresnel)} (F)"
                + $"   始点を浮かせる {OnOff(_settings.OffsetOrigin)} (E)   画素内のずらし {OnOff(_settings.Jitter)} (J)   表示 {ViewName(_settings.View)} (V)",
        };

        // 操作の一覧は、下の欄を出していないときだけ。欄を出すと縦が足りなくなる(自己チェックは 21 行ある)。
        if (_panelTitle is null)
        {
            lines.Add("1 2 3: 場面   ドラッグ: 回す   ホイール: 寄る   R: カメラを戻す   右クリック: 1画素を追う");
        ...
        {
            // 入りきらない行は切り捨てて、何行あったかだけ残す。
            int capacity = (int)((_renderer.Height - y - 10.0f) / lineHeight);
            var panel = new List<string> { _panelTitle + "   (X で消す)" };
            if (_panelLines.Count + 1 <= capacity)
            {
                panel.AddRange(_panelLines);
            }
            else
            {
                int room = Math.Max(1, capacity - 2);
                panel.AddRange(_panelLines.Take(room));
                panel.Add($"…(あと {_panelLines.Count - room} 行)");
            }
    ...
    /// <summary>
    /// 1画素を追う(右クリック)。描画スレッドとは別に、UI スレッドでその画素の光線を1本だけ追って木を記録する。
    ...
    private void TracePixel(int x, int y)
    {
        ...
        var tracer = new WhittedTracer(_scene, _settings);
        TraceLog log = tracer.TracePixel(CreateCamera(), x, y, out Vector3 color, out RayCounters rays);

        _panelTitle = $"1画素を追う ({x}, {y})   光線 {rays.Total} 本(カメラ {rays.Camera} / 影 {rays.Shadow} / 反射 {rays.Reflection} / 屈折 {rays.Refraction})"
    ...
    private void SaveImage()
    {
        ...
        string path = Path.Combine(
            directory,
            $"day59-scene{_sceneIndex + 1}-{_renderer.Status.Samples}spp-{DateTime.Now:yyyyMMdd-HHmmss}.png");
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        switch (e.KeyCode)
        {
            ...
            case Keys.V:
                ChangeSettings(_settings with { View = (ViewMode)(((int)_settings.View + 1) % 3) });
                break;
