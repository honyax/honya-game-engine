...
internal sealed class ProgressiveRenderer : IDisposable
{
    ...
    public void Start(Scene scene, Camera camera, RenderSettings settings)
    {
        ...
        var tracer = new WhittedTracer(scene, settings);
    ...
    private void RenderLoop(WhittedTracer tracer, Camera camera, RenderSettings settings, CancellationToken token)
    {
    ...
    private void RenderPreview(WhittedTracer tracer, Camera camera, bool encodeSrgb, ParallelOptions options)
    {
        ...
        Parallel.For(0, blockRows, options, blockY =>
        {
            ...
            for (int left = 0; left < Width; left += PreviewBlock)
            {
                ...
                Vector3 color = tracer.Sample(camera, (left + right) * 0.5f, (top + bottom) * 0.5f, ref rays);
