...
/// <summary>
/// 自己チェック(C キー)。<b>16 項目</b>(1〜10 は Day 59、11〜16 が Day 60 で追加)。
///
/// <para>
...
/// 11〜16 は、答えが解析的に分かっている入力(既知の積分、白い炉、Whitted との一致)だけを選んである(要点7)。
/// </para>
/// <para>
/// 乱数は種を固定している(<c>new Random(59)</c> / <see cref="Rng.Create"/> の番号)。何度押しても同じ数字が出る。
/// 全部で 2〜3 秒かかる(11〜16 が数百万本の光線を撃つため)。
/// </para>
/// </summary>
internal static class SelfCheck
{
    ...
    private static void CheckInvisibleGlass(Action<bool, string, string> report)
    {
        ...
        // Day 60 から、場面は使う前に Prepare で光る形を集める必要がある。
        scene.Prepare();
    ...
    /// パストレーシングの絵がなかなか綺麗にならない理由がこれ(要点7)。
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// 14. <b>白い炉のテスト</b>(furnace test)。今日いちばん鋭いチェック(要点7)。
    ///
    /// <para>
    ...
    private static void CheckFurnace(Action<bool, string, string> report)
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            // 代わりに<b>その本数が全体のごく一部であること</b>を条件にする(要点7)。
    ...
    private static void CheckDirectLightMatchesWhitted(Action<bool, string, string> report)
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            if (scene.Intersect(ray, out _, out _))
            {
                hits++;
            }
