...
internal sealed class ViewerWindow : Form
{
    ...
    /// <summary>下の欄の先頭行(PageUp / PageDown で動かす)。自己チェックが 32 行あって入りきらないため。</summary>
    ...
    public ViewerWindow(int width, int height)
    {
        ...
        Text = "Day60 - CPU レイトレーサ(2): パストレーシング、モンテカルロ積分";
    ...
    /// <summary>
    /// 描き直す。<b>場面を作り直す(光る形を集める)前に、必ず描画を止める</b>(Day 60 で追加)。
    ///
    /// <para>
    ...
    private void DrawHud(Graphics g)
    {
        ...
        // 操作の一覧は、下の欄を出していないときだけ。欄を出すと縦が足りなくなる(自己チェックは 32 行ある)。
        if (_panelTitle is null)
        {
            lines.Add("1〜5: 場面   ドラッグ: 回す   ホイール: 寄る   R: カメラを戻す   右クリック: 1画素を追う");
        ...
        {
            // 入りきらない行は PageUp / PageDown でめくる(Day 60 で追加。自己チェックが 16 項目 32 行になったため)。
