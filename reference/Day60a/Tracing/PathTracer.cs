...
/// <item><b>点光源のハイライト</b> → 面光源が本当に鏡に映り、影の縁が本当にぼける(場面4・5)</item>
/// <item><b>ガラスの真っ黒な影</b> → ガラスを通った光が床に集まる(集光。場面5)</item>
/// </list>
///
/// <para>
...
internal sealed class PathTracer : Tracer
{
    ...
    public override Vector3 Trace(in Ray ray, ref Rng rng, ref RayCounters counters, TraceLog? log)
    {
        ...
        {
            if (!TryHit(current, out SurfaceHit hit, out Shape shape))
            {
    ...
    private Vector3 SampleDirectLight(in SurfaceHit hit, Vector3 albedo, int depth, ref Rng rng, ref RayCounters counters, TraceLog? log)
    {
        ...
        if (pick < pointCount)
        {
            ...
            if (Scene.FindOccluder(shadowRay, distance) is Shape occluder)
            {
        ...
        if (Scene.FindOccluder(ray, maxDistance) is Shape blocker)
        {
