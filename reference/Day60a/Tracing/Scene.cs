...
///
/// <para>
/// 問い合わせは Day 59 と同じ<b>全部の形に順番に聞くだけ</b>(総当たり)。形が N 個なら光線1本で N 回の交差判定になる。
/// 今日の場面は多くて 11 個なので気にならないが、形が数百個になると光線1本が数百回の判定になり、
/// そこで BVH(形を箱の木に入れて、光線が通らない箱の中身をまとめて飛ばす)を入れる(Day 60b)。
/// <b>呼ぶ側(<see cref="Tracer"/>)は、中身が総当たりか BVH かを知らない</b>——入れ替えるのはこのクラスだけで済む。
/// </para>
/// <para>
...
internal sealed class Scene
{
    ...
    /// <summary>
    /// 光る形(面光源)を集める。<b>描画を始める前に1回だけ</b>呼ぶ(Day 60 で追加)。
    ///
    /// <para>
    ...
    /// パストレーサでは、その光は<b>NEE ではなく散乱の経路のほうが拾う</b>(場面5)。
    /// </para>
    /// </summary>
