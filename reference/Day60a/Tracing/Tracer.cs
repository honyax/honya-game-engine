...
internal abstract class Tracer
{
    ...
    public Vector3 Sample(Camera camera, float px, float py, ref Rng rng, ref RayCounters counters)
    {
        ...
        switch (Settings.View)
        {
            ...
            case ViewMode.RayCount:
                ...
                return HeatColor(counters.Total - raysBefore);
    ...
    /// </summary>
    protected bool TryHit(in Ray ray, out SurfaceHit hit, out Shape shape)
    {
        if (!Scene.Intersect(ray, out float t, out Shape? candidate))
        {
            hit = default;
            shape = null!;
            return false;
    ...
    {
        if (!TryHit(ray, out SurfaceHit hit, out _))
        {
            return Vector3.Zero;
        }
    ...
    /// <summary>
    /// 光線の数を色にする。<b>2倍ごとに1段</b>(1 紺 → 2 青 → 4 水色 → 8 緑 → 16 黄 → 32 赤 → 64 以上 白)。
    /// </summary>
