...
/// <summary>
/// 6つの場面(1〜6 キー)。どれも「どこを見てほしいか」が1つずつある。
///
/// <para>
...
/// </para>
/// <para>
/// 1〜3 は Day 59 のまま(<b>点光源</b>で照らす)。4〜6 が Day 60 で足したもので、
/// 4 と 6 は<b>面光源</b>、5 は<b>空</b>で照らす。
...
internal static class SceneLibrary
{
    ...
    /// <summary>
    /// 場面5「球がたくさん」。<b>BVH の効き目を見るための場面</b>(要点7・8)。
    ///
    /// <para>
    /// RTIOW の最後の絵を真似た並び。小さな玉を格子状に、位置と材質をばらつかせて置く。
    /// <b>B キーで総当たり / 中央分割 / SAH を切り替えると、絵は同じまま速さだけが変わる</b>。
    /// </para>
    /// <para>
    /// 照らしているのは<b>空</b>と点光源1つ。空はそれ自体が無限に遠い面光源なので、
    /// 玉の影が上から柔らかく落ちる。<b>床の平面だけは BVH に入らない</b>(無限なので箱で囲めない)——
    /// HUD の「BVH 外」がその1枚。
    /// </para>
    /// <para>
    /// 乱数の種を固定しているので、何度起動しても同じ並びになる。
    /// </para>
    /// </summary>
    private static Scene CreateSphereField()
    {
        var scene = new Scene("球がたくさん", new OrbitView(new Vector3(0.0f, 0.55f, 0.0f), 0.25f, 0.17f, 9.0f, 32.0f))
        {
            Ambient = new Vector3(0.10f, 0.11f, 0.13f),
            SkyHorizon = new Vector3(0.85f, 0.90f, 1.00f),
            SkyZenith = new Vector3(0.32f, 0.52f, 0.95f),
        };
        ...
        scene.Shapes.Add(new Plane(
            ...
            Vector3.UnitY,
            0.0f,
            Material.Checker(new Vector3(0.62f), new Vector3(0.28f), 1.0f)));

        // 大きな玉3つ。ここに重なる小さな玉は置かない。
        var anchors = new[]
        {
            (Center: new Vector3(0.0f, 0.62f, 0.0f), Radius: 0.62f),
            (Center: new Vector3(-2.3f, 0.62f, 0.4f), Radius: 0.62f),
            (Center: new Vector3(2.3f, 0.62f, -0.4f), Radius: 0.62f),
        };
        ...
        for (int a = -11; a < 11; a++)
        {
            for (int b = -11; b < 11; b++)
            {
                var center = new Vector3(
                    a * 0.55f + 0.34f * random.NextSingle(),
                    0.11f,
                    b * 0.55f + 0.34f * random.NextSingle());

                bool tooClose = false;
                foreach (var anchor in anchors)
                {
                    if (Vector3.Distance(center, anchor.Center) < anchor.Radius + 0.35f)
                    {
                        tooClose = true;
                    }
                }
                ...
                Material material = roll switch
                {
                    // 拡散の色は、一様な乱数を2つ掛けて作る(RTIOW と同じ)。
                    // 掛けると小さい値に寄るので、彩度の高い色が多めに出る。
                    < 0.70f => Material.Diffuse(new Vector3(
                        random.NextSingle() * random.NextSingle(),
                        random.NextSingle() * random.NextSingle(),
                        random.NextSingle() * random.NextSingle())),
                    < 0.90f => Material.Metal(new Vector3(
                        0.5f + 0.5f * random.NextSingle(),
                        0.5f + 0.5f * random.NextSingle(),
                        0.5f + 0.5f * random.NextSingle())),
                    _ => Material.Glass(1.5f),
                };

                scene.Shapes.Add(new Sphere($"小さな玉({a},{b})", center, 0.11f, material));
            }
        }

        scene.Shapes.Add(new Sphere("ガラスの大玉", anchors[0].Center, anchors[0].Radius, Material.Glass(1.5f)));
        scene.Shapes.Add(new Sphere("赤い大玉", anchors[1].Center, anchors[1].Radius, Material.Diffuse(new Vector3(0.62f, 0.14f, 0.10f))));
        scene.Shapes.Add(new Sphere("金の大玉", anchors[2].Center, anchors[2].Radius, Material.Metal(new Vector3(1.00f, 0.71f, 0.29f))));

        // Whitted 法でも見えるように点光源を1つ。パストレーサではこれが NEE の相手になる。
        scene.Lights.Add(new PointLight("光", new Vector3(-6.0f, 9.0f, -7.0f), new Vector3(1.0f, 0.97f, 0.92f), 420.0f));
        return scene;
    }

    /// <summary>
    /// 場面6「ガラスの集光」。Day 59 の改造課題1で予告した絵(要点4・9)。
    ///
    /// <para>
