...
/// <summary>
/// 自己チェック(C キー)。<b>23 項目</b>(1〜10 は Day 59、11〜18 は Day 60、19〜23 が Day 61 で追加)。
///
/// <para>
...
/// 全部で 3〜4 秒かかる(11〜18 が数百万本の光線を撃ち、19〜23 が GPU と突き合わせるため)。
/// </para>
/// <para>
/// <b>Day 61 の 19〜23 は「移植したものが同じか」を見る</b>。GPU 版は絵を見ても正しさが分からない——
/// ノイズに埋もれるうえ、CPU 版と並べても「どちらもノイズだらけ」にしか見えないので、
/// <b>乱数を揃えて1サンプル目を1画素ずつ突き合わせる</b>(20)のがいちばん効く。
/// </para>
/// </summary>
internal static class SelfCheck
{
    public static IReadOnlyList<string> Run(GpuRenderer? gpu, GpuDevice? device, out string summary)
    {
        ...
        // ---- Day 61 ----
        CheckGpuAvailable(Report, gpu, device);
        CheckGpuMatchesCpuPath(Report, gpu);
        CheckGpuFurnace(Report, gpu);
        CheckGpuConverges(Report, gpu);
        CheckGpuBvhMatchesBruteForce(Report, gpu);
    ...
    /// <summary>
    /// 隣り合う画素の「最初の1個目の乱数」どうしの相関係数。
    ///
    /// <para>
    /// <b>サンプル番号を <c>i</c> にして毎回変える</b>のが Day 61 で直したところ。
    /// Day 60 までは 0 に固定していたが、画素の番号を混ぜないと 10 万組が全部同じ1つの値になり、
    /// ばらつきが 0 の数列どうしの相関(0 ÷ 0)を計算することになる。
    /// 64 ビットの乱数器では丸めの都合でたまたま 1.0000 に出ていたが、
    /// 32 ビットに替えたら 0.0000 になった——<b>測り方が落ちていただけ</b>で、乱数は何も悪くない。
    /// サンプル番号を変えれば、どちらの設定でも数列がばらつくので、相関がまともに定義される。
    /// </para>
    /// </summary>
    private static double NeighborCorrelation(bool decorrelate)
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            var left = Rng.Create(i % 320, i / 320, i, decorrelate);
            var right = Rng.Create((i % 320) + 1, i / 320, i, decorrelate);
    ...
    // =========================================================================
    //  Day 61: GPU への移植を確かめる
    // =========================================================================

    /// <summary>コーネルボックス(場面4)を、探し方を指定して作る。19〜23 で使い回す。</summary>
    private static Scene PreparedCornellBox(AccelerationMode mode = AccelerationMode.Sah)
    {
        Scene scene = SceneLibrary.Create(3);
        scene.Prepare(mode);
        return scene;
    ...
    /// <summary>
    /// 19. <b>GPU が使えて、C# の構造体と GLSL の std430 の並びが合っている</b>(要点4)。
    ///
    /// <para>
    /// バイト数の突き合わせがこの項目の値打ち。<b>ずれても絵は出る</b>——
    /// ずれたぶんだけ「半径を材質の番号として読む」ことになり、真っ黒な玉が並ぶだけで、
    /// GL のエラーは1つも出ない。<c>glGetProgramResource</c> に
    /// <b>GPU 自身が思っている1要素のバイト数</b>を聞いて、<see cref="Unsafe.SizeOf{T}"/> と比べる。
    /// </para>
    /// <para>
    /// uniform の名前も見る。GL は存在しない名前に値を入れても<b>黙って何もしない</b>ので、
    /// <c>uMaxDpth</c> のような打ち間違いは、絵が変にならない限り気づけない。
    /// </para>
    /// </summary>
    private static void CheckGpuAvailable(Action<bool, string, string> report, GpuRenderer? gpu, GpuDevice? device)
    {
        if (gpu is null || device is null)
        {
            report(false, "GPU が使えない(19〜23 は飛ばした)", "OpenGL 4.3 のコンテキストか、コンピュートシェーダの組み立てに失敗している");
            return;
        }

        (string Name, int Managed, int Gpu)[] blocks =
        [
            ("Material", Unsafe.SizeOf<GpuMaterial>(), gpu.StorageBlockStride("MaterialBuffer")),
            ("Sphere", Unsafe.SizeOf<GpuSphere>(), gpu.StorageBlockStride("SphereBuffer")),
            ("Plane", Unsafe.SizeOf<GpuPlane>(), gpu.StorageBlockStride("PlaneBuffer")),
            ("Node", Unsafe.SizeOf<GpuNode>(), gpu.StorageBlockStride("NodeBuffer")),
            ("PointLight", Unsafe.SizeOf<GpuPointLight>(), gpu.StorageBlockStride("PointLightBuffer")),
        ];

        bool layoutOk = blocks.All(b => b.Managed == b.Gpu);
        string sizes = string.Join(" / ", blocks.Select(b => $"{b.Name} {b.Managed}={b.Gpu}"));

        // uniform を1つも渡していないと名前を引いていないので、小さく1枚描いてから見る。
        using var probe = new GpuRenderer(device, 8, 8);
        Scene scene = PreparedCornellBox();
        probe.RenderLinear(scene, new Camera(scene.DefaultView, 8, 8), new RenderSettings(), 1);
        string[] missing = probe.MissingUniforms.Distinct().ToArray();

        report(
            layoutOk && missing.Length == 0,
            $"GPU が使える({device.Renderer} / OpenGL {device.Version})。"
            + $"呼び出し上限 {device.MaxWorkGroupInvocations} / 共有メモリ {device.MaxSharedMemoryBytes / 1024}KB / SSBO の口 {device.MaxStorageBindings} 本",
            $"std430 のバイト数(C# = GPU): {sizes} / 渡せなかった uniform "
            + (missing.Length == 0 ? "なし" : string.Join(",", missing)));
    }

    /// <summary>
    /// 20. <b>CPU と GPU が同じ道を歩く</b>(要点2・7)。<b>今日いちばん鋭いチェック</b>。
    ///
    /// <para>
    /// 乱数を CPU と GPU で揃えてある(<see cref="Rng"/>)ので、同じ画素の同じサンプル番号は
    /// <b>同じ向きへ散り、同じ光源を選び、同じところでロシアンルーレットに当たる</b>はず。
    /// 1サンプルだけ撃って1画素ずつ突き合わせれば、<b>ノイズに紛れずに</b>移植の間違いが出る——
    /// π を落とした・引く順番が違う・材質の番号がずれた、のどれでも画面の半分が変わる。
    /// </para>
    /// <para>
    /// ぴったり 0 にはならない。<c>sin</c> / <c>cos</c> / <c>sqrt</c> / <c>normalize</c> の
    /// 最後の1〜2ビットが CPU と GPU で違うので、跳ね返るたびに差が育ち、
    /// <b>形の輪郭では「隣の形に当たる」まで振れる</b>ことがある(要点5)。
    /// だから「平均の差」で見て、大きく外れた画素の数を添える。
    /// </para>
    /// </summary>
    private static void CheckGpuMatchesCpuPath(Action<bool, string, string> report, GpuRenderer? gpu)
    {
        if (gpu is null)
        {
            report(false, "CPU と GPU が同じ道を歩く", "GPU が無いので測れない");
            return;
        }
        ...
        float[] cpu = RenderCpuLinear(scene, settings, camera, Width, Height, 1);

        using var small = new GpuRenderer(gpu.Device, Width, Height);
        float[] rgba = small.RenderLinear(PreparedCornellBox(), camera, settings, 1);
        ...
        for (int i = 0; i < Width * Height; i++)
        {
            ...
            for (int c = 0; c < 3; c++)
            {
                float difference = MathF.Abs(cpu[(i * 3) + c] - rgba[(i * 4) + c]);
                sum += difference;
                worst = MathF.Max(worst, difference);
                outlier |= difference > 0.01f;
            }
        ...
        double outlierRatio = (double)outliers / (Width * Height);

        report(
            mean < 0.002 && outlierRatio < 0.02,
            $"CPU と GPU が同じ道を歩く(コーネルボックス {Width}x{Height} を 1 サンプルだけ。乱数は同じ数列)",
            $"1成分あたりの差の平均 {mean:E1} / 最大 {worst:E1} / 0.01 を超えた画素 {outliers} 個({outlierRatio:P2})"
            + "。差の出どころは sin cos sqrt の最後の1ビット");
    }

    /// <summary>
    /// 21. <b>GPU の白い炉</b>(Day 60 の自己チェック14 の GPU 版。要点7)。
    ///
    /// <para>
    /// 反射率 1 の面だけを、一様な明るさ L の空に置く。どこをどう見ても<b>1本ごとに厳密に L</b> が返るはず。
    /// <c>BRDF × cosθ / pdf</c> の打ち消しが GLSL のどこか1か所でもずれていれば、ここで落ちる——
    /// π を落としても、cos を2回掛けても、pdf を間違えても。
    /// </para>
    /// <para>
    /// <b>CPU 版と突き合わせる 20 とは別の役目</b>。20 は「CPU と同じか」しか見ないので、
    /// <b>両方が同じように間違っていたら通ってしまう</b>。21 は GPU 単体で「答えが合っているか」を見る。
    /// </para>
    /// </summary>
    private static void CheckGpuFurnace(Action<bool, string, string> report, GpuRenderer? gpu)
    {
        if (gpu is null)
        {
            report(false, "GPU の白い炉", "GPU が無いので測れない");
            return;
        }
        ...
        var scene = new Scene("白い炉", new OrbitView(new Vector3(0.0f, 1.0f, 0.0f), 0.3f, 0.12f, 4.5f, 45.0f))
        {
            SkyHorizon = new Vector3(SkyLevel),
            SkyZenith = new Vector3(SkyLevel),
        };
        ...
        var settings = new RenderSettings { MaxDepth = 128, RussianRoulette = false };
        using var small = new GpuRenderer(gpu.Device, Size, Size);
        float[] rgba = small.RenderLinear(scene, new Camera(scene.DefaultView, Size, Size), settings, 1);
        ...
        {
            float value = rgba[i * 4];

            // 真っ黒は「深さ 128 に着いた道」。球の輪郭をかすめる光線が丸め誤差で球の内側に入り、
            // 反射率 1 の閉じた空洞に閉じ込められる(Day 60 の検証で分かったのと同じ現象)。
            if (value <= 0.0f)
            {
                trapped++;
                continue;
            ...
            tested++;
            worst = MathF.Max(worst, MathF.Abs(value - SkyLevel));
        }

        double bounces = (double)small.Status.PassRays.Scatter / Math.Max(1, tested);
        report(
            worst < 1e-5f && tested > (Size * Size / 2) && trapped < (Size * Size / 50),
            $"GPU の白い炉(反射率 1 の床と球を、一様な明るさ {SkyLevel} の空に置く。{tested} 画素)",
            $"空の色とのずれの最大 {worst:E1}(期待 0)/ 跳ね返り {bounces:F2} 回 / 閉じ込められた画素 {trapped} 個は除いた");
    }

    /// <summary>
    /// 22. <b>十分に重ねれば CPU と GPU が同じ絵に落ち着く</b>。ついでに<b>速さの比</b>を測る(要点6)。
    ///
    /// <para>
    /// 20 は1サンプルの一致を見たが、こちらは<b>何十サンプルも重ねた平均</b>で見る。
    /// 1サンプルでわずかにずれた画素があっても、重ねれば同じ値に寄る——
    /// パストレーシングの答えは「期待値」なので、そこが一致していなければ移植が間違っている。
    /// </para>
    /// <para>
    /// 速さの比は<b>この項目の副産物</b>。同じ場面・同じサンプル数を CPU と GPU で撃った時間を並べる。
    /// 画面の HUD の数字は窓の描き直しと混ざるので、<b>比べるならここの数字</b>を見る。
    /// </para>
    /// </summary>
    private static void CheckGpuConverges(Action<bool, string, string> report, GpuRenderer? gpu)
    {
        if (gpu is null)
        {
            report(false, "CPU と GPU が同じ絵に落ち着く", "GPU が無いので測れない");
            return;
        }
        ...
        float[] cpu = RenderCpuLinear(scene, settings, camera, Width, Height, Samples);
        double cpuSeconds = clock.Elapsed.TotalSeconds;

        using var small = new GpuRenderer(gpu.Device, Width, Height);
        clock.Restart();
        float[] rgba = small.RenderLinear(PreparedCornellBox(), camera, settings, Samples);
        double gpuSeconds = clock.Elapsed.TotalSeconds;
        ...
        for (int i = 0; i < Width * Height; i++)
        {
            double a = Luminance(cpu[i * 3], cpu[(i * 3) + 1], cpu[(i * 3) + 2]);
            double b = Luminance(rgba[i * 4], rgba[(i * 4) + 1], rgba[(i * 4) + 2]);
            cpuMean += a;
            gpuMean += b;
        ...
        double meanError = Math.Abs(gpuMean - cpuMean) / cpuMean;

        report(
            meanError < 0.01 && difference < 0.02,
            $"CPU と GPU が同じ絵に落ち着く(コーネルボックス {Width}x{Height} を {Samples} サンプル)",
            $"明るさの平均 CPU {cpuMean:F4} / GPU {gpuMean:F4}(差 {meanError:P2})/ 画素ごとの差の平均 {difference:F4} / "
            + $"時間 CPU {cpuSeconds * 1000.0:F0}ms → GPU {gpuSeconds * 1000.0:F0}ms({cpuSeconds / gpuSeconds:F0} 倍)");
    }

    /// <summary>
    /// 23. <b>GPU でも BVH と総当たりが同じ絵を出す</b>(Day 60 の自己チェック18 の GPU 版)。
    ///
    /// <para>
    /// 木を辿る順が変わっても、<b>いちばん近い当たり</b>は同じ1つに決まる。
    /// 乱数も同じなので、道も同じになり、<b>1ビットも違わない絵</b>が出るはず。
    /// GLSL のスタックの積み方や「近い子から辿る」の入れ替えを間違えると、ここで落ちる。
    /// </para>
    /// </summary>
    private static void CheckGpuBvhMatchesBruteForce(Action<bool, string, string> report, GpuRenderer? gpu)
    {
        if (gpu is null)
        {
            report(false, "GPU の BVH と総当たりが一致", "GPU が無いので測れない");
            return;
        }
        ...
        Scene brute = Build(AccelerationMode.BruteForce);
        var camera = new Camera(brute.DefaultView, Width, Height);
        var settings = new RenderSettings { Acceleration = AccelerationMode.BruteForce };

        using var small = new GpuRenderer(gpu.Device, Width, Height);
        float[] bruteImage = small.RenderLinear(brute, camera, settings, Samples);
        RayCounters bruteRays = small.Status.PassRays;

        float[] sahImage = small.RenderLinear(
            Build(AccelerationMode.Sah), camera, settings with { Acceleration = AccelerationMode.Sah }, Samples);
        RayCounters sahRays = small.Status.PassRays;

        float worst = 0.0f;
        for (int i = 0; i < bruteImage.Length; i++)
        {
            worst = MathF.Max(worst, MathF.Abs(bruteImage[i] - sahImage[i]));
        }

        double bruteTests = (double)(bruteRays.BoxTests + bruteRays.ShapeTests) / Math.Max(1, bruteRays.Total);
        double sahTests = (double)(sahRays.BoxTests + sahRays.ShapeTests) / Math.Max(1, sahRays.Total);

        report(
            worst < 1e-6f && sahTests < bruteTests * 0.2,
            $"GPU の BVH と総当たりが一致(球がたくさん {Width}x{Height} を {Samples} サンプル)",
            $"画素の差の最大 {worst:E1}(期待 0。乱数が同じなので道まで同じ)/ "
            + $"1光線あたりの交差判定 総当たり {bruteTests:F1} → SAH {sahTests:F1}({bruteTests / sahTests:F1} 分の1)");
    }

    /// <summary>
    /// CPU 側で小さく描いて、<b>線形の平均の色</b>を画素ごとに返す(rgb の3つずつ)。
    /// GPU の <see cref="GpuRenderer.RenderLinear"/> と<b>同じ順で同じ乱数</b>を引くように書いてある。
    /// </summary>
    private static float[] RenderCpuLinear(Scene scene, RenderSettings settings, Camera camera, int width, int height, int samples)
    {
        Tracer tracer = Tracer.Create(scene, settings);
        ...
        Parallel.For(0, height, y =>
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    var rng = Rng.Create(x, y, i, settings.DecorrelatePixels);
                    Vector2 offset = settings.Jitter ? ProgressiveRenderer.SampleOffset(i) : new Vector2(0.5f);
                    sum += tracer.Sample(camera, x + offset.X, y + offset.Y, ref rng, ref rays);
                }

                int index = ((y * width) + x) * 3;
                result[index] = sum.X / samples;
                result[index + 1] = sum.Y / samples;
                result[index + 2] = sum.Z / samples;
            }
        });
