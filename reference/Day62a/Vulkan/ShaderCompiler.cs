...
internal sealed class ShaderCompiler : IDisposable
{
    ...
    public unsafe uint[] CompileComputeFile(string path)
    {
        string source = File.ReadAllText(path);
        return Compile(source, Path.GetFileName(path), ShaderKind.ComputeShader);
    }
    ...
    /// </para>
    /// </summary>
    public unsafe uint[] Compile(string source, string name, ShaderKind kind)
    {
        ...
        try
        {
            ...
            fixed (byte* pSource = sourceBytes)
            {
                result = _api.CompileIntoSpv(
                    ...
                    (byte*)namePtr, (byte*)entryPtr, _options);
            }
