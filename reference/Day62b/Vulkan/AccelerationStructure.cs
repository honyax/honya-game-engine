...
internal sealed unsafe class AccelerationStructure : IDisposable
{
    ...
    /// </list>
    /// </summary>
    public static AccelerationStructure Build(VulkanDevice device, GpuSphere[] spheres)
    {
        ...
        var aabbGeometry = new AccelerationStructureGeometryKHR
        {
            SType = StructureType.AccelerationStructureGeometryKhr,
            GeometryType = GeometryTypeKHR.AabbsKhr,

            // **Opaque を立てると any-hit を呼ばなくなる**ので速い。
            // 今日は透ける物が無いので立てておく(Day 62c の any-hit の話につながる)。
            ...
            Geometry = new AccelerationStructureGeometryDataKHR
            {
                Aabbs = new AccelerationStructureGeometryAabbsDataKHR
                {
                    ...
                    Stride = (ulong)Unsafe.SizeOf<GpuAabb>(),
                },
            },
        };

        (AccelerationStructureKHR blas, VulkanBuffer blasBuffer, ulong blasBytes, ulong blasScratch) =
            BuildOne(device, api, AccelerationStructureTypeKHR.BottomLevelKhr, aabbGeometry, (uint)spheres.Length);
        ...
        (AccelerationStructureKHR tlas, VulkanBuffer tlasBuffer, ulong tlasBytes, ulong tlasScratch) =
            BuildOne(device, api, AccelerationStructureTypeKHR.TopLevelKhr, instanceGeometry, 1);
    ...
    private static (AccelerationStructureKHR Handle, VulkanBuffer Buffer, ulong Size, ulong ScratchSize) BuildOne(
        VulkanDevice device, KhrAccelerationStructure api,
        AccelerationStructureTypeKHR type, AccelerationStructureGeometryKHR geometry, uint primitiveCount)
    {
        Vk vk = device.Api;
        AccelerationStructureGeometryKHR localGeometry = geometry;
        ...
        uint counts = primitiveCount;
        api.GetAccelerationStructureBuildSizes(
            device.Handle, AccelerationStructureBuildTypeKHR.DeviceKhr, &buildInfo, &counts, &sizeInfo);
        ...
        device.SubmitAndWait(cmd => RecordBuild(device, api, cmd, info, range));
    ...
    private static void RecordBuild(
        ...
        AccelerationStructureBuildGeometryInfoKHR buildInfo, AccelerationStructureBuildRangeInfoKHR range)
    {
        // 第4引数が**ポインタのポインタ**なのは、加速構造ごとに「範囲の配列」を渡す形だから
        // (複数の加速構造を1回のコマンドで建てられる)。今日は1つずつ建てるので要素も1つ。
