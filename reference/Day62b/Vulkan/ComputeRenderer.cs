...
/// </list>
/// <para>
/// <b>2つとも起動時に作っておく</b>のが Vulkan らしいところ。切り替えは
...
internal sealed unsafe class ComputeRenderer : IDisposable
{
    ...
    public ComputeRenderer(
        VulkanDevice device, int width, int height, GpuSphere[] spheres,
        uint[] bruteSpirv, uint[]? rayQuerySpirv)
    {
        ...
        {
            _acceleration = AccelerationStructure.Build(device, spheres);
        }
    ...
    /// </para>
    /// </summary>
    /// <param name="useRayQuery">
    /// true なら ray query 版のパイプラインを使う(Day 62b で追加)。
    /// 持っていないときは黙って総当たりに落ちる。
    /// </param>
    public void Render(Camera camera, int mode, bool useRayQuery, int[] destination)
    {
        ...
        bool rayQuery = useRayQuery && RayQueryAvailable;
        vk.CmdBindPipeline(_commands, PipelineBindPoint.Compute, rayQuery ? _rayQueryPipeline : _brutePipeline);
