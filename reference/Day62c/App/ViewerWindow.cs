...
/// </para>
/// <para>
/// <b>Day 62c の差分も `B` キーの回りだけ</b>。探し方が2つから3つになり、
/// HUD に SBT の大きさが1つ増える。絵を作る場所は何も変わっていない。
/// </para>
/// </summary>
internal sealed class ViewerWindow : Form
{
    ...
    /// <summary>
    /// 探し方(Day 62c で3つになった)。起動時はレイトレーシングパイプライン——
    /// 今日の主役なので最初から見せる。
    /// </summary>
    private TraceMode _trace = TraceMode.Pipeline;
    ...
    public ViewerWindow(int width, int height)
    {
        ...
        Text = "Day62c - レイトレーシングパイプラインと SBT: 分岐がシェーダの分割になる";
    ...
    public void Run()
    {
        ...
        while (_running)
        {
            ...
            {
                _renderer.Render(new Camera(_view, _width, _height), _mode, _trace, _pixels);
    ...
    /// <summary>
    /// いま実際に使われる探し方。対応していないものを選んでいたら総当たりに落ちる
    /// (<see cref="ComputeRenderer.Render"/> と同じ判断を HUD 側でもする)。
    /// </summary>
    private TraceMode Effective() => _trace switch
    {
        TraceMode.Pipeline when _renderer?.PipelineAvailable != true => TraceMode.BruteForce,
        TraceMode.RayQuery when _renderer?.RayQueryAvailable != true => TraceMode.BruteForce,
        _ => _trace,
    };
    ...
    private void DrawHud(Graphics g)
    {
        ...
        string traceName = Effective() switch
        {
            TraceMode.Pipeline => "RT パイプライン + SBT",
            TraceMode.RayQuery => "ray query (BLAS/TLAS)",
            _ => "総当たり",
        };
        ...
        // SBT の大きさ(Day 62c で追加)。**取っ手5つぶんしかない**ことを見るための行。
        if (_renderer?.PipelineAvailable == true)
        {
            lines.Add($"SBT   : {_renderer.ShaderBindingTableBytes} バイト"
                + $" (取っ手 {_device!.ShaderGroupHandleSize} B x 5 グループ"
                + $" / 区画の境界 {_device.ShaderGroupBaseAlignment} B)");
        }
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        switch (e.KeyCode)
        {
            ...
            // 探し方を切り替える(Day 62c で3つになった)。**作り直しは起きない**——
            // 3本のパイプラインは起動時に全部できていて、束ねるハンドルが変わるだけ。
            case Keys.B:
                _trace = (TraceMode)(((int)_trace + 1) % 3);
                _averageMilliseconds = 0.0;
                break;
