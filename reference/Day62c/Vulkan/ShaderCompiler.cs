...
internal sealed class ShaderCompiler : IDisposable
{
    ...
    public unsafe uint[] CompileComputeFile(string path, params string[] defines)
        => CompileFile(path, ShaderKind.ComputeShader, defines);

    /// <summary>
    /// 種類を指定して翻訳する(Day 62c で追加)。今日は .rgen / .rmiss / .rchit / .rint が増えた。
    /// </summary>
    ...
    /// <summary>
    /// <c>#include "..."</c> を、そのファイルの中身で置き換える(Day 62c で追加)。
    ///
    /// <para>
    ...
    /// </para>
    /// <para>
    /// 今日は<b>翻訳に渡す前に文字列として差し込む</b>——「SPIR-V の世界では、
    /// ソースをどう組み立てるかはアプリの責任」という要点2 の話が、いちばん素朴な形で出ている。
    ...
    private static string ResolveIncludes(string path)
    {
        ...
        foreach (string line in File.ReadLines(path))
        {
            ...
            // **入れ子は許さない**。許すと循環の検出が要るし、今日は1段で足りる。
