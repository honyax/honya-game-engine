...
/// <summary>
/// バッファ1本。<b>「入れ物」と「メモリ」が別々の概念</b>なのが Vulkan らしいところ(今日の要点3)。
///
/// <para>
...
/// 今日は本数が少ないので、<b>1本につき1回確保する</b>という素直だが実戦的でない形にしてある。
/// </para>
/// </summary>
internal sealed unsafe class VulkanBuffer : IDisposable
{
    ...
    public static VulkanBuffer Create(VulkanDevice device, ulong size, BufferUsageFlags usage, MemoryPropertyFlags properties)
    {
        ...
        // **GPU のアドレスで指したいバッファは、確保するときにもそう宣言する**(Day 62b で追加)。
        // 使い道(usage)に ShaderDeviceAddressBit を立てるだけでは足りず、
        // メモリ側にも MemoryAllocateFlagsInfo を鎖でつなぐ必要がある。
        // 忘れると vkGetBufferDeviceAddress が未定義動作になる(検証レイヤは捕まえてくれる)。
        bool wantsAddress = (usage & BufferUsageFlags.ShaderDeviceAddressBit) != 0;
        ...
        var allocInfo = new MemoryAllocateInfo
        {
            ...
            PNext = wantsAddress ? &flagsInfo : null,
    ...
    /// シェーダから何度も読まれるデータ(今日の球の配列)は、この手間をかけて VRAM に置く値打ちがある。
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// このバッファの<b>GPU から見たアドレス</b>(Day 62b で追加)。
    ///
    /// <para>
    /// 加速構造まわりの API は、入力も出力も<b>ハンドルではなく生のアドレス</b>で指す。
    /// 頂点・AABB・インスタンス・スクラッチのどれもそう。
    /// これは BLAS/TLAS の中身が<b>GPU 側から作られる</b>(<c>vkCmdBuildAccelerationStructures</c>)ためで、
    /// GPU の中にはディスクリプタの概念が無い場所があるから。
    /// </para>
    /// <para>
    /// 使うには <see cref="BufferUsageFlags.ShaderDeviceAddressBit"/> と
    /// <c>bufferDeviceAddress</c> 機能の両方が要る。
    /// </para>
    /// </summary>
    public ulong DeviceAddress
    {
        get
        {
            ...
            return _device.Api.GetBufferDeviceAddress(_device.Handle, &info);
        }
    }
