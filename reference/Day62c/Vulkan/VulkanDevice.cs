...
/// </para>
/// <para>
/// <b>Day 62c の差分もこのファイルの同じ場所</b>。拡張が1つ(<c>VK_KHR_ray_tracing_pipeline</c>)、
/// 機能が1つ(<c>rayTracingPipeline</c>)、そして<b>SBT を組むのに要る3つの数</b>を聞いておく。
/// </para>
/// <para>
...
internal sealed unsafe class VulkanDevice : IDisposable
{
    ...
    [
        ...
        // Day 62c で追加。**vkCmdTraceRaysKHR と、raygen / miss / closest-hit / intersection
        // という段を持つパイプライン**を使えるようにする。
        // ray_query が「どのシェーダからでも加速構造をたどれる」入口なのに対して、
        // こちらは「加速構造をたどる専用のパイプラインを組む」入口(要点1)。
    ...
    /// <summary>レイトレーシングパイプラインの API(Day 62c で追加)。</summary>
    private readonly KhrRayTracingPipeline? _rayTracingPipelineApi;
        ...
        string name, string driver, bool validation)
    {
        ...
        _rayTracingPipelineApi = rayTracingPipelineApi;
        Api = vk;
        Instance = instance;
        ...
        if (accelerationStructureApi is not null)
        {
            ...
            // Day 62c: レイトレーシングパイプラインの数も同じ鎖でまとめて聞く。
    ...
    /// <summary>
    /// レイトレーシングパイプラインが使えるか(Day 62c で追加)。
    /// 加速構造と同じ拡張の束で有効にするので、<see cref="RayQuerySupported"/> と必ず一致する。
    /// </summary>
    [MemberNotNullWhen(true, nameof(RayTracingPipelineApi))]
    public bool RayTracingPipelineSupported => _rayTracingPipelineApi is not null;

    /// <summary>レイトレーシングパイプラインの API。</summary>
    public KhrRayTracingPipeline? RayTracingPipelineApi => _rayTracingPipelineApi;

    /// <summary>
    /// シェーダグループの<b>取っ手</b>1つのバイト数(Day 62c で追加。手元の RTX 3070 は 32)。
    ///
    /// <para>
    /// SBT は「取っ手の並び」でできている。取っ手は <c>vkGetRayTracingShaderGroupHandlesKHR</c> が
    /// 返す不透明なバイト列で、<b>中身に意味を求めてはいけない</b>(GPU ごとに別物)。
    /// </para>
    /// </summary>
    public uint ShaderGroupHandleSize { get; }

    /// <summary>取っ手1つを置くときの境界。レコードの<b>刻み幅</b>はこの倍数でなければならない。</summary>
    public uint ShaderGroupHandleAlignment { get; }

    /// <summary>
    /// SBT の<b>区画</b>(raygen / miss / hit / callable)の先頭が乗るべき境界。
    /// 手元の RTX 3070 では 64 で、取っ手 32 バイトより大きい——
    /// だから区画の間に隙間が空く(<see cref="ShaderBindingTable"/>)。
    /// </summary>
    public uint ShaderGroupBaseAlignment { get; }

    /// <summary>
    /// シェーダの中から光線を撃てる深さの上限(手元の RTX 3070 は 31)。
    /// パイプラインを作るときに「どこまで潜るか」を宣言し、その値がこれを超えてはいけない。
    /// </summary>
    public uint MaxRayRecursionDepth { get; }
    ...
    private static VulkanDevice? Create(Vk vk, out string message)
    {
        ...
        KhrRayTracingPipeline? rtApi = null;
        if (rayTracing && vk.TryGetDeviceExtension(instance, device, out KhrAccelerationStructure loaded))
        {
            asApi = loaded;
            if (vk.TryGetDeviceExtension(instance, device, out KhrRayTracingPipeline rtLoaded))
            {
                rtApi = rtLoaded;
            }
        }
        ...
        return new VulkanDevice(
            ...
            debugUtils, messenger, callback, asApi, rtApi, name, driver, useValidation);
    }
