...
internal sealed unsafe class VulkanImage : IDisposable
{
    ...
    /// </para>
    /// </summary>
    /// <param name="sourceStage">
    /// 画像に書いた段(Day 62c で追加)。コンピュートなら <c>ComputeShaderBit</c>、
    /// レイトレーシングパイプラインなら <c>RayTracingShaderBitKhr</c>。
    /// <b>ここを間違えても大抵は動いてしまう</b>のが怖いところで、
    /// 「書き終わる前に読む」が起きるのは GPU が混んだときだけ。
    /// </param>
    public void RecordCopyToBuffer(CommandBuffer cmd, VulkanBuffer destination, PipelineStageFlags sourceStage)
    {
        var barrier = new ImageMemoryBarrier
        {
            SType = StructureType.ImageMemoryBarrier,
