...
///
/// <para>
/// <b>Day 56 での変更</b>: <b>灯り</b>(<c>lights</c>)と<b>夜の強さ</b>(<c>lighting.night</c>)が書けるようになった。
/// どちらも Day 52 で <c>Program</c> が持っていたもの(漂う光の群れと、3% の決め打ち)を、シーンの側へ移したもの。
/// 灯りを書いた JSON は <c>scenes/demo-final.json</c> で、<c>demo-v1.json</c> は1文字も変えていない——
/// <b>assets/ は全部の Day で共有している</b>ので、v1 に板を1枚足すだけで Day 39〜55 の絵が変わってしまう。
/// </para>
/// </summary>
/// <summary>
...
internal sealed class DemoScene : IDisposable
{
    ...
    /// <summary>
    /// **夜の強さの既定**(Day 52 の <c>Program.NightDim</c> をここへ移した。Day 56)。
    /// 太陽と空を 3% に落とす。JSON に <c>lighting.night</c> が無ければこれになるので、
    /// <c>demo-v1.json</c> の夜は Day 52〜55 と1ビットも変わらない。
    /// </summary>
    public const float DefaultNightScale = 0.03f;

    /// <summary>
    /// 夜にしたときの光(Day 56)。<c>lighting.night</c>。
    ///
    /// <para>
    /// <b>露出も夜の値を持つ</b>。本物のカメラは暗くなれば露出を上げる(人の目も瞳孔を開く)。
    /// 昼の露出のまま太陽を 3% に落とすと、灯りが照らしたところ以外は真っ黒になり、
    /// 通りの形が読めなくなる。どこまで上げるかは「夜の絵をどう見せたいか」なので、シーンが決める。
    /// </para>
    /// </summary>
    /// <param name="SunScale">太陽(夜は月明かりの代わり)に掛ける倍率。</param>
    /// <param name="IblScale">空(IBL)に掛ける倍率。</param>
    /// <param name="Exposure">夜の露出。書いていなければ昼の露出のまま(Day 52 と同じ)。</param>
    internal readonly record struct NightSettings(float SunScale, float IblScale, float Exposure);

    /// <summary>夜にしたときの光(Day 56)。<c>lighting.night</c> が無ければ Day 52 の 3%。</summary>
    public NightSettings Night { get; private set; } = new(DefaultNightScale, DefaultNightScale, 1.0f);

    /// <summary>
    /// **灯り**(Day 56)。<c>lights</c> が無ければ空(Day 39〜55 の JSON)。
    /// 点け消しと揺らぎは <see cref="SceneLights.Update"/> が毎フレーム決める。
    /// 空の列もシーンごとに1つ持つ(揺らぎの時計はシーンの持ち物で、シーンをまたいで共有しない)。
    /// </summary>
    public SceneLights Lights { get; private set; } = new([]);

    /// <summary>
    /// 光る板の照らす側を、板からどれだけ手前に置くか [m](Day 56)。
    ///
    /// <para>
    /// <b>板の上に置くと、板を貼った壁が照らされない</b>。壁の面の上にある点光源は、壁から見ると真横(N·L = 0)に居るから。
    /// 少し手前に出すと、窓のまわりの壁にも足元の地面にも光だまりができる——「窓から漏れた光」の置き方。
    /// </para>
    /// </summary>
    public const float PanelLightOffset = 0.4f;
        ...
        Func<string, string> resolveAsset)
    {
        ...
        if (root.TryGetProperty("lighting", out JsonElement lighting))
        {
            ...
            // **夜の強さもシーンの持ち物**(Day 56)。書いていなければ Day 52 の 3% で、露出は昼のまま。
            scene.Night = lighting.TryGetProperty("night", out JsonElement night)
                ? new NightSettings(
                    SunScale: GetFloat(night, "sunScale", DefaultNightScale),
                    IblScale: GetFloat(night, "iblScale", DefaultNightScale),
                    Exposure: GetFloat(night, "exposure", scene.Exposure))
                : new NightSettings(DefaultNightScale, DefaultNightScale, scene.Exposure);
        }
        ...
        // --- 4. 灯り(Day 56)---
        //
        // **小物より後に読む**。見える側は「街灯のガラス」のように小物の部品を名前で指すので、
        // 小物が並び終わっていないと探せない。
        if (root.TryGetProperty("lights", out JsonElement lights))
        {
            scene.Lights = scene.ReadLights(lights, shader, quad);
        }
    ...
    /// <summary>
    /// **灯りを読む**(Day 56)。1つの項目に照らす側と見える側を書く。
    ///
    /// <code>
    ///   color / intensity / radius / flicker … 照らす側(点光源)
    ///   glow.strength                      … 見える側の明るさ(色は照らす側と共有)
    ///   glow.prop + glow.match             … 見える側 その1: 小物の部品を光らせる(街灯のガラス)
    ///   glow.size / rotation / position    … 見える側 その2: 光る板を足す(窓・看板)
    ///   position                           … 照らす側の置き場所。書かなければ見える側から決める
    /// </code>
    ///
    /// <para>
    /// <b>置き場所を書かなくてよい</b>ようにしてあるのが、今日いちばん効いた作り。
    /// 部品を光らせるなら<b>その部品の真ん中</b>、板なら<b>板の少し手前</b>(<see cref="PanelLightOffset"/>)。
    /// 見える側から決めておけば、街灯を 1m 動かしても光だけ元の場所に取り残されることが無い。
    /// </para>
    ///
    /// <para>
    /// <b>見つからなければ例外</b>(<see cref="Load"/> の方針)。部品の名前を書き間違えると
    /// 「光るはずのガラスが光らないまま、床だけ照らされる」絵になり、どこが悪いのか分からない。
    /// </para>
    /// </summary>
    private SceneLights ReadLights(JsonElement lights, Handle<Shader> shader, Mesh<Vertex> quad)
    {
        var fixtures = new List<SceneLights.Fixture>();

        foreach (JsonElement entry in lights.EnumerateArray())
        {
            string name = GetString(entry, "name", $"灯り{fixtures.Count + 1}");
            Vector3 color = GetVector3(entry, "color", Vector3.One);

            var glowMaterials = new List<Material>();
            float glow = 0.0f;
            Vector3? source = null;

            if (entry.TryGetProperty("glow", out JsonElement visible))
            {
                glow = GetFloat(visible, "strength", 1.0f);
                source = TryGetString(visible, "prop", out string? prop)
                    ? FindGlowParts(name, prop, GetString(visible, "match", string.Empty), glowMaterials)
                    : AddGlowPanel(name, visible, shader, quad, glowMaterials);
            }

            if (source is null && !entry.TryGetProperty("position", out _))
            {
                throw new InvalidDataException($"灯り「{name}」: position か glow のどちらかを書くこと(置き場所が決まらない)");
            }

            fixtures.Add(new SceneLights.Fixture(
                name: name,
                position: GetVector3(entry, "position", source ?? Vector3.Zero),
                color: color,
                intensity: GetFloat(entry, "intensity", 1.0f),
                radius: GetFloat(entry, "radius", 4.0f),
                flicker: GetFloat(entry, "flicker", 0.0f),
                glow: glow,
                glowMaterials: glowMaterials));
        }

        return new SceneLights(fixtures);
    }

    /// <summary>
    /// 見える側 その1: **小物の部品を光らせる**。部品の真ん中を返す(照らす側の既定の置き場所)。
    ///
    /// <para>
    /// 部品は「小物の名前/部品の名前」で並んでいる(<see cref="AddProp"/>)。<paramref name="match"/> は
    /// <b>部品の名前か材質の名前</b>に含まれていればよい——街灯はガラスが材質で分かれていて(<c>street_lamp_01_glass</c>)、
    /// 吊りランタンは部品で分かれている(<c>LanternPole_Lantern</c>)。どちらの分け方もよくあるので、両方で探す。
    /// </para>
    /// </summary>
    private Vector3 FindGlowParts(string light, string prop, string match, List<Material> materials)
    {
        string prefix = prop + "/";

        var parts = _items
            .Where(item => item.Name.StartsWith(prefix, StringComparison.Ordinal)
                && (item.Name[prefix.Length..].Contains(match, StringComparison.OrdinalIgnoreCase)
                    || item.Material.Name.Contains(match, StringComparison.OrdinalIgnoreCase)))
            .ToList();

        if (parts.Count == 0)
        {
            throw new InvalidDataException($"灯り「{light}」: 小物「{prop}」に「{match}」を含む部品が見つかりません");
        }

        // 同じ材質を複数の部品が使っていることがある。**材質は1回ずつ**でよい。
        materials.AddRange(parts.Select(part => part.Material).Distinct());
    ...
    /// <summary>
    /// 見える側 その2: **光る板を足す**。板の少し手前を返す(照らす側の既定の置き場所)。
    ///
    /// <para>
    /// 板は壁と同じ <see cref="Primitives.CreateQuad"/> の1枚で、書き方も <c>surfaces</c> と同じ(size / rotation / position)。
    /// 違うのは<b>影を落とさず、当たらない</b>こと。壁から 2cm 浮かせて貼るだけの薄い板で、
    /// 窓の明かりが影を落としたり、キツネが窓の板にぶつかったりするのは変だから。
    /// </para>
    ///
    /// <para>
    /// <b>ベースカラーは暗いガラス</b>。灯りを消すと発光が 0 になり、
    /// 暗くつるつるした板が残る——夜の窓の、明かりが消えた姿。
    /// </para>
    ///
    /// <para>
    /// <b>白い 1x1 を貼る</b>(Day 35 の材質グリッドと同じ 1 枚を借りる)。貼らないと
    /// <see cref="RenderResources.GetTexture"/> が仮の絵(マゼンタの市松)を返し、灯りを消した窓に市松がうっすら浮く——
    /// 灯りを点けている間は発光に埋もれて見えないので、消して初めて気づく(計画書「検証の途中で分かったこと」)。
    /// </para>
    /// </summary>
    private Vector3 AddGlowPanel(
        string light, JsonElement panel, Handle<Shader> shader, Mesh<Vertex> quad, List<Material> materials)
    {
        Matrix4x4 transform = ReadTransform(panel);

        var material = new Material(shader)
        {
            Name = $"灯り/{light}",
            MainTexture = Track(_resources.LoadTextureFromPixels(
                "pbr/white", [255, 255, 255, 255], 1, 1, generateMipmaps: false, srgb: false)),
            BaseColorFactor = new Vector4(0.02f, 0.02f, 0.02f, 1.0f),
            MetallicFactor = 0.0f,
            RoughnessFactor = 0.25f,
        };

        (Vector3 min, Vector3 max) = TransformBounds(
            new Vector3(-0.5f, -0.5f, 0.0f), new Vector3(0.5f, 0.5f, 0.0f), transform);
        ...
        _items.Add(new Item(
            ...
            BoundsMin: min,
            BoundsMax: max,
            Collision: SceneCollisionKind.None));

        TriangleCount += 2;
        materials.Add(material);

        // 板は +Z を向いている(ReadTransform の説明)。拡大は Z に掛からないので、向きは正規化するだけで出る。
        Vector3 normal = Vector3.Normalize(Vector3.TransformNormal(Vector3.UnitZ, transform));
        return transform.Translation + (normal * PanelLightOffset);
    }
