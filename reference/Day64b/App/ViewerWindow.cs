...
internal sealed class ViewerWindow : Form
{
    ...
    public ViewerWindow(int width, int height)
    {
        ...
        Text = "Day64b - タスクシェーダ: 要らないメッシュレットを GPU の上で捨てる";
    ...
    public void Run()
    {
        ...
        while (_running)
        {
            ...
            if (_renderer is not null)
            {
                ...
                _renderer.Render(camera, cullCamera, _mode, _path, _culling, _pixels);
    ...
    private void DrawHud(Graphics g)
    {
        ...
        if (_renderer is not null)
        {
            ...
            {
                DrawPath.TaskMesh => $"タスクシェーダ {_renderer.TaskWorkGroups:N0} 組 x 32 人 → メッシュシェーダ {c.Visible:N0} 組",
