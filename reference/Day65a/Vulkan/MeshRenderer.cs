...
internal sealed unsafe class MeshRenderer : IDisposable
{
    ...
    /// <summary>体を選ぶコンピュートのパイプライン(Day 65a で追加)。cull.comp。</summary>
    private readonly Pipeline _cullPipeline;

    private readonly ShaderModule _cullModule;
        ...
        ShaderCompiler compiler, string shaderDirectory)
    {
        ...
        (_cullPipeline, _cullModule) = CreateComputePipeline(device, _pipelineLayout, cull);
        ...
        // ---- 7. 計測の入れ物 ------------------------------------------------------------
        _timestamps = CreateQueryPool(device, QueryType.Timestamp, 4, 0);
        ...
        Camera camera, Camera cullCamera, int mode, DrawPath path, CullingMode culling, InstanceSelection selection, int[] destination)
    {
        ...
        vk.CmdResetQueryPool(_commands, _timestamps, 0, 4);
        ...
        ulong* timestamps = stackalloc ulong[4];
        VulkanDevice.Check(
            vk.GetQueryPoolResults(_device.Handle, _timestamps, 0, 4, sizeof(ulong) * 4, timestamps, sizeof(ulong), QueryResultFlags.Result64Bit),
    ...
    public void Dispose()
    {
        ...
        vk.DestroyPipeline(_device.Handle, _cullPipeline, null);
        vk.DestroyShaderModule(_device.Handle, _cullModule, null);
    ...
    private void RecordGpuSelection(Vk vk)
    {
        ...
        vk.CmdBindPipeline(_commands, PipelineBindPoint.Compute, _cullPipeline);
        DescriptorSet set = _set;
    ...
    /// <summary>
    /// SPIR-V からコンピュートパイプラインを1本作る(Day 65a で追加。Day 62c の <c>ComputeRenderer</c> から持ってきた)。
    /// グラフィックスと違って、固定機能の設定は1つも無い。シェーダ1本とレイアウトだけ。
    /// </summary>
    private static (Pipeline Pipeline, ShaderModule Module) CreateComputePipeline(
