...
/// </para>
/// <para>
/// <b>Day 65c の差分</b>: <c>G</c> の先頭に「2パス(隠れたものも捨てる)」が増えた(起動時もこれ)。<c>V</c>(低い視点)が増えた。
/// HUD の「体」「判定」「内訳」に隠れていた数と早いパス・遅いパスの内訳が、「1枚」に2パスの時間の内訳が出る。
/// </para>
/// </summary>
internal sealed class ViewerWindow : Form
{
    ...
    /// <summary>描く体をどう選ぶか(Day 65a で追加)。Day 65c から起動時は2パス——今日の主役なので最初から見せる。</summary>
    private InstanceSelection _selection = InstanceSelection.GpuTwoPass;
    ...
    public ViewerWindow(int width, int height)
    {
        ...
        Text = "Day65c - GPU 駆動: 前のフレームの深度 (Hi-Z) で、隠れたものを捨てる 2 パス";
    ...
    public void Run()
    {
        ...
        while (_running)
        {
            ...
            if (_renderer is not null)
            {
                ...
                // Day 65c: 描く命令だけでなく「選び始めてから描き終わるまで」にした(2パスは描く命令が2回あり、間に Hi-Z が挟まるので)。
                _averageDrawMilliseconds = Smooth(_averageDrawMilliseconds, _renderer.LastTotalMilliseconds);
            }
    ...
    private void DrawHud(Graphics g)
    {
        ...
        if (_renderer is not null)
        {
            ...
            {
                InstanceSelection.GpuTwoPass => "2パス (隠れたものも捨てる)",
            ...
            TwoPassStatistics t = _renderer.LastTwoPass;
            if (_selection != InstanceSelection.GpuTwoPass)
            {
                lines.Add($"体    : {selectionName} (G)  {_renderer.InstanceCount} 体 → 描く {_renderer.LastDrawnInstances} 体{selectionCost}");
            }
            else if (t.LateRan)
            {
                // Day 65c: 遅いパスが全部の体を調べ直した結果。
                // タスクの道では、遅いパスは見える体を全部調べ直す(どのメッシュレットを描き足すかはタスクシェーダが決める)。
                int visible = _renderer.InstanceCount - t.FrustumInstances - t.OccludedInstances;
                lines.Add($"体    : {selectionName} (G)  {_renderer.InstanceCount} 体 → 画面の外 {t.FrustumInstances} / 隠れている {t.OccludedInstances}"
                    + $" / 見える {visible}  (早いパス {t.EarlyInstances} 体・遅いパス {t.LateInstances} 体)");
            }
            else
            {
                lines.Add($"体    : {selectionName} (G)  早いパスだけ {t.EarlyInstances} 体(目を止めている間は、見えた印も Hi-Z も止まる)");
            }

            // Day 64b: メッシュレットの行き先。タスクシェーダが数えた値(要点5)。
            CullingStatistics c = _renderer.LastCulling;
            if (_selection == InstanceSelection.GpuTwoPass && !t.LateRan)
            {
                // Day 65c: 目を止めている間は遅いパスが走らないので、内訳を数える人がいない。
                lines.Add("判定  : 目を止めている間は数えない(早いパスが、止めた瞬間に見えていたものだけを描く)");
            }
            else if (Effective() == DrawPath.TaskMesh)
            {
                double percent = 100.0 / Math.Max(c.Tested, 1);
                lines.Add($"判定  : {c.Tested:N0} 個 → 画面の外 {c.FrustumCulled * percent:F1}%"
                    + $" / 裏向き {c.BackfaceCulled * percent:F1}% / 隠れている {c.OcclusionCulled * percent:F1}% / 描く {c.Visible * percent:F1}%");
            }
            else
            {
            ...
            // Day 65c: GPU の合計(選ぶ + 描く + Hi-Z)。2パスなら、直前の1枚の内訳も出す。
            string breakdown = t.LateRan
                ? $" (直前の1枚: 早いパス {_renderer.LastSelectMilliseconds + _renderer.LastDrawMilliseconds:F2} + Hi-Z {_renderer.LastPyramidMilliseconds:F2}"
                    + $" + 遅いパス {_renderer.LastLateMilliseconds:F2})"
                : string.Empty;
            lines.Add($"1枚   : {ms:F2} ms ({(ms > 0.0 ? 1000.0 / ms : 0.0):F0} 枚/秒)"
                + $"  うち GPU {_averageDrawMilliseconds:F2} ms{breakdown}");
            ...
            string shader = Effective() switch
            {
                ...
                // Day 65c: 2パスでは、タスクシェーダは2回(早いパス + 遅いパス)、メッシュシェーダは描いた数(早い + 遅い)。
                DrawPath.TaskMesh when _selection == InstanceSelection.GpuTwoPass =>
                    $"タスクシェーダ {_renderer.TaskGroupsFor(t.EarlyInstances + t.LateInstances):N0} 組"
                    + $" → メッシュシェーダ {t.EarlyMeshlets:N0} + {t.LateMeshlets:N0} 組",
        ...
        lines.Add(string.Empty);
        lines.Add("ドラッグ: 回す / ホイール: 寄る / R: 視点を戻す / V: 低い視点 / H: この表示 / Esc: 終了");
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        switch (e.KeyCode)
        {
            ...
            // Day 65c: 先頭に2パスが入った。2パス → GPU → CPU → 選ばない → 2パス。
            case Keys.G:
                _selection = _selection switch
                {
                    InstanceSelection.GpuTwoPass => InstanceSelection.Gpu,
                    InstanceSelection.Gpu => InstanceSelection.Cpu,
                    InstanceSelection.Cpu => InstanceSelection.None,
                    _ => InstanceSelection.GpuTwoPass,
                };
                _averageDrawMilliseconds = 0.0;
                break;
            ...
            // 低い視点(Day 65c)。隠れるものが多い視点。R で元の視点に戻る。
            case Keys.V:
                _view = SceneData.LowView(SceneData.Presets[_presetIndex]);
                break;
