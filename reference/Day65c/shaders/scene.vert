...
void main()
{
    // gl_InstanceIndex が「何番目に描く体か」。vkCmdDrawIndexed の instanceCount ぶん繰り返される。
    // Day 65a: 本当の体の番号は一覧(binding 8)で引く。描かない体を飛ばして詰めてあるので、
    // gl_InstanceIndex をそのまま体の番号にすると、別の体の行列で描いてしまう。
    uint instance = drawnInstance(gl_InstanceIndex);   // Day 65c: 引き方を関数にした(common.glsl)
