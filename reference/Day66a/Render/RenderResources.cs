...
internal sealed class RenderResources : IDisposable
{
    ...
    /// <summary>
    /// **ジオメトリシェーダを挟んだプログラムを読む**(Day 63a)。
    ///
    /// <para>
    /// 窓口としての仕事は <see cref="LoadShader(string, string)"/> と同じ(重複排除と寿命)。
    /// <b>キーに3本目のパスが入る</b>だけが違う——ここを入れ忘れると、
    /// 同じ頂点・画素シェーダを使う別々のジオメトリシェーダが
    /// <b>同じものとして数えられ</b>、あとから頼んだほうに先の GS のプログラムが返る。
    /// Day 57 が <c>#define</c> をキーに混ぜたのとまったく同じ話。
    /// </para>
    /// </summary>
    public Handle<Shader> LoadShader(string vertexPath, string geometryPath, string fragmentPath)
    {
        string key =
            $"{Path.GetFullPath(vertexPath)}|{Path.GetFullPath(geometryPath)}|{Path.GetFullPath(fragmentPath)}";
        ...
        var shader = new Shader(_gl, vertexPath, geometryPath, fragmentPath);
        Handle<Shader> handle = _shaders.Add(shader);
        _shaderByKey[key] = handle;
        return handle;
    }
    ...
    /// <summary>
    /// **テッセレーションの2段を挟んだプログラムを読む**(Day 63b)。
    ///
    /// <para>
    /// キーに<b>4本のパスと <c>#define</c> の両方</b>が入る。
    /// 同じ3本のシェーダから割り方だけ違うプログラムを3つ作るので、
    /// ここを混ぜ忘れると<b>3通りが全部 <c>equal_spacing</c> になる</b>——
    /// しかも絵は出る(細かさは同じなので、よく見ないと気付けない)。
    /// Day 57 の <see cref="LoadComputeShader"/> とまったく同じ話。
    /// </para>
    /// </summary>
    public Handle<Shader> LoadShader(
        string vertexPath,
        string tessControlPath,
        string tessEvaluationPath,
        string fragmentPath,
        params string[] defines)
    {
        string key =
            $"{Path.GetFullPath(vertexPath)}|{Path.GetFullPath(tessControlPath)}|"
            + $"{Path.GetFullPath(tessEvaluationPath)}|{Path.GetFullPath(fragmentPath)}|"
            + string.Join(',', defines);
        ...
        var shader = new Shader(_gl, vertexPath, tessControlPath, tessEvaluationPath, fragmentPath, defines);
        Handle<Shader> handle = _shaders.Add(shader);
        _shaderByKey[key] = handle;
        return handle;
    }
    ...
    /// <summary>
    /// **コンピュートシェーダを読む**(Day 57)。
    ///
    /// <para>
    /// 窓口としての仕事は <see cref="LoadShader"/> とまったく同じ(重複排除と寿命)。
    /// <b>キーに <c>#define</c> を混ぜる</b>のがひとつだけ違うところで、
    /// 同じファイルから作った違うバリアントを別物として数えるため。
    /// ここを混ぜ忘れると、あとから <c>LOCAL_SIZE 64</c> で頼んだ人に
    /// 256 で焼いたプログラムが返る——**絵は出るが数が合わない**という壊れ方をする。
    /// </para>
    /// </summary>
    public Handle<Shader> LoadComputeShader(string computePath, params string[] defines)
    {
        string key = $"{Path.GetFullPath(computePath)}|compute|{string.Join(',', defines)}";
        if (_shaderByKey.TryGetValue(key, out Handle<Shader> existing) && _shaders.IsAlive(existing))
        {
            _shaders.Retain(existing);
            CacheHits++;
        ...
        var shader = new Shader(_gl, computePath, defines);
        Handle<Shader> handle = _shaders.Add(shader);
        _shaderByKey[key] = handle;
        return handle;
    }
