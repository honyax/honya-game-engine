...
internal sealed class Shader : IDisposable
{
    ...
    /// <summary>
    /// **頂点 + テッセレーション制御 + テッセレーション評価 + 画素**の4段のプログラム(Day 63b)。
    ///
    /// <para>
    /// ここも段の並びを足すだけで済んだ(Day 63a の3段と同じ)。違うのは
    /// <b><c>#define</c> を受け取ること</b>で、理由はテッセレーションの
    /// <b>割り方(<c>equal_spacing</c> / <c>fractional_odd_spacing</c> / <c>fractional_even_spacing</c>)が
    /// layout 修飾子で、uniform にできない</b>から。
    /// 3通りを見比べるには<b>同じソースを3回コンパイルする</b>しかない。
    /// </para>
    ///
    /// <para>
    /// Day 57 がコンピュートのワークグループの大きさのために作った
    /// <see cref="InjectDefines"/> が、そのまま描画用のプログラムにも効く——
    /// <b>差し込みは段の種類を見ていない</b>ので、書き足すものは無かった。
    /// 実際のエンジンが「シェーダバリアント」と呼んでいるものの、いちばん小さい形。
    /// </para>
    /// </summary>
    public Shader(
        GL gl,
        ...
        : this(
            ...
            [
                (ShaderType.VertexShader, vertexPath),
                (ShaderType.TessControlShader, tessControlPath),
                (ShaderType.TessEvaluationShader, tessEvaluationPath),
                (ShaderType.FragmentShader, fragmentPath),
            ],
            defines)
    {
