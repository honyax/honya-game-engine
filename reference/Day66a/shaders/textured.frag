...
// **出口に番号を振る**(Day 52)。Day 51 までは `out vec4 FragColor;` だけで、
// 出口が1つなら番号は 0 に決まっていた。2つ以上になると、書かないとリンカが勝手に振る。
//
// 普段の本描画(フォワード)では 0 番の FragColor だけを書く。
// シーンのバッファには1枚しか挿さっていないので、1〜3 番は**どこにも繋がらず捨てられる**。
// G-Buffer パスでは4枚が挿さっていて(GBuffer)、4つとも行き先がある。
layout (location = 0) out vec4 FragColor;

// --- Day 52: G-Buffer の残り3枚(GBuffer.Layout と two-way の約束)---
layout (location = 1) out vec4 GMaterial;
layout (location = 2) out vec4 GNormalDepth;
layout (location = 3) out vec4 GEmissive;

/// **G-Buffer パスか**(Day 52)。1 なら「表面を決める」ところまでで G-Buffer に書いて終わる。
///
/// シェーダを別に書かず、**本描画のシェーダを真ん中で打ち切る**形にした。
/// 視差・法線マップ・金属度の上書き……と、表面を決める手順は Day 32 から積み上げてきたもので、
/// 別のシェーダに写すと2か所を同じに保つ仕事が生まれる。
/// 代わりに、使わない後半のぶんまでレジスタを確保するので、専用のシェーダより少し重い
/// (本番のエンジンは #define で2本にコンパイルし分ける。Day 32 のシェーダバリアントの話)。
uniform int uGBufferPass;

/// ビュー行列(Day 52)。G-Buffer の2枚目は**ビュー空間**の法線と距離(Day 37 の形)なので、
/// 世界で出した法線と位置をここで運ぶ。
uniform mat4 uView;

// --- Day 52: 点光源(フォワード)---

/// **PointLight.MaxForward と必ず同じ値**にする。
///
/// 位置と色で vec4 を2本ずつ食うので、64 個で float 512 個。
/// OpenGL 3.3 が画素シェーダに保証する uniform の枠は float 1024 個で、
/// このシェーダの他の uniform がすでに百個近く使っている。
/// **フォワードは「全部の光を1本のシェーダに渡す」ので、枠がそのまま光の数の上限になる**。
const int MAX_FORWARD_LIGHTS = 64;

/// 実際に効かせる数。上限を超えたぶんは C# 側で切り捨ててある。
uniform int uPointLightCount;

/// xyz = 位置 / w = 届く距離。
uniform vec4 uPointLightPositions[MAX_FORWARD_LIGHTS];

/// rgb = 色と強さ。w は空き(vec3 の配列でも枠は vec4 1本ぶん食う実装が多い)。
uniform vec4 uPointLightColors[MAX_FORWARD_LIGHTS];

// --- Day 53: Forward+(クラスタード)---
//
// **光の一覧を uniform ではなくテクスチャバッファで受け取る**。
// uniform の枠(float 1024 個)は光 64 個で埋まったが、テクスチャバッファには
// 6万5千テクセル以上(保証値)が入る。受け取り方を変えただけで、上限の壁が消える。
//
// 一覧は3本に分かれている(ClusteredLighting が毎フレーム詰め直す)。
//
//   uClusters      クラスターごとの (始まり, 数)   … どこから何個読むか
//   uLightIndices  光の番号の並び                  … 始まりから数個ぶんが、そのクラスターの光
//   uLightData     光そのもの(1個 = 2テクセル)   … 番号で引く
//
// **2段の間接参照にしてある**のは、同じ光が何十個ものクラスターに入るから。
// クラスターごとに光の中身を書き写すと 32 バイト × 組の数になるが、番号なら 4 バイトで済む。

/// 1 なら Forward+ のループ(自分のクラスターの光だけ)。0 なら Day 52 のループ(全部の光)。
uniform int uClusteredLights;

/// **サンプラの型は中身の形式と対**。整数(RG32UI / R32UI)は usamplerBuffer、
/// 小数(RGBA32F)は samplerBuffer。取り違えると texelFetch が黙って 0 を返す。
uniform usamplerBuffer uClusters;
uniform usamplerBuffer uLightIndices;
uniform samplerBuffer uLightData;

/// 升目の数(横のタイル, 縦のタイル, 奥行きの切り身)。
uniform ivec3 uClusterCount;

/// タイル1枚の大きさ [画素]。
uniform float uClusterTileSize;

/// 奥行きの切り方。切り身 = log(深さ / near) × scale を切り捨てる(ClusterGrid.SliceOf と同じ式)。
uniform float uClusterNear;
uniform float uClusterSliceScale;

/// クラスターの見せ方(ClusterView)。0 通常 / 1 光の数 / 2 切り口 / 3 生の値(自己チェック用)
uniform int uClusterView;
...
//   11〜13 Forward+ の光の一覧(Day 53。ClusteredLighting.Apply が刺す。テクスチャバッファ)
...
/// **点光源の減衰**(Day 52)。PointLight.Attenuation と deferred.frag と同じ式。
///
/// 1/d² に「半径ちょうどで 0 に落ちる窓」を掛けたもの(式の意味は PointLight.Attenuation)。
/// 窓があるので、**半径の外の画素には1つも効かない**——フォワードでもそれは同じだが、
/// フォワードは「効かないこと」を確かめるために、全部の光について距離を測る。
...
/// **点光源1つぶんの明るさ**(Day 52)。deferred.frag にもまったく同じ関数がある。
///
/// 中身は太陽と同じ Cook-Torrance で、違うのは2つだけ——
/// L が「光への向き」で画素ごとに変わることと、距離で減衰すること。
/// 半径の外なら BRDF を計算する前に帰る(フォワードでいちばん効く節約)。
    ...
    vec3 albedo, float metallic, float roughness)
{
    ...
    float g;
    vec3 f;
    CookTorrance(n, v, l, albedo, metallic, roughness, diffuse, specular, d, g, f);

    return (diffuse + specular) * radiance
        * PointAttenuation(distance, positionRadius.w) * max(dot(n, l), 0.0);
}

/// **この画素のクラスター**(Day 53)。ClusterGrid.CoordOf と同じ式。
///
/// タイルは画面の座標だけで決まる(gl_FragCoord)。切り身は深さの対数で決まる——
/// 等比に切ってあるので、対数を取ると等間隔になり、掛け算1回で番号が出る。
/// **深度バッファを読まない**。深さは、頂点から補間してきた位置をビュー空間へ運べば分かる。
ivec3 ClusterCoord(float viewDepth)
{
    ivec2 tile = ivec2(gl_FragCoord.xy / uClusterTileSize);
    int slice = int(log(viewDepth / uClusterNear) * uClusterSliceScale);
    return clamp(ivec3(tile, slice), ivec3(0), uClusterCount - 1);
}

/// (タイル x, タイル y, 切り身) → 通し番号。ClusterGrid.IndexOf と同じく x がいちばん速く回る。
int ClusterIndex(ivec3 coord)
{
    return coord.x + (uClusterCount.x * (coord.y + (uClusterCount.y * coord.z)));
}

/// **光の数を色にする**(Day 53。ヒートマップ)。
///
/// 4倍ごとに色が1段進む(対数)。1個と4個の差も、16個と64個の差も同じくらい目に付く——
/// 代償は「何倍か」で効くので、等間隔の色より読みやすい。
/// 目盛りを 64 個まで取ってあるのは、光が 1024 個のときの升目が数十個を抱えるから
/// (16 個で頭打ちにすると、切り身を増やしても画面が赤いまま変わらず、比べられない)。
vec3 HeatColor(uint count)
{
    ...
    // 1 → 0.0 / 4 → 1/3 / 16 → 2/3 / 64 以上 → 1.0
    float t = clamp(log2(float(count)) / 6.0, 0.0, 1.0);

    vec3 blue = vec3(0.05, 0.25, 1.0);
    vec3 green = vec3(0.1, 0.9, 0.2);
    ...
    if (t < 2.0 / 3.0)
    {
        return mix(green, yellow, (t * 3.0) - 1.0);
    }

    return mix(yellow, red, (t * 3.0) - 2.0);
}

/// **切り口を色にする**(Day 53)。切り身ごとに色相を回し、タイルは市松で明暗を付ける。
/// 床の上で色の帯が奥ほど広がって見えるのが、等比に切っている姿。
vec3 SliceColor(ivec3 coord)
{
    vec3 palette[6] = vec3[6](
        vec3(0.90, 0.30, 0.25), vec3(0.95, 0.75, 0.20), vec3(0.35, 0.80, 0.30),
        vec3(0.25, 0.70, 0.90), vec3(0.40, 0.40, 0.95), vec3(0.85, 0.40, 0.85));

    vec3 color = palette[coord.z % 6];
    return ((coord.x + coord.y) % 2) == 0 ? color : color * 0.55;
}

/// **クラスターを見る**(Day 53。「Forward+」の F3)。
///
/// 色だけだと形が読めないので、カメラから当てた光で軽く陰影を付け、タイルの縁に線を引く。
/// 生の値(3)は自己チェック専用で、CPU が振り分けた答えと GPU が引いた答えを突き合わせるのに使う。
vec4 ClusterDebugColor(ivec3 coord, uint count, vec3 normal, vec3 viewDir)
{
    if (uClusterView == 3)
    {
        return vec4(vec3(coord), float(count));
    }

    vec3 color = uClusterView == 1 ? HeatColor(count) : SliceColor(coord);

    float facing = 0.45 + (0.55 * max(dot(normal, viewDir), 0.0));

    vec2 inTile = mod(gl_FragCoord.xy, uClusterTileSize);
    float edge = (inTile.x < 1.0 || inTile.y < 1.0) ? 0.35 : 1.0;

    return vec4(color * facing * edge, 1.0);
}
...
void main()
{
    ...
    // **発光は SSAO より前へ上げた**(Day 52)。下の G-Buffer の枝で書くので、
    // 枝より前に出来上がっていないといけない。
    ...
    // --- Day 52: G-Buffer パス。**ここで打ち切る** ---
    //
    // ここまでで「表面」が全部決まった——ベースカラー・法線・金属度・粗さ・AO・発光。
    // この先は全部「光を当てる」側で、それは deferred.frag が画素ごとにやり直す。
    // **切る場所がここなのには理由がある**。すぐ下の SSAO は、
    // G-Buffer から作るもの(Day 52 からは G-Buffer の2枚目を借りる)なので、
    // このパスの時点ではまだ出来ていない。読む前に帰る。
    if (uGBufferPass == 1)
    {
        // 0: ベースカラー。**平方根で詰める**。
        //
        // 8bit にリニアのまま入れると、暗い側の刻みが足りない——
        // 256 段を 0〜1 に均等に配るので、0.002 未満は全部 0 に潰れる。
        // 目は暗い側の差に敏感なので、夕暮れの壁の陰で縞が見える。
        // 平方根にしてから入れると暗い側に段が寄る(sRGB の符号化と同じ発想の簡易版)。
        // 読む側(deferred.frag)で2乗して戻す。
        FragColor = vec4(sqrt(max(base.rgb, vec3(0.0))), 1.0);

        // 1: AO / 粗さ / 金属度。**glTF の ORM テクスチャと同じ並び**にしてある
        // (Material のコメント「R は空き(AO を入れる流儀もある)」の、その流儀)。
        GMaterial = vec4(occlusion, roughness, metallic, 1.0);

        // 2: ビュー空間の法線と距離。**Day 37 の geometry.frag とまったく同じ形**なので、
        // SSAO がそのまま読める。違うのは法線が**法線マップを当てたあと**のものになったこと
        // (Day 37 の幾何パスは頂点法線だった)——レンガの目地にも細かい暗がりが乗るようになる。
        vec3 viewNormal = normalize(mat3(uView) * normal);
        float viewDepth = -(uView * vec4(vWorldPos, 1.0)).z;
        GNormalDepth = vec4(viewNormal, viewDepth);

        // 3: 発光。1.0 を超えるので 16F の1枚に入れる。
    ...
    // --- Day 52: 点光源(フォワード)---
    //
    // **全部の光を、描いた画素ごとに回す**。これがフォワードの多光源の姿そのもの。
    // 半径の外なら関数の頭で帰るが、「外かどうか」を確かめる距離の計算は全部の光ぶん走る。
    // さらに、あとから手前の物に上書きされる画素(オーバードロー)のぶんも同じだけ払う。
    //
    // 代償は **描いた画素の数 × 光の数**。ディファードは同じ仕事を
    // **光が覆った画素の数**だけで済ませる(deferred.frag)。
    // Day 53 からは、このループは下の else 側に入った(Forward+ の枝と分かれる)。
    //
    // **ランバートの枝(上)には入れていない**。「PBR」の F2 は Day 35 以前との見比べ用の窓なので、
    // 点光源まで持ち込むと何と比べているのか分からなくなる。
    vec3 pointLight = vec3(0.0);

    // クラスターの表示(下)で使うので、枝の外に置いておく。
    ivec3 clusterCoord = ivec3(0);
    uint clusterLights = 0u;

    if (uClusteredLights == 1)
    {
        // --- Day 53: Forward+ ---
        //
        // **回すのは自分のクラスターに入っている光だけ**。1024 個あっても、ここで回るのは数個。
        // どの光が入っているかは CPU が先に決めてある(ClusterGrid.Assign)。
        //
        // **1周の中身は Day 52 のループと1文字も違わない**(同じ PointLightContribution)。
        // 変わったのは「どの光を回すか」の選び方だけで、BRDF は今日1つも増えていない——
        // Day 52 でディファードのために書き写した3つ目の BRDF とは、ここが対照的。
        float viewDepth = -(uView * vec4(vWorldPos, 1.0)).z;
        clusterCoord = ClusterCoord(viewDepth);

        uvec2 cluster = texelFetch(uClusters, ClusterIndex(clusterCoord)).rg;
        clusterLights = cluster.y;

        for (uint i = 0u; i < cluster.y; i++)
        {
            int light = int(texelFetch(uLightIndices, int(cluster.x + i)).r);

            pointLight += PointLightContribution(
                texelFetch(uLightData, light * 2), texelFetch(uLightData, (light * 2) + 1).rgb,
                vWorldPos, normal, viewDir, base.rgb, metallic, roughness);
        }
    }
    else
    {
        ...
        {
            pointLight += PointLightContribution(
                uPointLightPositions[i], uPointLightColors[i].rgb,
                vWorldPos, normal, viewDir, base.rgb, metallic, roughness);
        }
    }
    ...
    if (uIblEnabled == 1)
    {
        ...
        vec3 irradiance = uProbeEnabled == 1
            ? ProbeIrradiance(vWorldPos, normal)
    ...
    vec3 color = directDiffuse + directSpecular + ambientDiffuse + ambientSpecular + pointLight;

    // --- Day 53: クラスターを見る(「Forward+」の F3)---
    // 光を当て終えたあとで絵を差し替える。ループは普段どおり回っているので、代償は変わらない。
    if (uClusteredLights == 1 && uClusterView != 0)
    {
        FragColor = ClusterDebugColor(clusterCoord, clusterLights, normal, viewDir);
        return;
    }
